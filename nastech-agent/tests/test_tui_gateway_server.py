...
from nastech_cli.active_sessions import active_session_registry_snapshot
from nastech_cli.browser_connect import ChromeDebugLaunch
...
def test_profile_scoped_mcp_discovery_uses_target_home(monkeypatch, tmp_path):
    """MCP discovery must start under the selected profile's NASTECH_HOME."""
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "default"))
    ...
    monkeypatch.setattr(
        ...
        lambda: seen.append(str(get_nastech_home())),
    )
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["NASTECH_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("NASTECH_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_VOICE", "1")
...
def test_prompt_submit_typed_stop_phrase_ends_voice_chat(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_VOICE", "1")
    monkeypatch.setenv("NASTECH_VOICE_TTS", "1")
    ...
    assert os.environ["NASTECH_VOICE"] == "0"
    assert os.environ["NASTECH_VOICE_TTS"] == "0"
...
def test_wake_owner_is_sticky_and_routes_detection_to_first_transport(monkeypatch):
    ...
    try:
        ...
        assert emitted == [(
            "wake.detected",
            "first-session",
            {"phrase": "hey nastech", "profile": None, "start_new_session": True},
            first,
        )]
...
def test_wake_toggle_persists_enabled_flag_only_on_explicit_gesture(monkeypatch):
    ...
    config = {"enabled": False, "phrase": "hey nastech", "surface": "auto",
              "start_new_session": True}
    persisted = []
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_VOICE", "1")
    # setenv (not delenv) — the handler writes NASTECH_VOICE_TTS directly, and
    ...
    monkeypatch.setenv("NASTECH_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # _get_platform_tools (a non-configurable platform toolset in nastech-cli's
    ...
    from nastech_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown NASTECH_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("NASTECH_MODEL", raising=False)
    monkeypatch.delenv("NASTECH_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nastech/nastech-test "}}
    )

    assert server._resolve_model() == "nastech/nastech-test"
...
def test_config_sync_switches_unpinned_session(monkeypatch):
    _patch_config_model(monkeypatch, "new/model", provider="nastech")
    session = _sync_test_session(config_model_seen=("old/model", "nastech"))
    calls = []
    monkeypatch.setattr(
        server,
        "_apply_model_switch",
    ...
    assert session["config_model_seen"] == ("new/model", "nastech")
...
def test_config_sync_switches_when_only_provider_differs(monkeypatch):
    _patch_config_model(monkeypatch, "old/model", provider="nastech")
    session = _sync_test_session(config_model_seen=("old/model", ""))
    calls = []
    monkeypatch.setattr(
        server,
    ...
    assert calls == ["old/model --provider nastech"]
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set NASTECH_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("NASTECH_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("NASTECH_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `nastech --tui -m <model>` sets NASTECH_MODEL/NASTECH_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("NASTECH_MODEL", "one-shot/model")
    monkeypatch.setenv("NASTECH_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("NASTECH_MODEL", "seed/model")
    monkeypatch.setenv("NASTECH_INFERENCE_MODEL", "seed/model")
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "nastech"}})

    assert server._config_model_target() == ("", "nastech")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "nastech_cli.model_switch.resolve_persist_behavior",
    ...
    monkeypatch.setattr(
        "nastech_cli.model_cost_guard.expensive_model_warning",
        lambda *a, **k: None,
    )
    session = {"agent": None}

    out = server._apply_model_switch(
        "sid", session, "new/model --provider nastech", persist_override=False
    )
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("NASTECH_MODEL", "nastech/nastech-test")
    monkeypatch.setenv("NASTECH_TUI_PROVIDER", "nastech")
    monkeypatch.delenv("NASTECH_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nastech/nastech-test", "nastech")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("NASTECH_MODEL", "nastech/nastech-test")
    monkeypatch.delenv("NASTECH_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("NASTECH_INFERENCE_PROVIDER", "nastech")
    monkeypatch.setattr(
        "nastech_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nastech/nastech-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("NASTECH_MODEL", "sonnet")
    monkeypatch.delenv("NASTECH_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("NASTECH_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "nastech_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_DESKTOP_TERMINAL", raising=False)
    monkeypatch.setattr(
        server,
        "_load_cfg",
        lambda: {
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "nastech_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "nastech_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_ensure_session_db_row_records_a_terminal_workspace(monkeypatch, tmp_path):
    ...
    The user cd'd there before running nastech. Leaving it null stranded the row
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_state.SessionDB", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_nastech_home", tmp_path)
...
def test_config_get_approval_mode_uses_smart_default_when_key_is_missing(
    ...
    monkeypatch.setattr(server, "_nastech_home", tmp_path)
    # Point the canonical resolver (load_config → env NASTECH_HOME) at the
    # temp home too, so the smart default is asserted against THIS config
    # rather than whatever the developer's real ~/.nastech happens to hold.
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via nastech_cli.config.load_config —
    # that path resolves NASTECH_HOME from the environment, not
    # server._nastech_home.
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_get_approval_mode_normalizes_yaml_off(tmp_path, monkeypatch):
    ...
    # load_config, which resolves NASTECH_HOME from the environment.
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    (tmp_path / "config.yaml").write_text(
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    monkeypatch.setattr(server, "_nastech_home", tmp_path)
    # config.set writes via server._nastech_home, but the post-write
    # session.info emit resolves the effective mode through the canonical
    # tools.approval resolver (load_config → env NASTECH_HOME).
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    emitted = []
    monkeypatch.setattr(server, "_emit", lambda *args: emitted.append(args))
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("NASTECH_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("NASTECH_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    ...
    assert server.os.environ["NASTECH_GATEWAY_SESSION"] == "1"
    assert server.os.environ["NASTECH_EXEC_ASK"] == "1"
    assert server.os.environ["NASTECH_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("nastech_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("nastech_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "nastech_cli.runtime_provider.resolve_runtime_provider",
...
def test_setup_runtime_check_honors_requested_provider(monkeypatch):
    ...
    scoped = server.handle_request(
        {"id": "1", "method": "setup.runtime_check", "params": {"provider": "nastech"}}
    )
    assert scoped["result"]["ok"] is True
    assert scoped["result"]["provider"] == "nastech"
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "nastech_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing NASTECH_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("NASTECH_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "nastech_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["NASTECH_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "NASTECH_TUI_PROVIDER" not in os.environ
        assert "NASTECH_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write NASTECH_MODEL / NASTECH_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("NASTECH_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("NASTECH_MODEL", raising=False)
    monkeypatch.delenv("NASTECH_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("nastech_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["NASTECH_TUI_PROVIDER"] == "openai-codex"
        assert "NASTECH_MODEL" not in os.environ
        assert "NASTECH_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_config_set_model_once_keeps_env_and_records_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("NASTECH_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setenv("NASTECH_MODEL", "old/model")
    monkeypatch.setattr(
    ...
    try:
        ...
        assert os.environ["NASTECH_INFERENCE_PROVIDER"] == "openrouter"
        assert os.environ["NASTECH_MODEL"] == "old/model"
    finally:
        server._sessions.clear()
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("nastech_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_personality_preserves_history_and_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
    # Persistence now flows through the single owner (nastech_cli.personality),
    # never _write_config_key / agent.system_prompt.
    import nastech_cli.personality as personality_mod
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        # attachments dir.
        assert not (workspace / ".nastech" / "desktop-attachments").exists()
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Nastech TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("nastech_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "nastech_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("nastech_state")
    ...
    monkeypatch.setitem(sys.modules, "nastech_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_nastech_home
    # fixture pins NASTECH_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("nastech_state.SessionDB", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("nastech_state.SessionDB", ProfileDB2)
...
def test_session_branch_installs_parent_profile_secret_scope(monkeypatch, tmp_path):
    ...
    session.branch already binds the parent's NASTECH_HOME and state.db, but the
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `nastech model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `nastech model` and the classic CLI /model picker.
    ...
    for providers like Nastech whose /models endpoint returns image/video
    ...
    with patch(
        "nastech_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("nastech_cli.models.provider_model_ids") as live_fetch:
            resp = server._methods["model.options"](99, {"session_id": ""})

    assert "result" in resp, resp
    providers = resp["result"]["providers"]
    nastech = next((p for p in providers if p.get("slug") == "nastech"), None)
    assert nastech is not None
    assert nastech["models"] == [
        "moonshotai/kimi-k2.5",
        "anthropic/claude-opus-4.7",
    ]
    assert nastech["total_models"] == 30
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from nastech_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "nastech_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "nastech_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "nastech_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr(
        "nastech_cli.auth.is_provider_explicitly_configured",
        lambda _slug: False,
    )
    monkeypatch.setattr("nastech_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("nastech_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr("nastech_cli.config.is_managed", lambda: False)
    save_credential = Mock()
    monkeypatch.setattr(
        "nastech_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``nastech --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"nastech_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "nastech_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("nastech_cli.browser_connect.local_port_in_use", return_value=False),
            patch("nastech_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "nastech_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with (
            patch("nastech_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("nastech_cli.browser_connect.local_port_in_use", return_value=True),
            patch("nastech_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_nastech_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.nastech/.env
    ...
    with patch.dict(sys.modules, {"nastech_cli.config": fake}):
...
def test_session_save_writes_under_nastech_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Nastech profile
    ...
    Regression: the gateway handler wrote ``nastech_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Nastech.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("nastech_conversation_*.json"))
    ...
    assert payload["model"] == "nastech-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Nastech."
...
def test_reap_idle_sessions_calls_periodic_trim(monkeypatch):
    ...
    # `from nastech_cli.mem_trim import trim_memory` at call time.
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import nastech_cli.nastech_billing as nb
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from get_nastech_home() (live
    # env var), always targeting NASTECH_HOME/config.yaml — point it at tmp_path.
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    monkeypatch.setattr(cli, "_nastech_home", tmp_path)
...
def test_tts_stream_begin_and_stop_lifecycle(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_VOICE_TTS", "1")
    monkeypatch.setenv("NASTECH_VOICE", "0")  # no barge-in monitor (no mic)
    playback_stops: list = []
...
def test_full_duplex_stop_phrase_mid_generation_ends_voice_chat(monkeypatch, tmp_path):
    ...
    assert os.environ.get("NASTECH_VOICE") == "0"  # voice chat ended
...
def test_build_persist_message_quotes_paths_containing_spaces(tmp_path):
    ...
    img_dir = tmp_path / "Application Support" / "Nastech" / "composer-images"
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(server, "render_message", lambda _t, _c: "")
        monkeypatch.setattr(server, "_emit", lambda *a: None)
        monkeypatch.setattr(server, "set_nastech_home_override", lambda _home: object())
        monkeypatch.setattr(
            server,
            "reset_nastech_home_override",
            lambda _token: cleanup_order.append("reset_home"),
        )
        monkeypatch.setattr("nastech_cli.mem_trim.trim_memory", _inspect_trim_frame)
...
# silently rewrote ~/.nastech/config.yaml top-to-bottom — top-level keys
...
def test_prompt_submit_truncation_archives_instead_of_deleting(monkeypatch):
    ...
    tests/nastech_state/test_replace_messages_archive_siblings.py.
    """
