...
Before the s6 migration, ``docker run nastechresearch/nastech-agent gateway
...
crash and a supervised dashboard alongside (when ``NASTECH_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``NASTECH_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``NASTECH_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``nastech`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'nastech.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'nastech.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → nastech gateway run) can take
    ...
    # (main-wrapper.sh → python → nastech gateway run) can take well over
    ...
    # top), NOT the static main-nastech service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-nastech
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The NASTECH_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``nastech gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``nastech gateway
    ...
    # (main-wrapper.sh → python → nastech gateway run) can take well
    ...
    # started either — so we'd see the CMD's `nastech gateway run`
    ...
    # `nastech gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*nastech.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `nastech gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-nastech`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``NASTECH_DASHBOARD=1`` is set, ``docker run <image> gateway
