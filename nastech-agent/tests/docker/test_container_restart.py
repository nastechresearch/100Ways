...
$NASTECH_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $NASTECH_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"nastech-restart-{safe}"
    volume = f"nastech-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "nastech", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "nastech", "profile", "create", "ghost").check_returncode()
