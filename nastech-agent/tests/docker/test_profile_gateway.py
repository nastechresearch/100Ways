...
Phase 4 wires `nastech -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"nastech profile create {PROFILE}")
    ...
    r = _sh(container_name, f"nastech -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"nastech -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
