...
def test_no_args_starts_nastech(built_image: str) -> None:
    """``docker run <image>`` should start nastech cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``nastech chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> nastech batch ...`` returns a
