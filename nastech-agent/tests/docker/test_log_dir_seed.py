...
registered later runs its log service as the dropped nastech user and
...
def test_logs_gateways_seeded_and_nastech_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by nastech after boot."""
    ...
    assert "logs=nastech" in r.stdout, (
        f"logs/ not owned by nastech: {r.stdout}"
    )
