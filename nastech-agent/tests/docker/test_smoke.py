...
Converted from the former ``.github/actions/nastech-smoke-test`` composite
...
def test_nastech_help(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"nastech --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"nastech --help produced a traceback: {r.stderr[-2000:]!r}"
    )
...
def test_dashboard_subcommand_present(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"nastech dashboard --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
...
def test_nastech_help_under_wrapped_init(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"nastech --help failed under `docker run --init` (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"nastech --help produced a traceback under --init: {r.stderr[-2000:]!r}"
    )
