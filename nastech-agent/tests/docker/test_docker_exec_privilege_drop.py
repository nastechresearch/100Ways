...
The shim (docker/nastech-exec-shim.sh, installed at /opt/nastech/bin/nastech)
...
failed: Nastech is not logged into Nastech Portal" on every message.

These tests verify:

1. ``docker exec <c> nastech …`` (defaulting to root) gets dropped to the
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-nastech-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown nastech:nastech`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to nastech:nastech,
...
def test_shim_drops_root_to_nastech_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `nastech` with a Python-style `-c` shim equivalent — there's no
    pure-nastech "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `nastech` is a console_scripts entry — under the
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "nastech", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by nastech, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "nastech:nastech", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected nastech:nastech. "
        "The shim did not drop privileges before invoking nastech."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/nastech/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid nastech nastech <args>` which resolves `nastech`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> nastech login`` (root) wrote
    ...
    as a parse failure, and resolve_nastech_runtime_credentials raised
    "Nastech is not logged into Nastech Portal" on every message.
    ...
    `docker exec` path produces is nastech-readable.

    Specifically: pretend the operator ran `nastech login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the nastech UID. We use `nastech auth list` since that touches the
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "nastech", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the nastech user "
