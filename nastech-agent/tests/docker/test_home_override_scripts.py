...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to nastech:nastech on every boot.
    ...
    # Verify files are now owned by nastech.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
    ...
    assert "nastech" in r.stdout, (
        f"expected nastech-owned files after restart, got: {r.stdout!r} — "
