...
def test_install_tree_not_writable_by_nastech(
    built_image: str, container_name: str,
) -> None:
    """The nastech user must not be able to modify /opt/nastech.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/nastech as the nastech user
        "touch /opt/nastech/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"nastech user can write to /opt/nastech (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/nastech/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"nastech user can write to /opt/nastech/.venv: {r.stdout}"
    )


def test_nastech_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
