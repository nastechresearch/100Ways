"""Harness: dashboard opt-in via NASTECH_DASHBOARD.

Today (tini): dashboard starts once when NASTECH_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``nastech`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without NASTECH_DASHBOARD, no dashboard process should be running."""
    ...
    assert r.returncode != 0, (
        "Dashboard should not be running without NASTECH_DASHBOARD"
    )
...
# whenever `NASTECH_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/nastech/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled nastech
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``nastech`` in the
    ...
    start_container(
        ...
        "NASTECH_DASHBOARD=1",
        "NASTECH_DASHBOARD_HOST=0.0.0.0",
        "NASTECH_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nastech" in provider_names, (
        "Bundled dashboard_auth/nastech provider should register when "
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and NASTECH_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``NASTECH_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
