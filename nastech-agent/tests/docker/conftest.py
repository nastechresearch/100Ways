...
Override the image with ``NASTECH_TEST_IMAGE`` env var to point at a pre-built
...
def built_image() -> str:
    ...
    Override with ``NASTECH_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("NASTECH_TEST_IMAGE"):
        return IMAGE_TAG
...
# fails (PermissionError swallowed) for nastech, so a test that ran as root
...
# helpers below run every probe under ``-u nastech`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: nastech).
