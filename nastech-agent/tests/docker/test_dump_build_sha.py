"""Regression test: ``nastech dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``nastech dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$NASTECH_GIT_SHA`` build-arg to ``/opt/nastech/.nastech_build_sha`` and
``nastech_cli/build_info.py`` reads it as a fallback.
...
is absent and ``nastech dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/nastech/.nastech_build_sha`` exists in the image, assert that
  ``nastech dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's nastech user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"nastech dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/nastech/.nastech_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/nastech/.nastech_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``NASTECH_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``nastech dump``
