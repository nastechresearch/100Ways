...
def test_config_migration_runs_on_boot(
    ...
    r = docker_exec_sh(
        container_name,
        "test -f /opt/nastech/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by nastech (migration ran as nastech)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "nastech", (
        f"config.yaml not owned by nastech (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
