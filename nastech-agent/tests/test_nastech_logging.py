"""Tests for nastech_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import nastech_logging
# Use whatever RotatingFileHandler class nastech_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. nastech_logging
...
from nastech_logging import RotatingFileHandler
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, nastech_home):
        log_dir = nastech_logging.setup_logging(nastech_home=nastech_home)
        assert log_dir == nastech_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, nastech_home):
    ...
    def test_idempotent_no_duplicate_handlers(self, nastech_home):
        nastech_logging.setup_logging(nastech_home=nastech_home)
        nastech_logging.setup_logging(nastech_home=nastech_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, nastech_home):
        ...
        test_logger = logging.getLogger("test_nastech_logging.write_test")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, nastech_home):
    ...
    def test_gateway_log_not_created_in_cli_mode(self, nastech_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, nastech_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, nastech_home):
    ...
    def test_gui_log_receives_only_gui_components(self, nastech_home):
        ...
        logging.getLogger("nastech_cli.web_server").info("dashboard online")
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, nastech_home):
        """When session context is set, log lines include [session_id]."""
        nastech_logging.setup_logging(nastech_home=nastech_home)
        nastech_logging.set_session_context("abc123")
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, nastech_home):
        nastech_logging.setup_logging(nastech_home=nastech_home)
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert nastech_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not nastech_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, nastech_home):
        level, max_size, backup = nastech_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, nastech_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> nastech_logging._ManagedRotatingFileHandler:
        handler = nastech_logging._ManagedRotatingFileHandler(
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        nastech_logging.setup_logging(nastech_home=nastech_home, mode="gateway")
        gw_path = nastech_home / "logs" / "gateway.log"
