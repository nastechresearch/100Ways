...
(SQLITE_PROTOCOL — typical on NFS/SMB), Nastech must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``NASTECH_HOME`` silently lose
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    monkeypatch.setattr(
        nastech_state, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: False
    )
    nastech_state._wal_reset_bug_warned_paths.clear()
    yield
    nastech_state._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see nastech_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("nastech_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
