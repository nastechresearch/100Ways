...
def nastech_home(monkeypatch, tmp_path):
    """Isolate NASTECH_HOME and clear module-level caches afterward so the
...
class TestPatchFailureEscalation:
    def test_first_two_failures_use_normal_hint(self, nastech_home, tmp_path, fresh_tracker):
    ...
    def test_different_tasks_have_independent_counters(
        self, nastech_home, tmp_path, fresh_tracker
    ):
        from tools.file_tools import _handle_patch
