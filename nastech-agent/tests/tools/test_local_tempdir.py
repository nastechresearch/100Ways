...
class TestLocalTempDir:
    def test_uses_os_tmpdir_for_session_artifacts(self, monkeypatch):
        ...
        assert env._snapshot_path == f"/data/data/com.termux/files/usr/tmp/nastech-snap-{env._session_id}.sh"
        assert env._cwd_file == f"/data/data/com.termux/files/usr/tmp/nastech-cwd-{env._session_id}.txt"
    ...
    def test_falls_back_to_tempfile_when_tmp_missing(self, monkeypatch):
        ...
        with patch("tools.environments.local.os.path.isdir", return_value=False), \
            ...
            assert env._snapshot_path == f"/cache/tmp/nastech-snap-{env._session_id}.sh"
            assert env._cwd_file == f"/cache/tmp/nastech-cwd-{env._session_id}.txt"
