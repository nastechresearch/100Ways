...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. nastech_cli/config.py:set_config_value
                       -> bridges via the canonical ``TERMINAL_CONFIG_ENV_MAP``
                          (one-shot when the user runs ``nastech config set …``)
...
dicts) keeps the test independent of the user's ~/.nastech/config.yaml and
mirrors the pattern used in tests/nastech_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``nastech config set foo bar``.
    ...
    """
    from nastech_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``nastech config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`nastech config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to TERMINAL_CONFIG_ENV_MAP "
        f"in nastech_cli/config.py (set_config_value bridges through it)."
    )
...
def test_docker_extra_args_is_bridged_everywhere():
    ...
    present in DEFAULT_CONFIG, TERMINAL_CONFIG_ENV_MAP (so ``nastech config
    ...
    producing the "Nastech partially reads the Docker config" symptom.  Guard
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Nastech exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Nastech sweeps stale
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``nastech config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
...
def test_docker_forward_env_is_bridged_everywhere():
    ...
    ``nastech config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
