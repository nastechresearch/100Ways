...
def test_output_path_rejects_nastech_oauth_store(tmp_path, monkeypatch):
    ...
    nastech_home.mkdir()
    monkeypatch.setattr(file_safety, "_nastech_home_path", lambda: nastech_home)
    monkeypatch.setattr(file_safety, "_nastech_root_path", lambda: nastech_home)

    target = nastech_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
