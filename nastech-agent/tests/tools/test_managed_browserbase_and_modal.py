...
def _enable_managed_nastech_tools(monkeypatch):
    """Ensure managed_nastech_tools_enabled() returns True even after module reloads.
    ...
    nastech_cli.nastech_account — so the function body returns True.
    """
    monkeypatch.setattr(
...
def _install_fake_tools_package():
    ...
    # nastech_cli.auth (imported transitively by nastech_account /
...
def test_browser_use_explicit_local_mode_stays_local_even_when_managed_gateway_is_ready(tmp_path):
    ...
    env.update({
        ...
        "TOOL_GATEWAY_USER_TOKEN": "nastech-token",
