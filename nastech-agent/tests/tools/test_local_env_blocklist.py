...
See: https://github.com/NastechResearch/nastech-agent/issues/1002
See: https://github.com/NastechResearch/nastech-agent/issues/1264
"""
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.nastech/.env must not leak."""
...
class TestNastechBinDirOnPath:
    ...
    even when the gateway was launched without the nastech install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_nastech_bin_dir / _prepend_nastech_bin_dir.
    """
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/nastech/bin/nastech" if name == "nastech" else None)
    ...
    def test_make_run_env_injects_nastech_bin_dir(self):
        """A gateway env missing the nastech dir gets it back in the subshell PATH.

        Platform-agnostic: ``_prepend_nastech_bin_dir`` uses ``os.pathsep`` on
        ...
        assert entries[0] == "/opt/nastech/bin"
...
class TestNastechInternalDynamicSecrets:
    ...
    ``_is_nastech_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``nastech_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_nastech_internal_secret
        assert _is_nastech_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_nastech_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_nastech_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_nastech_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_nastech_internal_secret
        assert _is_nastech_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_nastech_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_nastech_internal_secret
        assert _is_nastech_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_nastech_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_nastech_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_nastech_internal_secret
        assert not _is_nastech_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_nastech_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_nastech_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_nastech_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_nastech_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_nastech_internal_secret("PATH")
        assert not _is_nastech_internal_secret("MY_APP_KEY")
