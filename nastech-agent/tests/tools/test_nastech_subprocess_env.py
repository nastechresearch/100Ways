"""Tests for nastech_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_NASTECH_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return nastech_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestBrowserPassthroughPattern:
    def test_browser_keys_recoverable_after_strip(self):
        ...
        with patch.dict(os.environ, {**_SAFE_SAMPLE, **leaked}, clear=True):
            env = nastech_subprocess_env(inherit_credentials=False)
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "NASTECH_KANBAN_TASK": "t_parent",
                "NASTECH_KANBAN_RUN_ID": "123",
                "NASTECH_KANBAN_DB": "/tmp/parent-kanban.db",
                "NASTECH_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
