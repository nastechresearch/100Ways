...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("nastech_cli.config.load_config", side_effect=_boom):
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in nastech_cli.config must expose tool_output so
    that ``nastech setup`` and default installs stay in sync with the
    helpers here."""
