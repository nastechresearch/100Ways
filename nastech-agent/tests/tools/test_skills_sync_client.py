...
the sync wire contract):
  ...
  * the access gate (Nastech admin) making sync inert
...
# ---------------------------------------------------------------------------
# Access gate (Nastech admin) + per-skill opt-in
# ---------------------------------------------------------------------------
...
class TestDevGate:
    def test_gate_open_with_claim(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ssc, "resolve_nastech_runtime_credentials",
        ...
        monkeypatch.setattr(auth_mod, "resolve_nastech_runtime_credentials",
                            lambda **kw: {"api_key": token, "base_url": "https://x"})
        ident = ssc.resolve_identity()
        assert ident["nastech_admin"] is True
    ...
    def test_gate_closed_without_claim(self, monkeypatch):
        ...
        monkeypatch.setattr(auth_mod, "resolve_nastech_runtime_credentials",
                            lambda **kw: {"api_key": token, "base_url": "https://x"})
        ident = ssc.resolve_identity()
        assert ident["nastech_admin"] is False
    ...
    def test_maybe_pull_inert_when_not_logged_in(self, monkeypatch):
        ...
        monkeypatch.setattr(auth_mod, "resolve_nastech_runtime_credentials", _raise)
...
def synced_env(tmp_path, monkeypatch):
    """A NASTECH_HOME with two opted-in skills + a token-carrying identity."""
...
# ---------------------------------------------------------------------------
# Env-var configuration (Nastech Cloud "on by default" via environment)
# ---------------------------------------------------------------------------

class TestEnvConfig:
    def test_base_url_env_wins(self, monkeypatch):
        monkeypatch.setenv("NASTECH_SYNC_BASE_URL", "https://plane.example/")
    ...
    def test_base_url_defaults_to_production(self, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_SYNC_BASE_URL", raising=False)
        monkeypatch.setattr("nastech_cli.config.load_config", lambda: {}, raising=False)
    ...
    def test_feature_enabled_env(self, monkeypatch):
        # Default off.
        monkeypatch.delenv("NASTECH_SYNC_ENABLED", raising=False)
        monkeypatch.setattr("nastech_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_feature_enabled() is False
        for truthy in ("1", "true", "YES", "on"):
            monkeypatch.setenv("NASTECH_SYNC_ENABLED", truthy)
            assert ssc.sync_feature_enabled() is True
        for falsy in ("0", "false", "off"):
            monkeypatch.setenv("NASTECH_SYNC_ENABLED", falsy)
            assert ssc.sync_feature_enabled() is False

    def test_default_opt_in_env(self, monkeypatch):
        monkeypatch.delenv("NASTECH_SYNC_DEFAULT_OPT_IN", raising=False)
        monkeypatch.setattr("nastech_cli.config.load_config", lambda: {}, raising=False)
        assert ssc.sync_default_opt_in() is False
        monkeypatch.setenv("NASTECH_SYNC_DEFAULT_OPT_IN", "true")
    ...
    def test_env_overrides_config_yaml(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_SYNC_ENABLED", "false")
        monkeypatch.setattr(
            "nastech_cli.config.load_config",
            lambda: {"sync": {"enabled": True}},
            raising=False,
...
class TestDeviceName:
    def test_default_is_hostname_seeded(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_SYNC_DEVICE_NAME", raising=False)
        monkeypatch.setattr(
    ...
    def test_existing_file_wins_over_default_and_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_SYNC_DEVICE_NAME", "cloud-seed")
        assert ssc.stable_device_id() == "Explicit Name"

    def test_env_seeds_first_use(self, tmp_path, monkeypatch):
        # Nastech Cloud path: NASTECH_SYNC_DEVICE_NAME seeds the first-use label.
        monkeypatch.setattr(ssc, "_skills_dir", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_SYNC_DEVICE_NAME", "nastech-cloud-ben-1")
        assert ssc.stable_device_id() == "nastech-cloud-ben-1"
        # persisted so it stays stable even if the env later changes
        assert (tmp_path / ".sync_device_id").read_text() == "nastech-cloud-ben-1"
        monkeypatch.setenv("NASTECH_SYNC_DEVICE_NAME", "changed")
        assert ssc.stable_device_id() == "nastech-cloud-ben-1"
...
def _org_identity(role=None, org_id="org-1", owner="owner1"):
    ...
    return {"api_key": token, "base_url": "http://x", "owner": owner,
            "nastech_admin": True, "claims": claims,
