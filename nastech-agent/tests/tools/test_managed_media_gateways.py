...
@pytest.fixture(autouse=True)
def _enable_managed_nastech_tools(monkeypatch):
    """Patch the source modules so managed_nastech_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr(
        "nastech_cli.nastech_account.get_nastech_portal_account_info",
        lambda: NastechPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
...
def test_managed_fal_submit_uses_gateway_origin_and_nastech_token(monkeypatch):
    captured = {}
    ...
    assert captured["client_key"] == "nastech-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "nastechresearch.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "nastech-token")
    ...
    assert captured["api_key"] == "nastech-token"
    assert captured["base_url"] == "https://openai-audio-gateway.nastechresearch.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.nastechresearch.com/v1"
