...
Coverage:
  nastech_cli/clipboard.py  — platform-specific image extraction (macOS, WSL, Wayland, X11)
...
from nastech_cli.clipboard import (
...
class TestSaveClipboardImage:
    def test_creates_parent_dirs(self, tmp_path):
        ...
        with patch("nastech_cli.clipboard.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("nastech_cli.clipboard._linux_save", return_value=False):
...
class TestMacosPngpaste:
    def test_success_writes_file(self, tmp_path):
        ...
        with patch("nastech_cli.clipboard.subprocess.run", side_effect=fake_run):
...
class TestMacosHasImage:
    ...
    def test_image_class_detection(self, stdout, expected):
        with patch("nastech_cli.clipboard.subprocess.run") as mock_run:
...
class TestIsWsl:
    def setup_method(self):
        # _is_wsl is nastech_constants.is_wsl; reset the function's own module
        # globals so this stays stable even if nastech_constants was imported
        # through a different module object earlier in a large xdist run.
        import nastech_constants
        nastech_constants._wsl_detected = None
...
class TestLinuxSave:
    ...
    def setup_method(self):
        import nastech_cli.clipboard as cb
        cb._wsl_detected = None

    def test_wsl_tried_first(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("nastech_cli.clipboard._is_wsl", return_value=True):
            with patch("nastech_cli.clipboard._wsl_save", return_value=True) as m:
                assert _linux_save(dest) is True
                m.assert_called_once_with(dest)

    def test_wayland_fails_falls_through_to_xclip(self, tmp_path):
        dest = tmp_path / "out.png"
        with patch("nastech_cli.clipboard._is_wsl", return_value=False):
            with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                with patch("nastech_cli.clipboard._wayland_save", return_value=False):
                    with patch("nastech_cli.clipboard._xclip_save", return_value=True) as m:
                        assert _linux_save(dest) is True
                        m.assert_called_once_with(dest)
...
class TestWindowsHasImage:
    ...
    def test_clipboard_has_image(self):
        with patch("nastech_cli.clipboard._get_ps_exe", return_value="powershell"):
            with patch("nastech_cli.clipboard.subprocess.run") as mock_run:
...
class TestConvertToPng:
    ...
    def test_imagemagick_failure_preserves_original(self, tmp_path, failure):
        ...
        with patch.dict(sys.modules, {"PIL": None, "PIL.Image": None}):
            with patch("nastech_cli.clipboard.subprocess.run", side_effect=side_effect):
...
class TestHasClipboardImage:
    ...
    def test_macos_dispatch(self):
        ...
        with patch("nastech_cli.clipboard._macos_has_image", return_value=True) as m:
    ...
    def test_wsl_falls_through_to_wayland_when_windows_path_empty(self):
        ...
        """
        with patch("nastech_cli.clipboard._is_wsl", return_value=True):
            with patch("nastech_cli.clipboard._wsl_has_image", return_value=False) as wsl:
                with patch.dict(os.environ, {"WAYLAND_DISPLAY": "wayland-0"}):
                    with patch("nastech_cli.clipboard._wayland_has_image", return_value=True) as wl:
                        assert has_clipboard_image() is True
...
class TestPreprocessImagesWithVision:
    ...
    @pytest.fixture
    def cli(self):
        """Minimal NastechCLI with mocked internals."""
