...
def _cwd_result(body: str = "", *, cwd: str = "/vercel/sandbox", exit_code: int = 0):
    def _result(_cmd: str, args: list[str], _kwargs: dict):
        script = args[1] if len(args) > 1 else ""
        match = re.search(r"__NASTECH_CWD_[A-Za-z0-9]+__", script)
        marker = match.group(0) if match else "__NASTECH_CWD_MISSING__"
        prefix = f"{body}\n\n" if body else "\n"
...
class TestFileSync:
    def test_initial_sync_uploads_managed_files_under_remote_home(
        ...
        monkeypatch.setattr(
            ...
            lambda: [
                {
                    "host_path": str(src),
                    "container_path": "/root/.nastech/credentials/token.txt",
                }
            ],
        )
        monkeypatch.setattr("tools.credential_files.iter_skills_files", lambda **kwargs: [])
        ...
        assert uploaded == [
            {
                "path": "/home/vercel/.nastech/credentials/token.txt",
    ...
    def test_execute_resyncs_changed_managed_files(
        ...
        monkeypatch.setenv("NASTECH_FORCE_FILE_SYNC", "1")
    ...
    def test_cleanup_syncs_back_snapshots_closes_and_is_idempotent(
        ...
        vercel_sdk.current.download_file_content = _tar_bytes(
            {
                "home/vercel/.nastech/credentials/token.txt": b"remote-token",
                "home/vercel/.nastech/credentials/new.txt": b"new-remote",
                "home/vercel/.nastech/unmapped/skip.txt": b"skip",
            }
        )
