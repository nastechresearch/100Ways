...
Older images made /opt/nastech/ui-tui and /opt/nastech/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV NASTECH_TUI_DIR=/opt/nastech/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R nastech:nastech /opt/nastech/ui-tui" not in text
    assert "chown -R nastech:nastech /opt/nastech/node_modules" not in text
