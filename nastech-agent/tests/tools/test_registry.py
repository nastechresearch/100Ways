...
class TestDeregisterAuthorization:
    ...
    def test_plugin_cannot_deregister_unowned_tool_without_opt_in(self):
        reg = self._reg()
        reg.register_plugin_override_policy("nastech_plugins.evil", False)
        with patch.object(ToolRegistry, "_caller_module", return_value="nastech_plugins.evil"):
            import pytest
    ...
    def test_plugin_root_module_can_deregister_submodule_handler(self):
        ...
        nastech_plugins.pkg (root cleanup code) must be allowed to deregister a
        tool whose handler was defined in nastech_plugins.pkg.handlers.  The
        exact module strings differ, but they share the same plugin package root
        (nastech_plugins.pkg) — ownership is bound to the package, not the leaf
        module (egilewski review, #55840).
        """
        reg = ToolRegistry()
        reg.register_plugin_override_policy("nastech_plugins.pkg", False)
        handler = eval("lambda *a, **k: 'sub'", {"__name__": "nastech_plugins.pkg.handlers"})
        reg.register(
            name="sub_tool", toolset="pkg-ts",
            schema={"name": "sub_tool", "description": "", "parameters": {"type": "object", "properties": {}}},
            handler=handler,
        )
        # Caller is the plugin root (nastech_plugins.pkg), handler is in a
        # submodule (nastech_plugins.pkg.handlers) — must be allowed.
        with patch.object(ToolRegistry, "_caller_module", return_value="nastech_plugins.pkg"):
    ...
    def test_opted_in_plugin_submodule_can_deregister(self):
        ...
        root (``nastech_plugins.allowed``).  If the caller is a submodule
        (``nastech_plugins.allowed.cleanup``), the old code looked up
        ``_plugin_override_policy.get("nastech_plugins.allowed.cleanup")`` →
        ...
        reg.register_plugin_override_policy("nastech_plugins.allowed", True)
        with patch.object(ToolRegistry, "_caller_module", return_value="nastech_plugins.allowed.cleanup"):
            reg.deregister("protected")
        assert reg._tools.get("protected") is None


    def test_core_code_deregister_always_allowed(self):
        """Non-plugin callers (core Nastech code) are never gated."""
        reg = self._reg()
    ...
    def test_full_bypass_blocked(self):
        ...
        with pytest.raises(PermissionError):
            evil_handler = eval("lambda *a, **k: 'hijacked'", {"__name__": "nastech_plugins.evil"})
