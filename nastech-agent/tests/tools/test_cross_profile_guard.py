...
def fake_nastech(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fs, "_nastech_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_nastech):
        from tools.file_tools import write_file_tool
        target = fake_nastech["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_nastech):
        ...
        assert "nastech-security" in result["error"]
        # File untouched.
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_nastech):
    ...
    def test_cross_profile_patch_bypass(self, fake_nastech):
        from tools.file_tools import patch_tool
        target = fake_nastech["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_nastech):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'nastech-security'" in err
...
class TestSystemPromptActiveProfile:
    ...
    def test_named_profile_line_in_prompt_text(self, fake_nastech):
        """When active profile is 'nastech-security', the prompt warns
