"""Cross-session NASTECH_SESSION_* leak guard for the local terminal backend.

Regression coverage for the bug where a terminal subprocess could observe a
*different concurrent session's* ``NASTECH_SESSION_KEY`` (and the other
``NASTECH_SESSION_*`` vars).
...
subprocess-env bridge resolves each ``NASTECH_SESSION_*`` from the ContextVar and,
...
from tools.environments.local import _make_run_env, _sanitize_subprocess_env, nastech_subprocess_env
...
def test_engaged_unset_contextvar_strips_foreign_session_key(monkeypatch):
    ...
    os.environ["NASTECH_SESSION_KEY"], this task's ContextVar is unset, and the
    ...
    assert "NASTECH_SESSION_KEY" not in env, (
        "Foreign concurrent session key leaked into subprocess env: "
        f"{env.get('NASTECH_SESSION_KEY')!r}"
    )
...
def test_set_session_vars_engages_and_overrides_foreign_global(monkeypatch):
    ...
    assert env.get("NASTECH_SESSION_KEY") == "agent:main:discord:group:MY_BUGS_ROOT:111"
...
def test_unengaged_process_preserves_os_environ_fallback(monkeypatch):
    ...
    Pure single-process CLI/one-shot sets NASTECH_SESSION_* directly in os.environ
    ...
    monkeypatch.setenv("NASTECH_SESSION_KEY", "cli-session-key")
    monkeypatch.setenv("NASTECH_SESSION_ID", "cli-session-id")

    env = _make_run_env({})

    assert env.get("NASTECH_SESSION_KEY") == "cli-session-key"
    assert env.get("NASTECH_SESSION_ID") == "cli-session-id"
...
def test_explicit_empty_thread_id_overrides_stale_value(monkeypatch):
    ...
    no thread id binds NASTECH_SESSION_THREAD_ID="" and that empty value must win
    over an older non-empty value left in os.environ.
    """
    monkeypatch.setenv("NASTECH_SESSION_THREAD_ID", "stale-thread-from-prior-turn")
    ...
    assert env.get("NASTECH_SESSION_THREAD_ID") == "", (
        "Bound-empty thread id did not override the stale value: "
        f"{env.get('NASTECH_SESSION_THREAD_ID')!r}"
    )
    assert env.get("NASTECH_SESSION_KEY") == "mm:chan"
...
def test_sanitize_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    ...
    stale_base = {
        "PATH": "/usr/bin:/bin",
        "NASTECH_SESSION_KEY": "agent:main:discord:thread:FOREIGN_BG:FOREIGN_BG",
        "NASTECH_SESSION_THREAD_ID": "FOREIGN_BG",
    }

    sanitized = _sanitize_subprocess_env(stale_base)

    assert "NASTECH_SESSION_KEY" not in sanitized, (
        f"Background subprocess inherited foreign key: {sanitized.get('NASTECH_SESSION_KEY')!r}"
    )
    assert "NASTECH_SESSION_THREAD_ID" not in sanitized
...
def test_sanitize_subprocess_env_set_contextvar_wins_when_engaged():
    ...
    assert sanitized.get("NASTECH_SESSION_KEY") == "agent:main:discord:group:REAL_BG:222"


# --------------------------------------------------------------------------- #
# Non-terminal spawn surface (nastech_subprocess_env) — sibling path
# --------------------------------------------------------------------------- #

def test_nastech_subprocess_env_strips_foreign_session_key_when_engaged(monkeypatch):
    """nastech_subprocess_env (browser/ACP/CLI/TUI-host spawns) must not leak a
...
def test_nastech_subprocess_env_unengaged_preserves_fallback(monkeypatch):
    """A pure single-process CLI (never engaged) keeps the inherited fallback."""
    monkeypatch.setenv("NASTECH_SESSION_KEY", "cli-fallback-key")
    # not engaged (autouse fixture leaves _session_context_engaged False)
    env = nastech_subprocess_env()
    assert env.get("NASTECH_SESSION_KEY") == "cli-fallback-key"
