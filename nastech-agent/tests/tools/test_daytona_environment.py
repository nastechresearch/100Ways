...
class TestPersistence:
    def test_persistent_resumes_via_get(self, make_env):
        ...
        env._mock_client.get.assert_called_once_with("nastech-mytask")
...
class TestSyncSafety:
    def test_single_upload_quotes_parent_path(self, make_env, tmp_path):
        ...
        remote_path = "/root/.nastech/skills/evil; touch /tmp/daytona-owned/file.txt"
        ...
        assert mkdir_cmd == (
            "mkdir -p '/root/.nastech/skills/evil; touch /tmp/daytona-owned'"
        )
        assert "; touch" not in mkdir_cmd.replace(
            "'/root/.nastech/skills/evil; touch /tmp/daytona-owned'", ""
        )
