...
code paths Nastech takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``nastech_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect NASTECH_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("nastech_cli.stdio", None)
        # Fresh import now; tests import from nastech_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import nastech_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("nastech_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from nastech_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("nastech_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "nastech_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, nastech_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "nastech_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call nastech_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """nastech_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from nastech_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from nastech_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self):
        ...
        Without it, the gateway-respawn watcher spawned by ``nastech update``
        (which runs under nastech-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# nastech_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "nastech_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the NASTECH_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / nastech_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "nastech_cli/tools_config.py",
            "nastech_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_nastech_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_nastech_home" in source
        assert 'cache_dir = get_nastech_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_nastech_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "nastech_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "nastech_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        nastech_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "nastech_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from nastech_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (NASTECH_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "NASTECH_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import nastech_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "nastech_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        it up — a pre-import so the lazy ``nastech_cli.gateway`` import didn't
        re-run ``gateway/status``'s ``import msvcrt`` branch, and a mock of
        ``get_nastech_home`` because the real one's ``Path.resolve()`` consults
        ...
        with mock.patch.object(
            gw, "_stable_gateway_working_dir", return_value="C:/nastech"
        ), mock.patch(
            "nastech_cli.config.get_nastech_home", return_value="C:/nastech"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        from nastech_cli._subprocess_compat import (
            windows_detach_flags_without_breakaway,
            windows_detach_popen_kwargs,
        )

        monkeypatch.setattr(gr, "_resolve_nastech_bin", lambda: ["nastech"])
        ...
        assert "_NASTECH_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("NASTECH_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("NASTECH_TEST_SECRET", "")
