...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_nastech_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_nastech_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_nastech_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "nastechresearch.com"}):
            with patch("tools.web_tools._read_nastech_access_token", return_value="nastech-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="nastech-token",
                        api_url="https://firecrawl-gateway.nastechresearch.com",
                    )
...
class TestBackendSelection:
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """Regression: a Nastech OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured TAVILY_API_KEY in the fallback path.
        Free Nastech tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a Nastech-managed
...
class TestCheckWebApiKey:
    ...
    def test_configured_firecrawl_backend_accepts_managed_gateway(self):
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}):
            with patch("tools.web_tools._peek_nastech_access_token", return_value="nastech-token"):
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_check_web_api_key_returns_true_for_custom_provider(self):
        ...
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("tools.web_tools._peek_nastech_access_token", return_value=None):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
