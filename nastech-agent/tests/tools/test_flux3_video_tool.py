...
def _call(handler, args, response, headers=None):
    ...
    with patch.object(
        ...
        return_value=headers if headers is not None else {"Authorization": "Bearer nastech-token"},
...
class TestGating:
    ...
    def test_visible_to_any_signed_in_account_whatever_its_entitlement(self):
        ...
        with patch.object(flux3, "peek_nastech_access_token", return_value="nastech-token"), \
                patch(
    ...
    def test_hidden_without_a_nastech_credential(self):
        # The gateway takes a Nastech bearer and nothing else, so with no token
        # every call could only ever answer "sign in" — six schemas on every
        # API call for something that cannot work.
        with patch.object(flux3, "peek_nastech_access_token", return_value=None):
            assert flux3.check_bfl_requirements() is False
    ...
    def test_a_profile_sees_a_credential_held_at_the_global_root(self, tmp_path, monkeypatch):
        ...
        # nastech_cli.auth, not here.
        monkeypatch.delenv("TOOL_GATEWAY_USER_TOKEN", raising=False)
        root = tmp_path / "root"
        (root / "profiles" / "work").mkdir(parents=True)
        (root / "auth.json").write_text(
            json.dumps({"version": 1, "providers": {"nastech": {"access_token": "root-token"}}}),
            encoding="utf-8",
        )
        monkeypatch.setenv("NASTECH_HOME", str(root / "profiles" / "work"))

        # The profile's own store is empty, so this passes only via the
        # global-root fallback — without which the tools would be hidden.
        assert flux3.peek_nastech_access_token() is None
        assert flux3.check_bfl_requirements() is True
    ...
    def test_the_credential_probe_never_forces_a_token_refresh(self, monkeypatch):
        ...
        with patch.object(flux3, "read_nastech_access_token", side_effect=AssertionError("refreshed")):
            assert flux3.check_bfl_requirements() is True

    def test_fails_closed_when_the_credential_probe_raises(self):
        with patch.object(flux3, "peek_nastech_access_token", side_effect=RuntimeError("auth store unreadable")):
            assert flux3.check_bfl_requirements() is False
...
class TestSubmitTransport:
    def test_text_to_video_posts_the_mode_and_arguments(self):
        ...
        assert requests[0]["headers"]["Authorization"] == "Bearer nastech-token"
    ...
    def test_a_401_asks_for_a_nastech_sign_in(self):
...
class TestPollTransport:
    ...
    def test_on_messaging_the_clip_lands_where_the_gateway_may_send_it(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_SESSION_PLATFORM", "telegram")
    ...
    def test_off_messaging_the_clip_stays_a_file_and_no_tag_is_offered(self, tmp_path, monkeypatch, platform):
        ...
        monkeypatch.setenv("NASTECH_SESSION_PLATFORM", platform)
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
    ...
    def test_a_cli_session_is_recognised_by_its_source(self, tmp_path, monkeypatch):
        # The CLI, TUI, and desktop leave NASTECH_SESSION_PLATFORM empty and
        # identify themselves on NASTECH_SESSION_SOURCE instead, so keying only
        # on the platform would miss them.
        monkeypatch.delenv("NASTECH_SESSION_PLATFORM", raising=False)
        monkeypatch.setenv("NASTECH_SESSION_SOURCE", "tui")
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
...
class TestMediaDelivery:
    ...
    def test_a_local_path_is_uploaded_and_replaced_with_a_reference(self):
        ...
        assert requests[0]["json"]["input_image"] == "nastech-upload:token-1"
    ...
    def test_video_fields_permit_video_only(self):
        ...
        assert requests[0]["json"]["input_video"] == "nastech-upload:token-v"
    ...
    def test_every_keyframe_path_is_uploaded(self):
        ...
        async def fake_uploader(data, mime):
            uploads.append(mime)
            return f"nastech-upload:token-{len(uploads)}"
        ...
        assert requests[0]["json"]["input_images"] == [
            "nastech-upload:token-1",
            "https://x/b.png",
            "nastech-upload:token-2",
        ]
...
class TestLocalPathDetection:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "nastech-upload:eyJhbGciOiJIUzI1NiJ9.e30.sig",
            "C:frame.png",
