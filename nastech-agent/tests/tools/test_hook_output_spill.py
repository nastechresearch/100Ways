...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("nastech_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="nastech-spill-test-")
    ...
    def test_default_directory_uses_nastech_home(self):
        """When no directory override, spill under NASTECH_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="nastech-home-")
        try:
            with patch.dict(os.environ, {"NASTECH_HOME": test_home}):
                # Also patch get_nastech_home to the env var to mirror production.
            ...
            # ~/.nastech/hook_outputs depending on get_nastech_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.nastech/hook_outputs/sess")),
            ]
