...
class TestSecurityGating:
    ...
    def test_config_failure_fails_open(self, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.setattr(
...
class TestIsSatisfiedVersionAware:
    ...
    def test_trace_upload_hub_at_core_locked_version_is_current(self, monkeypatch):
        ...
        core installs satisfies the trace-upload spec, so the `nastech update`
...
# ---------------------------------------------------------------------------
# active_features + refresh_active_features (Piece A — nastech update wiring)
# ---------------------------------------------------------------------------
...
class TestActiveFeatures:
    ...
    def test_shared_dependency_does_not_activate_feature(self, monkeypatch):
        ...
        # reasons. It must not make nastech update try to refresh Matrix unless
...
class TestRefreshActiveFeatures:
    ...
    def test_windows_matrix_refresh_is_skipped_before_pip(self, monkeypatch):
        ...
        # path. `nastech update` must not retry that doomed install every run.
        #
