...
class TestStripBlockedTools(unittest.TestCase):
    ...
    def test_mixed_composite_is_subtracted_at_child_assembly(self):
        ...
        ``nastech-cli`` contains both allowed tools and every sensitive
        ...
        parent.enabled_toolsets = ["nastech-cli"]
...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_nastech_child_rederives_api_mode_from_model(self):
        ...
        parent = _make_mock_parent(depth=0)
        parent.base_url = "https://inference-api.nastechresearch.com/v1"
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            self.assertEqual(kwargs["provider"], "nastech")
            self.assertEqual(kwargs["model"], "nastech-4-405b")
...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    @patch("nastech_cli.runtime_provider.resolve_runtime_provider")
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on Nastech, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT Nastech
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    def test_load_config_prefers_active_persistent_config_over_cli_defaults(self):
        ...
        with patch.dict("sys.modules", {"cli": stale_cli}):
            with patch(
                "nastech_cli.config.load_config_readonly", return_value=active_config
            ):
