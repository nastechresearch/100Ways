...
Tests cover:
- managed_nastech_tools_enabled() subscription-based gate
...
from nastech_cli.nastech_account import NastechPaidServiceAccessInfo, NastechPortalAccountInfo
...
# managed_nastech_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedNastechToolsEnabled:
    """Subscription-based gate: True for paid Nastech subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "nastech_cli.nastech_account.get_nastech_portal_account_info",
            lambda: NastechPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_nastech_tools_enabled() is False
...
class TestNastechToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "nastech_cli.nastech_account.get_nastech_portal_account_info",
            lambda force_fresh=False: NastechPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
        ...
        message = nastech_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, nastech_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_nastech_tools_enabled",
            lambda: nastech_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, nastech_enabled=True)
    ...
    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, nastech_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, nastech_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---


    def test_managed_blocked_when_nastech_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, nastech_enabled=False)
        assert result["selected_backend"] is None
