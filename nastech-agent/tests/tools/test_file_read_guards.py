...
class TestFileDedup(unittest.TestCase):
    ...
    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("nastech-dedup-")
...
class TestConfigOverride(unittest.TestCase):
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("nastech_cli.config.load_config", return_value={"file_read_max_chars": 50})
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("nastech_cli.config.load_config", return_value={"file_read_max_chars": 500_000})
...
class TestWriteInvalidatesDedup(unittest.TestCase):
    ...
    Regression test for https://github.com/NastechResearch/nastech-agent/issues/13144
    """

    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("nastech-write-dedup-")
