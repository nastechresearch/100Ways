...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "nastech-token",
        )
...
def test_resolve_managed_tool_gateway_is_inactive_without_nastech_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "nastechresearch.com",
...
def test_read_nastech_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth.resolve_nastech_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_nastech_access_token() == "fresh-token"
...
def test_managed_gateway_auth_headers_carry_the_bearer():
    with patch.object(managed_tool_gateway, "managed_nastech_tools_enabled", return_value=True):
    ...
    assert headers == {"Authorization": "Bearer nastech-token"}


def test_managed_gateway_auth_headers_reflect_a_rotated_token():
    # Read fresh on every call: a Nastech access token expires within the hour,
...
class TestManagedMediaUploader:
    """The presign -> PUT -> ``nastech-upload:<token>`` protocol.
    ...
    def _uploader(self, **kwargs):
        return managed_tool_gateway.build_managed_media_uploader(
            ...
            token_reader=kwargs.pop("token_reader", lambda: "nastech-token"),
        )
    ...
    def test_presign_declares_the_exact_type_and_length_the_put_then_sends(self):
        ...
        assert calls["presign"][0]["headers"]["Authorization"] == "Bearer nastech-token"
        ...
        assert calls["result"] == "nastech-upload:tok-1"
    ...
    def test_a_missing_credential_fails_before_any_request(self):
        ...
        with patch.object(managed_tool_gateway, "managed_nastech_tools_enabled", return_value=True), \
            ...
            with pytest.raises(RuntimeError, match="no Nastech credential"):
