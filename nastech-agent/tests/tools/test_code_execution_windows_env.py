...
``nastech_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``NASTECH_`` prefix was dropped
...
# ``nastech_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``nastech_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        """
        from tools.code_execution_tool import generate_nastech_tools_module
        import tempfile, ast
        stub = generate_nastech_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_nastech_tools_module(["terminal"], transport="uds")
...
# let the child import nastech_tools, a user script that printed non-ASCII
