...
def _run_terminal(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        monkeypatch.setattr("nastech_cli.plugins.invoke_hook", invoke_hook)
...
def test_terminal_output_transform_still_runs_strip_and_redact(monkeypatch, tmp_path):
    # Ensure redaction is active regardless of host NASTECH_REDACT_SECRETS state
...
def test_large_process_output_is_bounded_before_sudo_and_plugin_hooks(
    ...
    monkeypatch.setattr("nastech_cli.plugins.invoke_hook", _hook_spy)
