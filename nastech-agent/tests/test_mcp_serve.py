"""
Tests for mcp_serve — Nastech MCP server.
...
def _isolate_nastech_home(tmp_path, monkeypatch):
    """Redirect NASTECH_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking nastech_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <NASTECH_HOME>/state.db for its mtime gate; the autouse
        # fixture points NASTECH_HOME at tmp_path.
        db_path = tmp_path / "state.db"
