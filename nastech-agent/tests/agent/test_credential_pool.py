...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "nastech" / "auth.json").read_text()
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "nastech_cli.env_loader.get_secret_source",
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://nastech/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://nastech/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "nastech" / "auth.json").read_text())["credential_pool"]["nastech"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.nastech/.env when
    ...
    # User edited ~/.nastech/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.nastech/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (nastech_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_mirrors_nastech_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "nastech"))
    ...
    auth_payload = json.loads((tmp_path / "nastech" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["nastech"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_nastech_runtime_api_key_rejects_opaque_agent_key():
    from agent.credential_pool import PooledCredential
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `nastech setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Nastech PKCE
    ...
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("nastech_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_nastech_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `nastech_pkce` into auth.json) and later switches to the API key at
    `nastech setup` would still have those OAuth entries dormant on disk.
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `nastech setup`.  When ANTHROPIC_TOKEN is set (and
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_nastech_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_custom_endpoint_pool_seeds_from_config(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "nastech" / "config.yaml"
    import yaml
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``nastech auth remove copilot gh_cli``.  The gate must run
    ...
    monkeypatch.setattr(
        "nastech_cli.copilot_auth.get_copilot_api_token",
        _boom,
    )
...
def test_load_pool_skips_resolve_when_all_copilot_sources_suppressed(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "nastech"))
    from nastech_cli.copilot_auth import COPILOT_ENV_VARS
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    monkeypatch.setattr("nastech_cli.copilot_auth.resolve_copilot_token", _boom)
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_nastech_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.nastech`` singleton, it must carry the mint/refresh
        ...
        tmp_path,
        {
            ...
            "providers": {
                "nastech": {
                    ...
                    "inference_base_url": "https://inference.nastechresearch.com/v1",
                    "token_type": "Bearer",
                    "scope": "openid profile",
                    "obtained_at": "2026-04-24T10:00:00+00:00",
                    "expires_at": "2026-04-24T11:00:00+00:00",
                    "expires_in": 3600,
                    "agent_key": "sk-nastech-AAAA",
                    "agent_key_id": "ak_123",
    ...
    assert e.agent_key == "sk-nastech-AAAA"
...
# ── PR #10160 salvage: Nastech OAuth cross-process sync tests ─────────────────
...
def test_persist_preserves_concurrent_disk_only_entry(tmp_path, monkeypatch):
    ...
    final = json.loads((tmp_path / "nastech" / "auth.json").read_text())
...
def _make_anthropic_claude_code_pool(tmp_path, monkeypatch, *, access_token, refresh_token, expires_at_ms=9_999_999_999_000):
    ...
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "credential_pool": {}})
    monkeypatch.setattr("nastech_cli.auth.is_provider_explicitly_configured", lambda pid: pid == "anthropic")
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_nastech_oauth_credentials",
        lambda: None,
    )
