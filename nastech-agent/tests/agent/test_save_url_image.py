...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate NASTECH_HOME under tmp_path."""
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / ".nastech"))
    (tmp_path / ".nastech").mkdir()

    # Force the constants/image cache helpers to re-read NASTECH_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("nastech_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_nastech_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under NASTECH_HOME — gateway cleanup
