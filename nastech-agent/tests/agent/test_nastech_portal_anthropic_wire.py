...
class TestApiModeRouting:
    """``nastech_api_mode`` / ``determine_api_mode`` split the Portal catalog."""
    ...
    def test_anthropic_prefixed_models_use_the_messages_wire(self, model):
        assert nastech_api_mode(model) == "anthropic_messages"
    ...
    def test_a_claude_model_without_the_vendor_prefix_is_not_rerouted(self):
        ...
        assert nastech_api_mode("claude-opus-4.8") == "chat_completions"

    def test_determine_api_mode_honors_the_model_for_nastech(self):
        ...
        overlay alone advertises openai_chat for every Nastech model."""
        ...
        assert (
            determine_api_mode("nastech", PORTAL_URL, model="nastech-4-405b")
            == "chat_completions"
        )
        # No model → historical OpenAI-wire default (safer than guessing).
        assert determine_api_mode("nastech", PORTAL_URL) == "chat_completions"
...
class TestRuntimeResolution:
    ...
    def _stub_portal_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "nastech")
    ...
    def test_anthropic_model_resolves_to_the_messages_wire(self, monkeypatch):
        monkeypatch.setattr(rp, "_get_model_config", lambda: {"provider": "nastech"})

        resolved = rp.resolve_runtime_provider(
            requested="nastech", target_model="anthropic/claude-opus-5"
        )

        assert resolved["provider"] == "nastech"
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "nastech", "default": "nastech-4-405b"},
        )
...
class TestPoolRuntimeResolution:
    ...
    def test_pool_entry_honors_the_model_derived_mode(self, monkeypatch, portal_entry):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="nastech", target_model="anthropic/claude-opus-4.8"
        )
...
class TestPortalBodyFields:
    ...
    They are produced by the Nastech provider profile, which only the OpenAI-wire
    transport consults — so the Anthropic branch has to merge them in itself.
    """

    def _build(self, provider="nastech", session_id="sess-abc123"):
    ...
    def test_helper_merge_is_a_no_op_for_non_nastech(self):
        from agent.chat_completion_helpers import (
            _merge_nastech_portal_messages_extra_body,
        )

        kwargs = {"model": "claude-opus-4-8", "messages": []}
        agent = SimpleNamespace(provider="anthropic", session_id="s")
        assert _merge_nastech_portal_messages_extra_body(agent, kwargs) is kwargs
...
class TestPortalThinkingReplay:
    ...
    def test_staging_host_with_env_override_keeps_signed_thinking(
        ...
        monkeypatch.setenv("NASTECH_INFERENCE_BASE_URL", STAGING_URL)
...
class TestAuxiliaryDualWire:
    """``resolve_provider_client('nastech', …)`` must wrap Claude onto Messages."""
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                "agent.auxiliary_client._try_nastech",
            ...
            client, model = resolve_provider_client("nastech", "nastech-4-405b")
