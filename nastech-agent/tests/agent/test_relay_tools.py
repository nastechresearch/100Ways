"""Tests for the core Relay-managed Nastech tool adapter."""
...
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "profile"))
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "nastech-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "nastech-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("nastech-test-tool-execution")
        relay.intercepts.deregister_tool_request("nastech-test-tool-request")
