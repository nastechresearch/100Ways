"""Regression tests for the codex_app_server → Nastech UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_nastech_tools_mcp_server_emits_bare_tool_name(self):
        """The nastech-tools MCP server wraps Nastech' own tools for codex;
        ...
        mcp.nastech-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "nastech-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "nastech-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
