...
@pytest.fixture
def nastech_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.nastech/`` with a config.yaml referencing one external dir."""
...
def test_cache_invalidates_on_mtime_change(nastech_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = nastech_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different NASTECH_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".nastech"
    ...
    home_b = tmp_path / "home_b" / ".nastech"
    ...
    monkeypatch.setenv("NASTECH_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("NASTECH_HOME", str(home_b))
