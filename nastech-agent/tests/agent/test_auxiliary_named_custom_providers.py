...
def _isolate(tmp_path, monkeypatch):
    """Redirect NASTECH_HOME and clear module caches."""
...
def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test NASTECH_HOME."""
    import yaml
...
class TestResolveProviderClientMainAlias:
    ...
    def test_main_resolves_github_copilot_alias(self, tmp_path):
        ...
        with (
            patch("nastech_cli.auth.resolve_api_key_provider_credentials", return_value={
...
class TestResolveVisionProviderClientModelNormalization:
    ...
    def test_vision_auto_strips_matching_main_provider_prefix(self, tmp_path):
        ...
        with (
            patch("agent.auxiliary_client._read_nastech_auth", return_value=None),
            patch("nastech_cli.auth.resolve_api_key_provider_credentials", return_value={
                "api_key": "glm-key",
                "base_url": "https://api.z.ai/api/paas/v4",
            }),
