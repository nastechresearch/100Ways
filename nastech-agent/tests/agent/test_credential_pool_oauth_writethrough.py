...
Companion to ``tests/nastech_cli/test_xai_oauth_writethrough.py``. That file
...
def profile_and_root(tmp_path, monkeypatch):
    ...
    only refuses the *real* user's ``$HOME/.nastech/auth.json``; a tmp_path
...
def test_codex_pool_refresh_holds_auth_store_lock_across_post(monkeypatch, tmp_path):
    ...
    Codex refresh tokens are single-use. If two Nastech processes both read the
