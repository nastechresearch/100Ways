...
class TestAuxiliaryConfigBridge:
    ...
    def test_web_extract_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "web_extract": {"provider": "nastech", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_WEB_EXTRACT_PROVIDER") == "nastech"
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in nastech_cli/config.py has correct auxiliary structure."""
