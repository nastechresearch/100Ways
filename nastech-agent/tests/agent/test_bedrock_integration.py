...
Note: Tests that import ``nastech_cli.auth`` or ``nastech_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from nastech_cli.models import _PROVIDER_ALIASES
...
class TestProviderLabels:
    ...
    def test_bedrock_label(self):
        from nastech_cli.models import _PROVIDER_LABELS
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("nastech_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("nastech_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("nastech_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("nastech_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into nastech_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from nastech_cli.providers import ALIASES
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "nastech-agent[bedrock]" not in extras["all"]
