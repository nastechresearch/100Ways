...
def curator_env(tmp_path, monkeypatch):
    """Isolated NASTECH_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # the *next* test has configured (or the default ~/.nastech once monkeypatch
    ...
    # thread here, while NASTECH_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from nastech_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `nastech model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from nastech_cli.main import _AUX_TASKS
    from nastech_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. nastech_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. nastech_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
