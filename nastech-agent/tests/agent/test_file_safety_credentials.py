...
Regression for https://github.com/NastechResearch/nastech-agent/issues/17656 —
...
for the credential stores while leaving arbitrary ``NASTECH_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_nastech_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "nastech_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_nastech_home_path", lambda: home)
    return home
...
def test_arbitrary_nastech_home_file_not_blocked(fake_home):
    """Non-credential files inside NASTECH_HOME stay readable."""
...
def test_identically_named_nastech_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Nastech-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside NASTECH_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside NASTECH_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside NASTECH_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, NASTECH_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_nastech_home_path", lambda: profile)
