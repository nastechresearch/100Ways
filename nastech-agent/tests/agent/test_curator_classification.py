...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"nastech-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "nastech-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "nastech-agent-dev"
...
# `nastech curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `nastech curator pin <umbrella>` in the rename block so
