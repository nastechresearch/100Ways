"""Tests for agent/nastech_rate_guard.py — cross-session Nastech Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    nastech_home = str(tmp_path / ".nastech")
    os.makedirs(nastech_home, exist_ok=True)
    monkeypatch.setenv("NASTECH_HOME", nastech_home)
    # Clear any cached module-level imports
    return nastech_home


class TestRecordNastechRateLimit:
    """Test recording rate limit state."""

    def test_records_with_header_reset(self, rate_guard_env):
        from agent.nastech_rate_guard import record_nastech_rate_limit, _state_path
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_nastech_rate_limit(headers=None, default_cooldown=120.0)
...
class TestNastechRateLimitRemaining:
    """Test checking remaining rate limit time."""


    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.nastech_rate_guard import record_nastech_rate_limit, nastech_rate_limit_remaining

        record_nastech_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = nastech_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.nastech_rate_guard import nastech_rate_limit_remaining, _state_path
        ...
        assert nastech_rate_limit_remaining() is None
        # File should be cleaned up
        assert not os.path.exists(_state_path())



class TestClearNastechRateLimit:
    """Test clearing rate limit state."""

    def test_clears_existing_file(self, rate_guard_env):
        from agent.nastech_rate_guard import (
            record_nastech_rate_limit,
            clear_nastech_rate_limit,
            nastech_rate_limit_remaining,
            _state_path,
        )

        record_nastech_rate_limit(headers={"retry-after": "600"})
        assert nastech_rate_limit_remaining() is not None
    ...
    def test_clear_when_no_file(self, rate_guard_env):
        from agent.nastech_rate_guard import clear_nastech_rate_limit
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.nastech_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.nastech_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_nastech_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.nastech_rate_guard import record_nastech_rate_limit

        # Record a rate limit
        record_nastech_rate_limit(headers={"retry-after": "600"})

        # Mock _read_nastech_auth to return valid creds (would normally succeed)
        import agent.auxiliary_client as aux
        monkeypatch.setattr(aux, "_read_nastech_auth", lambda: {
            "access_token": "test-token",
            "inference_base_url": "https://api.nastech.test/v1",
        })

        result = aux._try_nastech()
        assert result == (None, None)

    def test_try_nastech_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        import agent.auxiliary_client as aux

        # No rate limit recorded — _try_nastech should proceed normally
        # (will return None because no real creds, but won't be blocked
        # by the rate guard)
        monkeypatch.setattr(aux, "_read_nastech_auth", lambda: None)
        result = aux._try_nastech()
        assert result == (None, None)


class TestIsGenuineNastechRateLimit:
    """Tell a real account-level 429 apart from an upstream-capacity 429.

    Nastech Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Nastech).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.nastech_rate_guard import is_genuine_nastech_rate_limit
        ...
        assert is_genuine_nastech_rate_limit(headers=headers) is True
    ...
    def test_bare_429_with_no_headers_is_upstream(self):
        ...
        assert is_genuine_nastech_rate_limit(headers=None) is False
        assert is_genuine_nastech_rate_limit(headers={}) is False
        assert is_genuine_nastech_rate_limit(
    ...
    def test_last_known_state_all_healthy_stays_upstream(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="nastech")
        assert is_genuine_nastech_rate_limit(
