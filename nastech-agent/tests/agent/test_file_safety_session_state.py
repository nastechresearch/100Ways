...
manipulation. Nastech keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point NASTECH_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_nastech_home /
    get_default_nastech_root) instead of monkeypatching private helpers —
    ...
    NASTECH_HOME=<root>/profiles/<name> makes get_default_nastech_root()
