...
pytestmark = pytest.mark.skipif(
    os.environ.get("NASTECH_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "NASTECH_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    registry = {"openai": _FakeImgProvider("openai"), "nastech": _FakeImgProvider("nastech")}
    ...
    # Listed in preference order: Nastech Portal before OpenAI.
    assert [p["name"] for p in listed] == ["nastech", "openai"]
