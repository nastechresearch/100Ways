...
``nastech_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``NASTECH_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "nastech_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "nastech_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / nastech_subprocess_env /
