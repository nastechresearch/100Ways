...
class TestCacheFileReadBlocking:
    """Internal Nastech cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        nastech_home = tmp_path / ".nastech"
        cache = nastech_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._nastech_home_path", return_value=nastech_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Nastech cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        nastech_home = tmp_path / ".nastech"
        hub = nastech_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_nastech_home(self, tmp_path):
        """The env basename guard does not depend on NASTECH_HOME resolution."""
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        nastech_home = tmp_path / ".nastech"
        cache = nastech_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
