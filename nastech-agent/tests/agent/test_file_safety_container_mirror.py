...
``…/sandboxes/<backend>/<task>/home/.nastech/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.nastech/…``.
...
class TestClassifyContainerMirrorTarget:
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.nastech/profiles/group1/SOUL.md",
            mirror_prefix="/root/.nastech",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.nastech")
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.nastech/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.nastech") is not None
