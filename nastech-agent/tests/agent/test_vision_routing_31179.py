...
def isolated_home(monkeypatch):
    """Temp NASTECH_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="nastech_test_31179_")
    nastech_home = os.path.join(test_home, ".nastech")
    os.makedirs(nastech_home)
...
def _fresh_modules():
    """Drop cached nastech modules so each test reloads against current env."""
    _drop_reload_targets()
