...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Nastech root with two profiles, monkeypatch the
...
def fake_nastech(tmp_path, monkeypatch):
    ...
    sec_home = root / "profiles" / "nastech-security"
...
def _set_active_home(monkeypatch, nastech_home: Path):
    """Point file_safety._nastech_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_nastech, monkeypatch):
        _set_active_home(monkeypatch, fake_nastech["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_nastech, monkeypatch):
        """If NASTECH_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
...
class TestGetCrossProfileWarning:
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_nastech, monkeypatch):
        _set_active_home(monkeypatch, fake_nastech["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "nastech-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_nastech, monkeypatch):
        _set_active_home(monkeypatch, fake_nastech["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_nastech["default_home"] / "skills" / "foo" / "SKILL.md")
