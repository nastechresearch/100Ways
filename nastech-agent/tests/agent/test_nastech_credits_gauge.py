"""Tests for the Nastech-credits subscription % gauge in build_nastech_credits_snapshot.
...
from agent.account_usage import build_nastech_credits_snapshot, render_account_usage_lines
...
def _acct(**kwargs):
    ...
    kwargs.setdefault("portal_base_url", "https://portal.nastechresearch.com")
...
def test_gauge_present_with_monthly_credits():
    snap = build_nastech_credits_snapshot(_acct(
...
def test_nan_remaining_no_window_no_nan_string():
    ...
    snap = build_nastech_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=NastechPortalSubscriptionInfo(monthly_credits=220, credits_remaining=float("nan")),
        paid_service_access_info=NastechPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
...
def test_failopen_none_and_logged_out():
    assert build_nastech_credits_snapshot(None) is None
    assert build_nastech_credits_snapshot(_acct(logged_in=False)) is None
