...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("nastech_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated NASTECH_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="nastech_api_content_")
    os.makedirs(os.path.join(test_home, ".nastech"))
    prev_home = os.environ.get("NASTECH_HOME")
    os.environ["NASTECH_HOME"] = os.path.join(test_home, ".nastech")
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["NASTECH_HOME"] = prev_home
