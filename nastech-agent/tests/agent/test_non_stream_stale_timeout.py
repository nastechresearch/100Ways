...
def _write_config(tmp_path: Path, body: str) -> None:
    nastech_home = tmp_path
    (nastech_home / "config.yaml").write_text(body or "{}\n", encoding="utf-8")
...
def test_default_base_is_90s(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NASTECH_API_CALL_STALE_TIMEOUT", raising=False)
...
""")
    ...
    import importlib
    from nastech_cli import timeouts as to_mod
