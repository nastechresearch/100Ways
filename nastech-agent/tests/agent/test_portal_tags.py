"""Tests for agent.portal_tags — Nastech Portal request tag contract."""
...
def test_ambient_context_propagates_via_thread_context_helper():
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            plain = ex.submit(nastech_portal_tags).result()
            propagated = ex.submit(
                propagate_context_to_thread(nastech_portal_tags)
            ).result()
    finally:
        reset_conversation_context(token)
...
def test_nastech_sticky_key_matches_conversation_tag():
