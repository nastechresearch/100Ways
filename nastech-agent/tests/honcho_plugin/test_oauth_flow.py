    ...
    def do_POST(self):  # noqa: N802
        ...
        if grant_type == "authorization_code":
            body["config"] = {
                ...
                "hosts": {"nastech": {"saveMessages": True, "recallMode": "hybrid"}},
            }
        payload = json.dumps(body).encode()
...
def fake_as(monkeypatch):
    ...
    monkeypatch.setenv("HONCHO_OAUTH_CLIENT_ID", "nastech-desktop")
    try:
        yield base
    finally:
        server.shutdown()
...
def test_full_loopback_flow_then_refresh(tmp_path, fake_as):
    ...
    assert saved["hosts"]["nastech"]["apiKey"] == "hch-at-1"
    assert saved["hosts"]["nastech"]["oauth"]["refreshToken"] == "hch-rt-1"
    assert saved["hosts"]["nastech"]["recallMode"] == "hybrid"
    ...
    token, refreshed = oauth.ensure_fresh_token(
        config_path, "nastech", now=saved["hosts"]["nastech"]["oauth"]["expiresAt"] + 10
    )
    assert refreshed is True
    assert token == "hch-at-2"
    rotated = json.loads(config_path.read_text())["hosts"]["nastech"]["oauth"]
...
def test_state_mismatch_is_rejected(fake_as, tmp_path):
    ...
    with pytest.raises(ValueError, match="unknown or expired"):
        oauth_flow.complete_authorization(
            ...
            config_path=tmp_path / "honcho.json", host="nastech",
        )


def test_source_tags_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, source="nastech-cli")
    assert "source=nastech-cli" in url
    untagged, _ = oauth_flow.begin_authorization(endpoints)
    assert "source=" not in untagged


def test_client_id_defaults_to_nastech_agent(monkeypatch):
    ...
    assert oauth_flow.resolve_endpoints(**common).client_id == "nastech-agent"
...
def test_grant_persists_default_client_id(tmp_path, fake_as, monkeypatch):
    ...
    # store client_id=nastech-agent so refresh reuses the right client.
    monkeypatch.delenv("HONCHO_OAUTH_CLIENT_ID", raising=False)
    config_path = tmp_path / "honcho.json"
    ...
    saved = json.loads(config_path.read_text())
    assert saved["hosts"]["nastech"]["oauth"]["clientId"] == "nastech-agent"


def test_config_path_rides_the_authorize_link(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    url, _ = oauth_flow.begin_authorization(endpoints, config_path="~/.nastech/honcho.json")
    q = parse_qs(urlparse(url).query)
    assert q["config_path"][0] == "~/.nastech/honcho.json"
...
def test_display_config_path_never_leaks_absolute_path():
    ...
    under_home = Path.home() / ".nastech" / "profiles" / "work" / "honcho.json"
    assert oauth_flow._display_config_path(under_home) == "~/.nastech/profiles/work/honcho.json"
...
def test_cli_flow_stores_tokens_without_applying_config(tmp_path, fake_as):
    ...
    config_path = tmp_path / "honcho.json"
    config_path.write_text(json.dumps({"hosts": {"nastech": {"saveMessages": False}}}))
...
def test_request_device_code_parses_response_and_sends_identity(fake_as):
    endpoints = oauth_flow.resolve_endpoints()
    device = oauth_flow.request_device_code(endpoints, source="nastech-cli")
    ...
    assert _FakeAS.last_device_form["client_id"] == "nastech-desktop"
    assert _FakeAS.last_device_form["scope"] == "write"
    assert _FakeAS.last_device_form["source"] == "nastech-cli"
...
def test_launcher_runs_flow_in_background_and_reports_connected(monkeypatch, reset_flow):
    ...
    st = oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="nastech")
    assert st["state"] == "pending"  # returns immediately, before the flow finishes
    assert _wait_until(lambda: seen.get("source") == "nastech-desktop")  # default source tag
    assert seen["host"] == "nastech"
    gate.set()
...
def test_get_flow_status_reports_stored_connection(tmp_path, monkeypatch, reset_flow):
    ...
    monkeypatch.setattr(honcho_client, "resolve_active_host", lambda: "nastech")
    monkeypatch.delenv("HONCHO_API_KEY", raising=False)

    cfgfile.write_text(json.dumps({"hosts": {"nastech": {}}}))
    assert oauth_flow.get_flow_status()["connected"] is False

    cfgfile.write_text(json.dumps({"hosts": {"nastech": {"apiKey": "hch-v3-static"}}}))
    ...
    cfgfile.write_text(json.dumps({"hosts": {"nastech": {
        ...
        "oauth": {"refreshToken": "hch-rt-x", "expiresAt": 9_999_999_999,
                  "clientId": "nastech-desktop", "tokenEndpoint": "http://x/oauth/token"},
    }}}))
    s = oauth_flow.get_flow_status()
...
def test_memory_oauth_router_dispatches_by_provider_convention():
    ...
    from nastech_cli.memory_oauth import _resolve_flow
