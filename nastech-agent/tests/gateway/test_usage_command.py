...
class TestUsageAccountSection:
    ...
    async def test_usage_command_uses_persisted_provider_when_agent_not_running(self, monkeypatch):
        ...
        async def _fake_to_thread(fn, *args, **kwargs):
            ...
            # with the provider positionally) and the Nastech credits fetch
            # (nastech_credits_lines, markdown-only) through to_thread — record every
        ...
        # The credits block routes through the shared nastech_credits_lines() helper;
        # stub it so this account-section test stays hermetic (no portal/auth lookup).
        monkeypatch.setattr("agent.account_usage.nastech_credits_lines", lambda markdown=False: [])
