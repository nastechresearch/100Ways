...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    nastech_home.mkdir()
    monkeypatch.setattr(base, "_NASTECH_HOME", nastech_home)
    monkeypatch.setattr(base, "_NASTECH_ROOT", nastech_home)
    path = nastech_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.nastech/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("NASTECH_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("NASTECH_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("NASTECH_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("NASTECH_KANBAN_HOME", str(tmp_path / "nastech"))
        monkeypatch.setenv("NASTECH_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "nastech" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("NASTECH_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("NASTECH_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("NASTECH_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.nastech/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        nastech_dir = fake_home / ".nastech"
        (nastech_dir / "mcp-tokens").mkdir(parents=True)
        secret = nastech_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._NASTECH_HOME",
            nastech_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._NASTECH_ROOT",
            nastech_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the NASTECH_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        nastech_dir = fake_home / ".nastech"
        nastech_dir.mkdir(parents=True)
        token = nastech_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._NASTECH_HOME", nastech_dir)
        monkeypatch.setattr("gateway.platforms.base._NASTECH_ROOT", nastech_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_nastech_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.nastech
        ...
        nastech_dir = fake_home / ".nastech"
        nastech_dir.mkdir(parents=True)
        artifact = nastech_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting NASTECH_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose NASTECH_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.nastech) emits
        ...
        nastech_root = denied_root / ".nastech"
        prof_cache = nastech_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._NASTECH_ROOT", nastech_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.nastech/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        nastech_home = tmp_path / ".nastech"
        cache = nastech_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.nastech/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "NASTECH_HUMAN_DELAY_MODE": "natural",
            "NASTECH_HUMAN_DELAY_MIN_MS": "oops",
            "NASTECH_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_tolerates_malformed_env_vars(self):
        env = {
            "NASTECH_HUMAN_DELAY_MODE": "custom",
            "NASTECH_HUMAN_DELAY_MIN_MS": "oops",
            "NASTECH_HUMAN_DELAY_MAX_MS": "still-bad",
        }
        with patch.dict(os.environ, env):
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"NASTECH_MEDIA_DELIVERY_STRICT": "1",
                                     "NASTECH_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Nastech
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../nastech/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.nastech/cache/media/sensitive_host_path_abc123.bin"
