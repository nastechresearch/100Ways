...
class TestReasoningCommand:
    ...
    def test_resolve_session_reasoning_prefers_session_override(self, tmp_path, monkeypatch):
        nastech_home = tmp_path / "nastech"
        nastech_home.mkdir()
        (nastech_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")
    ...
    def test_run_agent_includes_enabled_mcp_servers_in_gateway_toolsets(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_nastech_home", nastech_home)
        monkeypatch.setattr(gateway_run, "_env_path", nastech_home / ".env")
...
class TestLoadShowReasoningCoercion:
    ...
    def _load_with_config(self, tmp_path, monkeypatch, yaml_body: str) -> bool:
        nastech_home = tmp_path / "nastech"
        nastech_home.mkdir()
        (nastech_home / "config.yaml").write_text(yaml_body, encoding="utf-8")
        monkeypatch.setattr(gateway_run, "_nastech_home", nastech_home)
