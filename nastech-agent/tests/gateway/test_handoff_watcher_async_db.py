...
async def test_watcher_wraps_calls_via_asyncio_to_thread(monkeypatch):
    ...
    nastech_state) and records which SessionDB callables were handed to it.
    ...
    wrapped = []
    real_to_thread = nastech_state.asyncio.to_thread

    async def _spy_to_thread(func, *args, **kwargs):
        wrapped.append(getattr(func, "__name__", repr(func)))
        return await real_to_thread(func, *args, **kwargs)

    monkeypatch.setattr(nastech_state.asyncio, "to_thread", _spy_to_thread)
