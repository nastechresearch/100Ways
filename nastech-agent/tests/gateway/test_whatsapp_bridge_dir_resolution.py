...
(/opt/nastech/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable NASTECH_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_nastech_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable NASTECH_HOME."""
    ...
    monkeypatch.setattr(
        "nastech_constants.get_nastech_home", lambda: nastech_home
    )
    ...
    expected = nastech_home / "scripts" / "whatsapp-bridge"
