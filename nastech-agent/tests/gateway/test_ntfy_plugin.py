...
class TestNtfyRequirements:
    ...
    def test_returns_false_when_httpx_unavailable(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "nastech-test")
...
class TestNtfyAdapterInit:
    ...
    def test_publish_topic_uses_extra_value(self):
        config = PlatformConfig(
            enabled=True,
            extra={"topic": "nastech-in", "publish_topic": "nastech-out"},
        )
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "nastech-out"
...
class TestConnect:
    ...
    def test_connect_starts_stream_task(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "nastech-test"})
        adapter = NtfyAdapter(config)
...
class TestSend:
    ...
    def _make_adapter(self, topic="nastech-in", publish_topic="", token="", markdown=False):
    ...
    def test_send_posts_to_publish_topic(self):
        adapter = self._make_adapter(topic="nastech-in", publish_topic="nastech-out")
        ...
        result = _run(adapter.send("nastech-in", "Hello ntfy!"))
        assert result.success is True
        assert result.message_id == "abc123"

        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/nastech-out")

    def test_send_falls_back_to_subscribe_topic(self):
        adapter = self._make_adapter(topic="nastech-in")
        ...
        result = _run(adapter.send("nastech-in", "Hello!"))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/nastech-in")
    ...
    def test_send_uses_metadata_publish_topic(self):
        ...
        result = _run(adapter.send(
            "nastech-in", "Hi!", metadata={"publish_topic": "override-out"}
        ))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
    ...
    def test_get_chat_info_returns_dict(self):
        adapter = NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "t"}))
        info = _run(adapter.get_chat_info("nastech-in"))
        assert info["name"] == "nastech-in"
        assert info["type"] == "dm"
...
class TestOnMessage:
    ...
    def _make_adapter(self):
        return NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "nastech-in"}))
    ...
    def test_duplicate_message_skipped(self):
        ...
        adapter.set_message_handler(handler)
        event = {"id": "dup-1", "event": "message", "topic": "nastech-in", "message": "hi", "time": None}
        _run(adapter._on_message(event))
        _run(adapter._on_message(event))
        assert len(calls) == 1
...
class TestEnvEnablement:
    ...
    def test_markdown_truthy_values(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "nastech-in")
...
class TestStandaloneSend:
    ...
    def test_emits_echo_tag_header(self, monkeypatch):
        ...
        monkeypatch.setenv("NTFY_TOPIC", "nastech-in")
        pconfig = MagicMock()
        pconfig.extra = {"topic": "nastech-in"}
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            ...
            _run(_standalone_send(pconfig, "nastech-in", "hi"))
