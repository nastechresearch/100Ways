...
closure the PR changed, against a real temp ``NASTECH_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_nastech_home() / config.yaml`` — point it here.
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_nastech_home", default_home)
    _stub_picker_dependencies(monkeypatch)
