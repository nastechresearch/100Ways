...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NASTECH_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("NASTECH_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("NASTECH_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("NASTECH_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("NASTECH_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
