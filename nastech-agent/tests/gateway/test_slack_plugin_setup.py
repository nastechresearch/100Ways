...
``nastech_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``nastech_cli.config`` (get_env_value / save_env_value / remove_env_value)
and ``nastech_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch
those source modules.
"""
import nastech_cli.config as config_mod
import nastech_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved, removed, existing):
    ...
    # Manifest writing reaches out to nastech_cli.slack_cli + filesystem; stub it.
    import nastech_cli.slack_cli as slack_cli_mod
