...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _nastech_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._nastech_home", tmp_path), \
        ...
        with patch("gateway.run._nastech_home", tmp_path):
            ...
            # project_root will be the real nastech-agent dir (which HAS .git).
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._nastech_home", nastech_home), \
             ...
             patch("shutil.which", side_effect=lambda x: "/usr/bin/nastech" if x == "nastech" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = nastech_home / ".update_pending.json"
        ...
        assert not (nastech_home / ".update_exit_code").exists()
    ...
    async def test_fallback_when_no_setsid(self, tmp_path):
        ...
        assert "Starting Nastech update" in result
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_MANAGED", "")
        ...
        # Later steps may legitimately return success ("Starting Nastech
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (nastech_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = nastech_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (nastech_home / ".update_output.txt").write_text("done")
        (nastech_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (nastech_home / ".update_pending.json").write_text(json.dumps(pending))
        (nastech_home / ".update_output.txt").write_text(
    ...
    async def test_cleans_up_on_error(self, tmp_path):
        ...
        pending_path = nastech_home / ".update_pending.json"
        output_path = nastech_home / ".update_output.txt"
        exit_code_path = nastech_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (nastech_home / ".update_pending.claimed.json").exists()
    ...
    async def test_completion_notification_tolerates_invalid_utf8_output(self, tmp_path):
        ...
        assert "Nastech update finished" in sent_text
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
...
class TestWatchUpdateProgress:
    ...
    async def test_invalid_utf8_update_output_does_not_crash_watcher(self, tmp_path):
        ...
        (nastech_home / ".update_pending.json").write_text(json.dumps({
            "platform": "telegram",
            "chat_id": "67890",
            "user_id": "12345",
        }))
        (nastech_home / ".update_output.txt").write_bytes(
        ...
        assert "Nastech update finished" in sent
        assert not (nastech_home / ".update_pending.json").exists()
