...
``nastech update``, serving pre-update bridge.js behavior forever (e.g.
...
def _fresh_node_modules(bridge_dir: Path) -> None:
    ...
    nm.mkdir()
    (nm / ".nastech-pkg-hash").write_text(
...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["NASTECH_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["NASTECH_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["NASTECH_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
