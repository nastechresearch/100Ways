...
def test_secondary_profile_pairing_stores_created(tmp_path, monkeypatch):
    ...
    with patch("nastech_cli.profiles.profiles_to_serve", return_value=[
        ("coder", tmp_path / ".nastech" / "profiles" / "coder"),
    ]), patch("nastech_cli.profiles.get_active_profile_name", return_value="default"):
...
def test_pairing_store_scoped_to_profile_dir(tmp_path, monkeypatch):
    ...
    with patch("nastech_cli.profiles.profiles_to_serve", return_value=[
        ("ops", tmp_path / ".nastech" / "profiles" / "ops"),
    ]), patch("nastech_cli.profiles.get_active_profile_name", return_value="default"):
