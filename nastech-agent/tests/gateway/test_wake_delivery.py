...
* the stateless API server (supports_async_delivery=False) self-POSTs
  /v1/chat/completions with the RAW session id in X-Nastech-Session-Id, so the
...
class ApiServerLikeAdapter:
    ...
    def __init__(self, host="0.0.0.0", port=0, key="test-key", model="nastech"):
        self._host = host
        self._port = port
        self._api_key = key
...
def test_deliver_wake_non_push_self_posts_raw_session_id(monkeypatch):
    ...
    async def handler(request):
        seen["session_id"] = request.headers.get("X-Nastech-Session-Id")
