...
class TestBlockingApprovalE2E:
    ...
    def setup_method(self):
        ...
        os.environ.pop("NASTECH_GATEWAY_SESSION", None)
        os.environ.pop("NASTECH_EXEC_ASK", None)
        os.environ.pop("NASTECH_SESSION_KEY", None)
    ...
    def test_blocking_approval_uses_canonical_timeout(self, approval_config, monkeypatch):
        ...
        def agent_thread():
            ...
            os.environ["NASTECH_SESSION_KEY"] = session_key
            try:
                with patch(
...
class TestFallbackNoCallback:
    ...
    def test_no_callback_returns_approval_required(self):
        ...
        os.environ["NASTECH_EXEC_ASK"] = "1"
        os.environ["NASTECH_SESSION_KEY"] = "no-callback-test"
        try:
...
class TestCrossSessionApprovalIsolation:
    ...
    process-global ``os.environ["NASTECH_SESSION_KEY"]`` inside
    ...
    def test_contextvar_wins_over_clobbered_environ(self):
        ...
        os.environ["NASTECH_SESSION_KEY"] = "session-B"
    ...
    def test_unset_contextvar_does_not_fall_back_to_clobbered_environ(self):
        ...
        os.environ["NASTECH_SESSION_KEY"] = "session-B-stale"
    ...
    def test_approval_prompt_routes_to_originating_session(self):
        ...
        def worker_a():
            ...
            # gateway, which no longer writes NASTECH_SESSION_KEY).
            token = set_current_session_key("session-A")
            try:
                result_holder[0] = check_all_command_guards(
                    "rm -rf /important", "local"
    ...
    def test_two_concurrent_sessions_route_to_own_queue_contextvar_only(self):
        ...
        ``os.environ["NASTECH_SESSION_KEY"]``. This proves the contextvar is
        ...
        # No NASTECH_SESSION_KEY in os.environ at all — pure contextvar routing.
        os.environ.pop("NASTECH_SESSION_KEY", None)
        os.environ["NASTECH_GATEWAY_SESSION"] = "1"
        os.environ["NASTECH_EXEC_ASK"] = "1"
