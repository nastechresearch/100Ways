...
substrings like 'foo@nastech_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="nastech_bot")
    return adapter
...
def _telegram_mention_entity(text, mention="@nastech_bot", entity_type="mention"):
...
class TestRealMentionsAreDetected:
    ...
    def test_mention_after_non_bmp_characters_uses_telegram_offsets(self):
        adapter = _make_adapter()
        text = "\U0001f680\U0001f680 @nastech_bot hello"
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@nastech_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@nastech_bot.example")
    ...
    def test_substring_inside_url_without_entity(self):
        ...
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@nastech_bot for details")
        assert adapter._message_mentions_bot(msg) is False


    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@nastech_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestEntityEdgeCases:
    ...
    def test_malformed_entity_with_negative_offset(self):
        adapter = _make_adapter()
        msg = _message(text="@nastech_bot hi",
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @NASTECH_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@NASTECH_BOT")])
        assert adapter._message_mentions_bot(msg) is True
...
class TestTelegramUtf16EntityOffsets:
    def test_extracts_bot_mention_username_after_non_bmp_characters(self):
        text = "\U0001f9ea\U0001f9ea @nastech_bot please"
        msg = _message(text=text, entities=[_telegram_mention_entity(text)])
        assert TelegramAdapter._extract_bot_mention_usernames(msg) == {"nastech_bot"}

    def test_bot_command_suffix_after_non_bmp_characters_mentions_bot(self):
        adapter = _make_adapter()
        text = "\U0001f9ea\U0001f9ea /new@nastech_bot"
        entity = _telegram_mention_entity(text, mention="/new@nastech_bot", entity_type="bot_command")
