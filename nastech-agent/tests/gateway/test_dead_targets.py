...
def isolate(tmp_path, monkeypatch):
    monkeypatch.setattr("gateway.delivery.get_nastech_home", lambda: tmp_path)
    monkeypatch.setattr("gateway.dead_targets.get_nastech_home", lambda: tmp_path)
    return tmp_path
...
class TestDeadTargetRegistry:
    ...
    def test_persists_across_instances(self, isolate):
        ...
        # New instance reads the same on-disk store under tmp NASTECH_HOME.
