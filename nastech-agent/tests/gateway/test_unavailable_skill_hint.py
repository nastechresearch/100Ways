...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`nastech_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + NASTECH_HOME so the real user config is untouched."""
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `nastech skills config` writes).
    monkeypatch.setattr(
    ...
    assert "nastech skills config" in msg
