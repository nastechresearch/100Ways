...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_runtime_config(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestConcurrencyCap:
    ...
    def test_resolve_reads_config_value(self):
        ...
        with patch("nastech_cli.config.load_config", return_value=cfg):
    ...
    async def test_health_detailed_returns_ok(self, adapter):
        ...
        with patch("gateway.status.read_runtime_status", return_value={
            ...
            async with TestClient(TestServer(app)) as cli:
                ...
                assert data["platform"] == "nastech-agent"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_nastech_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "nastech-agent"
            assert data["data"][0]["owned_by"] == "nastech"
    ...
    def test_resolve_model_name_default_profile(self):
        """Default profile falls back to 'nastech-agent'."""
        with patch("nastech_cli.profiles.get_active_profile_name", return_value="default"):
            assert APIServerAdapter._resolve_model_name("") == "nastech-agent"
    ...
    async def test_model_options_returns_shared_inventory(self, adapter, monkeypatch):
        ...
        ctx = object()
        payload = {
            "providers": [{"slug": "nastech", "name": "Nastech Portal", "models": ["gpt-5.5"]}],
            "model": "gpt-5.5",
            "provider": "nastech",
        }
        seen = {"thread_calls": 0}
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "nastech.api_server.capabilities"
            assert data["platform"] == "nastech-agent"
            assert data["model"] == "nastech-agent"
            ...
            assert data["features"]["session_continuity_header"] == "X-Nastech-Session-Id"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "nastech_cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "nastech_cli.tools_config._get_platform_tools",
            return_value={"default"},
        ), patch(
            "nastech_cli.tools_config.get_nastech_subscription_features",
            return_value=feature_snapshot,
        ) as resolve_features, patch(
            "nastech_cli.tools_config._toolset_has_keys",
            return_value=True,
        ) as has_keys, patch(
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: nastech.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``nastech.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: nastech.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: nastech.tool.progress":
                    continue
    ...
    async def test_result_error_fallback_is_redacted(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nastech-agent", "input": "Hello"},
                )
    ...
    async def test_stream_task_done_callback_enqueues_eos_for_responses(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with (
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nastech-agent", "input": "hi", "stream": True},
                )
                assert resp.status == 200
    ...
    async def test_get_stored_response(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nastech-agent", "input": "Hi"},
                )
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "nastech-agent", "input": "What is 6*7?"},
                )
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "nastech-agent", "messages": [{"role": "user", "content": "hello"}]},
                )
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-Nastech-Error", "")
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["nastech"]["failed"] is True
...
# ---------------------------------------------------------------------------
# X-Nastech-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_traversal_session_id_header_rejected(self, auth_adapter):
        """Security (#5958): a path-traversal X-Nastech-Session-Id must be
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={"X-Nastech-Session-Id": bad, "Authorization": "Bearer sk-secret"},
                        json={"model": "nastech-agent", "messages": [{"role": "user", "content": "hi"}]},
                    )
    ...
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-Nastech-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Nastech-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
...
# ---------------------------------------------------------------------------
# X-Nastech-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Nastech-Session-Key": "agent:main:webui:dm:user-7",
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Nastech-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Nastech-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "nastech-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Nastech-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Nastech-Session-Key"
...
class TestApiKeyStartupGuardFailsClosed:
    ...
    def _blocking_auth_import():
        ...
        def _blocked(name, *args, **kwargs):
            if name == "nastech_cli.auth":
                raise ImportError("simulated: nastech_cli.auth unavailable")
...
class TestCreateAgentModelRecovery:
    def test_create_agent_defaults_to_provider_catalog_model_when_empty(self, monkeypatch):
        ...
        `nastech auth add openai-codex` without `nastech model`) —
        ...
        monkeypatch.setattr(
            "nastech_cli.models.get_default_model_for_provider",
