...
def _clear_env(monkeypatch):
    monkeypatch.delenv("NASTECH_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("NASTECH_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("NASTECH_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
