...
``nastech_cli.config`` (get_env_value / save_env_value / remove_env_value),
``nastech_cli.cli_output`` (prompt / prompt_yes_no / print_*), and
``nastech_cli.setup`` (prompt_choice); we patch each at its source module so
...
import nastech_cli.setup as setup_mod
