...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="NastechBot"),
    )
...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "nastech_cli.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("nastech_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
async def test_rename_thread_edits_only_when_current_name_matches(adapter):
    ...
    thread.edit.assert_awaited_once_with(
        ...
        reason="Nastech semantic session title",
    )
...
def test_register_skill_command_callback_dispatches_by_name(adapter):
    ...
    with patch(
        "nastech_cli.commands.discord_skill_commands_by_category",
