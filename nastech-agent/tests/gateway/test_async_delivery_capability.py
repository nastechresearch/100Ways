...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("NASTECH_SESSION_PLATFORM") == "api_server"
        finally:
...
class TestDeclareStatelessChannel:
    """``nastech -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``NASTECH_SESSION_*`` routing keys, so an async delegation's
