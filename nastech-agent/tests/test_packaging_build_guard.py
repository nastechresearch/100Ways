...
def _build_artifact(kind: str, tmp_path, *, nix_build: bool) -> subprocess.CompletedProcess[str]:
    ...
    if nix_build:
        env["NASTECH_NIX_BUILD"] = "1"
    else:
        env.pop("NASTECH_NIX_BUILD", None)
...
def test_artifact_build_rejects_nix_development_shell_environment(kind, tmp_path):
    ...
    assert "Building wheels or sdists for nastech-agent is not supported" in result.stderr


@pytest.mark.parametrize(
    ("kind", "artifact_glob"),
    [("sdist", "nastech_agent-*.tar.gz"), ("wheel", "nastech_agent-*.whl")],
)
