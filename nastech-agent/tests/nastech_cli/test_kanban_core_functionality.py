...
Complements tests/nastech_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/nastech_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/nastech_cli/test_kanban_db.py.
    monkeypatch.setenv("NASTECH_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    nastech subprocess (which would hang trying to call an LLM).
    """
    captured = {}
    ...
    env = captured["env"]
    assert env.get("NASTECH_KANBAN_TASK") == tid
    assert env.get("NASTECH_PROFILE") == "some-profile"
...
def test_cli_daemon_help_marks_deprecated():
    ...
    from nastech_cli import kanban as kb_cli
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `nastech kanban --help` output"
    )
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``nastech_cli.kanban_db`` fresh and uses that single
