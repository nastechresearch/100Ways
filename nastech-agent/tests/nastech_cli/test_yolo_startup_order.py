"""Regression tests for #60328: --yolo must set NASTECH_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing NASTECH_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("NASTECH_YOLO_MODE")

    monkeypatch.setattr(
        "nastech_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("nastech_cli.main.cmd_chat", lambda args: None)
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """nastech --yolo must set NASTECH_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["nastech", "--yolo"]
    )
    assert result == "1", (
        "NASTECH_YOLO_MODE was not '1' when _prepare_agent_startup was "
