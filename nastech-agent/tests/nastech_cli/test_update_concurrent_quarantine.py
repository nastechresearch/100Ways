"""Tests for issue #26670 — concurrent nastech.exe detection and improved
quarantine retry / reboot-deferred fallback during `nastech update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_nastech_instances
# helper (and need the autouse stub in tests/nastech_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "nastech.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "nastech.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_nastech_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "nastech.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_nastech_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "nastech.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_nastech_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("nastech.exe.old.")
...
def test_quarantine_falls_back_to_reboot_schedule(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_nastech_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    import nastech_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "nastech_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "nastech_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m nastech_cli.main gateway run"
