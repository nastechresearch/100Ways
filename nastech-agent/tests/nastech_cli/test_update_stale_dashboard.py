"""Tests for the stale-dashboard handling run at the end of ``nastech update``.

``nastech update`` detects ``nastech dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``nastech_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``nastech_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from nastech_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"nastech_cli.main.X\")``
    ...
    live = sys.modules.get("nastech_cli.main")
    if live is None:
        live = importlib.import_module("nastech_cli.main")
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m nastech_cli.main dashboard"),
                    _ps_line(12345, "nastech dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "nastech-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="nastech-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "nastech-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "nastech-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "nastech-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("nastech_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "nastech-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "nastech-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "nastech-dashboard.service"],
            ["systemctl", "--user", "restart", "nastech-dashboard.service"],
        ]
        assert all(call[:1] != ["sudo"] and call[:2] != ["systemctl"] for call in calls)
        find_pids.assert_not_called()
        kill.assert_not_called()
        assert "✓ restarted nastech-dashboard.service" in capsys.readouterr().out
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "nastech_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `nastech update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m nastech_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["nastech_cli.main"]
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(live, "_get_pid_cgroup_path",
                          return_value="/system.slice/nastech-serve.service"), \
             patch.object(live, "_get_systemd_service_for_pid",
                          return_value="nastech-serve.service"), \
        ...
        restart.assert_called_once_with(
            "nastech-serve.service", "/system.slice/nastech-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service nastech-serve.service" in out
...
class TestManualBackendRespawn:
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["nastech", "dashboard", "--port", "8300"],
                ["nastech", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["nastech", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["nastech", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["nastech", "serve"]])

        assert failed == [["nastech", "serve"]]
        out = capsys.readouterr().out
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00nastech_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "nastech_cli.main", "serve"]
