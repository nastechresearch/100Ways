"""Tests for nastech_cli.auth._default_verify platform-aware fallback.
...
from nastech_cli.auth import _default_verify, _resolve_verify
...
class TestResolveVerifyIntegration:
    ...
    def test_no_ca_uses_default_verify_on_linux(self, monkeypatch):
        for var in ("NASTECH_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE"):
    ...
    def test_insecure_wins_over_everything(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NASTECH_CA_BUNDLE", str(bundle))
