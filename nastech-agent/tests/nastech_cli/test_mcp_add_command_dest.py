"""Regression test: ``nastech mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``nastech_cli/_parser.py``).  The dispatcher in ``nastech_cli/main.py``
...
result, ``nastech mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the nastech parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`nastech mcp add foo --url ...` must keep args.command == "mcp".
