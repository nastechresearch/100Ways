"""Tests for ``nastech debug`` CLI command and debug utilities."""
...
def nastech_home(tmp_path, monkeypatch):
    """Set up an isolated NASTECH_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO nastech_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from nastech_cli.debug import _upload_paste_rs
        ...
        with patch("nastech_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "nastech_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from nastech_cli.debug import upload_to_pastebin

        with patch("nastech_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("nastech_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, nastech_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = nastech_home / "logs" / "agent.log"
        from nastech_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, nastech_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from nastech_cli.debug import _capture_log_snapshot
        ...
        (nastech_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `nastech debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, nastech_home):
        from nastech_cli.debug import _capture_log_snapshot

        (nastech_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, nastech_home):
        from nastech_cli.debug import _capture_log_snapshot

        (nastech_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "Nastech Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(nastech_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, nastech_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, nastech_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from nastech_cli.debug import _capture_log_snapshot

        (nastech_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, nastech_home):
        """The uploaded report — what people paste into support — must explain it."""
        from nastech_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of NASTECH_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def nastech_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated NASTECH_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".nastech"
        home.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(home))
    ...
    def test_default_redacts_tail_and_full_text(self, nastech_home_with_secret):
        from nastech_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, nastech_home_with_secret):
        from nastech_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, nastech_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when NASTECH_REDACT_SECRETS=false, and the share-time
        ...
        assert os.environ.get("NASTECH_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, nastech_home_with_secret
    ):
        from nastech_cli.debug import _capture_log_snapshot

        log_path = nastech_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, nastech_home_with_secret):
        from nastech_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from nastech_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, nastech_home):
        from nastech_cli.debug import collect_debug_report

        with patch("nastech_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- nastech dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- nastech dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, nastech_home, capsys):
        ...
        with patch("nastech_cli.dump.run_dump"), \
             patch("nastech_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("nastech_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, nastech_home, capsys):
        ...
        with patch("nastech_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- nastech dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, nastech_home_with_secret, capsys
    ):
        ...
        with patch("nastech_cli.dump.run_dump"), \
             patch("nastech_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("nastech_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "nastech debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, nastech_home):
        from nastech_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from nastech_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from nastech_cli.debug import delete_paste
        ...
        with patch("nastech_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.nastech/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``nastech debug``
    invocation.
    """


    def test_records_pending_to_json(self, nastech_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from nastech_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, nastech_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from nastech_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, nastech_home):
        ...
        with patch("nastech_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, nastech_home):
        from nastech_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("nastech_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, nastech_home):
        ...
        with patch(
            "nastech_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, nastech_home):
        from nastech_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("nastech_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from nastech_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("nastech_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``nastech debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, nastech_home):
        from nastech_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (nastech_home / "logs" / "agent.log").write_text(
        ...
        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("nastech_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, nastech_home):
        from nastech_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + Nastech-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:

    def test_no_redact_omits_banner(self, nastech_home):
        from nastech_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, nastech_home):
        from nastech_cli.debug import collect_share_bundle
...
class TestBuildNastechBundle:
    def test_envelope_shape_and_gzip(self, nastech_home):
        import gzip
        import json as _json

        from nastech_cli.debug import build_nastech_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_nastech_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "nastech-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_nastech_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugShareNastech:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_nastech_success_prints_view_url(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share
        ...
        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.diagnostics_upload.share_to_nastech", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "Nastech-INTERNAL" in out
        assert "https://support.example.com/diagnostics/id-1" in out
        assert "2026-06-20T00:00:00Z" in out
        # The blob passed to share_to_nastech must be gzip bytes.
        blob = share.call_args[0][0]
        assert isinstance(blob, (bytes, bytearray)) and blob[:2] == b"\x1f\x8b"

    def test_nastech_failure_suggests_local(self, nastech_home, capsys):
        from nastech_cli.debug import run_debug_share

        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.diagnostics_upload.share_to_nastech",
        ...
        assert "Nastech upload failed" in err
        assert "--local" in err

    def test_nastech_does_not_touch_pastebin(self, nastech_home):
        from nastech_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("nastech_cli.dump.run_dump"), patch(
            "nastech_cli.diagnostics_upload.share_to_nastech", return_value=res
        ), patch("nastech_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [nastech|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``NastechCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("nastech_cli.debug.run_debug_share", _fake_run):
            self._handler()(cmd_original)
        return captured

    def test_bare_debug_defaults_to_paste(self):
        c = self._captured("/debug")
        assert c["nastech"] is False and c["local"] is False
    ...
    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug NASTECH")
        assert c["nastech"] is True
...
class TestShareConsentGate:
    """`nastech debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, nastech=False,
    ...
    def test_non_interactive_requires_yes(self, nastech_home, capsys, monkeypatch):
        ...
        with patch("nastech_cli.dump.run_dump"), \
             patch("nastech_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, nastech_home, capsys, monkeypatch):
