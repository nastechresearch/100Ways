...
from nastech_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``nastech_session_at`` / ``nastech_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a self-hosted-OIDC session hits the ``nastech``
    provider first, which tries to reach Nastech Portal's JWKS; if that's
    unreachable ``nastech`` raises ProviderError. The gate must keep trying the
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["nastech_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
