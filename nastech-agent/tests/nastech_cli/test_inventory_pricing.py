...
columns + Free/Pro badges and gate paid models on free Nastech accounts, the
same way the `nastech model` CLI picker does.
"""

import nastech_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_nastech_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_omits_sale_for_free_models_even_with_original(monkeypatch):
    ...
    rows = [{"slug": "nastech", "models": ["a/free"]}]
    inv._apply_pricing(rows)
