...
reset).  Nastech must detect that and lift the stale local cooldown instead
...
def _write_auth_store(nastech_home, payload):
...
def test_resolver_recovers_when_probe_confirms_reset(tmp_path, monkeypatch):
    ...
    _write_auth_store(nastech_home, _pool_only_rate_limited_store())
    ...
    store = json.loads((nastech_home / "auth.json").read_text())
...
def test_pool_probe_not_fired_for_non_quota_exhaustion(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path / "nastech", store)
