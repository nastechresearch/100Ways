...
from nastech_cli.memory_setup import _CANCELLED, _curses_select
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_nastech_home", lambda: tmp_path)
    monkeypatch.setattr("nastech_cli.config.load_config", lambda: {"memory": {}})
    monkeypatch.setattr("nastech_cli.config.save_config", save_config)
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on nastech update."""
