...
from nastech_cli.slack_cli import _build_full_manifest, slack_manifest_command
from nastech_cli.subcommands.slack import build_slack_parser


def _parse_slack_args(argv):
    """Build the real `nastech slack` parser and parse argv against it."""
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from nastech_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from nastech_cli import main as main_module
    from nastech_cli import slack_cli
...
class TestSlackManifestArgparse:
    ...
    def test_long_description_file_reports_tilde_expansion_errors(
        ...
        source = "~nastech-user-that-does-not-exist-20260716/AGENTS.md"
...
class TestSlackFullManifest:
    """Generated full Slack app manifest used by `nastech slack manifest`."""
    ...
    def test_assistant_features_remain_enabled(self):
        manifest = _build_full_manifest("Nastech", "Your Nastech agent on Slack")
    ...
    def test_no_assistant_preserves_core_surface(self):
        ...
        manifest = _build_full_manifest(
            "Nastech", "Your Nastech agent on Slack", include_assistant=False
        )
