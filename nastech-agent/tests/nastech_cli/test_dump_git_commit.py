"""Tests for nastech_cli.dump._get_git_commit — git SHA resolution for ``nastech dump``.

``nastech dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``NASTECH_GIT_SHA`` build-arg.
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    with patch("nastech_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("nastech_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``nastech dump`` doesn't have to special-case
    ...
    with patch("nastech_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("nastech_cli.dump.subprocess.run", return_value=failed), \
         patch("nastech_cli.build_info.get_build_sha", return_value="b2f477a3"):
