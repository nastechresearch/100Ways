...
prefix (e.g. ``mission-control.tilos.com/nastech/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /nastech`` on every request.

The dashboard already honours this for the SPA bundle (rewriting asset
URLs and the bootstrap ``__NASTECH_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/nastech/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/nastech/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/nastech`` when behind a prefix so they
...
from nastech_cli.dashboard_auth import clear_providers, register_provider
from nastech_cli.dashboard_auth import prefix as prefix_mod
...
class TestForwardedPrefixNormalisation:
    def test_home_assistant_ingress_prefix_with_subpath_is_accepted(
        ...
        the SPA receives the correct __NASTECH_BASE_PATH__ and asset prefix."""
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/nastech"},
        ...
        assert r.headers["location"].startswith("/nastech/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/nastech/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/nastech/auth/callback`` — the
        ...
        assert parsed.path == "/nastech/auth/callback", (
...
# ---------------------------------------------------------------------------
# NASTECH_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``NASTECH_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``nastech_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "nastech_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "NASTECH_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("NASTECH_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set NASTECH_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("NASTECH_DASHBOARD_PUBLIC_URL", "nastech.domain.com")
        ...
        assert any(
            "NASTECH_DASHBOARD_PUBLIC_URL" in m
            and "nastech.domain.com" in m
            and "scheme" in m
            for m in warnings
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-nastech_session_pkce=")
        ]
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from nastech_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("nastech_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/nastech, so the next request under
        ...
        (``/nastech/auth/login``, ``/nastech/auth/callback``). A cookie
        set with ``Path=/nastech`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/nastech/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "nastech_session_pkce" in c
        )
        # Parse "__Secure-nastech_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-nastech_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/nastech cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/nastech",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-nastech_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/nastech" in at_cookies[0]
