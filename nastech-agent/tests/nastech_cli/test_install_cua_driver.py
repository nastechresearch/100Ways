...
* Be supported-platform-only — no-op silently elsewhere so ``nastech update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Nastech
...
class TestRequireConfirmedUpdate:
    """`nastech update` passes require_confirmed_update=True: the full
    ...
    Explicit `nastech computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `nastech computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `nastech update`.
    """
