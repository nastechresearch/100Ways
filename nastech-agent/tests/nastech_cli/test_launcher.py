"""Tests for the top-level `./nastech` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nastech` should use `nastech_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nastech"
    called = []

    fake_main_module = types.ModuleType("nastech_cli.main")

    def fake_main():
        called.append("nastech_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nastech_cli.main", fake_main_module)
    ...
    assert called == ["nastech_cli.main"]
