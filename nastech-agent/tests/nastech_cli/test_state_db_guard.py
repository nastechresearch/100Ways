...
from nastech_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    NASTECH_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def nastech_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("nastech_cli.config") or mod == "nastech_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, nastech_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, nastech_home, capsys, monkeypatch):
