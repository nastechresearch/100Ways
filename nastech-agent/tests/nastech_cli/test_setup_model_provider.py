...
from nastech_cli.main, these tests mock the delegation point and verify
...
from nastech_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("nastech_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("nastech_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside nastech setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("nastech_cli.main.select_provider_and_model", fake_select)
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("nastech_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("nastech_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    ``get_nastech_subscription_features`` so the surface stays aligned with the
    ...
    monkeypatch.setattr("nastech_cli.nastech_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
