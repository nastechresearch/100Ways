"""Tests for the per-profile Projects store (nastech_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Nastech Agent", folders=["/tmp/nastech"])
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "nastech-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' NASTECH_HOME.
