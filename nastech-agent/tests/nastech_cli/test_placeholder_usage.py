...
from nastech_cli.config import config_command, show_config
from nastech_cli.setup import _print_setup_summary
...
def test_config_set_usage_marks_placeholders(capsys):
    ...
    assert "Usage: nastech config set" in out
    assert "<key>" in out
    assert "<value>" in out
...
def test_show_config_marks_placeholders(tmp_path, capsys):
    ...
    out = capsys.readouterr().out
    assert "nastech config set <key> <value>" in out
