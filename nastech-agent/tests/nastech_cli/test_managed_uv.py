"""Tests for nastech_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from nastech_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from nastech_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path):
            from nastech_cli.managed_uv import managed_uv_path
...
class TestResolveUv:
    ...
    def test_existing_executable(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path):
            from nastech_cli.managed_uv import resolve_uv
            result = resolve_uv()
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("nastech_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from nastech_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "nastech_cli.managed_uv.get_nastech_home",
            return_value=tmp_path,
        ), patch(
            "nastech_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "nastech_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``nastech update`` runs the call site from the old, already-imported
    ``nastech_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("nastech_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from nastech_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from nastech_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\nastech\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from nastech_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from nastech_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / "uv"
        _make_executable(uv)
        # Fresh stamp under the isolated NASTECH_HOME.
        import nastech_constants
        stamp = nastech_constants.get_nastech_home() / "cache" / ".uv_self_update_stamp"
        stamp.parent.mkdir(parents=True, exist_ok=True)
        stamp.touch()

        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv.subprocess.run") as mock_run, \
             patch(
                 "nastech_cli.managed_uv.repair_vulnerable_runtime",
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from nastech_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from nastech_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".nastech-runtime" / "python"
        assert alpha == expected
        assert beta == expected

    def test_environment_is_private_and_sanitized(self, tmp_path):
        from nastech_cli.managed_uv import managed_python_env
        ...
        assert env["UV_PYTHON_INSTALL_DIR"] == str(
            checkout / ".nastech-runtime" / "python"
        )
        for key in (
            "CONDA_DEFAULT_ENV",
            "CONDA_PREFIX",
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from nastech_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "nastech_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "nastech_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".nastech-runtime").exists()
        mock_install.assert_not_called()

    def test_stage_candidate_sync_keeps_uv_project_config(self, tmp_path):
        from nastech_cli.managed_uv import _stage_candidate_venv

        root = tmp_path / "checkout"
        root.mkdir()
        (root / "uv.lock").write_text("# lock\n", encoding="utf-8")
        generation = root / ".nastech-runtime" / "python" / "gen"
        ...
        with patch("nastech_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch(
                 "nastech_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".nastech-runtime" / "python" / "generation-test"
        ...
        with patch(
             ...
             patch(
                 "nastech_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
        ...
        reacquired = _acquire_repair_lock(root / ".nastech-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".nastech-runtime" / "venv-candidate"
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from nastech_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".nastech-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from nastech_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".nastech-runtime"
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv._install_uv_posix") as mock_posix:
            from nastech_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from nastech_cli.managed_uv import _runtime_request
    ...
    def _run_generation(tmp_path, monkeypatch, current_version, candidate_version):
        ...
        import nastech_cli.managed_uv as managed_uv
        from nastech_cli.sqlite_runtime import SQLiteRuntimeInfo
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv._install_uv"), \
             patch(
                 "nastech_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "nastech_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".nastech-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every nastech update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from nastech_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a nastech_constants predating its symbol.

    ``nastech update`` imports nastech_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from nastech_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'nastech_constants'
        (~/.nastech/nastech-agent/nastech_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from nastech_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(nastech_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/nastech/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/nastech/venv/bin/python")
        assert _venv_python(Path("/opt/nastech/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through nastech_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_nastech_cli` bans.
        """
        import nastech_constants
        ...
        assert _venv_python(Path("/opt/nastech/venv")) == sentinel
