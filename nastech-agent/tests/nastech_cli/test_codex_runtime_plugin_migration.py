...
from nastech_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_nastech_tools_mcp_entry,
    _format_toml_value,
...
# ---- end-to-end migrate(, expose_nastech_tools=False) ----
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from nastech_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_nastech_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(nastech_cfg, codex_home=tmp_path, expose_nastech_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Nastech' managed block, re-running
        ...
        migrate({"mcp_servers": {"nastech-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.nastech-mcp]" in final
    ...
    def test_summary_reports_migration_count(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_nastech_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``nastech codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_nastech_tools=False)
...
class TestNastechHomeLeakGuard:
    ...
    once codex spawned the nastech-tools MCP subprocess.
    """




    def test_real_nastech_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate NASTECH_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.nastech"
        monkeypatch.setenv("NASTECH_HOME", real_path)
        entry = _build_nastech_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("NASTECH_HOME") == real_path

    def test_unset_nastech_home_omits_env_key(self, monkeypatch):
        """When NASTECH_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever NASTECH_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_nastech_tools_mcp_entry()
        env = entry.get("env", {})
        assert "NASTECH_HOME" not in env, (
            f"NASTECH_HOME should not be set when env var is unset, got: "
            f"{env.get('NASTECH_HOME')!r}"
        )
