...
from nastech_cli.copilot_auth import _try_gh_cli_token
...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        assert "nastech" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["nastech"].auth_type == "oauth_device_code"
...
from nastech_cli.auth import PROVIDER_REGISTRY as _REGISTRY

_EXTRA_ENV_VARS = (
    # Checked directly in resolve_provider("auto"), not via the registry.
    "OPENROUTER_API_KEY", "NASTECH_API_KEY",
    ...
    "NASTECH_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "NASTECH_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth._load_auth_store", lambda: {})
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "nastech_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "nastech_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("nastech_cli.copilot_auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        from nastech_cli.runtime_provider import resolve_runtime_provider
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "nastech_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "nastech_cli.models.fetch_github_model_catalog",
    ...
    def test_runtime_copilot_acp_uses_process_runtime(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setenv("NASTECH_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Nastech is unconfigured."""
        from nastech_cli import config as config_module
        from nastech_cli.auth import PROVIDER_REGISTRY
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: nastech_home / ".env")
        monkeypatch.setattr(config_module, "get_nastech_home", lambda: nastech_home)
        monkeypatch.setattr("nastech_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("nastech_cli.auth.get_auth_status", lambda _pid: {})
        ...
        from nastech_cli.main import _has_any_provider_configured
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        config_file = nastech_home / "config.yaml"
    ...
    def test_config_provider_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        """
        import yaml
        nastech_home = self._setup_home(monkeypatch, tmp_path)
        (nastech_home / "config.yaml").write_text(yaml.dump({
            "model": {"default": "anthropic/claude-opus-4.6", "provider": "openrouter"},
        }))
        sweep_calls = []
        ...
        monkeypatch.setattr("nastech_cli.auth.get_auth_status", _trap)
        from nastech_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert sweep_calls == [], (
    ...
    def test_auth_json_skips_registry_sweep(self, monkeypatch, tmp_path):
        ...
        def _guarded_status(provider_id):
            ...
            assert provider_id == "nastech", "sweep must be skipped"
            return {"logged_in": True}

        monkeypatch.setattr("nastech_cli.auth.get_auth_status", _guarded_status)
        from nastech_cli.main import _has_any_provider_configured
        assert _has_any_provider_configured() is True
        assert calls == ["nastech"], (
...
class TestKimiCodeCredentialAutoDetect:
    ...
    def test_non_kimi_providers_unaffected(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
...
class TestZaiEndpointAutoDetect:
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestZaiParallelProbe:
    ...
    def test_candidate_model_fallback_within_endpoint(self, monkeypatch):
        ...
        from nastech_cli.auth import ZAI_ENDPOINTS, detect_zai_endpoint
    ...
    def test_priority_order_wins_over_completion_order(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.auth.httpx.post", _slow_first)
        result = detect_zai_endpoint("test-key", timeout=1.0)
        assert result is not None
        assert result["id"] == first[0]

    def test_all_fail_returns_none(self, monkeypatch):
        from nastech_cli.auth import detect_zai_endpoint

        monkeypatch.setattr("nastech_cli.auth.httpx.post", self._mock_post({}))
    ...
    def test_early_exit_does_not_wait_for_slow_losers(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.auth.httpx.post", _slow_losers)
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from nastech_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from nastech_cli.main import _PROVIDER_MODELS as main_models
        from nastech_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from nastech_cli import models as models_mod
...
def _deepinfra_cache_isolation(monkeypatch):
    ...
    """
    import nastech_cli.models as _models_mod
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        from nastech_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from nastech_cli.models import _fetch_deepinfra_models_by_tag
        import nastech_cli.models as _m
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from nastech_cli.auth import PROVIDER_REGISTRY, resolve_provider
        from nastech_cli.config import OPTIONAL_ENV_VARS
        from nastech_cli.models import CANONICAL_PROVIDERS
