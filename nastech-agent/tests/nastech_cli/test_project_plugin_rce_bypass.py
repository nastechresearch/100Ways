...
Execution via the ``NASTECH_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``nastech_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.nastech/plugins/`` source via
   ``os.environ.get("NASTECH_ENABLE_PROJECT_PLUGINS")`` — truthy for
...
2. ``nastech_cli.web_server._mount_plugin_api_routes`` then imported
...
# ---------------------------------------------------------------------------
# Layer 1 — NASTECH_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.nastech/plugins``
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("NASTECH_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("NASTECH_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"NASTECH_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
...
class TestEndToEndPocBlocked:
    ...
    ``NASTECH_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_ENABLE_PROJECT_PLUGINS", "0")
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "nastech_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "nastech_dashboard_plugin_evil" not in sys.modules
