"""Tests for nastech_cli/completion.py — shell completion script generation."""
...
from nastech_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real nastech structure."""
    p = argparse.ArgumentParser(prog="nastech")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_nastech_completion()" in out
        assert "complete -F _nastech_completion nastech" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_nastech_profiles'" in out
...
class TestProfileCompletion:
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'nastech profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _nastech_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_nastech_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _nastech_profiles"
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__nastech_profiles" in out
        count = out.count("(__nastech_profiles)")
