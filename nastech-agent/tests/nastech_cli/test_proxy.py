"""Tests for the `nastech proxy` subcommand and its upstream adapters."""
...
# ---------------------------------------------------------------------------
# NastechPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(nastech_home: Path, nastech_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nastech state into a hermetic NASTECH_HOME."""
    auth_path = nastech_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"nastech": nastech_state},
    }))
    return auth_path
...
def test_nastech_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "nastech_cli.proxy.adapters.nastech_portal.resolve_nastech_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic NASTECH_HOME."""
    auth_path = nastech_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from nastech_cli.proxy.server import create_app  # noqa: E402
