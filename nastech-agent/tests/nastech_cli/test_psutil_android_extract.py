...
from nastech_cli.psutil_android import (
    MARKER,
    REPLACEMENT,
    PSUTIL_URL,
...
def test_install_psutil_android_compat_uses_patched_tree(tmp_path):
    ...
    with patch("urllib.request.urlretrieve", side_effect=fake_urlretrieve), \
         patch.object(nastech_main, "_run_install_with_heartbeat", side_effect=fake_run_install):
        nastech_main._install_psutil_android_compat(
            ["uv", "pip"],
            env={"NASTECH_TEST": "1"},
        )

    assert captured["cmd"][:4] == ["uv", "pip", "install", "--no-build-isolation"]
    assert captured["env"] == {"NASTECH_TEST": "1"}
