...
`NASTECH_REDACT_SECRETS` at module-import time. `nastech_cli/main.py` at
...
in .env), the toggle was silently ignored in both `nastech chat` and
`nastech gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `NASTECH_REDACT_SECRETS`
env var in `nastech_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports nastech_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("NASTECH_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import nastech_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('NASTECH_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["NASTECH_HOME"] = str(nastech_home)
    env.pop("NASTECH_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env NASTECH_REDACT_SECRETS takes precedence over config.yaml."""
    nastech_home = tmp_path / ".nastech"
    nastech_home.mkdir()
    (nastech_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (nastech_home / ".env").write_text("NASTECH_REDACT_SECRETS=true\n")
