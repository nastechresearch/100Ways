"""Test the platform-branched PTY bridge import in nastech_cli.web_server.
...
def test_web_server_uses_posix_pty_bridge_on_posix():
    ...
    from nastech_cli.pty_bridge import PtyBridge as PosixBridge
    from nastech_cli.pty_bridge import PtyUnavailableError as PosixErr
...
def test_pty_bridge_import_block_is_platform_branched():
    ...
    #   else:
    #       try:
    #           from nastech_cli.pty_bridge import PtyBridge, PtyUnavailableError
    #       ...
    assert 'sys.platform.startswith("win")' in src or "sys.platform.startswith('win')" in src
    assert "from nastech_cli.win_pty_bridge import" in src
    assert "from nastech_cli.pty_bridge import" in src
