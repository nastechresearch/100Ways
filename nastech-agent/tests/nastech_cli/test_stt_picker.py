"""Tests for the Speech-to-Text category in `nastech tools` (tools_config).
...
from nastech_cli.tools_config import (  # noqa: E402
...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from nastech_cli.nastech_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY

        managed = [p for p in _stt_cat()["providers"] if p.get("managed_nastech_feature")]
        assert managed, "expected a Nastech Subscription row"
        for p in managed:
            assert p["managed_nastech_feature"] == "stt"
    ...
    def test_apply_provider_selection_stt(self):
        ...
        with patch(
            ...
            feats.return_value = MagicMock(
                nastech_auth_present=False, account_info=None
            )
...
class TestModelPicker:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "nastech_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from nastech_cli.tools_config import _POST_SETUP_READY
