...
fires SIGTERM + app.quit() and spawns nastech-setup, but its Python backend
(``python.exe -m nastech_cli.main serve``) survives the teardown race. The
Desktop is gone — nothing will respawn that backend — yet the venv-holder
guard refused on it and the update dead-ended with "Nastech is still running"
...
reap (with its full child tree — the managed .nastech-runtime interpreter
...
def _holders(pid=200, cmdline="python.exe -m nastech_cli.main serve"):
...
def test_backend_with_live_parent_keeps_refusal():
    parent = _proc(50, ["Nastech.exe"], create_time=10.0)
...
def test_non_backend_holder_keeps_refusal():
    repl = _proc(300, ["python.exe", "-m", "nastech_cli.main", "chat"], ppid=999)
    fake = _fake_psutil({300: repl})
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = _holders(pid=300, cmdline="python.exe -m nastech_cli.main chat")
...
def test_orphan_root_plus_managed_runtime_descendant_qualifies():
    ...
    # serve root and its .nastech-runtime interpreter child. The child's live
    # parent IS the orphan root, so the set is safe — only the root is
    # returned (taskkill /T reaps the descendant with it).
    backend = _proc(200, _SERVE_ARGV, ppid=999)
    child_argv = [
        "C:\\nastech\\.nastech-runtime\\python\\generation-1\\python.exe",
        "worker.py",
    ]
