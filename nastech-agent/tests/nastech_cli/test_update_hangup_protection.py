"""Tests for SIGHUP protection and stdout mirroring in ``nastech update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``nastech_cli/main.py``.  These exist so
that ``nastech update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_ACTION_ID", "a" * 32)

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== nastech-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_ACTION_ID", "not-safe\nforged")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        import nastech_cli.config as _cfg
        if hasattr(_cfg, "_NASTECH_HOME_CACHE"):
            _cfg._NASTECH_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "nastech update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_nastech_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "nastech_cli.config.get_nastech_home", _boom, raising=True
        )
