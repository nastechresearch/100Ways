"""Tests for ``nastech dashboard register``.
...
The portal HTTP call and the Nastech token resolution are both mocked — this
...
import nastech_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        err = AuthError("not logged in", provider="nastech", relogin_required=True)
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "nastech_cli.auth.resolve_nastech_access_token", side_effect=err
            ), patch("nastech_cli.config.is_managed", return_value=False):
                with pytest.raises(SystemExit) as exc:
                    dr.cmd_dashboard_register(_ns())
        assert exc.value.code == 1
        out = capsys.readouterr().out
        assert "not logged into Nastech Portal" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nastechresearch.com",
        ...
        # (idempotency key) and once for NASTECH_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "NASTECH_DASHBOARD_OAUTH_CLIENT_ID":
                return existing_client_id
            return None

        with patch(
            "nastech_cli.auth.resolve_nastech_access_token", return_value=account_token
        ), patch("nastech_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "nastech_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "nastech_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["NASTECH_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the NASTECH_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["NASTECH_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / NASTECH_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        NASTECH_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "NASTECH_DASHBOARD_PORTAL_URL":
        ...
        with patch(
            ...
            # The ambient process env may carry NASTECH_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("NASTECH_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists NASTECH_DASHBOARD_PUBLIC_URL in .env.

    --redirect-uri is the full public callback (e.g.
    https://nastech.example.com/auth/callback). At serve time the dashboard auth
    layer reconstructs that callback by appending "/auth/callback" to
    NASTECH_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as NASTECH_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        NASTECH_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "NASTECH_DASHBOARD_PUBLIC_URL":
    ...
    def test_no_redirect_flag_does_not_overwrite_existing(self, capsys):
        ...
        assert "NASTECH_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://nastech.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "nastech_cli.config.get_env_value", return_value=None
        ), patch(
            "nastech_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://nastech.example.com/auth/callback",
                )
            )
        assert saved["NASTECH_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["NASTECH_DASHBOARD_PUBLIC_URL"] == "https://nastech.example.com"
