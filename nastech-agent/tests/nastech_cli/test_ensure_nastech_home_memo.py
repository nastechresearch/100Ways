"""ensure_nastech_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".nastech"
    second = tmp_path / "b" / ".nastech"

    monkeypatch.setenv("NASTECH_HOME", str(first))
    cfg.ensure_nastech_home()

    # Profile switch: NASTECH_HOME moves → the new path is ensured too.
    monkeypatch.setenv("NASTECH_HOME", str(second))
    cfg.ensure_nastech_home()
