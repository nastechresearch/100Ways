...
def _build_app(use_https: bool = True, prefix: str = ""):
    ...
    def set_endpoint():
        ...
        set_session_cookies(
            ...
            prefix=prefix, provider="nastech",
        )
        return r
...
def test_session_cookies_use_host_prefix_on_https_direct():
    ...
    provider = next(c for c in cookies if c.startswith(f"__Host-{SESSION_PROVIDER_COOKIE}=nastech"))
...
def test_session_cookies_use_secure_prefix_when_proxied():
    """HTTPS + /nastech prefix → __Secure- prefix (__Host- forbids
    Path != "/"; __Secure- keeps the Secure-required hardening)."""
    client = TestClient(_build_app(use_https=True, prefix="/nastech"))
    r = client.get("/set")
    cookies = r.headers.get_list("set-cookie")
    at = next(c for c in cookies if c.startswith(f"__Secure-{SESSION_AT_COOKIE}="))
    assert "Path=/nastech" in at
    assert "Secure" in at
