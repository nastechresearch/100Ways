...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # Nastech Portal opts in via include_sale_original=True.
...
def test_resolve_nastech_pricing_credentials_honors_inference_env_override(monkeypatch):
    """Staging profiles set NASTECH_INFERENCE_BASE_URL — pricing must follow it.

    Without this, anonymous/failed-auth fallback hits prod and sale
    ``pricing.original`` never reaches Desktop/CLI pickers.
    """
    monkeypatch.setenv(
        "NASTECH_INFERENCE_BASE_URL",
        "https://stg-inference-api.nastechresearch.com/v1",
    )
    ...
    assert api_key == ""
    assert base_url == "https://stg-inference-api.nastechresearch.com/v1"
