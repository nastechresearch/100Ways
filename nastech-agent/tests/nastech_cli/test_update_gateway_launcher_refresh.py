...
2. ``nastech_cli.main._refresh_windows_gateway_launchers`` — ``nastech
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "nastech_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("nastech_cli.config.get_nastech_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: nastech update regenerates launchers
# ---------------------------------------------------------------------------
