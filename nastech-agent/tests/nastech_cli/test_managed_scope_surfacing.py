"""Surfacing tests — managed scope shown in `config show` and `nastech doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    home.mkdir()
    monkeypatch.setenv("NASTECH_HOME", str(home))
    monkeypatch.setenv("NASTECH_MANAGED_DIR", str(tmp_path / "nope"))
    (home / "config.yaml").write_text("model:\n  default: user/model\n", encoding="utf-8")
    import nastech_cli.config as cfg
    from nastech_cli import managed_scope
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("NASTECH_MANAGED_DIR", str(tmp_path / "nope"))
    from nastech_cli import managed_scope, doctor
