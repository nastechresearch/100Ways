"""Tests for the resolve_nastech_access_token startup-burst memo (PR #66016).
...
def test_repeat_calls_within_ttl_hit_memo(monkeypatch, tmp_path):
    ...
    first = auth.resolve_nastech_access_token()
    second = auth.resolve_nastech_access_token()
    third = auth.resolve_nastech_access_token()
...
def test_insecure_callers_bypass_memo(monkeypatch, tmp_path):
    ...
    auth.resolve_nastech_access_token()
    auth.resolve_nastech_access_token(insecure=True)
