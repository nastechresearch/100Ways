...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NASTECH_HOME with .container-mode file."""
    nastech_home = tmp_path / ".nastech"
    nastech_home.mkdir()
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.delenv("NASTECH_DEV", raising=False)

    container_mode = nastech_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nastech-agent\n"
        "exec_user=nastech\n"
        "nastech_bin=/data/current-package/bin/nastech\n"
    )
    return nastech_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nastech_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nastech-agent"
    assert info["exec_user"] == "nastech"
    assert info["nastech_bin"] == "/data/current-package/bin/nastech"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nastech_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "nastech"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "nastech-agent" in cmd
    assert "/data/current-package/bin/nastech" in cmd
    assert "chat" in cmd
