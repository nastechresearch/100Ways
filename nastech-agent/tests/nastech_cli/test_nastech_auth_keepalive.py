from nastech_cli import nastech_auth_keepalive as keepalive
...
def test_keepalive_refreshes_stale_pool_entry(monkeypatch):
    ...
    assert keepalive.refresh_nastech_auth_keepalive_once() is True
...
def test_keepalive_falls_back_to_singleton_state(monkeypatch):
    ...
    def _resolve_nastech_runtime_credentials(**kwargs):
        calls.append(kwargs)
        return {
            "provider": "nastech",
    ...
    monkeypatch.setattr(
        ...
        _resolve_nastech_runtime_credentials,
    )

    assert keepalive.refresh_nastech_auth_keepalive_once(timeout_seconds=15.0) is True
