...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``nastech_session_rt`` cookie (contract V1: no RT to persist).
...
from nastech_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from nastech_cli.dashboard_auth.cookies import (
    SESSION_AT_COOKIE,
...
class TestApi401Envelope:
    ...
    def test_login_url_drops_next_for_deep_api_path(self, gated_app):
        ...
        from ``next=`` entirely; the SPA's own ``nastech.lastLocation``
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    the browser deletes ``nastech_session_at`` the instant the token lapses,
    while ``nastech_session_rt`` lives for 30 days. From that moment the
    ...
    def _build_rt_only_app(self):
        ...
        """
        import time as _t
        from tests.nastech_cli.conftest_dashboard_auth import _sign
    ...
    def test_provider_hint_routes_refresh_to_token_owner(self, gated_app):
        """A Nastech-style RT must not be rejected by Basic just because Basic
...
class TestAutoSsoRedirect:
    ...
    from nastech_cli.dashboard_auth.cookies import SSO_ATTEMPT_COOKIE
...
class TestNextSameOriginValidation:
    ...
    def test_safe_next_validator_accepts_same_origin(self):
        from nastech_cli.dashboard_auth.middleware import _safe_next_target
...
class TestValidatePostLoginTarget:
    ...
    def test_accepts_same_origin_paths(self):
        from nastech_cli.dashboard_auth.routes import _validate_post_login_target
...
class TestRenderLoginHtmlNext:
    ...
    def test_no_next_emits_plain_button(self):
        from nastech_cli.dashboard_auth.login_page import render_login_html
...
class TestAuthLoginPkceCookieNext:
    ...
    def test_no_next_query_omits_next_segment(self, gated_app):
        ...
        assert r.status_code == 302
        cookies = r.headers.get_list("set-cookie")
        pkce = next(c for c in cookies if "nastech_session_pkce" in c)
