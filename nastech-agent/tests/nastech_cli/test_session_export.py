...
from nastech_cli.session_export import export_record_count, render_sessions_export
from nastech_cli.session_export_html import (
...
def test_sessions_export_cli_prompt_only_stdout(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["nastech", "sessions", "export", "-", "--session-id", "sess", "--only", "user-prompts"],
    )
