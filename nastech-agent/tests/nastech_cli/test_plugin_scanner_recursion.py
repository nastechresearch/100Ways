...
def _enable(nastech_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<nastech_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        nastech_home = Path(os.environ["NASTECH_HOME"])  # set by hermetic conftest fixture
        user_plugins = nastech_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(nastech_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        nastech_home = Path(os.environ["NASTECH_HOME"])  # set by hermetic conftest fixture
        _write_plugin(nastech_home / "plugins", ["p1"])
        _enable(nastech_home, "p1")
...
class TestBackendGate:
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(nastech_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(nastech_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(nastech_home, "bad-img-plugin")
