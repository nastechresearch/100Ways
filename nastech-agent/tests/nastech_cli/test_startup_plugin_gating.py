...
def _live_subcommand_names() -> set[str]:
    """Run ``nastech --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from nastech_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["nastech", "--help"]
    buf = io.StringIO()
    try:
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``nastech photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain nastech-sweeper
    asked for:
    ...
    def _fake_loader():
        ...
        # top-level nastech <name> CLI command via PluginContext.
