"""Regression tests for the OAuth dispatcher in nastech_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "nastech_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "nastech_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "nastech_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "nastech auth add fake-oauth",
...
def test_nastech_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    from nastech_cli import auth as auth_mod
    from nastech_cli import web_server as ws

    session_id = "nastech-effective-scope-test"
    ws._oauth_sessions[session_id] = {
        "session_id": session_id,
        "provider": "nastech",
        "flow": "device_code",
    ...
    def fake_refresh_nastech_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_nastech_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_nastech_credentials", lambda state: None)

    try:
        ws._nastech_poller(session_id)
        assert captured_state["scope"] == auth_mod.DEFAULT_NASTECH_SCOPE
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `nastech model` universe) must be offered by /api/providers/oauth. This keeps
    the desktop Accounts tab in lockstep with the CLI picker — no provider the
    CLI can sign into may be missing from the GUI.
    """
    from nastech_cli.provider_catalog import provider_catalog
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `nastech model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Nastech."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``nastech auth remove
    xai-oauth``.
    """
    from nastech_cli import auth as auth_mod
    from nastech_cli import web_server as ws
    ...
    # Prior `nastech auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to nastech_cli.auth.get_auth_status (the
    ...
    with patch("nastech_cli.auth.get_auth_status", return_value=fake_status):
