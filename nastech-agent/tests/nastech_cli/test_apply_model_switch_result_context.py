...
``NastechCLI._apply_model_switch_result()`` printed ``ModelInfo.context_window``
...
def _run_display(monkeypatch, result):
    ...
    # Avoid writing to ~/.nastech/config.yaml during the test.
    monkeypatch.setattr(cli_mod, "save_config_value", lambda *a, **k: None)
    cli_mod.NastechCLI._apply_model_switch_result(_StubCLI(), result, False)
    return captured
...
def test_global_switch_clears_context_pin_owned_by_previous_route(monkeypatch):
    ...
    with (
        ...
        patch("nastech_cli.config.load_config_readonly", return_value=configured),
    ):
        cli_mod.NastechCLI._apply_model_switch_result(cli, result, True)
