"""Tests for npm ``EBADENGINE`` recovery (``nastech_cli/npm_engine.py``).

The behaviour under test is a contract about *reacting* to npm's own engine
check: npm states the range it wants in the failure, Nastech upgrades only an
...
from nastech_cli.npm_engine import (
    actual_npm_version,
    is_ebadengine,
    managed_npm_prefix,
...
npm error engine Not compatible with your version of node/npm: nastech-agent@1.0.0
npm error notsup Not compatible with your version of node/npm: nastech-agent@1.0.0
...
class TestManagedDetection:
    ...
    no other npm — this is the boundary between "Nastech fixes it" and "the
...
class TestRepairDecision:
    ...
    def test_foreign_npm_provisions_managed_runtime_instead(
        ...
        """A system/nvm/brew/Nix npm is never modified — Nastech provisions its
        ...
        import nastech_cli.npm_engine as npm_engine
        ...
        upgrades = []
        monkeypatch.setattr(
            npm_engine, "bootstrap_nastech_managed_node", fake_bootstrap
        )
        monkeypatch.setattr(
            npm_engine,
            "upgrade_managed_npm",
    ...
    def test_foreign_npm_failed_bootstrap_prints_manual_fix(
        ...
        monkeypatch.setattr(
            npm_engine, "bootstrap_nastech_managed_node", lambda: None
        )
