...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(status_mod, "get_nastech_home", lambda: tmp_path, raising=False)
    monkeypatch.setattr(status_mod, "load_config", lambda: {"model": "gpt-5.4"}, raising=False)
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "openai-codex", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "openai-codex", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "OpenAI Codex", raising=False)
    monkeypatch.setattr(auth_mod, "get_nastech_auth_status_local", lambda: {}, raising=False)
    ...
    assert "Start with:   nastech gateway" in output
...
def test_show_status_reports_vercel_backend_contract(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(auth_mod, "get_nastech_auth_status", lambda: {}, raising=False)
    monkeypatch.setattr(auth_mod, "get_codex_auth_status", lambda: {}, raising=False)
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in nastech status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.nastech/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.nastech/auth.json" in out
    ...
    def test_import_failure_does_not_break_other_oauth_providers(self, monkeypatch, capsys, tmp_path):
        """Nastech/Codex/MiniMax rows must still appear when xAI import fails."""
        import nastech_cli.auth as auth_mod
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_nastech_auth_status_local",
    ...
    def test_status_function_returns_none_does_not_crash(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: nastech auth add xai-oauth)" in out


def test_show_status_reports_gateway_session_last_activity(monkeypatch, capsys, tmp_path):
    """nastech status should surface freshest gateway last_active (#72016)."""
    from nastech_cli import status as status_mod
    import nastech_cli.auth as auth_mod
    import nastech_cli.gateway as gateway_mod
    ...
    monkeypatch.setattr(nastech_state, "SessionDB", _FakeDB)
