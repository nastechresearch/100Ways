"""Tests for the nastech_cli models module."""

from unittest.mock import patch, MagicMock

from nastech_cli.nastech_account import NastechPortalAccountInfo
from nastech_cli.models import (
    OPENROUTER_MODELS, fetch_openrouter_models, model_ids, detect_provider_for_model,
    is_nastech_free_tier, partition_nastech_models_by_tier,
    check_nastech_free_tier, _FREE_TIER_CACHE_TTL,
...
class TestFetchOpenRouterModels:
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        nastech-agent is tool-calling-first — surfacing a non-tool model leads to
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from nastech_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from nastech_cli.models import _find_openrouter_slug
        with patch("nastech_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestIsNastechFreeTier:
    """Tests for is_nastech_free_tier — account tier detection."""

    def test_paid_service_access_allowed_true_is_not_free(self):
        assert is_nastech_free_tier({"paid_service_access": {"allowed": True}}) is False


    def test_empty_subscription_not_free(self):
        """Empty subscription dict defaults to not-free (don't block users)."""
        assert is_nastech_free_tier({"subscription": {}}) is False


    def test_empty_response_not_free(self):
        """Completely empty response defaults to not-free."""
        assert is_nastech_free_tier({}) is False


class TestPartitionNastechModelsByTier:
    """Tests for partition_nastech_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_nastech_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_all_paid_models(self):
        ...
        sel, unav = partition_nastech_models_by_tier(models, pricing, free_tier=True)
        assert sel == []
        assert unav == models
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "nastech_cli.models.fetch_nastech_recommended_models",
...
class TestCheckNastechFreeTierCache:
    """Tests for the TTL cache on check_nastech_free_tier()."""
    ...
    @patch("nastech_cli.nastech_account.get_nastech_portal_account_info")
    def test_result_is_cached(self, mock_account):
        """Second call within TTL returns cached result without account lookup."""
        mock_account.return_value = NastechPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=False,
        )
        result1 = check_nastech_free_tier()
        result2 = check_nastech_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_nastech_free_tier() is False
        assert check_nastech_free_tier(force_fresh=True) is False
...
class TestNastechRecommendedModels:
    """Tests for fetch_nastech_recommended_models + get_nastech_recommended_aux_model."""
    ...
    def setup_method(self):
        _models_mod._nastech_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from nastech_cli.models import fetch_nastech_recommended_models
        ...
        with patch("nastech_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            a = fetch_nastech_recommended_models("https://portal.example.com")
            b = fetch_nastech_recommended_models("https://portal.example.com")
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("nastech_cli.models.fetch_nastech_recommended_models", return_value=payload):
            text = get_nastech_recommended_aux_model(vision=False, free_tier=False)
            vision = get_nastech_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("nastech_cli.models.fetch_nastech_recommended_models", return_value=payload),
            patch("nastech_cli.models.check_nastech_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_nastech_recommended_aux_model(vision=False) == "paid-model"
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from nastech_cli.models import validate_requested_model
