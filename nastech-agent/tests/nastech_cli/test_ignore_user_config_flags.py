"""Tests for --ignore-user-config and --ignore-rules flags on `nastech chat`.
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("NASTECH_IGNORE_USER_CONFIG", "NASTECH_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``NASTECH_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<nastech_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._nastech_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With NASTECH_IGNORE_USER_CONFIG=1, user config.yaml is ignored.
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``nastech_cli/main.py`` must set
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "NASTECH_IGNORE_USER_CONFIG" not in os.environ
