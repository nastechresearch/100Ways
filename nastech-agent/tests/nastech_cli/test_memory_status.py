"""Tests for `nastech memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from nastech_cli.memory_setup import cmd_status
    ...
    with patch("nastech_cli.config.load_config", return_value=config):
        with patch("nastech_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
