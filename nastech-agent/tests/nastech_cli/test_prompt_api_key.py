...
``nastech setup`` / ``nastech model`` when an API key already exists in ``.env``.
...
def _run_prompt(existing_key, choice, new_key="", provider_id="", pconfig_name="deepseek"):
    ...
    with patch("builtins.input", return_value=choice), \
         patch("nastech_cli.secret_prompt.masked_secret_prompt", return_value=new_key):
...
def test_pool_only_key_does_not_offer_or_execute_clear(profile_env, monkeypatch, capsys):
    ...
    with patch("nastech_cli.config.save_env_value") as save_env:
