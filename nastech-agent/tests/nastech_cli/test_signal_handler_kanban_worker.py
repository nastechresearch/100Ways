...
for interactive ``nastech chat -q`` invocations, but kanban workers spawned by
...
The fix: when the process is a dispatcher-spawned worker (``NASTECH_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full nastech_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors nastech_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With NASTECH_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"NASTECH_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            "process still alive 2s after SIGTERM with NASTECH_KANBAN_TASK set "
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    """Without NASTECH_KANBAN_TASK, the original KeyboardInterrupt path runs.

    This is the contrast case proving the fix is gated on the env var: in
    interactive ``nastech chat -q`` (no env var), behavior is unchanged. The
