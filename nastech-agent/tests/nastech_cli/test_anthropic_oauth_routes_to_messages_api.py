...
* ``nastech_cli.providers.determine_api_mode`` correctly mapped
  ``api.anthropic.com`` to ``anthropic_messages``.
* ``nastech_cli.runtime_provider._detect_api_mode_for_url`` did NOT, so
...
class TestExplicitRuntimeForAnthropic:
    ...
    Exercised whenever the user (or a Nastech subcommand) passes an
...
class TestPoolEntryForAnthropic:
    ...
    has added an OAuth credential via ``nastech auth add anthropic
