...
from nastech_cli.console_engine import NastechConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/nastech-console-test.zip",
    "import /tmp/nastech-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_nastech_home):
...
def test_sessions_export_rejects_oversized_single_before_touching_output(
    ...
    monkeypatch.setattr(nastech_state, "resolved_max_export_messages", lambda: 2)
    materialized = []
    ...
    result = NastechConsoleEngine().execute(
...
def test_sessions_export_all_uses_per_session_budget(
    ...
    monkeypatch.setattr(nastech_state, "resolved_max_export_messages", lambda: 3)
...
def test_sessions_export_zero_limit_disables_guard(
    ...
    monkeypatch.setattr(nastech_state, "resolved_max_export_messages", lambda: 0)
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_nastech_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_nastech_home):
    ...
    assert code == 0
    assert "Nastech Console" in stdout.getvalue()
    assert "nastech>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_capture_output_surfaces_string_exit_code_as_command_error():
    from nastech_cli.console_engine import ConsoleCommandError, _capture_output
...
def test_execute_handler_string_exit_returns_error_not_crash(_isolate_nastech_home):
    result = NastechConsoleEngine().execute(
