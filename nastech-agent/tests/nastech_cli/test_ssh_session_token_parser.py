...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    ``$HOME/.nastech/desktop-ssh`` (a literal ``~/.nastech/desktop-ssh`` in
    apps/desktop/electron/remote-lifecycle.ts, expanded against the account's
    $HOME). A non-default sticky profile re-homes ``get_nastech_home()`` to
    ``<root>/profiles/<name>``, and a Docker-style ``NASTECH_HOME`` can point
    elsewhere entirely — neither must move the validator off
    ``$HOME/.nastech/desktop-ssh``, or the token is wrongly rejected."""
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_dir = home / ".nastech" / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"

    # A sticky profile and a custom (Docker) root both point get_nastech_home()
    # away from $HOME/.nastech; the anchor must ignore both.
    for elsewhere in (home / ".nastech" / "profiles" / "coder", tmp_path / "opt" / "data"):
        token_path.write_text("b" * 64)
        token_path.chmod(0o600)
        override = set_nastech_home_override(elsewhere)
        try:
            assert _read_ssh_session_token_file(str(token_path)) == "b" * 64
            assert not token_path.exists()
        finally:
...
def test_token_under_profile_desktop_ssh_is_rejected(tmp_path, monkeypatch):
    ...
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    profile_home = home / ".nastech" / "profiles" / "coder"
    token_dir = profile_home / "desktop-ssh" / ("a" * 32)
    token_dir.mkdir(parents=True, mode=0o700)
    token_path = token_dir / "0123456789abcdef.token"
    token_path.write_text("b" * 64)
    token_path.chmod(0o600)
    override = set_nastech_home_override(profile_home)
    try:
...
def test_token_file_rejects_symlink(tmp_path, monkeypatch):
    ...
    override = set_nastech_home_override(home / ".nastech")
    try:
...
def test_token_file_rejects_parent_escape(tmp_path, monkeypatch):
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    token_root = home / ".nastech" / "desktop-ssh"
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from nastech_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-nastech-root"
    monkeypatch.setenv("NASTECH_HOME", str(machine_root / "profiles" / "writer_2"))
