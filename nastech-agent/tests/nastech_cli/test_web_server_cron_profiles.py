...
def isolated_profiles(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(profiles, "_get_default_nastech_home", lambda: default_home)
...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_nastech_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_nastech_home()
    ...
    outer_token = set_nastech_home_override(default_home)
    try:
        ...
        assert scheduler._get_nastech_home() == default_home
    finally:
        reset_nastech_home_override(outer_token)
...
def test_create_registers_scheduler_inside_target_profile(
    ...
    class RecordingProvider(CronScheduler):
        ...
        def register_job(self, job):
            captured["job"] = job
            captured["runtime_home"] = get_nastech_home()
            captured["jobs_file"] = cron_jobs._current_cron_store().jobs_file
