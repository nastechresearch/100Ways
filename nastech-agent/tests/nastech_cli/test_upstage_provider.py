...
Regression guard for the bug where `nastech model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`nastech_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in NASTECH_OVERLAYS
        overlay = NASTECH_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `nastech_cli/main.py` (cmd_model): read
