"""Tests for nastech_cli.relaunch — unified self-relaunch utility."""
...
class TestBuildRelaunchArgv:
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/nastech", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/nastech", ["/usr/bin/nastech", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        nastech).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead.
        ...
        that motivated it.
        """
        monkeypatch.setattr(relaunch_mod, "resolve_nastech_bin", lambda: r"C:\Users\test\nastech.exe")
        ...
        monkeypatch.setattr(relaunch_mod.sys, "argv", [r"C:\Users\test\nastech.exe"])
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\nastech.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_nastech_bin", lambda: r"C:\nastech.exe")
...
class TestResolveNastechBinWindowsPyGuard:
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a nastech.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\nastech.exe" if name == "nastech" else None,
        )

        bin_path = relaunch_mod.resolve_nastech_bin()
        # Must NOT be the .py — must be the nastech.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\nastech.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "nastech"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_nastech_bin() == str(script)

    @pytest.mark.windows_only
    def test_windows_py_argv0_with_no_nastech_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND nastech.exe
        isn't on PATH, return None so the caller falls back to
        python -m nastech_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
