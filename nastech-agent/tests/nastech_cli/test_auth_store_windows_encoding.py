...
``_load_auth_store`` and the Codex/Nastech shared-store readers previously called
...
def nastech_home(tmp_path, monkeypatch):
    """Point NASTECH_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.nastech/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, nastech_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, nastech_home, windows_default_encoding):
        ...
        assert not (nastech_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, nastech_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, nastech_home):
        auth_path = nastech_home / "auth.json"
...
# --- sibling readers of the same ~/.nastech/auth.json in other modules -------
#
# _load_auth_store lives in nastech_cli/auth.py, but several other modules read
# the same ~/.nastech/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, nastech_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(nastech_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_nastech_provider_reads_non_ascii_store(self, nastech_home, windows_default_encoding, monkeypatch):
        """agent/auxiliary_client's Nastech-provider lookup reads the same store.

        The lookup returns None on any read failure, silently disabling Nastech as
        ...
        store = {
            ...
            "providers": {"nastech": {"agent_key": "k", "label": "工作账号"}},
        }
        _write_utf8(nastech_home / "auth.json", store)
        ...
        # NASTECH_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", nastech_home / "auth.json")

        # _read_nastech_auth consults the credential pool FIRST and returns early
        ...
        provider = aux._read_nastech_auth()
        assert provider is not None
        assert provider.get("agent_key") == "k"

    def test_read_shared_nastech_state_reads_non_ascii_store(
        self, tmp_path, monkeypatch, windows_default_encoding
    ):
        """nastech_cli.auth._read_shared_nastech_state must read a non-ASCII store.

        The shared Nastech store (``nastech_auth.json``) is written as UTF-8. A
        ...
        _write_utf8(shared_dir / "nastech_auth.json", payload)

        provider = auth._read_shared_nastech_state()
        assert provider is not None
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, nastech_home, monkeypatch, windows_default_encoding
    ):
        """nastech_cli.main._has_any_provider_configured reads auth.json.
        ...
        # nastech_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_nastech_state(
        self, nastech_home, windows_default_encoding
    ):
        """tools.managed_tool_gateway._read_nastech_provider_state reads auth.json.

        The Nastech provider entry can carry a non-ASCII label. Under the
        ...
        from tools.managed_tool_gateway import _read_nastech_provider_state

        nastech = _read_nastech_provider_state()
        assert nastech is not None
        assert nastech.get("agent_key") == "k"
        # The non-ASCII label round-trips intact.
        assert nastech.get("label") == "工作账号"
