"""Tests for nastech_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
from nastech_cli.telegram_managed_bot import (
    DEFAULT_MANAGER_BOT,
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("nastech_")
        assert name.endswith("_bot")
        assert len(name) == len("nastech_") + 16 + len("_bot")
        assert len(name) <= 32
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Nastech & Friends",
        )
        assert "Nastech+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "nastech_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/NastechSetupBot/nastech_abcdefghijklmnop_bot?name=Nastech+Agent",
            "qr_payload": "https://t.me/newbot/NastechSetupBot/nastech_abcdefghijklmnop_bot?name=Nastech+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "nastech_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Nastech Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="nastech_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/NastechSetupBot/nastech_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/NastechSetupBot/nastech_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "nastech_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "nastech_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("nastech_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("nastech_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("nastech_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from nastech_cli.setup import _setup_telegram_auto
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        seen = {}
        profile_home = tmp_path / ".nastech" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_nastech_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "nastech_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from nastech_cli.setup import _profile_name_from_nastech_home

        assert (
            _profile_name_from_nastech_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\nastech\profiles\oracle")
            )
            == "oracle"
        )
