"""Tests for the unified provider catalog (nastech_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``nastech model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from nastech_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
