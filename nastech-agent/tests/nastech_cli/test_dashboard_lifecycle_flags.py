"""Tests for ``nastech dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``nastech update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("nastech_cli.main._scan_dashboard_processes", return_value=[]), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "No nastech dashboard processes running" in out

    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "nastech dashboard --port 9119"),
            (12346, "python -m nastech_cli.main dashboard --host 0.0.0.0 --port 9120"),
        ]
        with patch("nastech_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("nastech_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        # Status is informational — always exits 0.
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 nastech dashboard process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids",
             ...
             patch("nastech_cli.main._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
        ...
        # for the post-`nastech update` path).
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("nastech_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``nastech dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"nastech_cli.web_server": fake_ws}), \
...
class TestArgparseWiring:
    ...
    ``nastech dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from nastech_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("nastech_cli.main")
