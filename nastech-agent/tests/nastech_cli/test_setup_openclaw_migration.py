...
from nastech_cli import setup as setup_mod
...
class TestOfferOpenclawMigration:
    ...
    def test_skips_when_user_declines(self, tmp_path):
        ...
        openclaw_dir.mkdir()
        script = tmp_path / "openclaw_to_nastech.py"
        script.write_text("# placeholder")
        with (
            patch("nastech_cli.setup.Path.home", return_value=tmp_path),
            patch.object(setup_mod, "_OPENCLAW_SCRIPT", script),
            patch.object(setup_mod, "prompt_yes_no", return_value=False),
        ):
            assert setup_mod._offer_openclaw_migration(tmp_path / ".nastech") is False
    ...
    def test_runs_migration_when_user_accepts(self, tmp_path):
        ...
        # Create a fake nastech home with config
        ...
        fake_mod = ModuleType("openclaw_to_nastech")
        ...
        fake_migrator.migrate.return_value = {
            ...
            "output_dir": str(nastech_home / "migration"),
        }
        ...
        with (
            ...
            result = setup_mod._offer_openclaw_migration(nastech_home)
...
class TestSetupWizardOpenclawIntegration:
    ...
    def test_migration_offered_during_first_time_setup(self, tmp_path):
        ...
        with (
            patch.object(setup_mod, "ensure_nastech_home"),
            patch.object(setup_mod, "load_config", return_value={}),
            patch.object(setup_mod, "get_nastech_home", return_value=tmp_path),
            patch.object(setup_mod, "get_env_value", return_value=""),
            patch.object(setup_mod, "is_interactive_stdin", return_value=True),
            patch("nastech_cli.auth.get_active_provider", return_value=None),
...
class TestSetupWizardSkipsConfiguredSections:
    ...
    def test_sections_skipped_when_migration_imported_settings(self, tmp_path):
        ...
        def fake_migration(nastech_home):
        ...
        # vars via nastech_cli.gateway.get_env_value, NOT setup_mod's. Patch
