"""Cross-process update mutual exclusion (``nastech_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``nastech
...
dashboard-spawned ``nastech update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".nastech-update-in-progress"


def test_marker_path_follows_process_nastech_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* NASTECH_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".nastech-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``nastech update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Nastech is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``nastech-setup`` under ``~/.nastech`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.update_lock._pid_alive", lambda pid: True)
