...
from nastech_cli.auth import is_runtime_provider_routable
...
def _rows_with_env(monkeypatch, env_name: str, provider: str) -> list[dict]:
    ...
    with (
        ...
        patch("nastech_cli.models.cached_provider_model_ids", return_value=["model-a"]),
        patch("nastech_cli.providers.NASTECH_OVERLAYS", {}),
    ):
