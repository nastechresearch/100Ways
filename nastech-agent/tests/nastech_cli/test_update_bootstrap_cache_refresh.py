...
Pre-#67193 nastech-setup binaries reuse ``bootstrap-cache/install-<branch>.ps1``
...
``Access denied``). Those binaries have no self-update path, so ``nastech
...
def _run(home, root, branch="main"):
    with patch.object(cli_main, "get_nastech_home", return_value=str(home)), patch.object(
...
def test_never_raises_on_io_error(tmp_path):
    home, root = _setup(tmp_path)
    with patch.object(cli_main, "get_nastech_home", side_effect=OSError("boom")):
