"""Focused tests for the Nastech shared-metrics durable store."""
...
from nastech_cli.observability import shared_metrics as shared_metrics_module
from nastech_cli.observability.shared_metrics import SharedMetricsStore
from nastech_cli.observability.shared_metrics_contract import (
...
SCHEMA_PATH = (
    ...
    / "observability"
    / "schemas"
    / "nastech.shared_metrics.v2.schema.json"
)
LEGACY_SCHEMA_PATH = SCHEMA_PATH.with_name("nastech.shared_metrics.v1.schema.json")
...
def _resource(
    nastech_version: str = "test-version",
    *,
    os_family: str = "linux",
    ...
    return {
        ...
        "nastech_version": nastech_version,
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "nastech.shared_metrics.v2"
...
def test_v2_package_preserves_pending_v1_model_counters(tmp_path):
    ...
    assert package["schema_version"] == "nastech.shared_metrics.v2"
...
def test_package_schema_matches_the_model_call_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "nastech.shared_metrics.v2"
...
def test_client_active_mark_accepts_only_an_empty_allowlisted_payload():
    event = SimpleNamespace(
        ...
        scope_category=None,
        metadata={
            "nastech.metrics.schema_version": "nastech.metrics.event.v2",
        },
        data={},
    )
    ...
    wrong_schema.metadata["nastech.metrics.schema_version"] = "unknown"
...
def test_v1_package_schema_retains_the_legacy_model_contract():
    ...
    assert schema["properties"]["schema_version"]["const"] == "nastech.shared_metrics.v1"
...
def test_auxiliary_logical_scope_projects_one_normalized_terminal_route():
    event = SimpleNamespace(
        ...
        metadata={
            ...
            "nastech.call_role": "auxiliary:compression",
        },
    )
    ...
    event.metadata["nastech.call_role"] = "primary"
...
def test_tool_subscriber_contract_accepts_only_bounded_events():
    terminal = SimpleNamespace(
        ...
        category="tool",
        category_profile={},
        name="nastech.tool_call",
        scope_category="end",
        metadata={"nastech.metrics.schema_version": "nastech.metrics.event.v2"},
        data={
...
def test_skill_subscriber_contract_accepts_only_bounded_marks():
    ...
    assert skill_counter(load) == ("nastech.skill.load.count", load.data)
...
def test_store_exports_task_started_and_terminal_counters(tmp_path):
    ...
    store.record_counter("nastech.task_run.finished", terminal, _resource())
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE nastech_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT nastech_version FROM counter_aggregates"
            ).fetchall()
        }
