"""Tests for nastech_cli.service_manager — the abstract ServiceManager
...
from nastech_cli.service_manager import (
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `nastech_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import nastech_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("nastech_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "nastech_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
...
# unprivileged nastech user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from nastech_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``nastech gateway run``, an agent action, the Open WebUI helper) holds
    the per-NASTECH_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root NASTECH_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `nastech gateway run --replace` (no -p flag).
    assert "nastech gateway run --replace" in default_text
    assert "nastech -p default" not in default_text
...
#
# `nastech gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_nastech_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as nastech.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    nastech). Restartable log/run must not pathname-chown or pathname-rm a
    nastech-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on nastech-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid nastech mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid nastech rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_nastech_idx = log_text.index('s6-setuidgid nastech mkdir -p "$log_dir"')
    rm_as_nastech_idx = log_text.index('s6-setuidgid nastech rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_nastech_idx < rm_as_nastech_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = nastech_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside NASTECH_HOME the way
    # a real nastech uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'NASTECH_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that nastech cannot unlink a foreign
        ...
        'if [ -n "$NASTECH_TEST_DROPPED" ]; then\n'
        "  exit 0\n"
        "fi\n"
        f'exec {real_rm} "$@"\n',
        encoding="utf-8",
    ...
    env = os.environ.copy()
    env["NASTECH_HOME"] = str(nastech_home)
