"""Tests for nastech tools disable/enable/list command (backend)."""
...
class TestToolsDisableBuiltin:
    ...
    def test_disable_removes_toolset_from_platform(self):
        ...
        with patch("nastech_cli.tools_config.load_config", return_value=config), \
             patch("nastech_cli.tools_config.save_config") as mock_save:
...
class TestToolsDisableMcp:
    ...
    def test_disable_unknown_server_prints_error(self, capsys):
        config = {"mcp_servers": {}}
        with patch("nastech_cli.tools_config.load_config", return_value=config), \
             patch("nastech_cli.tools_config.save_config"):
