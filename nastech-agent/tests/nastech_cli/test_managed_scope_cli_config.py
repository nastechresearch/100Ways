...
nastech_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `nastech config`/`doctor`
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_MANAGED_DIR", str(managed))
...
def _load_cli_config(home):
    ...
    cli.py binds ``_nastech_home = get_nastech_home()`` at import time (module
    singleton), so monkeypatching NASTECH_HOME after import doesn't move it.
    ...
    this only matters for tests that swap NASTECH_HOME.)
    """
    import cli

    cli._nastech_home = home
