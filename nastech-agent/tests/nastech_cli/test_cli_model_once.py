...
def test_cli_model_once_records_restore_and_does_not_persist(monkeypatch):
    ...
    stub._snapshot_model_runtime = cli_mod.NastechCLI._snapshot_model_runtime.__get__(stub)
    printed = []
    ...
    monkeypatch.setattr("nastech_cli.model_switch.resolve_display_context_length", lambda *a, **k: None)

    cli_mod.NastechCLI._handle_model_switch(
        stub,
...
def test_cli_restore_model_runtime_snapshot_restores_agent():
    ...
    cli_mod.NastechCLI._restore_model_runtime_snapshot(stub, snapshot)
