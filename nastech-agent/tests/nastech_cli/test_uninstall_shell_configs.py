...
This rewrites files Nastech does not own (``~/.bashrc``, ``~/.zshrc``, ...) and
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``NASTECH_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("NASTECH_HOME", str(home / ".nastech"))
    return home


class TestHappyPath:
    def test_nastech_path_block_is_removed(self, fake_home: Path):
        rc = fake_home / ".zshrc"
        ...
        assert "# Nastech Agent" not in text
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/NASTECH_HOME patches the fake_home fixture installed.
