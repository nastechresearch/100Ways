...
def test_auto_detected_nastech_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks Nastech but credentials are revoked, fall through to OpenRouter."""
    from nastech_cli.auth import AuthError
    ...
    # resolve_provider returns "nastech" (stale active_provider in auth.json)
    ...
    # Nastech credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_nastech_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="nastech", code="invalid_grant", relogin_required=True)
        ),
    )
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from nastech_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from nastech_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from nastech_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
