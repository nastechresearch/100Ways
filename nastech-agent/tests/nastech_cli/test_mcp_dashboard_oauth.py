...
def test_hosted_auth_start_returns_public_authorization_url(monkeypatch):
    ...
    with patch(
        "nastech_cli.dashboard_auth.prefix.resolve_public_url",
...
def test_hosted_auth_allows_same_server_name_in_different_profiles(tmp_path, monkeypatch):
    ...
    existing = DashboardOAuthFlow(
        ...
        server_name="reports",
        profile=None,
        nastech_home=str(tmp_path / "default"),
    ...
    with patch("nastech_cli.mcp_config._get_mcp_servers", return_value={"reports": {"url": "https://mcp.example"}}), \
