...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_nastech_home):
...
class TestProfileScopedHubActions:
    def test_hub_install_spawns_with_profile_flag(
        ...
        """Hub installs must go through a fresh ``nastech -p <profile>``
        ...
        monkeypatch.setattr(web_server, "_spawn_nastech_action", _fake_spawn)
        resp = client.post(
