"""Unit tests for nastech_cli.managed_scope (resolver + loaders + key helpers)."""
import textwrap
...
def test_managed_dir_env_scrubbed_by_default():
    """conftest must scrub NASTECH_MANAGED_DIR so a dev-shell value can't leak in."""
    import os

    assert "NASTECH_MANAGED_DIR" not in os.environ
