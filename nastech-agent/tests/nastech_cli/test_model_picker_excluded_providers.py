...
interactive ``nastech model`` CLI picker.

The CLI picker (``nastech_cli.main.select_provider_and_model``) builds its
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``nastech model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    nastech_id / alias names."""
    # 'openai' is an alias-style nastech id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from nastech_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
