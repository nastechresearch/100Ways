"""Tests for nastech_cli.security_advisories.
...
import nastech_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect NASTECH_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full nastech_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "nastech_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "nastech_cli.config.save_config",
