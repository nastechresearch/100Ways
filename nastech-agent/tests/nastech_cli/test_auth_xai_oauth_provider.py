"""Tests for xAI Grok OAuth — tokens stored in Nastech auth store (~/.nastech/auth.json)."""
...
def _setup_nastech_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Nastech auth store at the given root."""
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("nastech_cli.auth.httpx.Client", _factory)
    return holder
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    nastech_home.mkdir(parents=True, exist_ok=True)
    (nastech_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    _setup_nastech_auth(nastech_home, access_token=near, refresh_token="rt-old")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    monkeypatch.setattr("nastech_cli.auth.refresh_xai_oauth_pure", _fake_pure)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
# ``XAI_BASE_URL`` / ``NASTECH_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    nastech_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    nastech_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    nastech_home = tmp_path / "nastech"
    _seed_xai_oauth_state(nastech_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nastech")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    monkeypatch.setattr("nastech_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((nastech_home / "auth.json").read_text())
    ...
    assert raw["active_provider"] == "nastech"
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``nastech model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``nastech model``
    ...
    assert "nastech model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from nastech_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Nastech that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `nastech model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `nastech auth list` reflects truth and
    ...
    _setup_nastech_auth(nastech_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
...
def test_credential_pool_device_code_seed_respects_suppression(tmp_path, monkeypatch):
    from agent.credential_pool import load_pool
    from nastech_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``nastech auth remove xai-oauth 1`` for a
    ...
    from agent.credential_pool import load_pool
    from nastech_cli.auth_commands import auth_remove_command
    ...
    # Act: the user runs `nastech auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((nastech_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Nastech invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``nastech model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``nastech model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``nastech auth
    ...
    (nastech_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.delenv("NASTECH_XAI_BASE_URL", raising=False)
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_nastech_auth(nastech_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    monkeypatch.setattr("nastech_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_runtime_provider_uses_pool_entry_for_xai_oauth(tmp_path, monkeypatch):
    ...
    nastech_home = tmp_path / "nastech"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_nastech_auth(nastech_home, access_token=fresh)
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.delenv("NASTECH_XAI_BASE_URL", raising=False)
    monkeypatch.delenv("XAI_BASE_URL", raising=False)
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_nastech_auth(nastech_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    def _fake_refresh(access_token, refresh_token, **kwargs):
        ...
        (nastech_home / "auth.json").write_text(json.dumps(raw))
        raise AuthError(
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`nastech auth add xai-oauth` entries (source='manual:xai_pkce') are
    ...
    _setup_nastech_auth(nastech_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    Nastech, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/Nastech refresh in a
    multi-provider setup silently overrides the user's chosen active
    provider (visible to ``nastech auth status``, ``nastech setup``, and the
    ``nastech`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    nastech_home = tmp_path / "nastech"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_nastech_auth(nastech_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "Nastech token rotations silently take over multi-provider users' "
