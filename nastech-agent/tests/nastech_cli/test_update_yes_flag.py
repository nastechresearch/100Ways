"""Tests for `nastech update --yes / -y` — assume yes for interactive prompts.
...
class TestUpdateYesConfigMigration:
    ...
    @patch("nastech_cli.update_cmd._reload_config_modules")
    @patch("nastech_cli.update_cmd._run_migrate_config_fresh")
    @patch("nastech_cli.update_cmd._run_config_check_fresh", return_value=(1, 2))
    @patch("nastech_cli.config.get_missing_config_fields", return_value=[])
    @patch("nastech_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``nastech_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``nastech_cli.main`` first could leave
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    the exception escape and crash `nastech update` mid-flight.
    """
    ...
    def test_unicode_decode_error_in_tty_skips_and_prints_hint(
        ...
        out = capsys.readouterr().out
        assert "nastech config migrate" in out
        mock_migrate.assert_not_called()

    def test_stash_restore_unicode_decode_error_falls_through_to_skip(self, tmp_path, capsys):
        from nastech_cli.update_cmd import _restore_stashed_changes
    ...
    def test_upstream_remote_prompt_unicode_decode_error_falls_through_to_skip(
        ...
        from nastech_cli.update_cmd import _sync_with_upstream_if_needed

        with patch(
            "nastech_cli.update_cmd._has_upstream_remote", return_value=False
        ), patch(
            "nastech_cli.update_cmd._should_skip_upstream_prompt", return_value=False
        ), patch(
            "nastech_cli.update_cmd._add_upstream_remote"
        ) as mock_add, patch(
            "builtins.input",
            side_effect=UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid byte"),
        ):
