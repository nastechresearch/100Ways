...
@pytest.fixture()
def isolated_nastech_home():
    """Per-test NASTECH_HOME dir (already redirected by the autouse conftest
...
def test_freshness_after_config_edit(isolated_nastech_home):
    from nastech_cli.config import read_raw_config_readonly

    cfg = _write_config(isolated_nastech_home, {"display": {"ephemeral_system_ttl": 1}})
    first = read_raw_config_readonly()
    assert first["display"]["ephemeral_system_ttl"] == 1

    _write_config(isolated_nastech_home, {"display": {"ephemeral_system_ttl": 7}})
...
def test_missing_config_returns_empty(isolated_nastech_home):
    from nastech_cli.config import read_raw_config_readonly

    cfg = isolated_nastech_home / "config.yaml"
    if cfg.exists():
        cfg.unlink()
