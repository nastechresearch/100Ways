"""Tests for nastech_cli/goals.py — persistent cross-turn goals."""
...
def nastech_home(tmp_path, monkeypatch):
    """Isolated NASTECH_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves NASTECH_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from nastech_cli.goals import _parse_judge_response
...
class TestGoalManager:
    ...
    def test_set_then_status(self, nastech_home):
    ...
    def test_continuation_prompt_shape(self, nastech_home):
        ...
        mgr.set("port goal command to nastech")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to nastech" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from nastech_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, nastech_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from nastech_cli import goals
        from nastech_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from nastech_cli.goals import GoalState
...
class TestMigrateGoalToSession:
    ...
    def test_migrates_active_goal_to_child(self, nastech_home):
        from nastech_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
    ...
    def test_does_not_clobber_existing_child_goal(self, nastech_home):
        from nastech_cli.goals import save_goal, load_goal, migrate_goal_to_session, GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, nastech_home):
        from nastech_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, nastech_home):
        import pytest
        from nastech_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, nastech_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, nastech_home):
        from unittest.mock import patch
        from nastech_cli import goals
...
class TestStatusLineSubgoalCount:
    ...
    def test_status_line_with_subgoals(self, nastech_home):
        from nastech_cli.goals import GoalManager
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, nastech_home):
        from nastech_cli import goals
        from nastech_cli.goals import GoalManager
    ...
    def test_stop_waiting_clears_barrier(self, nastech_home):
        from nastech_cli.goals import GoalManager
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_pid_parks_loop(self, nastech_home):
        from nastech_cli import goals
        from nastech_cli.goals import GoalManager
    ...
    def test_time_barrier_clears_after_deadline(self, nastech_home):
        from nastech_cli.goals import GoalManager
    ...
    def test_continue_verdict_still_continues_with_background(self, nastech_home):
...
class TestSessionTriggerBarrier:
    ...
    def test_registry_releases_on_watch_match_while_alive(self, nastech_home):
    ...
    def test_wait_on_session_validation(self, nastech_home):
        from nastech_cli.goals import GoalManager
...
class TestParseContract:
    ...
    def test_inline_fields_parsed(self):
        from nastech_cli.goals import parse_contract
...
class TestGoalContractSerialization:
    def test_roundtrip_with_contract(self):
        from nastech_cli.goals import GoalState, GoalContract
    ...
    def test_render_block_omits_empty_fields(self):
        from nastech_cli.goals import GoalContract
...
class TestGoalManagerContract:
    ...
    def test_set_contract_after_the_fact(self, nastech_home):
        from nastech_cli.goals import GoalManager, GoalContract
        ...
        # Survives reload.
        from nastech_cli.goals import GoalManager as GM2
        assert GM2(session_id="c-after").has_contract()

    def test_persistence_roundtrip(self, nastech_home):
        from nastech_cli.goals import GoalManager, GoalContract
...
class TestJudgeWithContract:
    ...
    def test_judge_uses_contract_template(self, nastech_home):
        from unittest.mock import patch
        from nastech_cli import goals
        from nastech_cli.goals import GoalContract
...
class TestDraftContract:
    def test_draft_parses_json(self, nastech_home):
        from unittest.mock import patch
        from nastech_cli import goals
    ...
    def test_draft_returns_none_when_no_client(self, nastech_home):
        from unittest.mock import patch
        from nastech_cli import goals
...
class TestContractAndBackgroundCompose:
    ...
    def test_judge_prompt_carries_contract_and_background(self, nastech_home):
        from unittest.mock import patch
        from nastech_cli import goals
        from nastech_cli.goals import GoalContract
