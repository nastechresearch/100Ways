...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for Nastech"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.nastech.cli_plugin",
        group="nastech_agent.plugins",
        dist=dist,
    )

    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "nastech_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for Nastech",
            "entrypoint",
            "adapters.nastech.cli_plugin",
            "wiki",
        )
    ]
