...
`nastech --tui --resume latest --in ./dir` (and the classic-CLI equivalents)
...
def test_top_level_in_value_not_mistaken_for_subcommand(monkeypatch):
    # `nastech --in chat` — "chat" is the flag's value, not the subcommand.
    import sys

    import nastech_cli.main as mod

    monkeypatch.setattr(sys, "argv", ["nastech", "--in", "chat", "--resume", "latest"])
