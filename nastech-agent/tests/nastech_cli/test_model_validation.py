"""Tests for provider-aware `/model` validation in nastech_cli.models."""
...
class TestNormalizeOpencodeBaseUrl:
    ...
    def test_strips_v1_for_anthropic_messages(self):
        from nastech_cli.models import normalize_opencode_base_url
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("nastech_cli.models.provider_model_ids", return_value=codex_models):
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Nastech User-Agent.
    ...
    def test_probe_sends_nastech_user_agent(self):
        ...
        assert ua.startswith("nastech-cli/"), (
            f"User-Agent must advertise nastech-cli, got {ua!r}"
        )
