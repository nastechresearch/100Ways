"""Tests for the get_nastech_auth_status() process-level cache.

The cache avoids re-validating Nastech credentials on every menu paint —
`nastech tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
against portal.nastechresearch.com during one render. The cache is keyed
...
also call invalidate_nastech_auth_status_cache().
"""
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test NASTECH_HOME.
...
def test_get_nastech_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    with patch.object(auth_mod, "_compute_nastech_auth_status", side_effect=fake_compute):
        first = auth_mod.get_nastech_auth_status()
        second = auth_mod.get_nastech_auth_status()
        third = auth_mod.get_nastech_auth_status()

    assert call_count["n"] == 1, (
        f"_compute_nastech_auth_status was called {call_count['n']}× — "
        "cache is not deduplicating within TTL."
    )
    # Each call returns a copy so callers can't mutate the cached dict.
    assert first == second == third
    first["mutated"] = True
    assert "mutated" not in auth_mod.get_nastech_auth_status()

    auth_mod.invalidate_nastech_auth_status_cache()


def test_get_nastech_auth_status_caches_failure_path(tmp_path, monkeypatch):
    ...
    with patch.object(auth_mod, "_compute_nastech_auth_status", side_effect=fake_compute):
        for _ in range(10):
            auth_mod.get_nastech_auth_status()
