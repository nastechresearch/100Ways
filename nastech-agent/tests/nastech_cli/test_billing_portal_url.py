"""Portal-URL resolution for Phase 2b billing errors (nastech_billing).
...
def _preview(monkeypatch):
    monkeypatch.setenv("NASTECH_PORTAL_BASE_URL", "https://nas-pr-412.nastechresearch.wtf")


def test_absolutize_resolves_relative(_preview):
    assert (
        _absolutize_portal_url("/billing?topup=open")
        == "https://nas-pr-412.nastechresearch.wtf/billing?topup=open"
    )
