"""Tests for nastech_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE nastech_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  nastech_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
from nastech_cli import _early_recovery as er
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing nastech_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"nastech_cli"}
            ...
            import nastech_cli._early_recovery  # noqa: F401
