"""Regression tests for TOCTOU-safe credential file writers in ``nastech_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.nastech/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Nastech shared-credential store write (inside _write_shared_nastech_state)
# ---------------------------------------------------------------------------


def test_shared_nastech_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The Nastech shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    # _nastech_shared_store_path() refuses to touch the real shared store during
    ...
    # get_default_nastech_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("NASTECH_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from nastech_cli import auth as auth_mod

        state = {
            "access_token": "nastech-access-xxx",
            "refresh_token": "nastech-refresh-xxx",
            "token_type": "Bearer",
        ...
        auth_mod._write_shared_nastech_state(state)
        path = auth_mod._nastech_shared_store_path()
    finally:
        os.umask(old_umask)

    assert path.exists(), "shared Nastech store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Nastech shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Nastech shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )

    data = json.loads(path.read_text())
    assert data["refresh_token"] == "nastech-refresh-xxx"
