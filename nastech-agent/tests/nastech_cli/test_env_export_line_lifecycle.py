...
``~/.nastech/.env``. ``load_env()`` parses the export prefix (#6659), so every
...
def test_classic_pat_save_via_endpoint_succeeds(nastech_home):
...
def test_plain_line_save_and_remove_still_work(nastech_home):
    """Sanity: the ordinary KEY= path is unchanged."""
    from nastech_cli.config import load_env, remove_env_value, save_env_value

    save_env_value("GITHUB_TOKEN", OLD_PAT)
    assert load_env()["GITHUB_TOKEN"] == OLD_PAT
    save_env_value("GITHUB_TOKEN", NEW_PAT)
    env_text = nastech_home.joinpath(".env").read_text(encoding="utf-8")
