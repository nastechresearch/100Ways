"""Guard: every `nastech update` path that reports user-modified skills must
also tell the user how to find them.

`nastech update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``nastech_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``nastech skills list-modified`` so the count is actionable — otherwise,
...
import nastech_cli.update_cmd as update_mod


_COUNT_RE = re.compile(r"user-modified \(kept\)")
_HINT_RE = re.compile(r"nastech skills list-modified")


def _source_lines() -> list[str]:
    # The update pipeline was extracted to nastech_cli/update_cmd.py
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`nastech skills list-modified` within the following lines — the "
