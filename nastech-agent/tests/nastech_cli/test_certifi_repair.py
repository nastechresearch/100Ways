...
1. The venv-repair import probes (early recovery + `nastech update`) must
...
2. ``nastech doctor`` must fail the certificate check in that state, and
   ``nastech doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
import nastech_cli._early_recovery as er
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `nastech update`'s venv repair must apply
    ...
    def _run_probe_script(self, monkeypatch, tmp_path, bundle_path):
        ...
        from nastech_cli import main as main_mod
...
# =========================================================================
# 2. nastech doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "nastech doctor --fix" in message
