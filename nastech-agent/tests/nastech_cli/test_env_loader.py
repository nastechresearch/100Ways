...
def test_utf8_bom_does_not_mangle_first_key(tmp_path, monkeypatch):
    ...
    loaded = load_nastech_dotenv(nastech_home=home)
...
def test_latin1_fallback_stream_honors_override(tmp_path, monkeypatch):
    ...
    from nastech_cli.env_loader import _load_dotenv_with_fallback
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from nastech_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "NASTECH_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="nastech_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    """Known Nastech keys inherited from parent process are removed when absent
    ...
    monkeypatch.setenv("NASTECH_ACP_AUTH_METHOD", "cursor_login")
    ...
    load_nastech_dotenv(nastech_home=home)
    ...
    # NASTECH_ACP_AUTH_METHOD and COPILOT_CLI_PATH are NOT in the profile .env → cleared
    assert "NASTECH_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    Empty ``NASTECH_ACP_AUTH_METHOD=`` tells the ACP adapter to skip
    ...
    home = tmp_path / "nastech"
    home.mkdir()
    (home / ".env").write_text("NASTECH_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "NASTECH_ACP_AUTH_METHOD" in os.environ
    assert os.environ["NASTECH_ACP_AUTH_METHOD"] == ""
    # COPILOT_CLI_PATH is absent from .env → cleared
    assert "COPILOT_CLI_PATH" not in os.environ


def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    """When no profile .env exists (bare profile), load_nastech_dotenv must not
    ...
    assert os.getenv("NASTECH_ACP_AUTH_METHOD") == "cursor_login"
    assert os.getenv("PATH") == "/usr/bin:/bin"


def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Nastech key that IS explicitly set in the profile .env survives
    the cleanup (overrides the inherited value).
    """
    home = tmp_path / "nastech"
    home.mkdir()
    (home / ".env").write_text(
        "NASTECH_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("NASTECH_ACP_AUTH_METHOD") == "claude_code_cli"


def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    """A known Nastech key defined with the bash-compatible ``export KEY=value``
    ...
    home.mkdir()
    (home / ".env").write_text(
        "export NASTECH_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    monkeypatch.setenv("NASTECH_ACP_AUTH_METHOD", "cursor_login")
    load_nastech_dotenv(nastech_home=home)
    assert os.getenv("NASTECH_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from nastech_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
#
# load_nastech_dotenv loads .env with override=True, so a stale
...
# the shared nastech_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process NASTECH_HOME (a different profile's
...
def test_config_yaml_terminal_backend_overrides_stale_env(tmp_path, monkeypatch):
    """Regression for #29186: a leftover TERMINAL_ENV=docker in ~/.nastech/.env
...
def test_other_profile_home_does_not_bridge_process_config(tmp_path, monkeypatch):
    ...
    load_nastech_dotenv(nastech_home=other_home)
