...
Covers:
 - Normal path (tokens already in Nastech auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Nastech
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `nastech auth openai-codex`. The old
...
@pytest.fixture()
def nastech_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Nastech auth store (no Codex CLI needed)."""
    ...
    for var in [
        ...
        "NASTECH_API_KEY", "DEEPSEEK_API_KEY",
    ]:
...
def test_normal_path_still_works(nastech_auth_only_env):
    """openai-codex appears when tokens are already in Nastech auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Nastech
    auth store."""
    nastech_home = tmp_path / ".nastech"
    nastech_home.mkdir()
...
def test_no_codex_when_no_credentials(tmp_path, monkeypatch):
    ...
    for var in [
        "OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
        "NASTECH_API_KEY", "DEEPSEEK_API_KEY", "COPILOT_GITHUB_TOKEN",
