...
NASTECH_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Nastech IDs ("copilot").  The slug
...
def test_copilot_uses_nastech_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._load_auth_store",
