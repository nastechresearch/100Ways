...
def nastech_home(tmp_path, monkeypatch):
    ...
    # /etc/nastech on the dev/CI box can't influence the test.
    monkeypatch.setenv("NASTECH_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(nastech_home, monkeypatch):
...
def test_env_expansion_in_user_config(nastech_home, monkeypatch):
    from nastech_cli.config import load_config, cfg_get
