"""Tests for the local-only Nastech session classifier exposed on /api/status."""
    ...
    return ".".join(
        (
            ...
            _encode(
                {
                    "sub": "test-user",
                    "scope": auth.DEFAULT_NASTECH_SCOPE,
...
# ── get_nastech_auth_status_local — refresh-free display snapshot ──
...
def test_local_status_not_logged_in_after_terminal_quarantine(monkeypatch):
    ...
    status = auth.get_nastech_auth_status_local()
...
def test_local_status_repeated_polling_never_uses_live_auth(monkeypatch):
    ...
    assert all(
        auth.get_nastech_auth_status_local()["logged_in"] for _ in range(10)
    )
