...
def test_client(monkeypatch, tmp_path):
    ...
    # Isolate NASTECH_HOME so config reads go to our tmp.
...
class TestPluginApiRuntimeGate:
    ...
    async def test_middleware_blocks_disabled_user_plugin(self):
        ...
        with patch.object(web_server, "_get_dashboard_plugins", return_value=[fake_plugin]), \
             patch("nastech_cli.plugins_cmd._get_enabled_set", return_value={"hot"}), \
             patch("nastech_cli.plugins_cmd._get_disabled_set", return_value={"hot"}):
...
class TestBundledPluginAssetGate:
    ...
    def test_bundled_asset_returns_404_when_disabled(self, test_client, tmp_path, monkeypatch):
        ...
        with patch.object(web_server, "_get_dashboard_plugins", return_value=[fake_plugin]):
            ...
            with patch(
                "nastech_cli.plugins_cmd._get_enabled_set", return_value=set()
            ), patch(
                "nastech_cli.plugins_cmd._get_disabled_set", return_value=set()
            ):
            ...
            with patch(
                ...
                "nastech_cli.plugins_cmd._get_disabled_set", return_value={"bundledx"}
            ):
                resp = test_client.get("/dashboard-plugins/bundledx/dist/index.js")
