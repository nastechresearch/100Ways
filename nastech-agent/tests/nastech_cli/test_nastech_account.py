"""Tests for normalized Nastech Portal account entitlement helpers."""
...
from nastech_cli.nastech_account import (
    ...
    nastech_portal_topup_url,
    reset_nastech_portal_account_info_cache,
)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("nastech_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("nastech_cli.auth.resolve_nastech_access_token", lambda: "fresh-token")
    monkeypatch.setattr("nastech_cli.nastech_account._fetch_nastech_account_info", lambda *a, **kw: payload)

    info = get_nastech_portal_account_info(force_fresh=True)

    assert isinstance(info, NastechPortalAccountInfo)
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_nastech_portal_entitlement_message(info, capability="Nastech model access")
