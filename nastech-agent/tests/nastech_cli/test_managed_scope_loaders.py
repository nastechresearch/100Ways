...
nastech_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # nastech_time checks an env override first; ensure it's unset so config wins.
    ...
    assert nastech_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (NASTECH_TIMEZONE, NASTECH_REDACT_SECRETS, NASTECH_MAX_ITERATIONS,
    ...
    # writing NASTECH_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
