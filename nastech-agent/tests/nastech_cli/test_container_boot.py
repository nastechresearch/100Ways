"""Tests for nastech_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $NASTECH_HOME under tmp_path; no real
...
from nastech_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running nastech containers, those
    ...
    """
    monkeypatch.setattr(
        "nastech_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under nastech_home/profiles/<name>/."""
    p = nastech_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `nastech profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    NASTECH_HOME root (the implicit default profile)."""
    if state is not None:
        (nastech_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (nastech_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (nastech_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        nastech_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from nastech_cli import container_boot
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            ...
            "top",
            "/opt/nastech/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "0.0.0.0",
            "--port",
