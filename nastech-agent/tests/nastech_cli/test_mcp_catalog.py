"""Tests for nastech_cli.mcp_catalog and nastech_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("NASTECH_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_nastech_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp NASTECH_HOME."""
    hh = tmp_path / "nastech-home"
    hh.mkdir()
    monkeypatch.setenv("NASTECH_HOME", str(hh))
    monkeypatch.setattr(
        "nastech_cli.config.get_nastech_home", lambda: hh
    )
    monkeypatch.setattr(
        "nastech_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "nastech_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_nastech_home() lazily through nastech_constants
    monkeypatch.setattr(
        "nastech_constants.get_nastech_home", lambda: hh
    )
    return hh
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from nastech_cli.mcp_catalog import list_catalog
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from nastech_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from nastech_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from nastech_cli.mcp_catalog import install_entry
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from nastech_cli.mcp_catalog import install_entry, uninstall_entry
        from nastech_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from nastech_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from nastech_cli.mcp_picker import show_catalog
    ...
    def test_run_picker_non_tty_falls_back(self, catalog_dir, capsys, monkeypatch):
        ...
        from nastech_cli.mcp_picker import run_picker
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from nastech_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from nastech_cli import mcp_catalog
        from nastech_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "nastech_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no NASTECH_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("NASTECH_OPTIONAL_MCPS", raising=False)
        from nastech_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Nastech fetches/launches is pinned
        to an exact version.
