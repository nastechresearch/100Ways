"""Tests for nastech_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("nastech_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from nastech_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("nastech_cli.config.load_config")
    ...
    @patch("nastech_cli.config.load_config")
    @patch.dict("os.environ", {"NASTECH_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """NASTECH_SESSION_PLATFORM should be used when NASTECH_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_nastech_platform_takes_precedence(self, tmp_path, monkeypatch):
        """NASTECH_PLATFORM should win over NASTECH_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from nastech_cli.skills_config import _get_categories
        skills = [
