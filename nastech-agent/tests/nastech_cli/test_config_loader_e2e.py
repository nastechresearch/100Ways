...
Runs a real subprocess with a temp NASTECH_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (NASTECH_MANAGED_DIR).
...
Subprocess (not in-process monkeypatching) so module-level ``_nastech_home``
globals, managed-scope caches, and ``_under_pytest`` guards behave like
production: ``NASTECH_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    code = textwrap.dedent(
        ...
        import os
        from pathlib import Path
        saved = Path(server._nastech_home, "config.yaml").read_text(encoding="utf-8")
    ...
    out = _run_py(
        code,
        {
            ...
            "NASTECH_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['NASTECH_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"NASTECH_HOME": str(home)}, tmp_path)
