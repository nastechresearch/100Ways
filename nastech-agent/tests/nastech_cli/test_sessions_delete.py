...
def test_sessions_delete_accepts_unique_id_prefix(monkeypatch, capsys):
    ...
    monkeypatch.setattr(nastech_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys,
        "argv",
        ["nastech", "sessions", "delete", "20260315_092437_c9a6", "--yes"],
    )
...
def _run_prune(monkeypatch, capsys, argv_tail, candidates=None):
    """Run `nastech sessions prune <argv_tail>` against a FakeDB, capturing
    ...
    monkeypatch.setattr(nastech_state, "SessionDB", lambda: FakeDB())
    monkeypatch.setattr(
        sys, "argv", ["nastech", "sessions", "prune", *argv_tail]
    )
...
def test_sessions_prune_bare_keeps_90_day_default(monkeypatch, capsys):
    """A truly bare `nastech sessions prune` keeps the implicit 90-day cutoff."""
...
def test_sessions_prune_preview_shows_oldest_newest(monkeypatch, capsys):
    ...
    from nastech_cli.session_filters import format_epoch
