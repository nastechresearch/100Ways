"""Tests for the Kanban DB layer (nastech_cli.kanban_db)."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import nastech_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("NASTECH_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Nastech' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Nastech' DB/metadata/logs on task completion.
    """
...
# spawned with `nastech -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's NASTECH_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's NASTECH_HOME."""

    def _set_home(self, monkeypatch, tmp_path, nastech_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.delenv("NASTECH_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.nastech/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.nastech/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.nastech/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # NASTECH_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile NASTECH_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # NASTECH_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is NASTECH_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["NASTECH_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["NASTECH_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["NASTECH_KANBAN_TASK"] == "t_dispatch_env"
        assert env["NASTECH_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "NASTECH_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see nastech_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import nastech_state as _nastech_state
    monkeypatch.setattr(
        _nastech_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _nastech_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    nastech_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("nastech_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="nastech_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        nastech_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "nastech_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'nastech kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_nastech_argv()
#
# Workers spawned by the dispatcher must use a `nastech` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["nastech", ...]`
...
def test_resolve_nastech_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m nastech_cli.main`.

    Pins the correct module name (NOT `nastech` — there is no top-level
    `nastech` package). Regression for #23198: the original PR shipped
    `python -m nastech` which fails with `No module named nastech` on every
    invocation.
    """
    import shutil
    import sys
    import nastech_cli.kanban_db as kb

    monkeypatch.delenv("NASTECH_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_nastech_argv()
    assert argv == [sys.executable, "-m", "nastech_cli.main"]


def test_resolve_nastech_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `nastech_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m nastech_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("NASTECH_BIN", None)
    ...
    assert "Nastech Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="nastech_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from nastech_cli.kanban_db import connect
