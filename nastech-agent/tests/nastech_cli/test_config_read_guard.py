...
Canonical owners:
  ...
  * ``nastech_cli/config.py`` — ``load_config()`` / ``load_config_readonly()``
...
ALLOWLIST = {
    ...
    # gateway.run._nastech_home (delegates to read_raw_config otherwise).
    "gateway/run.py",
    # Reads the MANAGED-scope config.yaml (/etc/nastech/...), not the user's —
    # it IS the overlay source; the canonical loaders call into it.
    "nastech_cli/managed_scope.py",
