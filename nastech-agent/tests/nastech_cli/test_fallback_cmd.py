"""Tests for `nastech fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate NASTECH_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".nastech" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from nastech_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nastech", "model": "Nastech-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_returns_copies_not_aliases(self):
        from nastech_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "nastech", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "nastech"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from nastech_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        _write_config(isolated_home, {
            ...
            "fallback_providers": [
                {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "nastech", "model": "Nastech-4"},
            ],
        })
        from nastech_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("nastech_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("nastech_cli.main._require_tty"):
            from nastech_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nastech / Nastech-4")
        with patch("nastech_cli.setup._curses_prompt_choice", return_value=1):
            from nastech_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from nastech_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `nastech fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nastech_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
