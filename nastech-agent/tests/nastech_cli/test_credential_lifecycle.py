...
NASTECH_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
def nastech_home(monkeypatch, tmp_path):
    """Fresh NASTECH_HOME with .env + auth.json + config.yaml fixtures."""
...
def test_delete_clears_provider_models_cache(nastech_home):
    _write_env(nastech_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(nastech_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = nastech_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(nastech_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(nastech_home, OPENAI_API_KEY=old)
    _write_config(
        nastech_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = nastech_home.joinpath("config.yaml").read_text(encoding="utf-8")
