...
The terminal `nastech model` flow already exposes `custom_providers`, but the
...
import nastech_cli.providers as providers_mod
import pytest
import yaml
...
def _disable_live_custom_provider_model_probe(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", lambda *_a, **_kw: None)
    monkeypatch.setattr(
        "nastech_cli.models.cached_provider_model_ids", lambda *_a, **_kw: []
    )
    monkeypatch.setattr(
        "nastech_cli.models.provider_model_ids", lambda *_a, **_kw: []
    )
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_list_authenticated_providers_can_probe_active_bare_custom_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "NASTECH_OVERLAYS", {})
    monkeypatch.setattr(
        "nastech_cli.models.fetch_api_models",
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("nastech_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("nastech_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_picker_selection_resolves_named_custom_provider_model_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models.validate_requested_model",
        lambda *a, **k: _MOCK_VALIDATION,
    )
    monkeypatch.setattr("nastech_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr(
        "nastech_cli.model_switch.get_model_capabilities",
        lambda *a, **k: None,
    )
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_custom_providers_uses_live_models_for_multi_model_endpoint(monkeypatch):
    ...
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("nastech_cli.providers.NASTECH_OVERLAYS", {})
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from nastech_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "nastech_cli.model_switch._save_discovered_models_to_config",
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    from nastech_cli.model_switch import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("nastech_cli.config.save_config", fake_save)
    monkeypatch.setattr(
...
def test_model_flow_named_custom_persists_discovered_models(monkeypatch):
    """The ``nastech model`` named-custom-provider flow persists the discovered
    ...
    ``nastech model`` collapsed to the single ``model:`` default everywhere but
    ...
    monkeypatch.setattr(
        "nastech_cli.curses_ui.curses_radiolist", lambda *a, **k: 0
    )
    # No-op downstream writes so the test never touches a real config.
    monkeypatch.setattr("nastech_cli.main._save_custom_provider", lambda *a, **k: None)
    monkeypatch.setattr("nastech_cli.auth._save_model_choice", lambda *a, **k: None)
    monkeypatch.setattr("nastech_cli.auth.deactivate_provider", lambda *a, **k: None)
    monkeypatch.setattr(
        "nastech_cli.config.load_config",
    ...
    from nastech_cli.model_setup_flows import _model_flow_named_custom
...
def test_custom_provider_context_length_models_dict_still_probes(monkeypatch):
    ...
    ``nastech model`` writes ``models: {default: {context_length: N}}`` for
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", fetch)
...
def _no_probe_local_row(monkeypatch, *, custom_providers=None, user_providers=None,
                        current_provider="nastech", **kwargs):
...
def test_keyless_endpoint_with_saved_catalog_still_reads_cache(monkeypatch):
    """A keyless local server must not be pinned by Nastech' own auto-save.
...
def test_keyless_endpoint_with_saved_catalog_is_still_not_probed(monkeypatch):
    ...
    The no-key + declared-catalog combination exists to keep Nastech from
