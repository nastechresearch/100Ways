...
# The curated list in nastech_cli/models.py defines the floor; models.dev only
...
def test_opencode_go_appears_when_api_key_set():
    ...
    # models.dev is reachable) or "nastech" (from NASTECH_OVERLAYS fallback when
    # the API is unavailable, e.g. in CI).
    assert opencode_go["source"] in {"built-in", "nastech"}
