...
class TestApply:
    ...
    def test_reapply_codex_app_server_runs_migration(self):
        ...
        ~/.codex/config.toml never gets the nastech-tools MCP callback
        ...
        with patch.object(crs, "check_codex_binary_ok",
             ...
             patch("nastech_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "nastech-tools"]
    ...
    def test_enable_triggers_mcp_migration(self):
        """Enabling codex_app_server should auto-migrate Nastech mcp_servers
        ...
        # User MCP servers are reported (excluding internal nastech-tools)
        assert "Migrated 1 MCP server" in r.message
        assert "filesystem" in r.message
        # Permissions default surfaces
        assert "Default sandbox: :workspace" in r.message
        # Nastech tool callback announcement
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("nastech_cli.codex_runtime_plugin_migration.migrate") as mig:
    ...
    def test_migration_failure_does_not_block_enable(self):
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("nastech_cli.codex_runtime_plugin_migration.migrate",
