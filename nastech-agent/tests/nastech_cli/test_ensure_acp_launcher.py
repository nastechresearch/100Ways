"""`nastech update` must self-heal the ``nastech-acp`` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``nastech-acp`` command name on the login-shell PATH. Fresh installs get the
launcher from ``scripts/install.sh``; existing installs get it from
``_ensure_acp_launcher()`` during ``nastech update``.
"""
...
def test_does_not_follow_symlink_into_venv(fake_home, tmp_path):
    """#21454 failure mode: never write through a symlinked nastech-acp."""
    (fake_home / "nastech").write_text("#!/bin/sh\n", encoding="utf-8")
    ...
    (fake_home / "nastech-acp").symlink_to(console_script)

    _ensure_acp_launcher()

    assert console_script.read_text(encoding="utf-8") == marker
    assert (fake_home / "nastech-acp").is_symlink()
...
def test_unwritable_bin_dir_is_skipped(fake_home):
    ...
    try:
        ...
        assert not (fake_home / "nastech-acp").exists()
    finally:
