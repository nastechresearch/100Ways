...
``nastech auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.nastech/.env`` — the picker was silently dropping back to
...
from nastech_cli.models import _resolve_copilot_catalog_api_key
...
class TestCopilotCatalogApiKeyResolution:
    ...
    def test_falls_back_to_pool_oauth_token(self):
        ...
        with patch(
            ...
            "nastech_cli.auth.read_credential_pool",
            return_value=[{"access_token": "gho_abc123"}],
        ), patch(
            "nastech_cli.copilot_auth.exchange_copilot_token",
