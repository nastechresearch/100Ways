"""Tests for the startup security posture audit (nastech_cli.security_audit_startup)."""
...
import nastech_cli.security_audit_startup as audit
...
def test_run_security_audit_aggregates(monkeypatch, tmp_path):
    ...
    findings = audit.run_security_audit(nastech_home=tmp_path, config={})
...
def test_run_security_audit_clean_posture(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(audit, "_in_container", lambda: False)
    assert audit.run_security_audit(nastech_home=tmp_path, config={}) == []
...
def test_log_startup_security_warnings_emits_and_is_idempotent(monkeypatch, tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="nastech.security_audit"):
        first = audit.log_startup_security_warnings(nastech_home=tmp_path, config={})
    assert len(first) == 1
    assert any("ROOT" in r.message for r in caplog.records)

    # Second call is a no-op (idempotent within a process) unless forced.
    second = audit.log_startup_security_warnings(nastech_home=tmp_path, config={})
    assert second == []
    forced = audit.log_startup_security_warnings(nastech_home=tmp_path, config={}, force=True)
    assert len(forced) == 1
