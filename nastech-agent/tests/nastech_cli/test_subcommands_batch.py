...
# ── deprecated `nastech login` fails gracefully, not with argparse error ────
#
# `nastech login` is a removed command; its handler (`login_command` in
# `nastech_cli/auth.py`) prints a deprecation notice pointing at `nastech auth` /
# `nastech model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `nastech --help`.
