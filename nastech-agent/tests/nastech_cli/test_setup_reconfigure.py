...
- Bare `nastech setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `nastech setup --quick` runs the narrower "fill in missing items" flow.
- `nastech setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("nastech_cli.setup.ensure_nastech_home", {}),
        ("nastech_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("nastech_cli.config.is_managed", {"return_value": False}),
        ("nastech_cli.setup.load_config", {"return_value": {}}),
        ("nastech_cli.setup.save_config", {}),
        ("nastech_cli.setup.get_env_value", {"return_value": None}),
        ("nastech_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("nastech_cli.setup._print_setup_summary", {}),
        ("nastech_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("nastech_cli.setup.load_config", {"return_value": {}}),
        ("nastech_cli.setup.save_config", {}),
        ("nastech_cli.auth.get_active_provider", {"return_value": None}),
        ("nastech_cli.setup.get_env_value", {"return_value": None}),
        ("nastech_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `nastech setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                ...
                quick="nastech_cli.setup._run_quick_setup",
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["nastech", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
