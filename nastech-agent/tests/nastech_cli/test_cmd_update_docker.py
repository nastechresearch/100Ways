"""Tests for ``nastech update`` / ``--check`` inside the Docker container.
...
fix, ``nastech update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Nastech, not an update to the running container, so the message
...
@patch("nastech_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``nastech update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nastechresearch/nastech-agent:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    """
    from nastech_cli.config import format_docker_update_message

    msg = format_docker_update_message()

    # Primary command — the entire reason this message exists.
    assert "docker pull nastechresearch/nastech-agent:latest" in msg
    ...
    assert "NASTECH_HOME" in msg or "/opt/data" in msg, (
