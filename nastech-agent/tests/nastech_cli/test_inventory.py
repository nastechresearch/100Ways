"""Behavior tests for nastech_cli.inventory.
...
def _nastech_row(model: str = "openai/gpt-5.5") -> dict:
    return {
        "slug": "nastech",
        "name": "Nastech",
        "models": [model],
...
def test_list_authenticated_providers_force_fresh_is_keyword_only():
    """``force_fresh_nastech_tier`` must be keyword-only on the public listing API.
    ...
    param = sig.parameters["force_fresh_nastech_tier"]
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "nastech"},
        {"slug": "nastech", "name": "Nastech", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "nastech"},
    ...
    with (
        ...
        patch("nastech_cli.config.read_raw_config", return_value={}),
        patch(
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("nastech_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    with patch("nastech_cli.models.clear_provider_models_cache") as clear:
