...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $NASTECH_HOME.

    Models a host git install whose $NASTECH_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'nastech update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("nastech_cli.config.get_managed_system", return_value=None), \
         patch("nastech_cli.config.get_nastech_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $NASTECH_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("nastech_cli.config.get_nastech_home", return_value=home):
        from nastech_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``nastech update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("nastech_cli.config.get_managed_system", return_value=None), \
         patch("nastech_cli.config.get_nastech_home", return_value=tmp_path), \
         patch("nastech_constants.is_container", return_value=True):
        from nastech_cli.config import detect_install_method
