...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    NASTECH_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_nastech_argv", lambda: ["nastech"])
    ...
    assert pid == 4242
    assert captured["env"]["NASTECH_HOME"] == str(profile)
    assert captured["env"]["NASTECH_KANBAN_TASK"] == "t_spawn_tools"
