"""Tests for Codex auth — tokens stored in Nastech auth store (~/.nastech/auth.json)."""
...
def _setup_nastech_auth(nastech_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Nastech auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_nastech_auth(nastech_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (nastech_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((nastech_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``nastech auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``nastech auth add openai-codex``.
    ...
    (nastech_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `nastech auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``nastech auth add openai-codex`` for accounts B and C
    ...
    (nastech_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``nastech auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Nastech auth store, not ~/.codex/."""
    nastech_home = tmp_path / "nastech"
    ...
    _save_codex_tokens({"access_token": "nastech-at", "refresh_token": "nastech-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Nastech store
    assert not (codex_home / "auth.json").exists()

    # Nastech auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "nastech-at"


def test_resolve_returns_nastech_auth_store_source(tmp_path, monkeypatch):
    nastech_home = tmp_path / "nastech"
    _setup_nastech_auth(nastech_home)
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "nastech-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run nastech auth".
    """
    from nastech_cli.auth import (
    ...
    assert "nastech auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from nastech_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from nastech_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch nastech_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("nastech_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
