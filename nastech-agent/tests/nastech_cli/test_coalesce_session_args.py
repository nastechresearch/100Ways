...
from nastech_cli.main import _coalesce_session_name_args
...
class TestCoalesceSessionNameArgs:
    ...
    def test_continue_multiword_unquoted(self):
        """nastech -c Pokemon Agent Dev → -c 'Pokemon Agent Dev'"""
    ...
    def test_no_session_flags_passthrough(self):
        """nastech -w chat -q hello (nothing to merge)"""
    ...
    def test_stops_at_setup_subcommand(self):
        """nastech -c my setup → 'setup' is a subcommand, not part of name"""
        assert _coalesce_session_name_args(
            ["-c", "my", "setup"]
        ) == ["-c", "my", "setup"]
