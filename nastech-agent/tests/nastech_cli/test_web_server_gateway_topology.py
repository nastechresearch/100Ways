...
from nastech_cli.web_server import (
...
def _patch_topology(monkeypatch, homes, running, runtimes):
    ...
    """
    import nastech_cli.profiles as profiles_mod
...
class TestStatusEndpointTopology:
    ...
    def _setup_client(self, monkeypatch, _isolate_nastech_home):
        try:
        ...
        monkeypatch.setattr(
            nastech_state, "DEFAULT_DB_PATH", get_nastech_home() / "state.db"
        )
    ...
    def test_profile_names_and_mode_public_when_auth_gated(self, monkeypatch):
        ...
        # Nastech Cloud Portal reads /api/status over the network (a gated bind)
        ...
        try:
            ...
            # alongside nastech_home / gateway_pid.
            assert "gateways" not in data
            assert "nastech_home" not in data
