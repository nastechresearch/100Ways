...
import pytest
from nastech_cli.model_switch import list_authenticated_providers, switch_model
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical Nastech write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though Nastech's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("nastech_cli.providers.NASTECH_OVERLAYS", {})
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical Nastech shape
    ...
    ``default_model``, so new-shape entries written by Nastech's own writer
...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", lambda *a, **k: None)
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("nastech_cli.model_switch.resolve_alias", return_value=None), \
         patch("nastech_cli.model_switch.list_provider_models", return_value=[]), \
         patch("nastech_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("nastech_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("nastech_cli.models.detect_provider_for_model", return_value=None), \
         patch("nastech_cli.model_switch.get_model_info", return_value=None), \
         patch("nastech_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("nastech_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", _fake_fetch)
...
def test_current_custom_model_not_leaked_into_other_provider_rows(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.providers.NASTECH_OVERLAYS", {})
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-test")
    monkeypatch.setenv("NASTECH_API_KEY", "sk-test")
    monkeypatch.setattr(
        "nastech_cli.models.cached_provider_model_ids",
