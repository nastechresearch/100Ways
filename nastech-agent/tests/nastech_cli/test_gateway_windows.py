"""Tests for nastech_cli.gateway_windows."""
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda nastech_home: "")
    monkeypatch.setattr("nastech_cli.config.get_nastech_home", lambda: str(nastech_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "nastech_cli.main"]
    assert cwd == str(nastech_home.resolve())
    assert env_overlay["VIRTUAL_ENV"] == str(project / "venv")
    assert str(project) in env_overlay["PYTHONPATH"].split(gateway_windows.os.pathsep)


class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_nastech_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".nastech"
        home.mkdir()
        monkeypatch.setattr("nastech_cli.config.get_nastech_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".nastech"
        project = tmp_path / "checkout"
        monkeypatch.setattr("nastech_cli.config.get_nastech_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Nastech_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Nastech_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Nastech_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Nastech\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Nastech\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Nastech_Gateway_alice")
    ...
    assert "Nastech_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "nastech_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("NASTECH_HOME", "PYTHONIOENCODING", "NASTECH_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
# resume_pending=True) never fires when `nastech gateway stop` kills the
