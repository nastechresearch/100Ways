"""Regression tests: `nastech dashboard` validates NASTECH_WEB_DIST before serving.

A custom NASTECH_WEB_DIST without --skip-build previously skipped BOTH the
...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "nastech_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "nastech_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )


def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """NASTECH_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("NASTECH_WEB_DIST", str(empty_dist))

    started = []
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.web_server",
    ...
    assert "NASTECH_WEB_DIST" in out and str(empty_dist) in out
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    _wire_common(main_mod, monkeypatch)
    monkeypatch.delenv("NASTECH_WEB_DIST", raising=False)
    project_root = tmp_path / "proj"
    dist = project_root / "nastech_cli" / "web_dist"
