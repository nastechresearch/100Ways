"""Unit tests for nastech_cli.security_audit — parsers + OSV plumbing.
...
from nastech_cli import security_audit as sa
...
class TestRunAudit:
    def test_no_components_returns_empty(self, tmp_path: Path):
        findings = sa.run_audit(
            skip_venv=True, skip_plugins=True, skip_mcp=True, nastech_home=tmp_path
        )
        assert findings == []
    ...
    def test_findings_sorted_by_severity_desc(self, tmp_path: Path):
        ...
        with patch.object(sa, "_osv_query_batch", side_effect=fake_batch), \
            ...
            findings = sa.run_audit(
                skip_venv=True, skip_plugins=False, skip_mcp=True, nastech_home=tmp_path
            )
...
class TestExitCodes:
    ...
    def test_discovery_runs_once_per_audit(self, tmp_path: Path, monkeypatch, capsys):
        ...
        """
        monkeypatch.setattr(sa, "get_nastech_home", lambda: str(tmp_path))
        calls = {"venv": 0}
