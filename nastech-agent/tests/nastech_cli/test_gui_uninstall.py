...
def _make_user_data(nastech_home: Path) -> None:
    (nastech_home / "config.yaml").write_text("x: 1\n")
    (nastech_home / ".env").write_text("KEY=secret\n")
    (nastech_home / "sessions").mkdir()
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `nastech desktop` installs is removable."""
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".nastech")
