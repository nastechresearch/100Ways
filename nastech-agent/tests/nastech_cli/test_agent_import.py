"""Tests for nastech_cli.agent_import — ``nastech import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Nastech stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/nastech_cli/test_profiles.py:
Path.home() and NASTECH_HOME are redirected to tmp_path so nothing touches
the real ~/.nastech.
"""
...
from nastech_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and NASTECH_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, NASTECH_HOME -> tmp/.nastech."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".nastech"
...
@pytest.fixture()
def nastech_home(profile_env):
    return profile_env / ".nastech"
...
def run_import(agent, source, nastech_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=nastech_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, nastech_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, nastech_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, nastech_home):
        before = snapshot_tree(nastech_home)
        report = run_import("claude-code", claude_tree, nastech_home, execute=False)
        assert snapshot_tree(nastech_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, nastech_home):
        preview = run_import("claude-code", claude_tree, nastech_home, execute=False)
        applied = run_import("claude-code", claude_tree, nastech_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, nastech_home):
        return run_import("claude-code", claude_tree, nastech_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, nastech_home):
        config = yaml.safe_load((nastech_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, nastech_home):
        return run_import("codex", codex_tree, nastech_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, nastech_home):
        config = yaml.safe_load((nastech_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, nastech_home):
        assert (nastech_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, nastech_home):
        run_import("claude-code", claude_tree, nastech_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in nastech_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, nastech_home):
        run_import("codex", codex_tree, nastech_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in nastech_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, nastech_home):
        report = run_import("claude-code", claude_tree, nastech_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, nastech_home):
        run_import("claude-code", claude_tree, nastech_home, execute=True)
        config = yaml.safe_load((nastech_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, nastech_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, nastech_home, execute=True)
        ...
        assert "still importable" in (
            nastech_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, nastech_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, nastech_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, nastech_home):
        dest = nastech_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, nastech_home):
        run_import("claude-code", claude_tree, nastech_home, execute=True)
        first = (nastech_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, nastech_home, execute=True)
        assert (nastech_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, nastech_home):
        memory = nastech_home / "memories" / "MEMORY.md"
...
class TestExistingConfigPreserved:
    ...
    @pytest.fixture()
    def config_path(self, nastech_home):
        return nastech_home / "config.yaml"
    ...
    def test_malformed_config_is_left_byte_identical(
            self, claude_tree, nastech_home, config_path):
    ...
    def test_malformed_config_records_an_error_not_an_import(
        ...
        for kind in CONFIG_WRITING_KINDS:
            ...
            assert "nastech config edit" in reason
    ...
    def test_unreadable_config_is_left_byte_identical(
        ...
        try:
            report = run_import("claude-code", claude_tree, nastech_home,
                                execute=True)
            statuses = {
                i["status"]
                for kind in CONFIG_WRITING_KINDS
    ...
    def test_import_preserves_every_pre_existing_config_key(
        ...
        assert merged["model"] == "nastech-4-405b"
    ...
    def test_config_write_is_atomic_and_leaves_no_temp_files(
        ...
        leftovers = [p.name for p in nastech_home.glob(".tmp*")]
        assert leftovers == []

    def test_symlinked_config_stays_a_symlink(
            self, claude_tree, nastech_home, tmp_path, config_path):
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
            self, nastech_home, config_path, monkeypatch):
        """An interrupted dump leaves the previous file complete."""
        from nastech_cli import agent_import
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, nastech_home, capsys):
        ...
        assert not (nastech_home / "memories" / "MEMORY.md").exists()
        assert not (nastech_home / "skills" / "claude-code-imports").exists()
        config_text = (nastech_home / "config.yaml").read_text(encoding="utf-8") \
            if (nastech_home / "config.yaml").exists() else ""
