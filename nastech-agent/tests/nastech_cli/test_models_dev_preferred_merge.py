...
These guard the contract:
  ...
  * OpenRouter and Nastech Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (Nastech) semantics.
...
appear in ``/model`` without a Nastech release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("nastech_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from nastech_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("nastech_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("nastech_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("nastech_cli.config.get_env_value", return_value=""),
            patch("nastech_cli.config.save_env_value"),
        ):
...
class TestOpenRouterAndNastechUnchanged:
    """Per Teknium: openrouter and nastech are NEVER merged with models.dev."""


    def test_openrouter_does_not_call_merge(self):
        """openrouter takes its own live path — merge helper must NOT run."""
        with patch(
            "nastech_cli.models._merge_with_models_dev",
