"""Tests for the ``nastech hooks`` CLI subcommand."""
...
from nastech_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("NASTECH_ACCEPT_HOOKS", raising=False)
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`nastech hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged —
        scripts tested with `nastech hooks test` saw different top-level
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `nastech hooks doctor` used to run every
