"""Unit tests for nastech_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/nastech_cli/test_pty_bridge.py.  Drives
...
``nastech_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from nastech_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from nastech_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from nastech_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp
    ...
    def test_non_numeric_falls_back_to_min(self):
        from nastech_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('NASTECH_PTY_TEST=' + os.environ.get('NASTECH_PTY_TEST',''))",
            ],
            env={**os.environ, "NASTECH_PTY_TEST": "pty-env-works"},
        )
        try:
