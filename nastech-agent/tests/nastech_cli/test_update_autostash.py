...
def _patch_managed_uv(request):
    ...
    with patch("nastech_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("nastech_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("nastech_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(nastech_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(nastech_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    nastech_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_nastech_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import nastech_constants

    monkeypatch.delattr(nastech_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(nastech_constants, "apply_subprocess_home_env")

    nastech_main._reload_updated_runtime_modules()

    assert callable(nastech_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        nastech_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        nastech_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(nastech_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        nastech_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        nastech_main, "_discard_stashed_changes",
        lambda *a, **kw: discard_calls.append(1) or True,
    )
    monkeypatch.setattr(
        nastech_config, "load_config",
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``nastech update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(nastech_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".nastech-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by nastech update (nastech_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "nastech-update-autostash")

    assert marker.exists(), (
        ".nastech-bootstrap-complete was swept into the update autostash — it must "
