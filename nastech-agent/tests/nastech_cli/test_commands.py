...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /nastech <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_nastech_norm = {_norm(n) for n in _SLACK_VIA_NASTECH_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_nastech_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `nastech slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from nastech_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch nastech_cli.plugins.get_plugin_commands() to a fixed dict."""
        from nastech_cli import plugins as _plugins_mod
