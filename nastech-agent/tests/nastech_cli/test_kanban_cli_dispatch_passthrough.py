...
def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: nastech kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "nastech_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.config.load_config", lambda: fake_config)
