...
from nastech_constants import venv_bin_dir, venv_python_path
...
def test_commit_swaps_every_entry(tmp_path):
    ...
    assert not [p for p in os.listdir(live) if "nastech-update" in p]
...
def test_staging_clears_leftovers_from_an_interrupted_run(tmp_path):
    ...
    stale = Path(f"{live / 'agent'}.nastech-update-staging")
    stale.mkdir()
...
def test_no_open_coded_venv_layout_remains_in_nastech_cli():
    ...
    exempt = {"stdio.py"}
    pkg = Path(nastech_cli.__file__).parent
    offenders = []
    ...
    assert not offenders, (
        "open-coded venv layout found (use nastech_constants.venv_bin_dir):\n"
        + "\n".join(offenders)
    )
...
def test_top_level_files_are_swapped_atomically(tmp_path):
    ...
    nastech_constants.py, ...). Covering only directories would leave exactly
...
def test_failed_staging_leaves_no_orphaned_copies(tmp_path, monkeypatch):
    ...
    leftovers = [p for p in os.listdir(live) if "nastech-update" in p]
...
def test_venv_helpers_honour_an_explicit_platform_verdict():
    ...
    `nastech_cli.main._is_windows`. A helper that reads `sys.platform`
...
def test_patched_is_windows_reaches_the_venv_path_derivation():
    ...
    with patch.object(nastech_main, "_is_windows", return_value=True):
        got = nastech_main._resolve_install_target_python(
...
def test_staging_restores_backup_when_dst_is_missing(tmp_path, monkeypatch):
    ...
    backup = live / "agent.nastech-update-old"
    backup.mkdir()
...
def test_commit_failure_plus_discard_leaves_no_staging_litter(tmp_path, monkeypatch):
    ...
    litter = [p for p in os.listdir(live) if "nastech-update" in p]
