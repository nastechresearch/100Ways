...
def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@nastech" / "ink" / "package.json"
...
def test_make_tui_argv_uses_bundled_tui_when_workspace_missing(
    ...
    nastech_cli/tui_dist/entry.js but never ships ui-tui/ (that directory only
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (nastech-ink etc.)
