"""Tests that provider selection via `nastech model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated NASTECH_HOME with a minimal string-format config."""
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        def _boom(path, data, **kwargs):
            ...
            assert data["model"]["provider"] == "nastech"
            assert data["model"]["base_url"] == "https://inference.example.com/v1"
            assert data["model"]["default"] == "some-old-model"
            assert kwargs["sort_keys"] is False
            raise OSError("simulated atomic write failure")

        with patch("nastech_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("nastech_cli.auth.deactivate_provider"), \
...
class TestBaseUrlValidation:
    ...
    def test_empty_base_url_keeps_default(self, config_home, monkeypatch):
        ...
        with patch("nastech_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("nastech_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestZaiEndpointPicker:
    ...
    def test_custom_proxy_rejects_invalid_url(self, config_home, monkeypatch, capsys):
        ...
        from nastech_cli.auth import ZAI_ENDPOINTS
        custom_idx = len(ZAI_ENDPOINTS)

        with patch("nastech_cli.main._prompt_provider_choice", return_value=custom_idx), \
             patch("nastech_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("nastech_cli.auth.deactivate_provider"), \
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from nastech_cli.auth import ZAI_ENDPOINTS
        from nastech_cli.model_setup_flows import _select_zai_endpoint
        ...
        with patch("nastech_cli.main._prompt_provider_choice", side_effect=fake_choice):
