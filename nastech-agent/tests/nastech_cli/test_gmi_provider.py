...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from nastech_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # nastech model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in NASTECH_OVERLAYS
        overlay = NASTECH_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        try:
            ...
            monkeypatch.setattr(_auth_mod, "get_nastech_auth_status", lambda: {})
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "nastech_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["nastech", "chat", "--provider", "gmi"])
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("nastech_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("nastech_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            "nastech_cli.auth.deactivate_provider",
        ), patch(
            "builtins.input",
            return_value="",
        ):
            from nastech_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_nastech_home() / "config.yaml").read_text()) or {}
