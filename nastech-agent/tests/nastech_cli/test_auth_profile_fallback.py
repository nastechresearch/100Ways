...
When ``NASTECH_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.nastech/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.nastech            (has its own auth.json fixture)
    * Profile     -> tmp_path/.nastech/profiles/coder   (active, NASTECH_HOME points here)
...
def test_missing_global_auth_file_is_safe(profile_env):
    ...
    from nastech_cli.auth import read_credential_pool
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "nastech": {"access_token": "nastech-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("nastech")
    assert state is not None
    assert state["access_token"] == "nastech-global"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("nastech") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_nastech_runtime_credentials`` and
# ``resolve_nastech_access_token``) call ``_load_provider_state`` directly with
...
# logged into Nastech Portal"`` even though the user has a valid global Nastech
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (NASTECH_HOME != ~/.nastech, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    nastech_home = tmp_path / "classic"
    nastech_home.mkdir()
