...
hand-rolled parsers — ``nastech_cli.config.load_env``,
``nastech_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from nastech_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("nastech_cli.config.get_env_path", return_value=env_path):
            env = load_env()
    finally:
        invalidate_env_cache()
...
def test_skills_tool_load_env_strips_export_prefix(tmp_path, monkeypatch):
    ...
    # skills_tool.load_env reads get_nastech_home()/.env directly.
    import importlib

    import tools.skills_tool as skills_tool

    importlib.reload(skills_tool)
    with patch.object(skills_tool, "get_nastech_home", return_value=tmp_path):
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import nastech_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from NASTECH_HOME (set above) → tmp_path/.env, so
    # no patching is needed. Re-clear os.environ provider keys that
    # load_nastech_dotenv may have populated at import/reload time, forcing the
