...
Before the fix, ``nastech whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``nastech gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    nastech = home / ".nastech"
    nastech.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("NASTECH_HOME", str(nastech))
...
def _env_value(nastech_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``nastech whatsapp`` with an existing paired session and
