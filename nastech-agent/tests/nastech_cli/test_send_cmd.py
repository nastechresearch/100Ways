"""Tests for the ``nastech send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``nastech_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from nastech_cli import send_cmd
...
def test_load_nastech_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``nastech send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``nastech config set``) lives
    ...
    nastech_home.mkdir()
    (nastech_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_nastech_home() to re-resolve under the patched env.
    from importlib import reload

    import nastech_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_nastech_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_nastech_env_utf8_bom_preserves_first_key(tmp_path, monkeypatch):
    ...
    `nastech send` failed to authenticate.
    """
    import os

    nastech_home = tmp_path / ".nastech"
    nastech_home.mkdir()
    (nastech_home / ".env").write_bytes(
...
def test_load_nastech_env_bomless_utf8_still_loads(tmp_path, monkeypatch):
    """BOM-less UTF-8 .env files must keep loading after the utf-8-sig switch."""
    import os

    nastech_home = tmp_path / ".nastech"
    nastech_home.mkdir()
    (nastech_home / ".env").write_bytes(b"SEND_PLAIN_TOKEN=plain-val\n")
...
def test_load_nastech_env_latin1_fallback_still_loads(tmp_path, monkeypatch):
...
def test_load_nastech_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (nastech_home / ".env").write_bytes(b"SEND_OVR_TOKEN=caf\xe9-file\n")
...
def test_load_nastech_env_fallback_read_error_is_swallowed(tmp_path, monkeypatch):
    ...
    (nastech_home / ".env").write_bytes(b"SEND_ERR_TOKEN=caf\xe9\n")
...
def test_load_nastech_env_bom_only_env_is_noop(tmp_path, monkeypatch):
    """A .env containing only a BOM must load zero vars without error."""
    import os

    nastech_home = tmp_path / ".nastech"
    nastech_home.mkdir()
    (nastech_home / ".env").write_bytes(b"\xef\xbb\xbf")
