...
def main_mod(monkeypatch):
    import nastech_cli.main as mod
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_nastech_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from nastech_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """nastech -z bypasses NastechCLI, but recall still needs SessionDB."""
    from nastech_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "nastech_state", mod("nastech_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.config",
        mod("nastech_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.models",
        mod("nastech_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.runtime_provider",
        mod(
            "nastech_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "nastech_cli.tools_config",
        mod("nastech_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["NASTECH_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nastech/nastech-test", provider="nastech", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["NASTECH_MODEL"] == "nastech/nastech-test"
    assert env["NASTECH_INFERENCE_MODEL"] == "nastech/nastech-test"
    assert env["NASTECH_TUI_PROVIDER"] == "nastech"
    assert env["NASTECH_INFERENCE_PROVIDER"] == "nastech"
    assert env["NASTECH_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["NASTECH_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("nastech-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_nastech_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("NASTECH_TUI_DIR", raising=False)
