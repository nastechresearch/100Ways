"""Verify `nastech -c` picks the session the user most recently used."""

from __future__ import annotations

from nastech_cli.main import _resolve_last_session
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = nastech_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    real_db = nastech_state.SessionDB
    ...
    monkeypatch.setattr(
        "nastech_cli.main.subprocess.run",
        lambda cmd, **kw: __import__("subprocess").CompletedProcess(
            cmd, 0, stdout=str(repo_a), stderr=""
        ),
    )
    monkeypatch.setattr("nastech_state.SessionDB", lambda: real_db(db_path=state_db))
