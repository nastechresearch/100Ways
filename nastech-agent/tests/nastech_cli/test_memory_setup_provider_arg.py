"""Tests for `nastech memory setup [provider]` routing.
...
``nastech memory setup honcho``) without the interactive picker — which matters
because the per-provider ``nastech <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "nastech memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets NASTECH_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"NASTECH_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("nastech_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("nastech_cli.tools_config.subprocess.run", fake_run):
