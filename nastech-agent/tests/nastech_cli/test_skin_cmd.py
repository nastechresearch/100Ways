"""`nastech skin set` — deterministic single-color tweak of the active skin.
...
def _skins():
    d = get_nastech_home() / "skins"
    d.mkdir(parents=True, exist_ok=True)
    return d


def _activate(name: str) -> None:
    (get_nastech_home() / "config.yaml").write_text(f"display:\n  skin: {name}\n", encoding="utf-8")
...
def test_set_forks_a_builtin_without_inventing_a_background():
    ...
    assert (get_nastech_home() / "config.yaml").read_text().find("default-custom") != -1
