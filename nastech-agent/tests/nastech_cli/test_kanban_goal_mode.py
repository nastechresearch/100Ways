...
2. Spawn: _default_spawn sets the NASTECH_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (nastech kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """nastech kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from nastech_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("nastech_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("nastech_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("nastech_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("nastech_cli.kanban._worker_run_id_for", lambda _: None)
