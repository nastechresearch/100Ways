...
Bug class: the checkout's ``.py`` files change (``nastech update``, manual
...
def _make_pycache(repo: Path, subdir: str = "nastech_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nastech_main, "PROJECT_ROOT", repo)
    # Stamp records a different (older) fingerprint.
    (repo / nastech_main._BYTECODE_FINGERPRINT_FILE).write_text(
        "git:refs/heads/main:" + "a" * 40, encoding="utf-8"
    )

    nastech_main._sweep_stale_bytecode_if_checkout_changed()

    assert not cache.exists()
    # Stamp updated to the current fingerprint.
    recorded = (repo / nastech_main._BYTECODE_FINGERPRINT_FILE).read_text(encoding="utf-8")
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.nastech/plugins/<name>
# ---------------------------------------------------------------------------

def test_clear_plugin_bytecode_removes_nested_caches(tmp_path):
    from nastech_cli import plugins_cmd
