"""Tests for the post-pull syntax guard in ``nastech update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``nastech_cli/config.py``), the CLI
becomes unbootable — every ``nastech`` invocation imports those files at
...
markers in ``nastech_cli/config.py``; users who ran ``nastech update`` in
the 7-minute window before #28458 landed could not run any ``nastech``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nastech_main.subprocess, "run", fake_run)

    assert nastech_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    for relpath in nastech_main._UPDATE_CRITICAL_FILES:
        path = root / relpath
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except nastech_constants.py
    for relpath in nastech_main._UPDATE_CRITICAL_FILES:
        if relpath == "nastech_constants.py":
            continue
        path = tmp_path / relpath
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("# stub\n")

    ok, failing_path, error = nastech_main._validate_critical_files_syntax(tmp_path)
...
# release; if a future ``nastech update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
