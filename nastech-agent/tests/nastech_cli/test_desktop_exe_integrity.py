...
Nastech.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors nastech_cli.main.
...
def _win_tree(tmp_path: Path) -> tuple[Path, Path]:
    ...
    exe = desktop_dir / "release" / "win-unpacked" / "Nastech.exe"
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Nastech.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("nastech_cli.main._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Nastech.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("nastech_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("nastech_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `nastech desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Nastech.exe cannot launch
    must exit nonzero (so nastech-setup's retry-once kicks in) and must restore
    ...
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "Nastech.exe", PE_AMD64)

    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    pack_ok = subprocess.CompletedProcess(["npm", "run", "pack"], 0)

    with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("nastech_cli.main._resolve_node_runtime_npm", return_value="npm.cmd"), \
         patch("nastech_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("nastech_cli.main._desktop_build_needed", return_value=True), \
         patch("nastech_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         patch("nastech_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("nastech_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("nastech_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("nastech_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("nastech_cli.main.subprocess.run", return_value=pack_ok), \
