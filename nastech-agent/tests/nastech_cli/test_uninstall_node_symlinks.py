"""Tests for nastech_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $NASTECH_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Nastech-managed node dir.
"""
...
def _make_nastech_node(nastech_home: Path) -> Path:
    """Create a fake $NASTECH_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = nastech_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    nastech_home = fake_home / ".nastech"
    _make_nastech_node(nastech_home)
    ...
    removed = uninstall.remove_node_symlinks(nastech_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    nastech_home = fake_home / ".nastech"
    node_bin = _make_nastech_node(nastech_home)
