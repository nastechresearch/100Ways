...
from nastech_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    assert auth_mod.get_active_provider() == "nastech"
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from nastech_cli.auth_commands import auth_status_command
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    Mirrors Nastech / xAI-OAuth / Codex-OAuth / MiniMax quarantine pattern.
    """
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    ...
    def _terminal_refresh(_state, **_kw):
        raise AuthError(
            "Spotify token refresh failed. Run `nastech auth spotify` again.",
            provider="spotify",
