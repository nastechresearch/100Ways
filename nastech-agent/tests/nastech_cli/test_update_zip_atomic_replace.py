...
which broke ``nastech --tui`` because ``ui-tui/`` had vanished.
...
from nastech_cli.main import _atomic_replace_dir
...
def test_atomic_replace_swaps_content_on_success(tmp_path: Path) -> None:
    ...
    assert not (dst.parent / "ui-tui.nastech-update-staging").exists()
    assert not (dst.parent / "ui-tui.nastech-update-old").exists()
