"""Regression tests for Nastech OAuth refresh and inference JWT interactions."""
...
def test_resolve_nastech_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    ...
    assert creds["source"] == auth_mod.NASTECH_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.NASTECH_AUTH_PATH_INVOKE_JWT
...
def test_resolve_nastech_runtime_credentials_invoke_jwt_is_idempotent(
    ...
    assert (
        payload["providers"]["nastech"]["agent_key_obtained_at"]
...
def test_get_nastech_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from nastech_cli.auth import get_nastech_auth_status

    nastech_home = tmp_path / "nastech"
    nastech_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Nastech provider entry
    (nastech_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))

    # Seed the credential pool with a Nastech entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("nastech")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("nastech", {
        "access_token": token,
...
# =============================================================================
# _login_nastech: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginNastechSkipKeepsCurrent:
    """When a user runs `nastech model` → Nastech Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "nastech" while
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        fake_auth_state = {
            "access_token": "fake-nastech-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.nastechresearch.com",
            "portal_base_url": "https://portal.nastechresearch.com",
        ...
        monkeypatch.setattr(
            models_mod, "partition_nastech_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        _login_nastech(args, PROVIDER_REGISTRY["nastech"])
        ...
        # auth.json: active_provider restored to openrouter, but Nastech creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "nastech" in auth_after["providers"]
        assert auth_after["providers"]["nastech"]["access_token"] == "fake-nastech-token"
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        auth_path = nastech_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "nastech" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Nastech creds are still saved
        assert "nastech" in auth_after.get("providers", {})


# =============================================================================
# persist_nastech_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _nastech_device_code_login /
    refresh_nastech_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_nastech_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.nastech and lets seeding
    ...
    persist_nastech_credentials(first)
    ...
    persist_nastech_credentials(second)

    payload = json.loads((nastech_home / "auth.json").read_text())

    # providers.nastech reflects the latest write (singleton semantics)
    assert payload["providers"]["nastech"]["access_token"] == second_token
    assert payload["providers"]["nastech"]["agent_key"] == second_token

    # credential_pool.nastech has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["nastech"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == NASTECH_DEVICE_CODE_SOURCE
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    When the Nastech Portal server returns ``invalid_grant`` with
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """Nastech refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-nastech-refresh-token"] == "refresh-1"
...
def shared_store_env(tmp_path, monkeypatch):
    ...
    Required for every test that exercises the shared Nastech store — the
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    """
    from nastech_cli.auth import _nastech_shared_store_path
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    path = _nastech_shared_store_path()
    assert path.is_file()
    ...
    loaded = _read_shared_nastech_state()
...
def test_persist_nastech_credentials_mirrors_to_shared_store(
    ...
    """persist_nastech_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `nastech auth add nastech
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from nastech_cli.auth import (
        _nastech_shared_store_path,
        _read_shared_nastech_state,
        persist_nastech_credentials,
    )
    ...
    persist_nastech_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((nastech_home / "auth.json").read_text())
    assert "nastech" in payload.get("providers", {})

    # Shared store populated with the same refresh_token
    shared = _read_shared_nastech_state()
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from nastech_cli.auth import _load_auth_store, DEFAULT_NASTECH_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "nastech": {
                    "portal_base_url": "https://api.nastechresearch.com",
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        store["providers"]["nastech"]["portal_base_url"] = (
            "http://portal.nastechresearch.com"
        )
...
class TestNastechDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        ...
        msg = _nastech_device_auth_timeout_message("https://portal.nastechresearch.com")
        assert "CAPTCHA" in msg
        assert "nastech portal" in msg
        assert "https://portal.nastechresearch.com/login" in msg
...
def test_nastech_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "device_code": "device",
            "user_code": "SMCL-97YT",
            "verification_uri": "https://portal.nastechresearch.com/manage-subscription",
            "verification_uri_complete": (
                "https://portal.nastechresearch.com/manage-subscription"
                "?user_code=SMCL-97YT"
            ),
            "expires_in": 600,
            "interval": 1,
