...
A credential added via `nastech auth add openrouter` lives in the credential
...
def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `nastech auth add openrouter <token>` — a manual pool entry."""
...
def test_auto_detects_openrouter_from_pool(tmp_path, monkeypatch):
    ...
    (tmp_path / "nastech").mkdir(parents=True, exist_ok=True)
