...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /nastech-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``NASTECH_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``NASTECH_HOME`` value, which
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["NASTECH_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        ...
        "NASTECH_GATEWAY_EXIT_DIAG": "0",
        "NASTECH_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("nastech_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "nastech-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test NASTECH_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="NASTECH_HOME=/home/alice/.nastech"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_nastech_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_nastech_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "nastech-gateway.service"
