...
@pytest.fixture
def cli():
    obj = NastechCLI.__new__(NastechCLI)  # bypass __init__ (no full app needed)
...
def _capture_opener(monkeypatch, *, opened_ok=False):
    ...
    monkeypatch.setattr(NastechCLI, "_open_url_in_browser", _open, raising=False)
    return seen
...
def test_free_prints_catalog_and_deep_links_with_plan(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("plus"), raising=False)
...
def test_upgrade_confirm_names_the_subscription_card(cli, monkeypatch, capsys):
    ...
    monkeypatch.setattr(NastechCLI, "_prompt_text_input_modal", _scripted_modal("change", "ultra", "cancel"), raising=False)
