...
1. Unit contract on :func:`nastech_cli._subprocess_compat.noninteractive_git_env`.
...
class TestNoninteractiveGitEnv:
    ...
    def test_defaults_to_process_environ_copy(self, monkeypatch):
        monkeypatch.setenv("NASTECH_TEST_SENTINEL", "xyz")
        env = noninteractive_git_env()
        assert env["NASTECH_TEST_SENTINEL"] == "xyz"
        assert env["GIT_TERMINAL_PROMPT"] == "0"
...
class _BasicAuthChallenge(http.server.BaseHTTPRequestHandler):
    def _challenge(self):
        ...
        self.send_header("WWW-Authenticate", 'Basic realm="nastech-test"')
