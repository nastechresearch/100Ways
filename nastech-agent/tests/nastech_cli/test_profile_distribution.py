"""Tests for nastech_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/nastech_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_nastech_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Nastech"):
                check_nastech_requires(spec, cur)
...
class TestInstall:
    ...
    def test_update_respects_distribution_owned_allowlist(self, profile_env):
        ...
        from nastech_cli.profile_distribution import read_manifest as _read
    ...
    def test_install_enforces_nastech_requires(self, profile_env, monkeypatch):
        # Pin current Nastech version to something well below the requirement
        import nastech_cli
        monkeypatch.setattr(nastech_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            nastech_requires=">=99.0.0",
        )
...
class TestSecurity:
    ...
    def test_install_rejects_symlinked_distribution_files(self, profile_env, tmp_path):
        ...
        from nastech_cli.profiles import get_profile_dir
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from nastech_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_installed_distribution_shows_in_list(self, profile_env):
        ...
        from nastech_cli.profiles import list_profiles
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from nastech_cli.profiles import create_profile, list_profiles, get_profile_dir
