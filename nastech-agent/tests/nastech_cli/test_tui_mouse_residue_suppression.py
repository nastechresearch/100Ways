...
The Python launcher (`nastech --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Nastech session
...
from nastech_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["nastech", "--tui"])
        monkeypatch.delenv("NASTECH_TUI", raising=False)
        monkeypatch.setenv("NASTECH_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["nastech", "--tui"])
        monkeypatch.delenv("NASTECH_TUI", raising=False)
        monkeypatch.delenv("NASTECH_TUI_NO_EARLY_DISABLE", raising=False)
