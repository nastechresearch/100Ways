"""Tests for ``nastech_cli.diagnostics_upload`` — the Nastech-S3 upload client.
...
class TestRequestUploadUrl:
    ...
    def test_base_url_env_override(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_DIAGNOSTICS_BASE_URL", "https://staging.example.com")
        import nastech_cli.diagnostics_upload as mod
        ...
        try:
            ...
            monkeypatch.delenv("NASTECH_DIAGNOSTICS_BASE_URL", raising=False)
...
# ---------------------------------------------------------------------------
# share_to_nastech (orchestration)
# ---------------------------------------------------------------------------

class TestShareToNastech:
    def test_orchestrates_request_then_put(self):
        ...
        with patch.object(mod, "request_upload_url", return_value=info) as req, \
            ...
            result = mod.share_to_nastech(blob)
    ...
    def test_share_succeeds_without_id_in_response(self):
        ...
        with patch.object(mod, "request_upload_url", return_value=info), \
             patch.object(mod, "put_bundle") as put:
            result = mod.share_to_nastech(b"data")
        assert result == info
