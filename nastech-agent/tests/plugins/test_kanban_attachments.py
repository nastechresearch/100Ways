...
Covers three layers:
  * ``nastech_cli.kanban_db`` accessors (add/list/get/delete + path helpers)
...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "nastech_dashboard_plugin_kanban_attach_test", plugin_file,
    )
...
def test_attachments_root_is_per_board(kanban_home, monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_KANBAN_ATTACHMENTS_ROOT", raising=False)
...
# ---------------------------------------------------------------------------
# CLI — nastech kanban attach / attachments / attach-rm
# ---------------------------------------------------------------------------
