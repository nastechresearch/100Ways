...
def _inject_fake_nastech_cli(monkeypatch):
    """Inject fake nastech_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("nastech_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("nastech_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_nastech_cli = types.ModuleType("nastech_cli")
    fake_nastech_cli.config = fake_config_mod
    fake_nastech_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "nastech_cli", fake_nastech_cli)
    monkeypatch.setitem(sys.modules, "nastech_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "nastech_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["nastech", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_nastech_home", lambda: tmp_path)
        _inject_fake_nastech_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "nastech-user"
