...
gateway process memory + sidecar child env — so cron/`nastech send`
standalone sends structurally could not authenticate. The adapter now
persists ``<nastech-home>/runtime/photon-sidecar.json`` after the sidecar
...
async def test_standalone_send_consumes_record_when_env_missing(
    ...
    assert ":9111/" in url
    assert headers["X-Nastech-Sidecar-Token"] == "record-token"
