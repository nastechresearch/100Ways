"""Tests for the bundled Nastech dashboard-auth plugin.
...
import plugins.dashboard_auth.nastech as nastech_plugin
...
def _patched_jwks(provider: nastech_plugin.NastechDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(nastech_plugin.NastechDashboardAuthProvider)

    def test_name_and_display(self):
        p = nastech_plugin.NastechDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "nastech"
        assert p.display_name == "Nastech Research"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nastech_plugin.NastechDashboardAuthProvider(
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        ...
        assert "NASTECH_DASHBOARD_OAUTH_CLIENT_ID" in nastech_plugin.LAST_SKIP_REASON
    ...
    def test_registers_with_default_portal_url_when_only_client_id_set(
        ...
        defaults to the production Nastech Portal. The user shouldn't have
...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``nastech_cli.config.load_config``
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in nastech_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {nastech_plugin.LAST_SKIP_REASON!r}"
        )
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return nastech_plugin.NastechDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://nastech.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "nastech_session_pkce" in result.cookie_payload
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["nastech_session_pkce"] != b.cookie_payload[
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://nastech.fly.dev/auth/callback")
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.nastech.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "nastech"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.nastech.httpx.post",
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self, provider):
        ...
        client_cls.assert_called_once_with(
            provider._jwks_url,
            cache_keys=True,
            lifespan=nastech_plugin._JWKS_CACHE_SECONDS,
            headers={
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.nastech"):
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.nastech.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-nastech-refresh-token"] == "rt_old_value"
