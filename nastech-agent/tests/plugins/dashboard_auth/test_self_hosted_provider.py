...
Covers, by analogy with ``test_nastech_provider.py``:
...
_ISSUER = "https://auth.example.com/application/o/nastech"
_CLIENT_ID = "nastech-dashboard"
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://nastech.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://nastech.example/auth/callback"
...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            ...
            "NASTECH_DASHBOARD_OIDC_SCOPES",
            "NASTECH_DASHBOARD_OIDC_CLIENT_SECRET",
        ):
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "NASTECH_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON


    def test_registers_from_env(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("NASTECH_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("NASTECH_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_secret_from_env(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("NASTECH_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("NASTECH_DASHBOARD_OIDC_CLIENT_SECRET", "env-secret")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_empty_env_secret_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_DASHBOARD_OIDC_CLIENT_SECRET", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
