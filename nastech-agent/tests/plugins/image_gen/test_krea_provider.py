...
class TestGenerate:
    ...
    def test_aspect_ratio_mapping(self):
        """Nastech 'square' must map to Krea '1:1' in the wire payload."""
        from plugins.image_gen.krea import KreaImageGenProvider
...
# ---------------------------------------------------------------------------
# Managed Nastech gateway path
# ---------------------------------------------------------------------------


def _managed_cfg(
    origin: str = "https://krea-gateway.example.com",
    token: str = "nastech-tok-abc",
):
    ...
    return SimpleNamespace(
        ...
        nastech_user_token=token,
        managed_mode=True,
    )


class TestManagedGateway:
    def test_managed_submit_uses_gateway_origin_and_nastech_token(self, monkeypatch):
        """Managed mode submits to the gateway origin with the Nastech token."""
