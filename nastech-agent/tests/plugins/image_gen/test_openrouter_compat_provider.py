#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + Nastech)."""
...
class TestProviderClass:
    def test_names(self):
        ...
        assert names == {"openrouter", "nastech"}
    ...
    def test_display_names(self):
        ...
        assert by_name["nastech"].display_name == "Nastech Portal"
    ...
    def test_nastech_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        nastech = {p.name: p for p in _build_providers()}["nastech"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert nastech._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        ...
        with patch(_RUNTIME, return_value=nastech_runtime), \
            ...
            nastech = {p.name: p for p in _build_providers()}["nastech"]
            result = nastech.generate(prompt="a pet")
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "nastech"}
    ...
    def test_both_are_reference_capable_for_pets(self):
        ...
        assert "nastech" in _REF_CAPABLE
