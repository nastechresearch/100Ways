"""Tests for nastech_constants module."""
...
class TestGetDefaultNastechRoot:
    ...
    def test_no_nastech_home_returns_native(self, tmp_path, monkeypatch):
        ...
        assert get_default_nastech_root() == tmp_path / ".nastech"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (NASTECH_HOME=<root>/profiles/<name>),
        ...
        assert get_default_nastech_root() == docker_root

    @pytest.mark.windows_only
    def test_no_nastech_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\nastech, not ~/.nastech."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NASTECH_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_nastech_root() == local_appdata / "nastech"



class TestGetNastechHome:
    """Tests for get_nastech_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When NASTECH_HOME is unset on Windows, use %LOCALAPPDATA%\\nastech."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NASTECH_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_nastech_home() == local_appdata / "nastech"


class TestGetProcessNastechHome:
    """Tests for get_process_nastech_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_nastech_home() == home




class TestNastechManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_nastech_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(home))
        monkeypatch.setattr(nastech_constants, "node_tool_runnable", lambda path: True)

        assert find_nastech_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nastech_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(nastech_constants, "heal_nastech_managed_node", lambda: False)
        monkeypatch.setattr(
            nastech_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert nastech_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Nastech-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nastech_constants, "heal_nastech_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = nastech_constants._NASTECH_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = nastech_constants.find_nastech_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert nastech_constants.find_nastech_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert nastech_constants.find_nastech_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(nastech_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(nastech_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from nastech_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".nastech"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import nastech_cli._subprocess_compat as subprocess_compat
...
class TestGetNastechDir:
    """Tests for ``get_nastech_dir(new_subpath, old_name)``.
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_nastech_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert nastech_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert nastech_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert nastech_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert nastech_constants.windows_path_to_wsl("/home/alex") is None
        assert nastech_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(nastech_constants, "is_wsl", lambda: True)
        assert nastech_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert nastech_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert nastech_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
