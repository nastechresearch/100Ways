"""The test suite must never write into the operator's real Nastech logs.

`nastech_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<NASTECH_HOME>/logs/agent.log`.

If NASTECH_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets NASTECH_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(nastech_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_nastech_home_is_sandboxed_before_imports(self):
        # Deliberately NOT os.environ: by test time the per-test `_isolate_env`
        # fixture has sandboxed NASTECH_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import NASTECH_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set NASTECH_HOME before test modules import"
        assert Path(home).resolve() != _real_nastech_home().resolve(), (
            f"NASTECH_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("nastech_cli.main")

        real_logs = str(_real_nastech_home() / "logs")
        offenders = [p for p in _all_file_destinations() if p.startswith(real_logs)]

        assert offenders == [], (
            "the test session is writing into the operator's real Nastech logs:\n  "
            + "\n  ".join(offenders)
        )
