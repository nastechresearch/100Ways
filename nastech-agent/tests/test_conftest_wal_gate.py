"""The conftest WAL gate must agree with nastech_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``nastech_state``. That is deliberate: importing
``nastech_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.nastech`` before the per-test ``NASTECH_HOME`` redirect, which makes tests
...
from nastech_state import is_sqlite_wal_reset_vulnerable
...
def test_conftest_gate_agrees_with_nastech_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and nastech_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_nastech_state_at_collection():
    """The gate must stay import-free of nastech_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.nastech, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``nastech_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "nastech_state" or name.startswith("nastech_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported nastech_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.nastech during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("nastech_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["nastech_state"] = saved
    assert not blocked
