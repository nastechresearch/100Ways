...
class TestGetDefaultModelForProvider:
    """Unit tests for nastech_cli.models.get_default_model_for_provider."""
    ...
    def test_catalog_label_overrides_constant(self):
        ...
        with patch(
            "nastech_cli.model_catalog.get_default_model_from_cache",
            return_value="qwen/qwen3.8-max",
        ):
            assert (
                models_mod.get_preferred_silent_default_model("nastech")
                == "qwen/qwen3.8-max"
            )
            # nastech catalog carries qwen3.8-max, so the full resolver follows.
            assert (
                models_mod.get_default_model_for_provider("nastech")
                == "qwen/qwen3.8-max"
            )
