...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"nastech-agent[{extra}]" in spec
        ]
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `nastech update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `nastech update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (nastech_cli/dingtalk_auth.py)
...
def test_every_lazy_deps_exact_pin_matches_uv_lock():
    ...
    install ships the locked version, and the ``nastech update`` lazy-refresh
    ...
    assert not drift, (
        ...
        "every package the core lock also ships — otherwise `nastech update` "
...
def test_huggingface_hub_lazy_pin_matches_uv_lock():
    ...
    a feature from mere package presence, the `nastech update` lazy-refresh
    ...
    assert lazy_pin == locked, (
        ...
        "huggingface-hub`), or `nastech update` will churn/downgrade the "
