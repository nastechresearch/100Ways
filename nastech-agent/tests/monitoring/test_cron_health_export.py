...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("nastech.gateway.up"), _M("nastech.gateway.active_agents"),
        _M("nastech.gateway.busy"), _M("nastech.gateway.drainable"),
        _M("nastech.gateway.restart_requested"),
        _M("nastech.platform.up"), _M("nastech.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("nastech.cron.scheduler.heartbeat_age_seconds"),
        _M("nastech.cron.scheduler.last_success_age_seconds"),
        _M("nastech.cron.scheduler.catch_up_occurrences"),
        _M("nastech.cron.jobs.enabled"), _M("nastech.cron.jobs.running"),
        _M("nastech.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Nastech Agent-owned Relay transport health" in text
