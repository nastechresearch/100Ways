...
def test_gateway_health_event_maps_to_span_with_attrs():
    ...
    assert spans[0].name == "nastech.gateway_health"
    attrs = dict(spans[0].attributes or {})
    assert attrs["nastech.old_state"] == "starting"
    assert attrs["nastech.new_state"] == "running"
    assert attrs["nastech.active_agents"] == 2
...
def test_trace_resource_includes_stable_hashed_instance():
    ...
    assert attrs["service.name"] == "nastech-gateway"
...
def test_streamer_receives_events_and_respects_filter(monkeypatch):
    ...
    spans = mem.get_finished_spans()
    assert [s.name for s in spans] == ["nastech.gateway_health"]
