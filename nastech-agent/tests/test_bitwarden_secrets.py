...
binary download are exercised manually by `nastech secrets bitwarden
...
def nastech_home(tmp_path, monkeypatch):
    """Point Nastech at an isolated home directory."""
    home = tmp_path / ".nastech"
    home.mkdir()
    monkeypatch.setenv("NASTECH_HOME", str(home))
    # Some modules cache get_nastech_home; clear if needed.
    import nastech_constants
    if hasattr(nastech_constants, "_NASTECH_HOME_CACHE"):
        nastech_constants._NASTECH_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(nastech_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from nastech_cli.env_loader import _apply_external_secret_sources
