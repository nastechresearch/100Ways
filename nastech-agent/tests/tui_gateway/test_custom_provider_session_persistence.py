...
"No LLM provider configured. Run `nastech model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real NASTECH home.
...
# provider (e.g. Nastech) but who switched THIS session to a self-hosted model
...
# found" (the b200/nastech-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-nastech-model", "provider": "nastech"},
    "providers": {
        "nastech-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
            "models": ["nastech-ultra-sft"],
        }
    },
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("nastech-ultra-sft")
            == "custom:nastech-ultra"
        )
