...
before ``prompt.submit`` installs the session's profile NASTECH_HOME override, so
...
def test_profile_home_session_writes_under_profile(tmp_path):
    ...
    profile_home = tmp_path / ".nastech" / "profiles" / "coder"
...
def test_launch_home_fallback_when_no_profile(tmp_path):
    ...
    launch_home = tmp_path / ".nastech"
    session = {}

    with patch("tui_gateway.server._nastech_home", launch_home):
...
def test_two_profiles_are_isolated(tmp_path):
    ...
    home_a = tmp_path / ".nastech" / "profiles" / "a"
    home_b = tmp_path / ".nastech" / "profiles" / "b"
