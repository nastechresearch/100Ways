...
by surface:
  ...
  * ``tui_gateway.entry`` — the stdio ``nastech --tui`` path.
  * ``nastech_cli.mcp_startup`` — the desktop app + dashboard WebSocket sidecar
    (``tui_gateway/ws.py``) and ``nastech dashboard``.
...
``nastech_cli.mcp_startup``), so the scheduler bailed immediately and a slow MCP
...
def test_entry_in_flight_sees_startup_thread(clean_discovery_globals):
    """Desktop/dashboard surface: discovery thread lives on nastech_cli.mcp_startup.
...
def test_startup_module_exposes_in_flight_helpers(clean_discovery_globals):
    """nastech_cli.mcp_startup gains the in-flight/join helpers entry delegates to."""
