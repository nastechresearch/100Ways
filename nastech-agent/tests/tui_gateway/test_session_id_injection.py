...
session id into NASTECH_SESSION_ID so terminal/execute_code subprocesses can
...
saw an empty NASTECH_SESSION_ID even though agent_init had set it via
...
def test_set_session_context_injects_agent_session_id(monkeypatch):
    """NASTECH_SESSION_ID must equal the live agent.session_id after binding."""
    _install_session(
    ...
    assert get_session_env("NASTECH_SESSION_ID") == "20260722_deadbeef"
...
def test_set_session_context_falls_back_to_session_key(monkeypatch):
    ...
    (never leave NASTECH_SESSION_ID empty for an identified session)."""
    _install_session(monkeypatch, session_key="skey-xyz", agent_session_id=None)

    server._set_session_context("skey-xyz")

    assert get_session_env("NASTECH_SESSION_ID") == "skey-xyz"
