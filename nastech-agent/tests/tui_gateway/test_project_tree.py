...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="nastech/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is NASTECH_HOME (config/state) must not spawn a
    # phantom project; it lands in the Home bucket. A real repo alongside it
    # still groups normally.
    resolve = _resolver(
        {
            "/home/me/.nastech": ("/home/me/.nastech", "/home/me/.nastech"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.nastech", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.nastech"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.nastech")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.nastech"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/nastech-agent": ("/www/nastech-agent", "/www/nastech-agent")})
    sessions = [
        _session("/www/nastech-agent", branch="main"),
        _session("/www/nastech-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/nastech-agent"]
    assert _lane_ids(project) == ["/www/nastech-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/nastech-agent"
