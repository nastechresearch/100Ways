...
Nastech's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``nastech_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call nastech_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call nastech_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Nastech root.  harden_import_path must relocate the
    Nastech root to the front so ``from utils import ...`` resolves to Nastech."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/nastech", "/usr/lib"]
        nastech_bootstrap.harden_import_path(src_root="/opt/nastech")
        assert sys.path[0] == "/opt/nastech"
        assert sys.path.index("/opt/nastech") < sys.path.index(
