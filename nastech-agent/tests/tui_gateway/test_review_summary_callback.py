    ...
    with patch.dict(
        "sys.modules",
        {
            "nastech_constants": MagicMock(
                get_nastech_home=MagicMock(return_value="/tmp/nastech_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'nastech-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'nastech-release' patched"
    }
