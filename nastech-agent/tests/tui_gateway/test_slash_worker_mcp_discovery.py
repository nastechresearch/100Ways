...
def test_profile_local_mcp_tool_is_visible_in_slash_worker(tmp_path):
    ...
    server.write_text(
        textwrap.dedent(
            ...
            @mcp.tool()
            def nastech_61922_profile_probe() -> str:
                return {marker!r}
    ...
    for key in list(env):
        if key.endswith("_API_KEY") or key.endswith("_TOKEN"):
            env.pop(key)
    env["NASTECH_HOME"] = str(profile_home)
    env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
    env["NASTECH_SLASH_WATCHDOG_GRACE_S"] = "0"
    env["NASTECH_SLASH_WATCHDOG_POLL_S"] = "0.05"
    ...
    try:
        ...
        assert "mcp__profileprobe__nastech_61922_profile_probe" in response["output"]
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=5)
