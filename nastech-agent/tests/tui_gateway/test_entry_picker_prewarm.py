...
guard is covered in ``tests/nastech_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``nastech_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from nastech_cli.model_switch,
