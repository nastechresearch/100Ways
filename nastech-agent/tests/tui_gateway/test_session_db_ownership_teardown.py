...
class _RecordingDB:
    """Stand-in for ``nastech_state.SessionDB`` that counts ``close()`` calls."""
...
def test_lazy_recall_open_is_owned_by_the_agent(monkeypatch):
    ...
    monkeypatch.setattr("nastech_state.SessionDB", _factory)
...
def build_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "set_nastech_home_override", lambda _home: None)
    monkeypatch.setattr(server, "reset_nastech_home_override", lambda _tok: None)
