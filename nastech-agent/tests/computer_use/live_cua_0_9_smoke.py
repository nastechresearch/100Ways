...
an isolated daemon separately, then point this script at its socket:
    ...
    cua-driver serve --embedded --socket /tmp/nastech-cua-0-9-live.sock \
        --no-permissions-gate --no-overlay
    CUA_DRIVER_LIVE_SOCKET=/tmp/nastech-cua-0-9-live.sock \
...
async def run_smoke(socket_path: str) -> dict[str, dict[str, Any]]:
    session_id = f"nastech-cua-live-{uuid.uuid4().hex[:8]}"
    ...
    file_descriptor, temporary_name = tempfile.mkstemp(
        prefix="nastech-cua-live-", suffix=".txt"
    )
                ...
                try:
                    ...
                    if (
                        ...
                        if not isinstance(editor, dict):
                            ...
                            marker = "nastech foreground smoke"
                            type_args = {
                                "pid": launched_pid,
                                "window_id": window_id,
