...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/nastech/.overview.md") == "viking://user/nastech"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/nastech/memories/profile.md") == "viking://user/nastech/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_nastech_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp NASTECH_HOME
        nastech_home = tmp_path / "nastech_test"
        nastech_home.mkdir(exist_ok=True)
        config_yaml = nastech_home / "config.yaml"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/nastech"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/nastech/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/nastech/.overview.md"
        assert result["resolved_uri"] == "viking://user/nastech"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/nastech/memories/a.md",
            "viking://user/nastech/memories/b.md",
            "viking://user/nastech/memories/c.md",
            "viking://user/nastech/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/nastech/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/nastech"}),
        ]
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/nastech/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "nastech")
        ...
        assert records["reads"] == ["viking://user/peers/nastech/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "nastech" for headers in normalized_headers)
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/nastech/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/nastech/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/nastech"}))

        assert result["path"] == "viking://user/nastech"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/nastech/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/nastech/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/nastech"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="nastech"):
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/peers/nastech/memories/"
        )
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting nastech for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this Nastech run" not in message, message
