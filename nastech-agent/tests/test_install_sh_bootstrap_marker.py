...
The marker at ``$INSTALL_DIR/.nastech-bootstrap-complete`` is what the desktop
...
def make_checkout(tmp_path):
    install_dir = tmp_path / "nastech-agent"
    install_dir.mkdir()
...
def test_marker_matches_the_schema_the_desktop_validates(tmp_path):
    ...
    marker = install_dir / ".nastech-bootstrap-complete"
...
def test_marker_publish_leaves_no_temp_sibling(tmp_path):
    ...
    assert (install_dir / ".nastech-bootstrap-complete").is_file()
    assert not (install_dir / ".nastech-bootstrap-complete.tmp").exists()
...
def test_explicit_commit_pin_wins_over_head(tmp_path):
    ...
    payload = json.loads((install_dir / ".nastech-bootstrap-complete").read_text())
...
def test_no_marker_written_when_head_cannot_be_resolved(tmp_path):
    ...
    assert not (install_dir / ".nastech-bootstrap-complete").exists()
