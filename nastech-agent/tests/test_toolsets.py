...
class TestToolsetConsistency:
    ...
    def test_nastech_platforms_share_core_tools(self):
        """All nastech-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on nastech-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["nastech-cli", "nastech-telegram", "nastech-discord", "nastech-whatsapp", "nastech-slack", "nastech-signal", "nastech-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_nastech_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("nastech-whatsapp")
