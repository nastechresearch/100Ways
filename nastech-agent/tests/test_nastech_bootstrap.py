"""Tests for nastech_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Nastech entry point
(nastech, nastech-agent, nastech-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Nastech entry point imports nastech_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported nastech_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("nastech_bootstrap", None)
    import nastech_bootstrap  # noqa: WPS433
    return nastech_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Nastech CLI
...
class TestEntryPointsImportBootstrap:
    """Every Nastech entry point must import nastech_bootstrap as its
    ...
    # Entry points that invoke Nastech as a process.  Each one must
    # import nastech_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "nastech_cli/main.py",   # nastech CLI (console_script)
        "run_agent.py",          # nastech-agent (console_script)
        "acp_adapter/entry.py",  # nastech-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import nastech_bootstrap' and that
        ...
        half-finished ``nastech update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``nastech_bootstrap`` as a top-level module) leaves nastech
        ...
        """
        # Resolve relative to the nastech-agent repo root.  Tests live
        # at tests/test_nastech_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # nastech start even when nastech_bootstrap hasn't been
        ...
        assert first_import_name == "nastech_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'nastech_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import nastech_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Nastech's own top-level modules — covering both
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/nastech", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Nastech root.  The guard must relocate Nastech to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/nastech"])
        assert result[0] == "/opt/nastech"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Nastech root.
        assert result.index("/opt/nastech") < result.index("/home/user/tg-ws-proxy")
