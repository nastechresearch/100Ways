...
def test_required_frontmatter_fields(p):
    ...
    nastech = (fm.get("metadata") or {}).get("nastech") or {}
    if not (nastech.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.nastech.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    nastech = (fm.get("metadata") or {}).get("nastech") or {}
    dangling = [
        rs for rs in (nastech.get("related_skills") or []) if rs not in _all_names()
    ]
