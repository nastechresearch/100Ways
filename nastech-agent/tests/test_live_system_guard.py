...
def test_argv_arguments_are_not_treated_as_executables(tmp_path):
    ...
    guard (the path contains "nastech" via the pytest tmp root)."""
...
def test_direct_killer_argv_is_still_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "nastech-guard-regression-nomatch"])
...
def test_wrapped_killer_command_is_still_blocked():
    ...
    shell wrapper."""
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["bash", "-c", "pkill -f nastech-guard-regression-nomatch"])


def test_env_wrapped_killer_command_is_still_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["env", "GUARD_TEST=1", "pkill", "-f", "nastech-guard-regression-nomatch"])
