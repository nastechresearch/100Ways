...
Provides a default ``NASTECH_MODEL`` for cron run_job tests so each one
doesn't have to spell out a model. The global conftest blanks
NASTECH_MODEL hermetically; without this autouse fixture every cron test
that exercises ``run_job`` would hit the fail-fast guard added in
``cron/scheduler.py`` (see issue #23979) and have to be rewritten.

Tests that specifically need ``NASTECH_MODEL`` unset — model-resolution
edge cases — call ``monkeypatch.delenv("NASTECH_MODEL", raising=False)``
