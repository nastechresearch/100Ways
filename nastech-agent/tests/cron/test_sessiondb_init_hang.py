...
which is already bounded by NASTECH_CRON_TIMEOUT — the worker thread submitted
...
timeout (NASTECH_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed NASTECH_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors NASTECH_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("NASTECH_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "NASTECH_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_CRON_SESSION_DB_TIMEOUT", raising=False)
