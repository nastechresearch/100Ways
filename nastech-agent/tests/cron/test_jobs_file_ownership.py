...
Running a state-writing ``nastech cron`` CLI command as root (the default for
...
2. Zombie-ticker surfacing: a failing tick must persist the failure reason
   (``ticker_last_error``) where ``nastech cron status`` can show it, and a
...
class TestTickerLoopRecordsErrors:
    ...
    def test_failing_tick_persists_error(self, cron_store, monkeypatch):
        ...
        msg = jobs.get_ticker_last_error()
        assert msg is not None, (
            "a failing tick must persist its reason for `nastech cron status` "
            "to surface (#68483)"
        )
        assert "Permission denied" in msg
...
# =========================================================================
# 3. `nastech cron status` surfaces the failure reason
# =========================================================================


class TestCronStatusSurfacesError:
    def test_status_shows_last_error_and_permission_hint(self, monkeypatch, capsys):
        from nastech_cli import cron as cron_cli

        monkeypatch.setattr("nastech_cli.gateway.find_gateway_pids", lambda: [4321])
