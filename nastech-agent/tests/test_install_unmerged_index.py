...
The ``nastech update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m nastech_cli.main gateway run`` — image name
    ``pythonw``, not ``nastech.exe`` — so the ``taskkill /IM nastech.exe`` guard
    ...
    # The nastech.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM nastech.exe' in text
