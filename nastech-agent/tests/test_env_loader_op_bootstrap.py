...
1. ``load_nastech_dotenv()`` auto-loads ``~/.nastech/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_nastech_dotenv(nastech_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
