...
def scenario(name):
    ...
    def wrap(fn):
        def run():
            ...
            from nastech_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "nastech_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from nastech_cli.kanban_db import resolve_workspace
...
def _(home, kb):
    ...
    try:
        from nastech_cli import web_server as ws  # noqa
    except Exception:
        pass
