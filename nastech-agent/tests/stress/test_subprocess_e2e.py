...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves nastech_cli.kanban_db on its own
...
def main():
    ...
    # Point the `nastech` CLI child processes will run at the worktree
    # nastech_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "nastech")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m nastech_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/nastech_e2e_*/worker_*.log
    import glob
