...
These run without Docker: the shim's NASTECH_TINI_SHIM_TARGET /
NASTECH_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["NASTECH_TINI_SHIM_TARGET"] = str(init)
    env["NASTECH_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "NASTECH_TINI_SHIM_TARGET" in text
