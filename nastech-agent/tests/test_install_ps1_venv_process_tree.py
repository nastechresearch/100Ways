"""Windows installer regression for Nastech children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Nastech process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = nastech_home / ".nastech-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Nastech launchers
