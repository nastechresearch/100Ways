...
Regression coverage for the #65998 class: a ``nastech --tui`` process whose
...
class TestSignalArmLogic:
    def test_arms_with_double_cleanup_timeout(self, monkeypatch):
        monkeypatch.setenv("NASTECH_EXIT_WATCHDOG_S", "7")
    ...
    def test_bad_env_value_falls_back_to_default(self, monkeypatch):
        monkeypatch.setenv("NASTECH_EXIT_WATCHDOG_S", "not-a-number")
        with patch.object(cli, "_arm_exit_watchdog") as arm:
            cli._arm_exit_watchdog_on_shutdown_signal()
...
def test_sigterm_on_wedged_process_forces_exit_within_leash():
    ...
    NASTECH_EXIT_WATCHDOG_S; without the signal it would live forever."""
    env = dict(os.environ, NASTECH_EXIT_WATCHDOG_S="1", PYTHONPATH=_REPO_ROOT)
