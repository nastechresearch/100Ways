...
class TestCliApprovalUi:
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        assert seen["approval"].__func__ is NastechCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is NastechCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    cli._paint_now = NastechCLI._paint_now.__get__(cli, NastechCLI)
    cli._invalidate = NastechCLI._invalidate.__get__(cli, NastechCLI)
