...
def _isolate(tmp_path, monkeypatch):
    """Isolate NASTECH_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal NastechCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = NastechCLI.__new__(NastechCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["nastech-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Nastech' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Nastech' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Nastech' minimum context length."""
