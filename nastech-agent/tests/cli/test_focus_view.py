...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = NastechCLI.__new__(NastechCLI)
        host.model = "opus"
        ...
        with patch.object(NastechCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(NastechCLI, "_is_session_yolo_active", return_value=False):
            text = NastechCLI._build_status_bar_text(host, width=width)
...
def _make_agent(tool_progress_mode: str):
    ...
    with (
        ...
        patch("nastech_cli.config.load_config", return_value={}),
...
class TestCommandRegistration:
    ...
    def test_verbose_cycle_releases_focus_view(self):
        ...
        with patch("cli.save_config_value", return_value=True), patch("cli._cprint"):
            NastechCLI._toggle_verbose(host)
