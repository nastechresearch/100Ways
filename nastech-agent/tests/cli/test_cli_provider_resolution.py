...
def test_nastech_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in NastechCLI.__init__")

    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("nastech_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.NastechCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("nastech_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_nastech_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    import yaml

    config_home = tmp_path / "nastech"
    config_home.mkdir()
    monkeypatch.setenv("NASTECH_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        ...
        lambda provider: {
            "access_token": "nastech-token",
            "portal_base_url": "https://portal.example.com",
        },
    )
    monkeypatch.setattr(
        "nastech_cli.auth.resolve_nastech_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "nastech_cli.models.get_curated_nastech_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("nastech_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("nastech_cli.models.check_nastech_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "nastech_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "nastech_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "nastech_cli.nastech_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    nastech_main._model_flow_nastech(stale_config, current_model="glm-5.2")

    config = yaml.safe_load(config_path.read_text()) or {}
    model = config.get("model")
    assert model["provider"] == "nastech"
    assert model["default"] == selected_model
    assert model["base_url"] == "https://inference-api.nastechresearch.com/v1"
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.NastechCLI(compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("nastech_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("nastech_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("nastech_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("nastech_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "nastech_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("nastech_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    nastech_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    saved_env = {}
    monkeypatch.setattr("nastech_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("nastech_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "nastech_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "nastech_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("nastech_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    nastech_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nastech_login_tls_options(monkeypatch):
    monkeypatch.setattr(nastech_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "nastech_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "nastech"}},
    )
    monkeypatch.setattr("nastech_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("nastech_cli.auth.resolve_provider", lambda requested, **kwargs: "nastech")
    ...
    monkeypatch.setattr("nastech_cli.auth._login_nastech", _fake_login)

    nastech_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nastechresearch.com",
            inference_url="https://inference.nastechresearch.com/v1",
...
def test_auto_provider_name_localhost():
    from nastech_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from nastech_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "nastech_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("nastech_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("nastech_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="NASTECH_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "NASTECH_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from nastech_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
