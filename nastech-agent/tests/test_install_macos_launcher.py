"""Regression coverage for the user-facing macOS Nastech launcher."""
...
def test_venv_launcher_bypasses_uv_console_script_that_requires_realpath(tmp_path: Path) -> None:
    """Stock macOS must start Nastech even when its uv console script needs realpath."""
    ...
    (install_dir / "nastech").write_text("# source entrypoint\n", encoding="utf-8")
    _make_executable(
        venv_bin / "nastech",
        "#!/bin/sh\n"
        f'PATH="{minimal_path}"\n'
