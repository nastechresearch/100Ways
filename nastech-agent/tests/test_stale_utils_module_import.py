...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.nastech/nastech-agent/utils.py)
...
2. ``nastech update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``nastech_cli/main.py`` (the ``nastech update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``nastech_constants`` after the
