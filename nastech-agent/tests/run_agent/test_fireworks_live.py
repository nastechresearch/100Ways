...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the Nastech runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "Nastech failed to build a Fireworks client"
    return client, model


def test_nastech_wires_fireworks_client():
...
def test_fireworks_basic_chat_through_runtime():
    """A single-turn completion via the Nastech-resolved client returns text."""
    client, model = _resolve_runtime_client()
