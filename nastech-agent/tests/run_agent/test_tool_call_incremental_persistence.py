...
def _make_agent():
    nastech_home = Path(tempfile.mkdtemp(prefix="nastech-test-home-"))
    (nastech_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._nastech_home", nastech_home),
