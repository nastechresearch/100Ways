...
class TestFallbackChainAdvancement:
    ...
    def test_nastech_anthropic_fallback_uses_the_messages_wire(self):
        """Portal Claude fallbacks must not stay on chat_completions.

        ``resolve_provider_client`` still returns an OpenAI client for Nastech;
        ...
        """
        portal = "https://inference-api.nastechresearch.com/v1"
        fbs = [
            {
                "provider": "nastech",
        ...
        with (
            ...
            patch(
                "nastech_cli.model_normalize.normalize_model_for_provider",
...
class TestFallbackChainDedup:
    ...
    def test_skips_entry_matching_current_provider_and_model(self):
        ...
        with patch("agent.auxiliary_client.resolve_provider_client", side_effect=_resolve):
            with patch("nastech_cli.model_normalize.normalize_model_for_provider", side_effect=lambda m, p: m):
