...
class TestCredentialPoolPreservedOnAutoDetect:
    ...
    def test_anthropic_pool_preserved_with_url_auto_detect(self):
        ...
        with patch("agent.auxiliary_client.resolve_provider_client", return_value=(None, None)), \
             ...
             patch('nastech_cli.model_normalize.normalize_model_for_provider', return_value='test-model'), \
             patch('agent.credential_pool.load_pool', return_value=MagicMock()), \
             patch('nastech_cli.config.load_config', return_value={}), \
             patch('nastech_cli.config.get_compatible_custom_providers', return_value=[]), \
             patch('agent.iteration_budget.IterationBudget'), \
             patch('nastech_cli.config.cfg_get', return_value=None):
