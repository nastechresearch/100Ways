...
class TestBuildApiKwargsNastechPortal:
    def test_includes_nastech_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == nastech_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Nastech) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_nastech_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _NASTECH_MODEL, get_text_auxiliary_client
        nastech_auth = {
    ...
    def test_custom_endpoint_when_no_nastech(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Nastech keys are available.
        ...
        with patch("agent.auxiliary_client._read_nastech_auth", return_value=None), \
