...
- 96vc: ACP didn't set NASTECH_EXEC_ASK, so `check_all_command_guards`
...
1. Setting NASTECH_EXEC_ASK inside _run_agent (wraps the agent call).
...
def _isolate_approval_state(monkeypatch):
    ...
    developer's real ``~/.nastech/config.yaml`` command allowlist into
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set NASTECH_INTERACTIVE so
    ...
    (NASTECH_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When NASTECH_INTERACTIVE is set and an approval callback is
        ...
        # Without NASTECH_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without NASTECH_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With NASTECH_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with NASTECH_INTERACTIVE the approval path should consult the "
