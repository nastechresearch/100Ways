...
class TestHandleFunctionCall:
    ...
    def test_post_tool_call_receives_non_negative_integer_duration_ms(self):
        ...
        with (
            ...
            patch("nastech_cli.plugins.has_hook", return_value=True),
            patch("nastech_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("nastech_cli.plugins.has_hook", return_value=False),
            patch("nastech_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.plugins.invoke_middleware", fake_invoke_middleware)
        monkeypatch.setattr("nastech_cli.plugins.get_plugin_manager", lambda: manager)
        hook_calls = []
        monkeypatch.setattr(
            "nastech_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: hook_calls.append((hook_name, kwargs)) or [],
        )
        monkeypatch.setattr("nastech_cli.plugins.has_hook", lambda name: True)
    ...
    def test_registry_exception_emits_terminal_tool_hook(self, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr("nastech_cli.plugins.invoke_hook", lambda *_args, **_kwargs: [])
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("nastech_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_relay_rewrite_is_visible_to_pre_tool_authorization(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nastech_cli.observability.relay_runtime.apply_tool_request_intercepts",
            rewrite,
        )
        monkeypatch.setattr("nastech_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("nastech_cli.plugins.has_hook", lambda name: True)
...
class TestDisabledToolsetsPlatformBundle:
    ...
    def test_disabling_platform_bundle_preserves_core_tools(self):
        ...
        assert names_telegram == names_no_yuanbao, (
            f"Tools lost after disabling nastech-yuanbao: "
            f"{names_telegram - names_no_yuanbao}"
        )

    def test_disabling_platform_bundle_removes_own_tools(self):
        """Disabling nastech-discord should remove discord-specific tools."""
        from model_tools import get_tool_definitions

        tools = get_tool_definitions(
            enabled_toolsets=["nastech-discord"],
            disabled_toolsets=["nastech-discord"],
            quiet_mode=True,
        )
    ...
    def test_bundle_non_core_tools_unknown_falls_back(self):
        ...
        assert bundle_non_core_tools("nastech-does-not-exist") == set()
...
class TestDisabledToolsetsPostureToolset:
    ...
    not own -- same non-core-delta subtraction as nastech-* bundles (#33924) --
