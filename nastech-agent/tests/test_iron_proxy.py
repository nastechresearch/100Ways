...
def nastech_home(tmp_path, monkeypatch):
    """Point NASTECH_HOME at a temp dir so install paths don't touch the real $HOME."""
...
def test_load_mappings_handles_corrupt_json(nastech_home):
...
def test_verify_checksums_signature_skips_without_gpg(nastech_home, monkeypatch, tmp_path):
...
def test_start_proxy_idempotent_when_already_running(nastech_home, monkeypatch):
    state = ip._proxy_state_dir()
...
def test_subprocess_env_strips_unrelated_secrets(nastech_home, monkeypatch):
...
def test_ca_key_created_with_0o600(nastech_home, monkeypatch):
...
def test_ensure_audit_log_creates_with_0o600(nastech_home, tmp_path):
...
def test_ensure_audit_log_tightens_existing_perms(nastech_home, tmp_path):
    audit = tmp_path / "audit.log"
...
def test_proxy_state_dir_is_0o700(nastech_home):
    state = ip._proxy_state_dir()
...
def test_mappings_roundtrip_preserves_headers_and_aliases(nastech_home):
    m = ip.TokenMapping(
...
def test_ensure_management_token_persists_and_is_stable(nastech_home):
    t1 = ip.ensure_management_token()
    t2 = ip.ensure_management_token()
    assert t1 == t2
    assert t1.startswith("nastech-mgmt-")
...
def test_reload_proxy_refuses_when_not_running(nastech_home, monkeypatch):
...
def test_reload_proxy_posts_bearer_to_management_endpoint(nastech_home, monkeypatch):
...
def test_start_proxy_injects_management_key_env(nastech_home, monkeypatch):
    ...
    cfg = ip.build_proxy_config(
        mappings=[_sample_mapping()],
        ca_cert=nastech_home / "ca.crt",
        ca_key=nastech_home / "ca.key",
        http_listen=["127.0.0.1:9090"],
    )
    ip.write_proxy_config(cfg)
    (nastech_home / "bin").mkdir(parents=True, exist_ok=True)
    fake_bin = nastech_home / "bin" / "iron-proxy"
    ...
    assert captured_env[ip._MGMT_API_KEY_ENV].startswith("nastech-mgmt-")
...
def test_docker_egress_node_options_uses_sentinel(nastech_home, monkeypatch):
    """``_egress_proxy_args_for_docker`` should NOT put NODE_OPTIONS in
    env_overrides directly; it uses a sentinel key
    ``_NASTECH_EGRESS_NODE_OPTIONS_APPEND`` so DockerEnvironment can
    ...
    assert env.get("_NASTECH_EGRESS_NODE_OPTIONS_APPEND") == "--use-openssl-ca"
...
def test_persisted_nonce_roundtrip(nastech_home, monkeypatch):
...
def test_get_status_probes_configured_bind_host(nastech_home, monkeypatch):
