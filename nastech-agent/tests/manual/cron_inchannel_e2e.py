...
Uses a throwaway NASTECH_HOME so it never touches ~/.nastech. No real names.
"""
...
def _fresh_home():
    """Point NASTECH_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_nastech_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["NASTECH_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER NASTECH_HOME is set.
...
# a different get_nastech_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"NASTECH_HOME (throwaway): {HOME}")
