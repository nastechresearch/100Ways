"""Global emergency stop (`nastech pause` / `nastech resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`nastech resume`)
...
def test_engage_creates_sentinel_and_is_engaged(nastech_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (nastech_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(nastech_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (nastech_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(nastech_home):
    ...
    raw = json.loads((nastech_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(nastech_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(nastech_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (nastech_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(nastech_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(nastech_home):
    ...
    assert "nastech resume" in notice


def test_paused_reply_without_reason(nastech_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(nastech_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(nastech_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(nastech_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(nastech_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(nastech_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(nastech_home):
...
# ── CLI: nastech pause / nastech resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(nastech_home, capsys):
    from nastech_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(nastech_home, capsys):
    from nastech_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(nastech_home, capsys):
    from nastech_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(nastech_home, capsys):
    from nastech_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from nastech_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── nastech status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(nastech_home):
    from nastech_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(nastech_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when NASTECH_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(nastech_home):
    ...
    try:
        ...
        return
    assert reply is None or "nastech is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(nastech_home):
    from gateway.run import GatewayRunner
