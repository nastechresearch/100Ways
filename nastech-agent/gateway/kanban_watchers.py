...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`nastech pause`) is engaged.
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # NASTECH_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `nastech kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours NASTECH_KANBAN_DISPATCH_IN_GATEWAY env var
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `nastech kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Nastech profile
            ...
            PATH, missing venv, credential loss for a real Nastech profile).
            """
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from nastech_cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("NASTECH_KANBAN_BOARD")
                try:
                    os.environ["NASTECH_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("NASTECH_KANBAN_BOARD", None)
                    else:
                        os.environ["NASTECH_KANBAN_BOARD"] = prev_env
            return successes
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`nastech pause`): skip auto-decompose
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`nastech kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
