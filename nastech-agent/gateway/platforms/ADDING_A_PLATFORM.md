...
There are two ways to add a platform to the Nastech gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nastech/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Nastech code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nastech gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nastech_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Nastech core.
...
And add it to the `nastech-gateway` composite:

```python
"nastech-gateway": {
    "includes": [..., "nastech-your-platform"]
}
```
...
## 12. Status Display (`nastech_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nastech_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nastech_cli/ toolsets.py \
  --include="*.py" -l | sort -u
