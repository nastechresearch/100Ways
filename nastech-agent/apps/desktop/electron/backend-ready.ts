...
// `nastech serve` announces NASTECH_BACKEND_READY; the legacy `nastech dashboard`
// backend announces NASTECH_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^NASTECH_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `nastech_cli.main` → `web_server` → FastAPI/
 ...
 * NASTECH_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.NASTECH_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
/**
 * Watch a child process's stdout for the `NASTECH_(BACKEND|DASHBOARD)_READY
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Nastech backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Nastech backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
