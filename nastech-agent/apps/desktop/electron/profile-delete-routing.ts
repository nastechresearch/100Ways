// Profile-delete routing logic for the `nastech:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_nastech_home() and
...
/**
 * Parse a `nastech:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `nastech:api` request away from the primary/window backend
 ...
 * ensure_nastech_home() recreates the directory the delete just removed.
 */
