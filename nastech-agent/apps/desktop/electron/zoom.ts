...
export const ZOOM_STORAGE_KEY = 'nastech:desktop:zoomLevel'
 ...
 * but forgot to emit 'nastech:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('nastech:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
