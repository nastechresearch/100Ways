...
test('storage key stays stable so persisted zoom survives upgrades', () => {
  assert.equal(ZOOM_STORAGE_KEY, 'nastech:desktop:zoomLevel')
})
...
// 'nastech:zoom:changed' events. applyZoomLevel is the single funnel every zoom
...
test('applyZoomLevel applies the level then notifies the renderer', () => {
  ...
  assert.deepEqual(wc.calls, [
    ['setZoomLevel', 3],
    ['send', 'nastech:zoom:changed', { level: 3, percent: zoomLevelToPercent(3) }]
  ])
})
...
test('applyZoomLevel clamps garbage before applying and notifying', () => {
  ...
  assert.deepEqual(wc.calls, [
    ...
    ['send', 'nastech:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) }]
  ])
})
