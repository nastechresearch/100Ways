...
export async function revalidateRemoteConnection<TConnection extends RemoteConnectionDescriptor>({
  ...
  try {
    ...
    if (!failure.shouldReset) {
      log(
        `Cached remote Nastech backend failed liveness probe (${failure.failures}/${REMOTE_LIVENESS_FAILURE_LIMIT}); keeping connection for retry.`
      )

      return { ok: true, rebuilt: false }
    }

    log('Cached remote Nastech backend failed liveness probe; dropping stale connection.')
    resetConnection()
