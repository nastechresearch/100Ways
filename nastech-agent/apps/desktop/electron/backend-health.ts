...
export interface NastechReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForNastechReady`).
   */
...
export async function waitForNastechReady(baseUrl: string, options: NastechReadyOptions): Promise<void> {
  ...
  throw new Error(`Nastech backend did not become ready: ${detail}`)
}
