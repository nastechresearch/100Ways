...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env NASTECH_DASHBOARD_SESSION_TOKEN=abc123deadbeef NASTECH_DESKTOP=1 nastech dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /NASTECH_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /NASTECH_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Nastech-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Nastech-Session-Token: hdr_888'), /X-Nastech-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Nastech-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('nastech', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v nastech', 15000)
  assert.equal(args[args.length - 1], 'command -v nastech')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `nastech-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `nastech-ssh-test-${process.pid}-${Date.now()}`)
