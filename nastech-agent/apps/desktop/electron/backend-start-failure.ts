 ...
 * startNastech() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startNastech() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startNastech`, re-emits
