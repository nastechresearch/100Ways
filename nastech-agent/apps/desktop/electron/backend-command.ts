// Backend subcommand routing for the desktop-managed Nastech process.
//
// The desktop app launches its own headless backend via `nastech serve` — it
...
// app hasn't updated yet, or an older `nastech` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Nastech profile to pin via `--profile`.
 */
 ...
 * `-m nastech_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `nastech_cli/subcommands/dashboard.py` source registers
