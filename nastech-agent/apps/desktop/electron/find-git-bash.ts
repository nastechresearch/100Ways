 ...
 *   1. NASTECH_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\nastech\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect NASTECH_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.NASTECH_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'nastech', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'nastech', 'git', 'usr', 'bin', 'bash.exe'))
  }
