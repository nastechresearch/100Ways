...
test('parseWorktrees: main checkout + linked worktree', () => {
  const out = [
    ...
    'HEAD def456',
    'branch refs/heads/nastech/feat',
    ''
  ].join('\n')
  ...
  assert.equal(trees[1].branch, 'nastech/feat')
})
...
test('ensureGitRepo: inits a plain dir with a root commit so worktrees branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-wt-'))
...
test('switchBranch: switches a normal checkout branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-switch-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: lists locals and flags the checked-out branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-branches-'))
...
test('listBranches: flags a free default branch as default, not checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-branches-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBranches: a branch claimed by a worktree is flagged checked out', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-branches-wt-'))
...
test('listBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-nonrepo-'))
...
test('addWorktree: existingBranch checks the branch out without a new branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-convert-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: existing default branch switches the main checkout, not .worktrees/main', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-convert-default-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: lists local branches and flags the default', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-base-branches-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('listBaseBranches: empty on a non-repo path', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-base-nonrepo-'))
...
test('addWorktree: base param branches off a specified local branch', async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-base-add-'))
  const git = (...args) => execFileSync('git', args, { cwd: dir }).toString().trim()
...
test('addWorktree: base origin/main does not set up upstream tracking', async () => {
  ...
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-remote-'))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-clone-'))
...
function seedRemoteAndClone(label, branches) {
  const remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), `nastech-${label}-remote-`))
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), `nastech-${label}-clone-`))
  ...
  execFileSync('git', ['init', '-b', 'main', remoteDir])
  remoteGit('-c', 'user.email=nastech@localhost', '-c', 'user.name=Nastech', 'commit', '--allow-empty', '-m', 'root')
