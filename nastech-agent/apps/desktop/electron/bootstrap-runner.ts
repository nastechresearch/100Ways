...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.nastech-bootstrap-complete'), 'utf8')
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NastechResearch/nastech-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(nastechHome, installRef.cacheKey)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, nastechHome }: any = {}) {
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, nastechHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, nastechHome, pinCommit = true }) {
...
async function fetchManifest({ scriptPath, installerKind, emit, nastechHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, nastechHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, nastechHome, pinCommit })
      ]
