...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(NASTECH_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Nastech-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.nastech/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'nastech-desktop-ssh')
  }

  return path.join(os.homedir(), '.nastech', 'desktop-ssh')
}
