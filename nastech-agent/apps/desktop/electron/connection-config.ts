 ...
 *     `X-Nastech-Session-Token` header; WS uses `?token=`.
...
// nastech_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - nastech_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - nastech_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / nastech #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-nastech_session_at', '__Secure-nastech_session_at', 'nastech_session_at']
const RT_COOKIE_VARIANTS = ['__Host-nastech_session_rt', '__Secure-nastech_session_rt', 'nastech_session_rt']

// The Nastech portal (NAS) does NOT use Nastech gateway session cookies — it is a
...
// Keep this aligned with nastech_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['nastech', 'test', 'tmp', 'root', 'sudo'])
...
// Nastech Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteNastechPath = String(entry.remoteNastechPath || '').trim()

  if (remoteNastechPath) {
    out.remoteNastechPath = remoteNastechPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Nastech installation. Preserve an explicit mapping
  // when it is a valid Nastech profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `NASTECH_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://nastech.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a nastech session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live Nastech PORTAL (Privy) session — a non-empty
 ...
 * the Nastech gateway session cookies, so cloud sign-in / discovery liveness
