...
test('sandbox marker round-trips through the userData file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-sandbox-marker-'))
...
test('buildIcaclsGrantArgs targets ALL APPLICATION PACKAGES with inherited RX', () => {
  assert.deepEqual(buildIcaclsGrantArgs('C:\\Nastech\\win-unpacked'), [
    'C:\\Nastech\\win-unpacked',
    '/grant',
...
test('grantAllApplicationPackagesAcl is a no-op off Windows and reports exec failures', () => {
  ...
  const ok = grantAllApplicationPackagesAcl('C:\\Nastech', {
    platform: 'win32',
  ...
  assert.deepEqual(calls[0]?.args, buildIcaclsGrantArgs('C:\\Nastech'))

  const failed = grantAllApplicationPackagesAcl('C:\\Nastech', {
    platform: 'win32',
    execFileSync() {
...
test('buildNoSandboxRelaunchArgs appends a single --no-sandbox flag', () => {
  assert.deepEqual(buildNoSandboxRelaunchArgs(['--foo', '--no-sandbox', 'nastech://x']), [
    '--foo',
    'nastech://x',
    '--no-sandbox'
  ])
