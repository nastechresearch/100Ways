...
test('NASTECH_GIT_BASH_PATH override takes precedence', () => {
  const result = findGitBash({
    isWindows: true,
    env: { NASTECH_GIT_BASH_PATH: 'D:\\CustomGit\\bin\\bash.exe' },
    fileExists: yes,
    findOnPath: () => null
  })
...
test('NASTECH_GIT_BASH_PATH invalid path falls through to candidates', () => {
  const env = {
    NASTECH_GIT_BASH_PATH: 'X:\\Missing\\bash.exe',
...
test('NASTECH_GIT_BASH_PATH empty string is ignored', () => {
  const result = findGitBash({
    isWindows: true,
    env: { NASTECH_GIT_BASH_PATH: '', LOCALAPPDATA: '' },
    fileExists: no,
