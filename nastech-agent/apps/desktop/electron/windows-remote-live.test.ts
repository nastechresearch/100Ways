...
//   NASTECH_WIN_SSH_HOST   ssh alias/host of the Windows box
//   NASTECH_WIN_SSH_USER   remote user
//   NASTECH_WIN_SSH_NASTECH absolute path to the remote nastech.exe under test
const liveHost = process.env.NASTECH_WIN_SSH_HOST || ''
const liveUser = process.env.NASTECH_WIN_SSH_USER || ''
const configuredNastech = process.env.NASTECH_WIN_SSH_NASTECH || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Nastech-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredNastech)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredNastech.replace('nastech.exe', 'python.exe')}' -m nastech_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredNastech.replace('nastech.exe', 'python.exe')
        const terminate = `& '${python}' -m nastech_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.nastechPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m nastech_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m nastech_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
