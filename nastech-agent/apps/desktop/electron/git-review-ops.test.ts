...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'nastech-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Nastech Test'], { cwd: dir })
