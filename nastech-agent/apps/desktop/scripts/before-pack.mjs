 ...
 * product name (`Nastech`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Nastech'
 *
 * This is a hard failure with no obvious cause for the user — `nastech desktop`
 ...
 * (the mac-unpacked Nastech.app bundle) and Windows (win-unpacked), so we
 ...
 * Nastech.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `nastech desktop --build-only` (nastech_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Nastech.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Nastech'}.exe`
