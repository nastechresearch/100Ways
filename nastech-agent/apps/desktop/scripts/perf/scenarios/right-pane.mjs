...
export default {
  ...
  async run(cdp, opts = {}) {
    ...
    const ready = await cdp.eval(
      `!!(window.__PERF_DRIVE__?.rightPaneSetup && window.__RIGHT_PANE_PERF__ && window.__NASTECH_LAYOUT_TREE__)`
    )
    ...
    try {
      ...
      await cdp.eval(`window.__NASTECH_LAYOUT_TREE__.reveal('files'); window.__NASTECH_LAYOUT_TREE__.reveal('terminal')`)
