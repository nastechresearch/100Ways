...
// session per flush via `__NASTECH_SESSION_TILES__`, no backend, no credits —
...
const setup = (tiles, seedTurns) => `
  (() => {
    const hook = window.__NASTECH_SESSION_TILES__
...
const reveal = sid => `window.__NASTECH_LAYOUT_TREE__.reveal(${JSON.stringify(`session-tile:${sid}`)})`
  ...
  (() => {
    if (window.__RC__) {
      ...
      for (const { sid, rid } of window.__RC__.ids) {
        const states = window.__NASTECH_SESSION_TILES__.states()
        window.__NASTECH_SESSION_TILES__.publish(rid, { ...states[rid], busy: false, streamId: null })
        window.__NASTECH_SESSION_TILES__.close(sid)
      }
      window.__RC__ = null
    }
