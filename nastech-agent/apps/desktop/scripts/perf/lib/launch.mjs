...
//   · its own NASTECH_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · NASTECH_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated NASTECH_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate NASTECH_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = nastechHome ?? mkTemp('nastech-perf-home-')
  const userData = userDataDir ?? mkTemp('nastech-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !nastechHome) {
    seedConfigFrom(join(homedir(), '.nastech'), home)
  }
  ...
  try {
    ...
    // NASTECH_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set NASTECH_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      NASTECH_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.NASTECH_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'nastech-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'nastech-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.nastech'), home)
