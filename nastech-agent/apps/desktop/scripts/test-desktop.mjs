...
// no longer bundles the Nastech Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Nastech.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Nastech'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Nastech.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Nastech'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NASTECH_HOME for non-sandboxed runs -- matches main.ts's
// resolveNastechHome(). On Windows it's %LOCALAPPDATA%\nastech; elsewhere
// it's ~/.nastech. The fresh-install sandbox launchFresh() sets its own
// NASTECH_HOME and never touches this.
const DEFAULT_NASTECH_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'nastech')
  }
  return path.join(os.homedir(), '.nastech')
})()
const VENV_ROOT = path.join(DEFAULT_NASTECH_HOME, 'nastech-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'nastech-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.NASTECH_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Nastech-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Nastech-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Nastech-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Nastech-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.NASTECH_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.NASTECH_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const nastechHome = path.join(sandbox, 'nastech-home')
  ...
  env.NASTECH_DESKTOP_CWD = cwd
  env.NASTECH_DESKTOP_IGNORE_EXISTING = '1'
  env.NASTECH_DESKTOP_TEST_MODE = 'fresh-install'
  env.NASTECH_DESKTOP_USER_DATA_DIR = userDataDir
  env.NASTECH_HOME = nastechHome
  delete env.NASTECH_DESKTOP_NASTECH
  delete env.NASTECH_DESKTOP_NASTECH_ROOT
  ...
  console.log(`  NASTECH_HOME: ${nastechHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(nastechHome, 'nastech-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Nastech Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of nastech_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'nastech-agent', 'nastech_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Nastech
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NASTECH_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  NASTECH_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
