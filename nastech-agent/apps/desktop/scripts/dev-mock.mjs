...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nastech-dev-mock-${Date.now()}`))
  ...
  return { root, nastechHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(nastechHome, mockUrl) {
  fs.writeFileSync(
...
`,
  ...
  fs.writeFileSync(path.join(nastechHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.nastechHome, mock.url)
  console.log(`  NASTECH_HOME: ${sandbox.nastechHome}`)
  ...
  const env = {
    ...
    NASTECH_DESKTOP_APP_NAME: `NastechDevMock-${Date.now()}`,
  }
