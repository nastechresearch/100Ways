...
//   NASTECH_DESKTOP_CDP_PORT=9333 node scripts/eval.mjs "document.title"
//
// Needs a renderer with a debugging port: launch `hgui` / `npm run dev` with
// NASTECH_DESKTOP_CDP_PORT set (see electron/dev-cdp.ts).
const port = Number(process.env.NASTECH_DESKTOP_CDP_PORT || 9222)
let targets
...
try {
  ...
  console.error(
    `no renderer debugging port on 127.0.0.1:${port}. ` +
      ...
      'and that NASTECH_DESKTOP_CDP_PORT is not set to "off" or another port.'
  )
  process.exit(1)
}
