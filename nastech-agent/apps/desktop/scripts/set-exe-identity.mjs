#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Nastech icon + version metadata onto the
// built Nastech.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Nastech.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `nastech desktop`, the installer's
...
// path (which rebuilds via `nastech desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Nastech.exe>
//
...
// Stamp the Nastech icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Nastech Research',
      LegalCopyright: 'Copyright (c) 2026 Nastech Research'
    }
  })

  console.log('[set-exe-identity] done — Nastech icon + identity stamped')
}
