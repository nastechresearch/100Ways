...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.nastechHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.nastechHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.nastechHome)
  const builder = await RealSessionBuilder.start(sandbox.nastechHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.nastechHome, mock.url)
  fs.appendFileSync(path.join(sandbox.nastechHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.nastechHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
