/**
 * Shared E2E fixtures for the Nastech desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → nastech serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `nastech desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nastech-e2e-${prefix}-${Math.random()}`))
  const nastechHome = path.join(root, 'nastech-home')
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(nastechHome, 'config.yaml')
 ...
 */
export function writeEnvFile(nastechHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(nastechHome, '.env')
 ...
 */
function writeEmptyConfig(nastechHome: string): void {
  const configPath = path.join(nastechHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - NASTECH_HOME → sandbox nastech-home (isolated config/sessions)
 *  - NASTECH_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - NASTECH_DESKTOP_IGNORE_EXISTING=1 → don't pick up `nastech` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - NASTECH_DESKTOP_NASTECH_ROOT → REPO_ROOT (dev checkout resolution)
 *  - NASTECH_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    NASTECH_HOME: sandbox.nastechHome,
    NASTECH_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    NASTECH_DESKTOP_IGNORE_EXISTING: '1',
    NASTECH_DESKTOP_NASTECH_ROOT: REPO_ROOT,
    NASTECH_DESKTOP_APP_NAME: `NastechE2E-${Date.now()}`,
    ...
    NASTECH_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated NASTECH_HOME + userData
 * @param env      - the process environment (already has NASTECH_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.nastechHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.nastechHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via NASTECH_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`nastech serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.nastechHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { NASTECH_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Nastech backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Nastech.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Nastech.app', 'Contents', 'MacOS', 'Nastech')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'nastech')
}
 ...
 * progress without spawning a real Nastech backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * NASTECH_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    NASTECH_DESKTOP_BOOT_FAKE: '1',
    NASTECH_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + nastech-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).NASTECH_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).NASTECH_DESKTOP_NASTECH
  delete (env as Record<string, string | undefined>).NASTECH_DESKTOP_NASTECH_ROOT
