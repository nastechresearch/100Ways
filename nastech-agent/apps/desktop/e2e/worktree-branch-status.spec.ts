...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Nastech E2E'], { cwd: repo })
...
function configureRepoCwd(nastechHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(nastechHome, mockUrl)
  fs.appendFileSync(path.join(nastechHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(nastechHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.nastechHome, mock.url, repo)
