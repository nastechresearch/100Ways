 ...
 * AIAgent loop, using the E2E mock provider configured in `nastechHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(nastechHome: string) {
  ...
  static async start(nastechHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(nastechHome)
