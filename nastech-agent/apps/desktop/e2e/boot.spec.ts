 ...
 * packaged binary) with a real `nastech serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → nastech serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Nastech title', async () => {
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → nastech serve is spawned → WS connects → config
