 ...
 *
 * Implements just enough of the /v1/* surface for `nastech serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Nastech should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `nastech-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
  return {
    path,
    release: () => {
