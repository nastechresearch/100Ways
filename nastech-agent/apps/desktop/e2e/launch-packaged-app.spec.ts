...
/**
 * E2E smoke tests for the packaged Nastech desktop app.
 *
 ...
 * (credential stripping, isolated NASTECH_HOME + userData, unique app name).
 *
...
test('window opens with the Nastech title', async () => {
  const title = await fixture!.page.title()
  expect(title).toContain('Nastech')
})
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      nastechDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).nastechDesktop?.hud?.open({ sessionId: null })
  )
