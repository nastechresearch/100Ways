...
//
// When someone is in a hands-free "Hey Nastech" voice chat, the natural way to
...
// phrase (optionally addressed to Nastech), so a real turn that merely contains
...
// Optional address prefixes so "nastech stop" / "ok stop" / "hey nastech, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey nastech', 'hey nastech,', 'nastech', 'nastech,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("nastech") is not a stop command on its own.
      continue
    }
 ...
 * to Nastech). Returns false for anything that merely contains "stop" as part of
