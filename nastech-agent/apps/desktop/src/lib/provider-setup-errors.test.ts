...
describe('isProviderSetupErrorMessage', () => {
  ...
  it('does not match bare env var mentions from auxiliary warnings', () => {
    ...
    expect(isProviderSetupErrorMessage('Run `nastech setup` or set OPENROUTER_API_KEY.')).toBe(false)
    expect(
      isProviderSetupErrorMessage(
        '⚠ No auxiliary LLM provider configured — context compression will drop middle turns without a summary. Run `nastech setup` or set OPENROUTER_API_KEY.'
      )
    ).toBe(false)
