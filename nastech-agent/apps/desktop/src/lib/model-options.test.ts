...
const globalOptions = { model: 'nastech-4', provider: 'nastech', providers: [] }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'Nastech', slug: 'nastech', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'nastech', 'nastech-4')).toBe(false)
  })
