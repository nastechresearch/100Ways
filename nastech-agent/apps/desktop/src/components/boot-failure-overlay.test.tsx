...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Nastech gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.nastechDesktop
  Object.defineProperty(window, 'nastechDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'nastechDesktop', { configurable: true, value: original })
}
