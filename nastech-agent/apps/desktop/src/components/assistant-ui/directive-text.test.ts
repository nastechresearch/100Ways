...
import { formatRefValue, nastechDirectiveFormatter } from './directive-text'
...
describe('nastechDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = nastechDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = nastechDirectiveFormatter.parse('@file:src/main.tsx the entry point')
  ...
  it('parses session links with profile/id values', () => {
    const segments = nastechDirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...nastechDirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = nastechDirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...nastechDirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
