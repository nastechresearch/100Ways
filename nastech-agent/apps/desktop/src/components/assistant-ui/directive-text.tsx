...
const NASTECH_REF_TYPES = WIRE_REFERENCE_KINDS
type NastechRefType = (typeof NASTECH_REF_TYPES)[number]
...
const NASTECH_DIRECTIVE_RE = referenceRe()
...
export const nastechDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(NASTECH_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...nastechDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Nastech directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.nastechDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.nastechDesktop?.readFileDataUrl(id)
