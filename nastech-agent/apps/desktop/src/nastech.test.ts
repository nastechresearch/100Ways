...
describe('Nastech REST helpers', () => {
    ...
    api.mockImplementation(({ path }: { path: string }) => {
      if (path.startsWith('/api/profiles/sessions/sidebar')) {
        ...
        return Promise.reject(
          new Error(
            'Error invoking remote method \'nastech:api\': Error: 404: {"detail":"No such API endpoint: /api/profiles/sessions/sidebar"}'
          )
        )
      }
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getNastechConfig, '/api/config'],
      [getNastechConfigDefaults, '/api/config/defaults'],
