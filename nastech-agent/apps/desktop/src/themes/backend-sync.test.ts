...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). Nastech re-affirms it — `nastech config set display.skin neon`
    // or a `nastech skin set` recolor. That explicit event must repaint even
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // Nastech authored a new skin
    expect($pendingSkinApply.get()).toBe('forest')
  })
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // Nastech switched back to default
