...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.nastech/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.nastech/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'Nastech TUI Status\n\nSession ID: s-1\nModel: nastech-nastech-3 (unknown)'
  ...
  describe('session.usage', () => {
    ...
    it('appends credits_lines when present', () => {
      const body = renderRpcResult(
        {
          ...
          output: 20,
          total: 30,
          credits_lines: ['Nastech credits: 8,420 remaining', 'Resets: 2026-08-01']
        },
        'usage'
      )
      ...
      expect(body.split('\n')).toEqual([
        ...
        'Nastech credits: 8,420 remaining',
        'Resets: 2026-08-01'
      ])
    })
  })
