...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    window.nastechDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // nastech:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onDeepLink?.(payload => {
    ...
    void window.nastechDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
