...
import type { EnvVarInfo, OAuthProvider } from '@/types/nastech'
...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `nastech auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'nastech' ? 'Nastech Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'nastech' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('nastech', true), provider('minimax-oauth', false)]
  })
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Nastech Portal' })
    await act(async () => {
      fireEvent.click(remove)
    })

    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('nastech'))
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Nastech Portal'))
    })

    expect(startManualProviderOAuth).toHaveBeenCalledWith('nastech')
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          cli_command: 'nastech auth add qwen-oauth',
          disconnect_hint: "Use `nastech auth add qwen-oauth` or that provider's CLI to remove it.",
