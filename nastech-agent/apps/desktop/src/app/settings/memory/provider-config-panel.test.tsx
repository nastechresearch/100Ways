...
import type { MemoryProviderConfig } from '@/types/nastech'
...
function honchoSchema(): MemoryProviderConfig {
  return {
    name: 'honcho',
    label: 'Honcho',
    docs_url: 'https://docs.honcho.dev/v3/guides/integrations/nastech',
    fields: [
      {
        key: 'apiKey',
        label: 'API key',
...
describe('ProviderConfigPanel', () => {
  ...
  it('shows an inline error with retry when the load fails, then recovers', async () => {
    getMemoryProviderConfig.mockRejectedValueOnce(new Error('Timed out connecting to Nastech backend'))
