...
describe('ToolsetConfigPanel', () => {
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out Nastech Subscription row and a
      ...
      getToolsetConfig.mockResolvedValue(
        config({
          providers: [
            ...
            {
              name: 'Nastech Subscription',
              badge: 'subscription',
              tag: 'Managed OpenAI TTS',
              env_vars: [],
              post_setup: null,
              requires_nastech_auth: true,
              is_active: false,
              status: 'needs_auth'
            },
            {
  ...
  describe('managed Nastech provider activation', () => {
    const nastechBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
          {
            name: 'Nastech Subscription (Browser Use cloud)',
            badge: 'subscription',
    ...
    it('surfaces a sign-in notice when the PUT reports needs_nastech_auth', async () => {
      ...
      // reports needs_nastech_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(nastechBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'Nastech Subscription (Browser Use cloud)',
        needs_nastech_auth: true,
        feature: 'browser'
      })
      ...
      // The single Nastech row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /Nastech Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'Nastech Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing Nastech OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      try {
        ...
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('nastech', 'sess-1'), { timeout: 8000 })
