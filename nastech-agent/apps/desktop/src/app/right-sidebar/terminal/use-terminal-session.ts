...
const NASTECH_PATHS_MIME = 'application/x-nastech-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(NASTECH_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(NASTECH_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.nastechDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (nastech --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.nastechDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[nastech-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.nastechDesktop?.terminal?.write(sessionId, `${command}\r`)
