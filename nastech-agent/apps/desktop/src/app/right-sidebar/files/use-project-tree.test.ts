...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { nastechDesktop: unknown }).nastechDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { nastechDesktop: unknown }).nastechDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.nastechDesktop is missing', async () => {
    delete (window as unknown as { nastechDesktop?: unknown }).nastechDesktop
