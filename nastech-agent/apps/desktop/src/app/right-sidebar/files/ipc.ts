...
function ignoredBy(rules: GitignoreRule[], entry: NastechReadDirEntry) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<NastechReadDirResult> {
  if (!window.nastechDesktop) {
