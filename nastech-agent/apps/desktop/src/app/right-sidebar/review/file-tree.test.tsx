...
const file = (path: string): NastechReviewFile => ({
  added: 1,
  path,
  removed: 0,
  staged: false,
...
function filesUnderPublish(count: number): NastechReviewFile[] {
  return Array.from({ length: count }, (_, i) => file(`publish/file-${String(i).padStart(4, '0')}.so`))
}

function topLevelFiles(count: number): NastechReviewFile[] {
