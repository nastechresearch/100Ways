...
export function PetOverlayApp() {
  ...
  const setIgnore = (ignore: boolean) => {
    if (ignoreRef.current !== ignore) {
      ...
      window.nastechDesktop?.petOverlay?.setIgnoreMouse(ignore)
    }
  }
  ...
  useEffect(() => {
    const off = window.nastechDesktop?.petOverlay?.onState(payload => {
    ...
    window.nastechDesktop?.petOverlay?.control({ type: 'ready' })
  ...
  useEffect(() => {
    ...
    window.nastechDesktop?.petOverlay?.setFocusable(composerOpen)
  ...
  const onPetPointerMove = (e: React.PointerEvent) => {
    ...
    window.nastechDesktop?.petOverlay?.setBounds({
      height: drag.height,
      width: drag.width,
  ...
  const onPetPointerUp = (e: React.PointerEvent) => {
    ...
    if (drag.moved) {
      ...
      window.nastechDesktop?.petOverlay?.control({
    ...
    if (e.shiftKey) {
      window.nastechDesktop?.petOverlay?.control({ type: 'pop-in' })
    ...
    if (clickTimerRef.current) {
      clearTimeout(clickTimerRef.current)
      clickTimerRef.current = undefined
      window.nastechDesktop?.petOverlay?.control({ type: 'toggle-app' })
  ...
  const send = () => {
    ...
    if (text) {
      window.nastechDesktop?.petOverlay?.control({ text, type: 'submit' })
    }
  ...
  const openApp = () => {
    ...
    setUnread(false)
    window.nastechDesktop?.petOverlay?.control({ type: 'open-app' })
  }
  ...
  const onScale = useCallback((next: number, anchor: PetZoomAnchor) => {
    ...
    window.nastechDesktop?.petOverlay?.control({ scale: next, type: 'scale' })
  }, [])
  ...
  useEffect(() => {
    ...
    window.nastechDesktop?.petOverlay?.setBounds(bounds)
    window.nastechDesktop?.petOverlay?.control({ bounds, type: 'bounds' })
