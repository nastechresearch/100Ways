...
export function canUseQuickEntry(): boolean {
  return typeof window !== 'undefined' && typeof window.nastechDesktop?.quickEntry?.getSettings === 'function'
}
...
export async function loadQuickEntrySettings(): Promise<void> {
  ...
  try {
    applyStatus(await window.nastechDesktop.quickEntry.getSettings())
  } catch {
...
export async function saveQuickEntrySettings(patch: { enabled?: boolean; shortcut?: string }): Promise<void> {
  ...
  try {
    applyStatus(await window.nastechDesktop.quickEntry.setSettings(patch))
  } catch {
...
export function initQuickEntryBridge(): () => void {
  const api = typeof window === 'undefined' ? undefined : window.nastechDesktop?.quickEntry
