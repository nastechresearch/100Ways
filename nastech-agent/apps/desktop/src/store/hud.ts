 ...
 * composer with the reply scrolling above it, so Nastech can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.nastechDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.nastechDesktop?.hud
...
export const reportHudSession = (sessionId: null | string): void => window.nastechDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.nastechDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
