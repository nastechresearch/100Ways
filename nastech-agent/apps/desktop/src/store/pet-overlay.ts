 ...
 * app's bounds and stays visible while Nastech is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'nastech.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'nastech.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.nastechDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.nastechDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.nastechDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.nastechDesktop?.petOverlay?.close()
}
