...
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.nastechDesktop?.openWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.nastechDesktop.openSessionWindow(sessionId, opts),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.nastechDesktop.openWindow(), 'Could not open a new window')
}
