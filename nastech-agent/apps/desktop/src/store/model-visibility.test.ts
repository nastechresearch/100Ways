...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "nastech" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('nastech')])

    const visible = effectiveVisibleKeys(stored, [
      provider('nastech', ['nastech-3-llama-3.1-70b', 'nastech-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('nastech', 'nastech-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('nastech', 'nastech-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('nastech'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "nastech" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('nastech'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('nastech'))
    afterToggle.add(modelVisibilityKey('nastech', 'nastech-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('nastech', 'nastech-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nastech', 'nastech-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('nastech')])
    const providers = [provider('nastech', ['nastech-x', 'nastech-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('nastech'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('nastech', 'nastech-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('nastech'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('nastech', ['nastech-x', 'nastech-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    // Hide ALL of nastech — its sentinel is now stored.
    let stored: Set<string> | null = null
    stored = apply(stored, 'nastech', 'nastech-x')
    stored = apply(stored, 'nastech', 'nastech-y')
    expect(stored.has(emptyProviderSentinelKey('nastech'))).toBe(true)

    // Toggle a model in another provider. nastech must NOT snap back on.
    stored = apply(stored, 'openai', 'gpt-a')

    expect(stored.has(emptyProviderSentinelKey('nastech'))).toBe(true)
    const visible = effectiveVisibleKeys(stored, providers)
    expect(visible.has(modelVisibilityKey('nastech', 'nastech-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('nastech', 'nastech-y'))).toBe(false)
  })

  it('clears only the toggled provider sentinel when a model is re-enabled', () => {
    let stored: Set<string> | null = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('nastech')])
  ...
  it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
    // openai hidden-all, nastech untouched.
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('nastech', 'nastech-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('nastech', 'nastech-x'))).toBe(true)
...
describe('featured defaults', () => {
  ...
  it('defaults to the featured shortlist when a provider publishes one', () => {
    const nastech = featuredProvider(
      'nastech',
      ['anthropic/opus', 'anthropic/haiku', 'google/gemini', 'x-ai/grok'],
      ['anthropic/opus', 'google/gemini', 'x-ai/grok']
    )

    const visible = defaultVisibleKeys([nastech])

    // Featured are visible; the non-featured model is hidden by default.
    expect(visible.has(modelVisibilityKey('nastech', 'anthropic/opus'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nastech', 'google/gemini'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nastech', 'x-ai/grok'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nastech', 'anthropic/haiku'))).toBe(false)
  })
...
describe('setProviderVisibility', () => {
  ...
  it('leaves other providers untouched (their sentinels survive)', () => {
    ...
    // Turn openai fully on; nastech must stay hidden.
    const next = setProviderVisibility(stored, providers, 'openai', true)

    expect(next.has(emptyProviderSentinelKey('nastech'))).toBe(true)
    const visible = effectiveVisibleKeys(next, providers)
    expect(visible.has(modelVisibilityKey('nastech', 'nastech-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('openai', 'gpt-a'))).toBe(true)
  })
  ...
  it('collapses model families to one key per family when enabling', () => {
    ...
    const ps = [provider('nastech', ['model', 'model-fast'])]

    const next = setProviderVisibility(null, ps, 'nastech', true)

    expect(next.has(modelVisibilityKey('nastech', 'model'))).toBe(true)
    // The -fast sibling is represented by its base family, not its own key.
    expect(next.has(modelVisibilityKey('nastech', 'model-fast'))).toBe(false)
  })
})
