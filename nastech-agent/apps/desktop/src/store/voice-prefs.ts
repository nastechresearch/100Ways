...
import { getNastechConfigRecord, saveNastechConfig } from '@/nastech'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getNastechConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveNastechConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
