...
/**
 * Feature store for backend (agent) plugins — the native Nastech plugins plus
 ...
 * primitives `nastech plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Nastech. */
  portable?: boolean
}
