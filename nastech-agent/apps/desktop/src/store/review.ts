...
const OPEN_KEY = 'nastech.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'nastech.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'nastech.desktop.reviewTreeMode'
const SELECTED_KEY = 'nastech.desktop.reviewSelectedPath'
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.nastechDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.nastechDesktop?.openExternal?.(url)
    }
