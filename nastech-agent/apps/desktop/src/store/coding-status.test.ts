...
import type { NastechRepoStatus } from '@/global'
...
const sampleStatus: NastechRepoStatus = {
...
const otherStatus: NastechRepoStatus = {
  ...sampleStatus,
...
function stubProbe(impl: (cwd: string) => Promise<NastechRepoStatus | null>) {
  ;(window as unknown as { nastechDesktop?: unknown }).nastechDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('never publishes an old worktree status onto the primary after the active cwd moves', async () => {
    let resolveOld!: (status: NastechRepoStatus | null) => void
    stubProbe(
      () =>
        new Promise(resolve => {
          resolveOld = resolve
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh per drain', async () => {
    const resolvers: Array<(status: NastechRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
...
describe('repoChangeKindForPath', () => {
  it('does not notify a row when only another path changes', () => {
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'b.ts', untracked: true } as NastechRepoStatus['files'][number]]
      }
    })
    ...
    $repoStatusByCwd.set({
      '/repo': {
        ...sampleStatus,
        files: [{ path: 'a.ts', untracked: true } as NastechRepoStatus['files'][number]]
      }
    })
