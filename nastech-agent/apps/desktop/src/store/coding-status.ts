...
import type { NastechGitWorktree, NastechRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: NastechGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, NastechRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, NastechGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<NastechRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<NastechGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<NastechRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<NastechGitWorktree[]>>()
const $noRepoStatus = atom<NastechRepoStatus | null>(null)
const $noWorktrees = atom<NastechGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<NastechRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<NastechGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: NastechRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: NastechGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<NastechRepoStatus | null>
  seq: number
  target: string
}
