...
// reaches the gateway); everything else routes through window.nastechDesktop.git,
...
function file(path: string, over: Partial<NastechReviewFile> = {}): NastechReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as NastechReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.nastechDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as NastechReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.nastechDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: NastechReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as NastechReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { nastechDesktop?: unknown }).nastechDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as NastechReviewShipInfo)
