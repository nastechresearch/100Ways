...
How to build Nastech Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Nastech surface can also
...
- A **runtime home change** (switching the underlying `NASTECH_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Nastech — do not assume
