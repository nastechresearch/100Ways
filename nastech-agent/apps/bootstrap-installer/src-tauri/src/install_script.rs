...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/NastechResearch/nastech-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "nastech-setup/0.0.1")
        .send()
        .await
...
mod tests {
    ...
    fn upgrade_cached_script_adds_bom_to_legacy_ps1() {
        ...
        let dir = std::env::temp_dir().join(format!("nastech-bom-test-{}", std::process::id()));
    ...
    fn upgrade_cached_script_leaves_sh_untouched() {
        let dir = std::env::temp_dir().join(format!("nastech-bom-sh-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
