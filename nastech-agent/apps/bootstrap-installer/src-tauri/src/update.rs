...
///
/// The marker is also the cross-process update lock: `nastech update` claims
/// the same file (see `nastech_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `nastech update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in nastech_cli/update_lock.py — all three read
...
/// pid (e.g. a dashboard-spawned `nastech update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(nastechHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    //
    // The same marker is the cross-process update lock (nastech_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `nastech update` — is already
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    // `nastech update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `nastech update` child claims that SAME lock (nastech_cli/update_lock.py).
    ...
    // sync with HANDOFF_PID_ENV in nastech_cli/update_lock.py.
    envs.push((
        "NASTECH_UPDATE_HANDOFF_PID".to_string(),
...
mod tests {
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/nastech-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "NASTECH_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the nastech update child claims the same marker we hold; without our pid \
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `nastech update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(nastechHome, child.pid) races ahead
