//! Nastech Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `Nastech-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under NASTECH_HOME/logs/ so install
    ...
    // Nastech is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Nastech installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Nastech is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Nastech" doubles as a normal launcher (first run installs, every
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::nastech_home().join("nastech-agent");
                if bootstrap::nastech_is_installed(&install_root) {
        ...
        .expect("error while running Nastech Setup");
}
