...
fn main() {
    ...
    // local checkout hasn't pushed). Set NASTECH_BUILD_PIN_COMMIT to bake an
    // immutable commit pin for reproducible/release installers.
    //
    // Commit pin resolution:
    //   - NASTECH_BUILD_PIN_COMMIT, if set and non-empty. Accepts a SHA, tag,
    ...
    //   1. NASTECH_BUILD_PIN_BRANCH, if set and non-empty.
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=nastech-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=nastech-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=nastech-bootstrap: following branch {b} HEAD (no commit pin; \
                 set NASTECH_BUILD_PIN_COMMIT for an immutable pin)"
            ),
        }
    }
    if commit.is_none() && branch.is_none() {
        ...
        // build environment.
        println!(
            "cargo:warning=nastech-bootstrap: no pin resolved at build time; binary will fail at runtime without NASTECH_SETUP_DEV_REPO_ROOT or runtime args"
        );
    }
    ...
    // NASTECH_BUILD_PIN_COMMIT resolves a moving ref (tag/branch) to a SHA, a
    ...
    println!("cargo:rerun-if-env-changed=NASTECH_BUILD_PIN_COMMIT");
    println!("cargo:rerun-if-env-changed=NASTECH_BUILD_PIN_BRANCH");

    // -----------------------------------------------------------------
    // Tauri windows manifest. See nastech-setup.manifest for rationale —
    // declares level="asInvoker" so Windows's installer-detection
    // heuristic doesn't refuse to launch us without UAC elevation.
    // -----------------------------------------------------------------
    #[cfg(target_os = "windows")]
    let attrs = {
        let manifest = include_str!("nastech-setup.manifest");
...
fn resolve_commit_pin() -> Option<String> {
    ...
    // asks for one via NASTECH_BUILD_PIN_COMMIT. With no env var, we return
    // None and the installer follows the branch HEAD at install time.
    let requested = std::env::var("NASTECH_BUILD_PIN_COMMIT").ok()?;
    ...
    panic!(
        "NASTECH_BUILD_PIN_COMMIT={requested:?} could not be resolved to a commit \
...
fn resolve_branch_pin() -> Option<String> {
    if let Ok(v) = std::env::var("NASTECH_BUILD_PIN_BRANCH") {
        if !v.trim().is_empty() {
