...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """
    home_dir = Path(tempfile.mkdtemp(prefix="nastech_ts_live_"))
    nastech_home = home_dir / ".nastech"
    nastech_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, nastech_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".nastech" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, nastech_home / ".env")
        ...
        load_nastech_dotenv(nastech_home=str(Path.home() / ".nastech"))
    ...
    (nastech_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return nastech_home
