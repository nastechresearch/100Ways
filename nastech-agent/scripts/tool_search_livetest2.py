...
def run_one(scenario: Dict[str, Any], mode: str, rep: int, out_dir: Path) -> Dict[str, Any]:
    ...
    nastech_home = base.setup_isolated_home(enabled, listing=("auto" if mode == "listing" else "off"))
    ...
    try:
        ...
        from nastech_cli.plugins import get_plugin_manager, discover_plugins
    ...
    shutil.rmtree(Path(os.environ["NASTECH_HOME"]).parent, ignore_errors=True)
    return rec
