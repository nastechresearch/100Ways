...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[nastech] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[nastech`]*' }) -join "`n"
    ...
    return @{
        ...
        NastechHome = $(if ($paths) { $paths.nastech_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\nastech\nastech-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-nastech'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-nastech') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
