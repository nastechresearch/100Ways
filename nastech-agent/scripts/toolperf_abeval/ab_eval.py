...
measured in the production session DB) through `nastech chat` twice — once per
...
(tracker: NastechResearch/nastech-agent#77056).
"""
import json
import os
import re
...
def run(arm: str, model: str, reps: int, pythonpath: str, only=None):
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "nastech_cli.main", "chat", "--query", q,
