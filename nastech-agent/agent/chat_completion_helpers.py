...
from nastech_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from nastech_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _merge_nastech_portal_messages_extra_body(agent, anthropic_kwargs: dict) -> dict:
    """Merge Portal ``tags`` / ``session_id`` onto an Anthropic Messages kwargs dict.

    The Nastech provider profile is only consulted by the OpenAI-wire transport;
    ...
    """
    if getattr(agent, "provider", None) not in {"nastech", "nastech-portal", "nastechresearch"}:
        return anthropic_kwargs
    try:
        from providers import get_provider_profile

        nastech_profile = get_provider_profile("nastech")
        if nastech_profile is not None:
            anthropic_kwargs.setdefault("extra_body", {}).update(
                nastech_profile.build_extra_body(
                    session_id=getattr(agent, "session_id", None)
                )
            )
    except Exception as exc:  # noqa: BLE001 — never block a turn on tagging
        logger.debug("Nastech Portal extra_body merge failed: %s", exc)
    return anthropic_kwargs
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        # Nastech Portal reads ``tags`` and ``session_id`` as top-level body fields
        ...
        return _merge_nastech_portal_messages_extra_body(agent, anthropic_kwargs)
    ...
    _is_nastech = "nastechresearch" in agent._base_url_lower
    ...
    # like write_file/patch.  OpenRouter/Nastech were the only routes covered
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "nastech",
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    if fb_provider != "nastech":
        return None
    try:
        from nastech_cli.auth import get_provider_auth_state

        state = get_provider_auth_state("nastech") or {}
    except Exception as exc:
        return f"nastech_auth_unreadable:{type(exc).__name__}"
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        elif fb_provider in {"nastech", "nastech-portal", "nastechresearch"}:
            # Portal is dual-wire: anthropic/* must land on /v1/messages.
            # resolve_provider_client still returns an OpenAI client for
            # Nastech; the anthropic_messages branch below rebuilds the native
            # client from that credential + base_url.
            from nastech_cli.providers import nastech_api_mode

            fb_api_mode = nastech_api_mode(fb_model)
        elif (
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Nastech-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Nastech bookkeeping,
        ...
        if _is_nastech:
            from agent.portal_tags import nastech_portal_tags as _portal_tags
        ...
        if agent.api_mode == "codex_responses":
            ...
            if agent.api_mode == "anthropic_messages":
                ...
                _ant_kw = _merge_nastech_portal_messages_extra_body(agent, _ant_kw)
        ...
        if final_response:
            ...
            if agent.api_mode == "codex_responses":
                ...
                _ant_kw2 = _merge_nastech_portal_messages_extra_body(agent, _ant_kw2)
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Nastech. Preserve only
        ...
        if agent.provider == "moa":
            # Nastech interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Nastech preserves
        ...
        #      Nastech dedicated endpoint) stall for minutes during large
