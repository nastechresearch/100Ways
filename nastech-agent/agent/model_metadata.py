...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_nastech_home() / "cache" / "local_endpoint_probes.json"
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "qwen3.8-max": 1_000_000,     # 1M context (OpenRouter & Nastech portal, verified 2026-08-03)
    ...
    # (native MiniMax-M3, OpenRouter/Nastech minimax/minimax-m3).
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.nastechresearch.com": "nastech",
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Nastech minimum.
...
def _resolve_endpoint_context_length(
    ...
    if not matched:
        if len(endpoint_metadata) == 1:
            ...
            # a 32K embedding model on the Nastech portal) and poison the
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Nastech phrasing of the same condition.
    ...
    # OpenRouter/Nastech format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Nastech
            and "maximum context length" in error_lower)
...
def _normalize_model_version(model: str) -> str:
    ...
    Nastech uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_nastech_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve Nastech Portal model context length.

    Tries the live Nastech inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Nastech model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Nastech /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        resolve_context_length) so the Nastech portal path does not short-circuit it.
        """
        ...
        if ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info(
                ...
                "(known 32K underreport, Nastech path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Nastech URLs,
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Nastech: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # Drop the stale entry and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Nastech Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "nastech":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Nastech portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider == "nastech":
        ctx, source = _resolve_nastech_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Nastech URLs.
    ...
    # the dedicated Nastech/Copilot/GMI branches above.
    ...
    # ``Nastech-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
