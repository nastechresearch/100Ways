...
Resolution order for text tasks (auto mode):
  ...
  3. Nastech Portal (~/.nastech/auth.json active provider)
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``NASTECH_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `nastech update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Nastech owns auxiliary retry + provider/model fallback policy (the
    ...
    # Nastech sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Nastech control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="nastech-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Nastech triggers summarization.  Higher values delay
...
# "google/gemini-3-flash" kept 404ing here once Nastech dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (Nastech
...
# Nastech catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from nastech_cli.auth import resolve_api_key_provider_credentials
        from nastech_cli.models import fetch_models_with_pricing
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (Nastech returns its compaction pick), so it
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``NASTECH_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``NASTECH_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("NASTECH_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("NASTECH_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
# Nastech Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Nastech Portal.
#
...
from agent.portal_tags import nastech_portal_tags as _nastech_portal_tags


def _nastech_extra_body() -> dict:
    """Return a fresh Nastech Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``nastech_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _nastech_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``NASTECH_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_nastech_extra_body()`` or import ``nastech_portal_tags`` directly.
NASTECH_EXTRA_BODY = _nastech_extra_body()

# Set at resolve time — True if the auxiliary client points to Nastech Portal
auxiliary_is_nastech: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3.6-flash"
_NASTECH_MODEL = "google/gemini-3.6-flash"
_NASTECH_DEFAULT_BASE_URL = "https://inference-api.nastechresearch.com/v1"
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for nastech).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nastech":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the NASTECH resolution.
        from nastech_cli.auth import _nastech_inference_env_override

        env_url = _nastech_inference_env_override()
        if env_url:
            return env_url
    # runtime_base_url handles provider-specific logic (e.g. nastech prefers inference_base_url).
...
def _nastech_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("NASTECH_NASTECH_MIN_KEY_TTL_SECONDS", "1800")))
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for Nastech Portal — a blanket fallback would flip
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from agent.anthropic_adapter import _is_nastech_portal_endpoint

                    if _is_nastech_portal_endpoint(candidate):
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Nastech plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``nastech_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nastech_auth() -> Optional[dict]:
    """Read and validate ~/.nastech/auth.json for an active Nastech provider.

    Returns the provider state dict if Nastech is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("nastech")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _NASTECH_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "nastech":
            return None
        provider = data.get("providers", {}).get("nastech", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Nastech auth: %s", exc)
        return None


def _nastech_api_key(provider: dict) -> str:
    """Extract a usable Nastech inference JWT from stored auth state."""
    from nastech_cli.auth import _nastech_invoke_jwt_is_usable
    ...
    for token_key, expiry_key in (
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _nastech_invoke_jwt_is_usable(
            token,
...
def _nastech_base_url() -> str:
    """Resolve the Nastech inference base URL from env or default."""
    return os.getenv("NASTECH_INFERENCE_BASE_URL", _NASTECH_DEFAULT_BASE_URL)


def _resolve_nastech_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve Nastech auxiliary credentials from the selected pool entry."""
    try:
        from nastech_cli.auth import _agent_key_is_usable

        pool = load_pool("nastech")
    except Exception as exc:
        logger.debug("Auxiliary Nastech pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Nastech pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _nastech_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary Nastech pool refresh failed: %s", exc)
            refreshed = None
    ...
    api_key = _nastech_api_key(provider)
    base_url = _pool_runtime_base_url(entry, _NASTECH_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_nastech_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Nastech runtime credentials when available.
    ...
    """
    pooled = _resolve_nastech_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled

    try:
        from nastech_cli.auth import resolve_nastech_runtime_credentials

        creds = resolve_nastech_runtime_credentials(
            timeout_seconds=env_float("NASTECH_NASTECH_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary Nastech runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``nastech auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``nastech_cli.auth``'s singleton runtime resolver for older
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    os.getenv("NASTECH_XAI_BASE_URL", "").strip().rstrip("/")
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Nastech auth store.
    ...
    try:
        from nastech_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from nastech_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    """
    try:
        from nastech_cli.config import cfg_get, load_config_readonly
...
def _try_nastech(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Nastech —
    # if another session already recorded a 429, skip Nastech entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.nastech_rate_guard import nastech_rate_limit_remaining
        _remaining = nastech_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Nastech Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("nastech", ttl=_remaining)
            return None, None
    except Exception:
        pass

    nastech = _read_nastech_auth()
    runtime = _resolve_nastech_runtime_api(force_refresh=False)
    if runtime is None and not nastech:
        logger.warning(
            "Auxiliary Nastech client unavailable: no Nastech authentication found "
            "(run: nastech auth)."
        )
        _mark_provider_unhealthy("nastech", ttl=60)
        return None, None
    if runtime is None and nastech:
        logger.debug(
            "Auxiliary Nastech: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_nastech
    auxiliary_is_nastech = True
    logger.debug("Auxiliary client: Nastech Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/nastech/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_nastech_recommended_aux_model
    # auto-detects the caller's tier via check_nastech_free_tier().  Fall back to
    # _NASTECH_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _NASTECH_MODEL
    if not _aux_probe_active():
        # Availability probes skip the recommended-model lookup: the exact
        # model is irrelevant to "is Nastech resolvable?", and the Portal
        # recommended-models fetch below can hit the network.
        try:
            from nastech_cli.models import get_nastech_recommended_aux_model
            recommended = get_nastech_recommended_aux_model(vision=vision)
            if recommended:
                model = recommended
                logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _nastech_api_key(nastech or {})
        if not api_key:
            logger.warning(
                "Auxiliary Nastech client unavailable: no usable inference JWT found "
                "(run: nastech auth add nastech)."
            )
            _mark_provider_unhealthy("nastech", ttl=60)
            return None, None
        base_url = str((nastech or {}).get("inference_base_url") or _nastech_base_url()).rstrip("/")
    return (
...
def _refresh_nastech_recommended_model(
    *, vision: bool, stale_model: Optional[str]
) -> Optional[str]:
    """Re-fetch the Nastech Portal's recommended model after a stale-model 404.
    ...
    the Nastech → OpenRouter catalog, every auxiliary call 404s with
    ...
    model name to retry with:
      ...
      * ``_NASTECH_MODEL`` (google/gemini-3-flash-preview), the known-good default,
    ...
    try:
        ...
        fresh = get_nastech_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug(
            "Nastech recommended-model refresh failed (%s); using default %s",
            exc, _NASTECH_MODEL,
        )
    ...
    if _NASTECH_MODEL.strip().lower() != stale:
        return _NASTECH_MODEL
    return None
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `nastech setup` or `nastech model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=nastech_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    Mirrors the ``_try_anthropic`` / ``_try_nastech`` shape but delegates to
    :func:`nastech_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from nastech_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second nastech
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``nastech config aux reset``)."""
...
def _nastech_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh Nastech account API says paid access is allowed."""
    try:
        from nastech_cli.nastech_account import get_nastech_portal_account_info

        account_info = get_nastech_portal_account_info(force_fresh=True)
        return account_info.paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary Nastech paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Nastech → OpenRouter catalog. The Nastech
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.nastechresearch.com"):
        return "nastech"
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Nastech routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.nastechresearch.com"):
        return "nastech"
    return normalized
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_nastech_fallback_destination", None)
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "nastech": "nastech",
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``nastech_cli.fallback_config.resolve_entry_api_key``).
    """
    from nastech_cli.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._nastech_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Nastech' built-in discovery
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Nastech get their chosen chat model; users
      ...
      2. OpenRouter → Nastech → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_nastech, _stale_base_url_warned
    auxiliary_is_nastech = False  # Reset — _try_nastech() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `nastech model` but the
    #    old OPENAI_BASE_URL lingers in ~/.nastech/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: nastech model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.nastech/.env",
    ...
    # on aggregators (OpenRouter, Nastech) who previously got routed to a
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_nastech_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_nastech_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = nastech_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Nastech owns
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "nastech", "openai-codex" (or "codex"),
    ...
    #
    # Nastech + vision is the one carve-out: the branch below resolves its model
    # from the Portal's tier-aware vision recommendation (``_try_nastech(vision=
    ...
    _nastech_portal_vision = provider == "nastech" and is_vision
    if not model and provider != "auto" and not _nastech_portal_vision:
    ...
    if provider == "nastech":
        ...
        client, default = _try_nastech(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: nastech requested "
                           "but Nastech Portal not configured (run: nastech auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        portal_mode = nastech_api_mode(final_model)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: nastech model)")
                return None, None
    ...
    # OpenRouter / Nastech bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: nastech model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        ...
        # entries that coincidentally match a canonical provider (e.g. ``nastech``)
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: nastech doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("nastech_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(nastech_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "nastech":
            return resolve_provider_client("nastech", model, async_mode)
        if provider == "openai-codex":
...
def _resolve_strict_vision_backend(
    ...
    if provider == "nastech":
        ...
        # recommendations wrap onto /v1/messages — _try_nastech alone returns
        # a bare OpenAI client and the call 404s.
        return resolve_provider_client("nastech", model, is_vision=True)
    if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Nastech → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Nastech — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). Nastech is the
        ...
        #   3. Nastech Portal (vision-capable aggregator fallback)
        ...
        if main_provider and main_provider not in {"auto", "", "moa"}:
            ...
            if main_provider == "nastech":
                # Nastech resolves its vision model from the Portal's tier-aware
                # recommended-models slots inside _try_nastech(vision=True).
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Nastech Portal product tags when the auxiliary client is backed
    by Nastech Portal. Returns empty dict otherwise.
    """
    return _nastech_extra_body() if auxiliary_is_nastech else {}
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    if (not or_key
            and _read_nastech_auth() is None
            and (
...
def _refresh_nastech_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh Nastech runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_nastech_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`nastech_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                ...
                pass
        _nastech_on_messages = False
        if _provider_norm in {"nastech", "nastech-portal", "nastechresearch"}:
            from nastech_cli.providers import nastech_api_mode

            _nastech_on_messages = nastech_api_mode(model) == "anthropic_messages"
        if (
            _is_anthropic_compat_endpoint(provider, _effective_base)
            or _nastech_on_messages
            or _is_nvidia_nim
            or _is_moa
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Nastech use
    ...
    if _provider_for_portal in {"nastech", "nastech-portal", "nastechresearch"}:
        if "tags" not in merged_extra:
            merged_extra["tags"] = _nastech_portal_tags()
    ...
    # Anthropic Messages adapters translate Nastech reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nastech_on_messages = False
        if provider_norm in {"nastech", "nastech-portal", "nastechresearch"}:
            from nastech_cli.providers import nastech_api_mode
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Nastech reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `nastech model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Nastech Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Nastech → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to Nastech-routed calls.
        _heal_is_nastech = (
            resolved_provider == "nastech"
            or base_url_host_matches(_base_info, "inference-api.nastechresearch.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nastech:
            healed_model = _refresh_nastech_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Nastech catalog; "
        ...
        # ── Nastech auth refresh parity with main agent ──────────────────
        client_is_nastech = (
            resolved_provider == "nastech"
            or base_url_host_matches(_base_info, "inference-api.nastechresearch.com")
        )
        if (
            ...
            and client_is_nastech
            and _nastech_portal_account_has_fresh_paid_access()
        ):
            refreshed_client, refreshed_model = _refresh_nastech_auxiliary_client(
                cache_provider=resolved_provider or "nastech",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Nastech runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nastech:
            refreshed_client, refreshed_model = _refresh_nastech_auxiliary_client(
                cache_provider=resolved_provider or "nastech",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Nastech runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Nastech
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        # the Nastech → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_nastech = (
            resolved_provider == "nastech"
            or base_url_host_matches(_client_base, "inference-api.nastechresearch.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nastech:
            healed_model = _refresh_nastech_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Nastech catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Nastech runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nastech:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Nastech runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
