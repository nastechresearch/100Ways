...
load_nastech_dotenv(
    nastech_home=_nastech_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.nastech/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_nastech_home, "logs", "tui_gateway_crash.log")
...
_LONG_HANDLERS = frozenset(
    {
        ...
        # pool checks, pricing fetch, Nastech tier check, optional custom-provider
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("nastech_gateway_runtime_session_record", default=None)
)
...
class _SlashWorker:
    """Persistent NastechCLI subprocess for slash commands."""
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from nastech_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `nastech --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `nastech --tui`, so it must NOT count as dead here (doing
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from nastech_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from nastech_cli.active_sessions import coerce_max_concurrent_sessions
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_nastech_home).resolve():
        return None
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from nastech_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``nastech dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `nastech` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            session_db = None
            if profile_home:
                home_token = set_nastech_home_override(profile_home)
                try:
            ...
            if home_token is not None:
                reset_nastech_home_override(home_token)
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``nastech`` / ``nastech --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from nastech_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from nastech_state import is_disk_full_error
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``nastech_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _nastech_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_nastech_home_override()
        home = override if isinstance(override, str) and override else _nastech_home
...
def _load_cfg() -> dict:
    ...
    ``nastech_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    nastech_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    path = _nastech_home / "config.yaml"
...
def resolve_skin() -> dict:
    try:
        from nastech_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``nastech config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_nastech_home_override()
    return Path(override if isinstance(override, str) and override else _nastech_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Nastech activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="nastech-change-watcher", daemon=True).start()
...
def _resolve_model() -> str:
    ...
    try:
        from nastech_cli.models import get_preferred_silent_default_model
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from nastech_cli.models import detect_static_provider_for_model
...
def _load_approval_mode() -> str:
    ...
    canonical ``nastech_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`nastech_constants.resolve_reasoning_config` (per-model override >
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base Nastech): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `nastech --tui` both land here. See
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `nastech --tui -m`
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from nastech_cli.personality import available_personalities
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from nastech_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from nastech_cli.banner import get_update_result
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Nastech window, and answers
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via nastech_cli.personality (single owner)."""
    from nastech_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as nastech_cli.personality.resolve_personality — (name,
    ...
    """
    from nastech_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to nastech_cli.personality (single owner).
    """
...
def _load_fallback_model():
    ...
    stays in parity with ``NastechCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses nastech_cli.mcp_startup; TUI stdio keeps
    ...
    cfg = _load_cfg()
    from nastech_cli.config import resolve_ephemeral_system_prompt_from_config
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Nastech has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_nastech_home() / "cache" / "pet-gen"
...
def _spawn_trees_root():
    ...
    root = get_nastech_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    def run():
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_nastech_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from nastech_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from nastech_cli.voice import is_continuous_active
            ...
            # billing_url, is_nastech, message) so the TUI/desktop render a
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: nastech_cli.voice unavailable")
                except Exception as e:
...
def _session_images_dir(session: dict) -> Path:
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _nastech_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile NASTECH_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from nastech_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from nastech_cli.models import resolve_fast_mode_overrides
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through nastech_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from nastech_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under NASTECH_HOME (~/.nastech by
    ...
    nastech_home = os.path.realpath(str(get_nastech_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == nastech_home
        or real.startswith(nastech_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    would create catch-all projects: NASTECH_HOME itself and the dirs in
    ...
    nastech_home = os.path.normcase(os.path.realpath(str(get_nastech_home())))
    return real in _non_workspace_dirs() or real == nastech_home
...
def _discover_repos_payload(
    ...
    repos even with zero nastech sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (nastech home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Nastech sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Nastech
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `nastech` is interactive — use `/nastech chat -q …` or run `nastech` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`nastech setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`nastech gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`nastech sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`nastech config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from nastech_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Nastech" if role == "assistant" else role.title()
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``nastech_cli.voice.set_voice_busy_probe``)
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via nastech_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from nastech_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from nastech_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from nastech_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import nastech_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from nastech_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from nastech_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
