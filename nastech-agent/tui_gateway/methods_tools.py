...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.nastech/.env`` into the gateway process via
    ``nastech_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nastech_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m nastech_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "nastech_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m nastech_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=nastech_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "init":
        ...
        from nastech_cli.init_command import build_init_prompt_for_cwd
    ...
    if name == "moa":
        ...
        try:
            from nastech_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("NASTECH_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("NASTECH_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_nastech_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``nastech journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from nastech_cli.skills_hub import do_install
        ...
        if action == "browse":
            from nastech_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from nastech_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``nastech plugins`` / the dashboard, so the three surfaces
    ...
    try:
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Nastech plugin.
        ...
        if action == "toggle":
            from nastech_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
