...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `nastech setup --portal`. See [Nastech Portal](/integrations/nastech-portal).
:::
...
2. **Nastech spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo nastech gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.nastech/cron/output/` and you can read them anytime.
:::
...
Nastech will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Nastech Agent
```
...
Before creating cron jobs, ensure Nastech has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Nastech what you want:
...
For faster briefings, tell Nastech to delegate each topic to a sub-agent:
...
Nastech will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
