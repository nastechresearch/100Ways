...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `nastech send`."
---
...
- Piping any tool's output anywhere (`make | nastech send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `nastech
...
1. **Positional argument** — `nastech send --to telegram "hi"`
2. **`--file PATH`** — `nastech send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | nastech send --to telegram`

When stdin is a TTY (no pipe), Nastech does **not** wait for input — you'll
...
`~/.nastech/.env` and `~/.nastech/config.yaml`. It's a standalone subprocess
...
"no channels discovered yet", start the gateway once (`nastech gateway
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `nastech send` shares with cron delivery.
