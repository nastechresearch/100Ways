...
This is **not** the same as the [Email gateway adapter](../user-guide/messaging/email.md), which lets people chat with Nastech *by* emailing it (send a mail, get a reply in-thread). This guide is about the agent *operating a mailbox* — reading, searching, composing, and organising mail as part of its tasks. You can run both, ideally on separate accounts.
:::
...
Ask Nastech to do this for you — the skill contains the full procedure — or do it manually:
...
- [Email gateway adapter](../user-guide/messaging/email.md) — chat with Nastech over email instead
