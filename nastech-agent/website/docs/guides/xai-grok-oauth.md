...
After the first login, credentials are stored under `~/.nastech/auth.json` and refreshed automatically before they expire.
...
```bash
nastech config set model.default grok-build-0.1
nastech config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.nastech/config.yaml` will contain:
...
```bash
nastech --provider xai-oauth        # canonical
nastech --provider grok-oauth       # alias
nastech --provider x-ai-oauth       # alias
nastech --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `nastech tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `nastech tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `nastech setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.
...
This is **not** a stale-token problem — re-running `nastech model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NastechResearch/nastech-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
nastech config set model.provider xai
```
...
**Fix:** run `nastech model` and pick the xAI Grok OAuth provider, or run `nastech auth add xai-oauth`.
...
```bash
nastech auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `nastech auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `nastech auth list xai-oauth` to see them).
