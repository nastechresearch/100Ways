...
Add these lines to `~/.nastech/.env`:
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nastech gateway install` to update the plist.
:::
...
**Option 2:** Set it manually in `~/.nastech/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nastech/config.yaml`:
...
```bash
# From the CLI
nastech cron list          # View all scheduled jobs
nastech cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nastech-gateway` (Linux) or `~/.nastech/logs/gateway.log` (macOS) |
| Cron job output | `~/.nastech/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nastech/cron/jobs.json` |
| Pairing data | `~/.nastech/pairing/` |
| Session history | `~/.nastech/sessions/` |
