...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`nastech webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `nastech webhook subscribe`) |
