...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. Nastech works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
On an 8 GB Mac, use `q4_0` KV cache and choose a smaller model that can still fit Nastech' 64K minimum context. On 16 GB, you can comfortably do 128K context. On 32 GB+, you can run larger models or multiple parallel slots.

If you're still running out of memory, reduce context only while staying at or above Nastech' 64K minimum; otherwise switch to a smaller model or smaller quantization (Q3_K_M instead of Q4_K_M).
...
Nastech automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
NASTECH_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `NASTECH_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `NASTECH_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `NASTECH_API_TIMEOUT` |
...
Nastech sends its system prompt and tool schemas on every call, so on slower hardware the first turn can involve minutes of silence while the model processes that prompt before generating anything. That's prefill at work, not a stalled session. See [Slow first response (prefill)](./local-ollama-setup.md#slow-first-response-prefill) in the Ollama guide for mitigations like keeping the model loaded and trimming the fixed prompt with `nastech prompt-size`.
:::
