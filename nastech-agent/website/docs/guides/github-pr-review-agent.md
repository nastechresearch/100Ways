...
Use `deliver: "local"` to save reviews to `~/.nastech/cron/output/`. Great for testing before wiring up notifications.
:::
...
Create `~/.nastech/skills/code-review/SKILL.md`:
...
Verify it loaded — start `nastech` and you should see `code-review` in the skills list at startup.
