...
Create `/etc/systemd/system/nastech-teams-pipeline-maintain.service`:
...
And `/etc/systemd/system/nastech-teams-pipeline-maintain.timer`:
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.nastech/.env` at the top of a wrapper script that crontab calls.
...
```bash
nastech teams-pipeline subscriptions   # should show expirationDateTime advanced
nastech teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
- run `nastech teams-pipeline maintain-subscriptions --dry-run`
- inspect `nastech teams-pipeline list --status failed`
...
- run `nastech teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `nastech teams-pipeline run`
