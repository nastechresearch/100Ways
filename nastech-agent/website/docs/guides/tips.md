...
description: "Practical advice to get the most out of Nastech Agent — prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---
...
Run `nastech setup --portal` — you get 300+ models including Claude, GPT-5, and Gemini under one subscription. See [Nastech Portal](/integrations/nastech-portal).
:::
...
Forgot something from your last session? Run `nastech -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `nastech -r "my research project"`.
...
Want Nastech to have a stable default voice? Edit `~/.nastech/SOUL.md` (or `$NASTECH_HOME/SOUL.md` if you use a custom Nastech home). Nastech now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with Nastech](/guides/use-soul-with-nastech).
...
Already have a `.cursorrules` or `.cursor/rules/*.mdc` file? Nastech reads those too. No need to duplicate your coding conventions — they're loaded automatically from the working directory.

### Discovery

Nastech loads the top-level `AGENTS.md` from the current working directory at session start. Subdirectory `AGENTS.md` files are discovered lazily during tool calls (via `subdirectory_hints.py`) and injected into tool results — they are not loaded upfront into the system prompt.
...
:::tip
Run `/usage` periodically to see your token consumption. Run `/insights` for a broader view of usage patterns over the last 30 days. To see what your *fixed* per-message cost is before any conversation — system prompt, skills index, memory, tool schemas — run [`nastech prompt-size`](/reference/cli-commands#nastech-prompt-size) (works offline).
:::
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `nastech sessions list` and resume with `nastech -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `nastech pairing approve telegram XKGH5N7P` — simple and secure.
...
:::tip
By default, messaging sessions never auto-reset — context lives until you `/reset` or compression kicks in. If you want sessions to reset automatically (after idle time or daily at a fixed hour), opt in via the `session_reset` section in `~/.nastech/config.yaml`.
:::
...
# In your .env:
TERMINAL_ENV=docker
TERMINAL_DOCKER_IMAGE=nastech-sandbox:latest
```
...
Nastech checks every command against a curated list of dangerous patterns before execution. This includes recursive deletes, SQL drops, piping curl to shell, and more. Don't disable this in production — it exists for good reasons.
