...
If you're expecting jobs to fire automatically, you need a running gateway (`nastech gateway` for foreground, or `nastech gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `nastech cron tick`.
...
```bash
date
nastech cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.nastech/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.nastech/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.nastech/.env` |
...
| `local` | Write access to `~/.nastech/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `nastech cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `nastech skills install <skill-name>` or via `/skills` in the CLI.
...
2. `~/.nastech/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `nastech cron list`
...
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `nastech skills install <skill-name>`.
...
Jobs are stored in `~/.nastech/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.nastech/cron/jobs.json
chmod 600 ~/.nastech/cron/jobs.json   # Your user should own it
```
...
3. Open an issue at [github.com/NastechResearch/nastech-agent](https://github.com/NastechResearch/nastech-agent) with:
