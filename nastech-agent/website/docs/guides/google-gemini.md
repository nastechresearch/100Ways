...
Model availability changes over time. If a model disappears or is not enabled for your key, run `nastech model` again and pick one from the current list.
...
If you have not configured Gemini yet, exit the session and run `nastech model` first. `/model` switches among already-configured providers and models; it does not collect new API keys.
...
The selected model is not available for your account, region, or key. Run `nastech model` again and pick another Gemini model from the current list.
