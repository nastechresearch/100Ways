...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.nastech/.env` at the end.
...
3. **Description:** `nastech-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.nastech/.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.nastech/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `nastech gateway restart`.
