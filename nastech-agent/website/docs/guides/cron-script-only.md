...
write_file(
    path="~/.nastech/scripts/memory-watchdog.sh",
...
```bash
# 1. Write your script
cat > ~/.nastech/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.nastech/scripts/memory-watchdog.sh
...
# 3. Verify
nastech cron list
nastech cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.nastech/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
nastech cron create "every 5m"        # interval
nastech cron create "every 2h"
nastech cron create "0 9 * * *"       # standard cron: 9am daily
nastech cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.nastech/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.nastech/.env` / `~/.nastech/config.yaml`.

## Editing and Lifecycle

```bash
nastech cron list                                    # see all jobs
nastech cron pause <job_id>                          # stop firing, keep definition
nastech cron resume <job_id>
nastech cron edit <job_id> --schedule "every 10m"    # adjust cadence
nastech cron edit <job_id> --agent                   # flip to LLM mode
nastech cron edit <job_id> --no-agent --script …     # flip back
nastech cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.nastech/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.nastech/scripts/disk-alert.sh

nastech cron create "*/15 * * * *" \
  --no-agent \
