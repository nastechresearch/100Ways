...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Nastech Agent — what gets migrated, how config maps, and what to check after."
---
...
nastech claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.nastech/` (by default a single restore-point archive is written before apply, under `~/.nastech/backups/pre-migration-*.zip`; restorable with `nastech import`). |
...
| What | OpenClaw source | Nastech destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.nastech/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.nastech/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.nastech/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.nastech/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Nastech destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.nastech/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.nastech/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.nastech/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.nastech/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Nastech skill, `overwrite` replaces it, `rename` creates a `-imported` copy.
...
| What | OpenClaw config path | Nastech config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Nastech config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Nastech field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.nastech/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Nastech `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Nastech path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `NASTECH_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Nastech equivalent)

These are saved to `~/.nastech/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Nastech |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Nastech has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use Nastech profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Nastech manually via `nastech config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.nastech/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `nastech status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart nastech-gateway`

6. **Check session policies** — run `nastech config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `nastech whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `nastech claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `nastech config set`.

### Skills not appearing after migration

Imported skills land in `~/.nastech/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `nastech config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
