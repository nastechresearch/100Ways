...
sidebar_position: 3
title: 'Learning Path'
description: 'Choose your learning path through the Nastech Agent documentation based on your experience level and goals.'
---

# Learning Path

Nastech Agent can do a lot — CLI assistant, Telegram/Discord bot, task automation, RL training, and more. This page helps you figure out where to start and what to read based on your experience level and what you're trying to accomplish.

:::tip Start Here
If you haven't installed Nastech Agent yet, begin with the [Installation guide](/getting-started/installation) and then run through the [Quickstart](/getting-started/quickstart). Everything below assumes you have a working installation.
:::

:::tip First-time provider setup
First-time users almost always want `nastech setup --portal` — one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nastech Portal](/integrations/nastech-portal).
:::
...
- **Just browsing?** Check the [Key Features](#key-features-at-a-glance) table for a quick overview of everything Nastech Agent can do.
...
Use Nastech Agent as an interactive terminal assistant for writing, reviewing, and running code.
...
:::tip
Pass files directly into your conversation with context files. Nastech Agent can read, edit, and run code in your projects.
:::

### "I want a Telegram/Discord bot"

Deploy Nastech Agent as a bot on your favorite messaging platform.
...
7. [Use Voice Mode with Nastech](/guides/use-voice-mode-with-nastech)
...
:::tip
Cron jobs let Nastech Agent run tasks on a schedule — daily summaries, periodic checks, automated reports — without you being present.
:::
...
page is for built-in Nastech core development, not the usual user/custom-tool path.
:::

### "I want to train models"

Use reinforcement learning to fine-tune model behavior with Nastech Agent's RL training pipeline (powered by [Atropos](https://github.com/NastechResearch/atropos)).

1. [Quickstart](/getting-started/quickstart)
2. [Configuration](/user-guide/configuration)
3. [Atropos RL Environments](https://github.com/NastechResearch/atropos) (external)
4. [Provider Routing](/user-guide/features/provider-routing)
5. [Architecture](/developer-guide/architecture)

:::tip
RL training works best when you already understand the basics of how Nastech Agent handles conversations and tool calls. Run through the Beginner path first if you're new.
:::

### "I want to use it as a Python library"

Integrate Nastech Agent into your own Python applications programmatically.
...
| **Code Execution** | Run Python scripts that call Nastech tools programmatically | [Code Execution](/user-guide/features/code-execution) |
