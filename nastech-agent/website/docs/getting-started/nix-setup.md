...
**For non-NixOS users**, this only changes the install step. Everything after (`nastech setup`, `nastech gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage nastech the same way you manage any other NixOS service.
:::
...
```bash
# Run the desktop app
nix run github:NastechResearch/nastech-agent#desktop

# Or install persistently
nix profile install github:NastechResearch/nastech-agent#desktop

# run the tui
nix run github:NastechResearch/nastech-agent -- setup
nix run github:NastechResearch/nastech-agent -- --tui

# or install it in your profile
nix profile install github:NastechResearch/nastech-agent
nastech setup
nastech --tui
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    nastech-agent.url = "github:NastechResearch/nastech-agent";
  };
...
That's it. `nixos-rebuild switch` creates the `nastech` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o nastech /dev/stdin /var/lib/nastech/env
```
...
Setting `addToSystemPackages = true` does two things: puts the `nastech` CLI on your system PATH **and** sets `NASTECH_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `nastech` in your shell creates a separate `~/.nastech/` directory.
:::
...
Users listed in `hostUsers` are automatically added to the `nastech` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo nastech chat` manually.
:::
...
{
  sops = {
    defaultSopsFile = ./secrets/nastech.yaml;
...
```yaml
# secrets/nastech.yaml (encrypted with sops)
nastech-env: |
...
:::tip
Environment variables in `env` values are resolved from `$NASTECH_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$NASTECH_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
# Native mode
sudo -u nastech NASTECH_HOME=/var/lib/nastech/.nastech \
  nastech mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
nastech mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.nastech/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/nastech/.nastech/mcp-tokens/
# Ensure: chown nastech:nastech, chmod 0600
```
...
When nastech runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `nastech setup` | Config is declarative — edit `settings` in your Nix config |
| `nastech config edit` | Config is generated from `settings` |
| `nastech config set <key> <value>` | Config is generated from `settings` |
| `nastech gateway install` | The systemd service is managed by NixOS |
| `nastech gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, nastech runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the nastech package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/nastech` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/nastech-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current nastech package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `nastech plugins install` needed.
...
The package's `site-packages` is added to PYTHONPATH in the nastech wrapper. `importlib.metadata` discovers the entry point at session start.
...
```nix
{
  inputs.nastech-agent.url = "github:NastechResearch/nastech-agent";
...
:::note
A build-time collision check prevents plugin packages from shadowing core nastech dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # NASTECH_MANAGED blocks mutation
...
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `NASTECH_BUNDLED_SKILLS` is set in wrapper |
...
| `stateDir` | `str` | `"/var/lib/nastech"` | State directory (`NASTECH_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `nastech` CLI to system PATH and set `NASTECH_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$NASTECH_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `nastech gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the nastech user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$NASTECH_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.nastech` symlink to the service stateDir and are auto-added to the `nastech` group |
...
```
/var/lib/nastech/                     # stateDir (owned by nastech:nastech, 0750)
├── .nastech/                         # NASTECH_HOME
