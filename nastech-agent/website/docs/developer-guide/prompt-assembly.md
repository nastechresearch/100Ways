...
2. **context** — caller-supplied `system_message` plus project context files (`.nastech.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
`SOUL.md` lives at `~/.nastech/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
1. `.nastech.md` / `NASTECH.md` (walks to git root)
...
- `~/.nastech/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
