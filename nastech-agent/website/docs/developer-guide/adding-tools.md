...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Nastech Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Nastech tool** to the repository itself.
...
2. **`toolsets.py`** — add tool name to `_NASTECH_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `nastech_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `nastech_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `nastech chat -q "Use the weather tool for London"`
