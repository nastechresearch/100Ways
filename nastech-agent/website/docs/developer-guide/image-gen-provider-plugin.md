...
description: "How to build an image-generation backend plugin for Nastech Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** Nastech supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Nastech Plugin](/developer-guide/plugins).
:::

## How discovery works

Nastech scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.nastech/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `nastech_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `nastech tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `nastech tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.nastech/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `nastech plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `nastech tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `nastech tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `nastech tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $NASTECH_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `nastech plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `nastech tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `nastech tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$NASTECH_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.nastech/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `nastech plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.
...
nastech plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $NASTECH_HOME/config.yaml
echo "  provider: my-backend" >> $NASTECH_HOME/config.yaml

# Exercise it
nastech -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `nastech tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a Nastech Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
