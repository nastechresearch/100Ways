...
- setup / `nastech model` menu entries
...
1. `nastech_cli/auth.py` decides how credentials are found.
2. `nastech_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `nastech_cli/models.py` and `nastech_cli/main.py` make the provider show up in the CLI. (`nastech_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `nastech_cli/auth.py`
2. `nastech_cli/models.py`
3. `nastech_cli/runtime_provider.py`
4. `nastech_cli/main.py`
...
:::tip
`nastech_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `nastech setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$NASTECH_HOME/plugins/model-providers/` both get picked up.
...
7. `nastech model` menu includes the provider
8. `nastech setup` wizard delegates to `main.py` automatically
...
User plugins at `$NASTECH_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (Nastech Portal, Codex, Qwen Portal, Copilot)
...
- Provider-specific `nastech model` menu entries with bespoke auth flows
...
## Step 2: Add auth metadata in `nastech_cli/auth.py`
...
- OAuth / auth-store path: Nastech, OpenAI Codex
...
## Step 3: Add model catalog and aliases in `nastech_cli/models.py`
...
## Step 4: Resolve runtime data in `nastech_cli/runtime_provider.py`
...
A provider is not discoverable until it shows up in the interactive `nastech model` flow.

Update these in `nastech_cli/main.py`:
...
:::tip
`nastech_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `nastech model` and `nastech setup` automatically.
:::
...
```bash
source venv/bin/activate
python -m nastech_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m nastech_cli.main model
python -m nastech_cli.main setup
```
...
- [ ] `ProviderConfig` added in `nastech_cli/auth.py`
- [ ] aliases added in `nastech_cli/auth.py` and `nastech_cli/models.py`
- [ ] model catalog added in `nastech_cli/models.py`
- [ ] runtime branch added in `nastech_cli/runtime_provider.py`
- [ ] CLI wiring added in `nastech_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `nastech model` but not `nastech setup`
