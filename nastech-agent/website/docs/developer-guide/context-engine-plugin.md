...
Users select your engine via `nastech plugins` → Provider Plugins → Context Engine, or by editing `config.yaml`:
...
When `compression.context_timeout_seconds > 0` (the default), Nastech runs the
