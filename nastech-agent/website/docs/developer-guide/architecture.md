...
sidebar_position: 1
title: "Architecture"
description: "Nastech Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Nastech Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ nastech_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NastechCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── nastech_state.py           # SQLite session/state database with FTS5
├── nastech_constants.py       # NASTECH_HOME, profile-aware paths
...
│
├── nastech_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `nastech` subcommands (large file)
...
│   ├── skills_config.py      # nastech skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # nastech tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # nastech gateway start/stop
│
...
```text
User input → NastechCLI.process_input()
...
Three discovery sources: `~/.nastech/plugins/` (user), `.nastech/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `nastech plugins` or `config.yaml`.
...
Exposes Nastech as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`nastech -p <name>`) gets its own NASTECH_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
