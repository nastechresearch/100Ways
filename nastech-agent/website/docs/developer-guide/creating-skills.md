...
description: "How to create skills for Nastech Agent — SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to Nastech Agent. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, Nastech prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. Nastech never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
- `prompt` (optional) — prompt text shown during `nastech config migrate`; falls back to `description`
...
2. **Discovery:** `nastech config migrate` scans all enabled skills, finds unconfigured settings, and prompts the user. Settings also appear in `nastech config show` under "Skill Settings."

3. **Runtime injection:** When a skill loads, its config values are resolved and appended to the skill message:
   ```
   [Skill config (from ~/.nastech/config.yaml):
...
4. **Manual setup:** Users can also set values directly:
   ```bash
   nastech config set skills.config.myplugin.path ~/my-data
   ```

:::tip When to use which
Use `required_environment_variables` for API keys, tokens, and other **secrets** (stored in `~/.nastech/.env`, never shown to the model). Use `config` for **paths, preferences, and non-sensitive settings** (stored in `config.yaml`, visible in config show).
:::
...
Each entry supports:
- `path` (required) — file path relative to `~/.nastech/`
- `description` (optional) — explains what the file is and how it's created

When loaded, Nastech checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
:::tip When to use which
Use `required_environment_variables` for simple API keys and tokens (strings stored in `~/.nastech/.env`). Use `required_credential_files` for OAuth token files, client secrets, service account JSON, certificates, or any credential that's a file on disk.
:::
...
Prefer stdlib Python, curl, and existing Nastech tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
| Token | Replaced with |
|---|---|
| `${NASTECH_SKILL_DIR}` | Absolute path to the skill's directory |
| `${NASTECH_SESSION_ID}` | The active session id (left in place if there is no session) |
...
To analyse the input, run:
    ...
    node ${NASTECH_SKILL_DIR}/scripts/analyse.js <input>
```
...
Git branch: !`git -C ${NASTECH_SKILL_DIR} rev-parse --abbrev-ref HEAD`
```
...
```bash
nastech chat --toolsets skills -q "Use the X skill to do Y"
```
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo, is discoverable via `nastech skills browse` (labeled "official"), and installs with built-in trust.

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a registry and share it via `nastech skills install`.

## Blueprints: skills that are also automations

A **blueprint** is an ordinary skill that additionally declares a schedule in its frontmatter. Add a `metadata.nastech.blueprint` block and the skill becomes a shareable, runnable automation:
...
Because a blueprint **is** a skill, it flows through the entire skills pipeline unchanged — search, inspect, install, security scan, provenance, taps, the centralized index, and `nastech skills publish` for sharing. Nothing new to learn.

**Installing a blueprint.** When you install a skill that carries a `blueprint:` block, Nastech registers it as a **suggested cron job** rather than scheduling it. Scheduling is **opt-in** — installing never silently creates a recurring job. You review and accept it via `/suggestions`:

```bash
nastech skills install owner/morning-brief
...
**Sharing an automation you built.** A blueprint loaded by a cron job (`nastech cron create --skill <name> ...`) can be exported back to a SKILL.md and published like any other skill, so an automation you tuned for yourself becomes a one-command install for someone else.
...
Nastech can *propose* automations and let you accept them with one tap, instead of making you assemble cron jobs by hand. Every proposal flows through one surface — the `/suggestions` command — regardless of where it came from:
...
```bash
nastech skills publish skills/my-skill --to github --repo owner/repo
```
...
```bash
nastech skills tap add owner/repo
```
...
Nastech can now consume third-party skills from multiple external discovery models:
