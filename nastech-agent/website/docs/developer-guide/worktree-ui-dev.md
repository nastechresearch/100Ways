...
| Variable | Meaning |
|----------|---------|
| `NASTECH_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `NASTECH_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `NASTECH_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Nastech itself — they're private to these helpers. The variables Nastech *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `nastech --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_nastech_root)" || { echo "htui: not in a Nastech checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$NASTECH_MAIN_CHECKOUT/.venv/bin/python" -m nastech_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `NASTECH_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_nastech_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `NASTECH_TUI_DIR` are mutually exclusive
`NASTECH_TUI_DIR` points Nastech at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `nastech --tui --dev` exits with an error. Run `unset NASTECH_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_nastech_root)" || { echo "hgui: not in a Nastech checkout" >&2; return 1; }
  deps="${NASTECH_GUI_DEPS_CHECKOUT:-$NASTECH_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _nastech_link_deps "$desktop" "$deps/apps/desktop"
    _nastech_link_deps "$root" "$deps"
  else
  ...
  trap '_nastech_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    NASTECH_DESKTOP_NASTECH_ROOT="$root" \
    NASTECH_DESKTOP_PYTHON="$NASTECH_MAIN_CHECKOUT/.venv/bin/python" \
    NASTECH_DESKTOP_IGNORE_EXISTING=1 \
    NASTECH_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `NASTECH_DESKTOP_NASTECH_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `nastech`. |
| `NASTECH_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `NASTECH_DESKTOP_IGNORE_EXISTING` | Ignores any `nastech` on `PATH` so it can't shadow the worktree. |
| `NASTECH_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Nastech checkout.
_nastech_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/nastech_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_nastech_gui_cleanup() {
  local root="$1"
  [[ -n "$root" ]] && pkill -TERM -f "${root}/apps/desktop/node_modules/electron" 2>/dev/null
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'nastech_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `nastech --tui --dev` and the `NASTECH_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NastechResearch/nastech-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `NASTECH_*` variable Nastech reads
