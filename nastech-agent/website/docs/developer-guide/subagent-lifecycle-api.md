...
Plugins can launch and supervise fresh Nastech child sessions without importing
...
active agent turn fails closed with `No active Nastech parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Nastech can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Nastech's existing unsafe-tool block remains enforced.
