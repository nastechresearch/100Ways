...
- `toolsets.py` — registers `browser_dialog` in `browser`, `nastech-acp`, `nastech-api-server`, and core toolsets (gated on CDP reachability)
- `nastech_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
