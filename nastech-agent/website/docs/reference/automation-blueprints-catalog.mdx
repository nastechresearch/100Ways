...
of fields, and Nastech schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Nastech asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.nastech.blueprint` block in its
