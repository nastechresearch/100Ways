...
5. Save the account credentials automatically to `~/.nastech/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.nastech/.env`:
...
- **QR code login** — scan-to-connect setup via `nastech gateway setup`
...
- Tokens are saved per account+peer to `~/.nastech/weixin/accounts/<account_id>.context-tokens.json`
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.nastech/nastech-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
