...
2. **Add** → name (e.g. `nastech-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.nastech/.env` → restart the gateway.
...
The `nastech whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.nastech/.env`.  Required values are in **bold**.
...
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT provider (local faster-whisper, OpenAI/Nastech, Groq, etc.), then handed to the agent as text.
...
Message-template support (the workaround for outside-window sends) is not yet implemented in Nastech. If you need it, please [open an issue](https://github.com/NastechResearch/nastech-agent/issues) — it's planned but waiting on a clear demand signal.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `nastech tools list` and verify the platform is using `nastech-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `nastech_cli/platforms.py` for the platform → default toolset mapping.
...
| | Baileys (`nastech whatsapp`) | Cloud API (`nastech whatsapp-cloud`) |
|---|---|---|
