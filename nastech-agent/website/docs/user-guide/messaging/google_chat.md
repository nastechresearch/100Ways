...
> Run `nastech gateway setup` and pick **Google Chat** for a guided walk-through.
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a nastech restart)
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/nastech-chat-events`.
...
nastech -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```
...
The token lands at `~/.nastech/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.nastech/google_chat_user_token.json` (if present from
...
3. Check `nastech gateway` logs for `[GoogleChat] Connected`. If you see
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.nastech/google_chat_user_tokens/<sanitized_email>.json` (filesystem
