...
> Run `nastech gateway setup` and pick **IRC** for a guided walk-through.

## Prerequisites

- An IRC server to connect to (e.g. `irc.libera.chat`)
- A channel to join (e.g. `#nastech`) — comma-separate to join several
- A nickname for the bot (default: `nastech-bot`)
...
| `IRC_NICKNAME` | ✅ | Bot nickname (default: `nastech-bot`) |
...
By default, only nicks listed in `allowed_users` (or `IRC_ALLOWED_USERS`) may talk to the bot. Leave the list empty **and** set `IRC_ALLOW_ALL_USERS=true` to let anyone in the channel chat with Nastech — useful for testing, but not recommended on public networks since IRC nicks are not authenticated unless the network enforces NickServ.
...
Check status with `nastech gateway status` — IRC connection state is reported there, including for env-only setups.
...
- The adapter acquires a scoped credential lock per server+nick, so two Nastech profiles won't fight over the same IRC identity.
