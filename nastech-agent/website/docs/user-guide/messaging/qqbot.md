...
Connect Nastech to QQ via the **Official QQ Bot API (v2)** — supporting private (C2C), group @-mentions, guild, and direct messages with voice transcription.
...
Set the required environment variables in `~/.nastech/.env`:
...
For fine-grained control, add platform settings to `~/.nastech/config.yaml`:
