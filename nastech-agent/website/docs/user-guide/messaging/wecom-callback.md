...
Connect Nastech to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Nastech supports two WeCom integration modes:
...
> Run `nastech gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Nastech decrypts the message, queues it for the agent
...
(Use `nastech gateway start` only after `nastech gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Nastech and the token the
...
in `~/.nastech/.env` values around `=` will also break signature checks. After
fixing, restart `nastech gateway run`.
...
Check `nastech gateway run` logs for the bound host/port. If the adapter bound to
