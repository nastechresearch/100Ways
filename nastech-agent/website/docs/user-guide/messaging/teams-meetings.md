...
> Run `nastech gateway setup` and pick **Teams Meetings** for a guided walk-through.
...
Operator actions stay in the CLI (the `teams-pipeline` subcommand is registered by the `teams_pipeline` plugin — enable it via `nastech plugins enable teams_pipeline` or set `plugins.enabled: [teams_pipeline]` in `config.yaml`):
...
Add Graph app-only credentials to `~/.nastech/.env`:
...
| Jobs do not appear in `nastech teams-pipeline list` | Confirm `msgraph_webhook` is enabled and that subscriptions point at the right notification URL |
