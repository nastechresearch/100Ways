...
Connect Nastech to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Nastech and the BlueBubbles server
...
Or set environment variables directly in `~/.nastech/.env`:
...
By default, Nastech responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
When someone messages your iMessage, Nastech automatically sends them a pairing code. Approve it with:
```bash
nastech pairing approve bluebubbles <CODE>
```
Use `nastech pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.nastech/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.nastech/.env`):
```bash
...
Nastech will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Nastech
Nastech → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Nastech sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Nastech wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.nastech/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Nastech resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `nastech logs gateway` for webhook errors (or `nastech logs -f` to follow in real-time)
