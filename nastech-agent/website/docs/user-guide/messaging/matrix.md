...
sidebar_position: 9
title: "Matrix"
description: "Set up Nastech Agent as a Matrix bot"
---

# Matrix Setup

Nastech Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Nastech Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Nastech works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Nastech behaves once it's connected.

## How Nastech Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Nastech responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Nastech requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Nastech supports Matrix threads (MSC3440). If you reply in a thread, Nastech keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Nastech auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Nastech accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Nastech normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Nastech isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
Nastech sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Nastech now includes the current Matrix room name, room ID, topic, message ID,
...
2. Choose a username like `nastech` — the full user ID will be `@nastech:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `nastech-bot`).

### Option C: Use Your Own Account

You can also run Nastech as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Nastech needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Nastech the bot's user ID and password. Nastech will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@nastech:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Nastech Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
# MATRIX_USER_ID=@nastech:matrix.example.org
...
Bridge and appservice deployments need extra loop protection. Nastech always
...
Optional behavior settings in `~/.nastech/config.yaml`:
...
:::tip
You can run `nastech gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Nastech supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with nastech extras
cd ~/.nastech/nastech-agent && uv pip install -e ".[matrix]"
```
...
When E2EE is enabled, Nastech:

- Stores encryption keys in `~/.nastech/platforms/matrix/store/` (legacy installs: `~/.nastech/matrix/store/`)
...
In Matrix conversations, Nastech exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Nastech knows who requested the action.

### Media Limits

Nastech uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Nastech rejects arbitrary
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Nastech imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Nastech bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.nastech/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Nastech detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@nastech:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@nastech:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@nastech:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@nastech:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40nastech%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart Nastech:
   ```bash
   rm -f ~/.nastech/platforms/matrix/store/crypto.db*
   # restart nastech
   ```
...
Nastech supports the same gateway commands in Matrix that it supports on other
...
Nastech only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NastechResearch/nastech-agent/issues/12614).
...
If you previously used Nastech with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@nastech:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Nastech Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.nastech/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.nastech/platforms/matrix/store/crypto.db
   rm -f ~/.nastech/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.nastech/.env`:
...
a working E2EE setup with a previous version of Nastech and are upgrading.
...
services:
  nastech-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "NASTECH_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.nastech/platforms/matrix/store
```
...
RUN cd ~/.nastech/nastech-agent && uv pip install -e ".[matrix]"
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Nastech API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Nastech-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Nastech uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Nastech's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Nastech logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Nastech gateway isn't running, or it failed to connect.

**Fix**: Check that `nastech gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.nastech/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: Nastech can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
