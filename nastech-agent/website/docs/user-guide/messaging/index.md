...
description: "Chat with Nastech from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Nastech from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Nastech](/guides/use-voice-mode-with-nastech).
...
```mermaid
flowchart TB
    subgraph Gateway["Nastech Gateway"]
...
For group chats, hooks, and automation flows, Nastech supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Nastech keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Nastech does not hide failures just because the text resembles a silence token.
...
```bash
nastech gateway setup        # Interactive setup for all messaging platforms
```
...
nastech gateway setup        # Configure messaging platforms interactively
nastech gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo nastech gateway install --system   # Linux only: install a boot-time system service
nastech gateway start        # Start the default service
nastech gateway stop         # Stop the default service
...
```yaml title="~/.nastech/config.yaml"
gateway:
...
```bash
nastech gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Nastech sends heartbeats
...
| `/update` | Update Nastech Agent to the latest version |
...
nastech pairing approve telegram XKGH5N7P

# Other pairing commands:
nastech pairing list          # View pending + approved users
nastech pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Nastech appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.nastech/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.nastech/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `NASTECH_HOME`:

- `~/.nastech/status_phrases.yaml` or any `*.yaml` in `~/.nastech/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to NASTECH_HOME
...
Off by default. When enabled, Nastech prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.nastech/config.yaml`:
...
```bash
nastech gateway install               # Install as user service
nastech gateway start                 # Start the service
nastech gateway stop                  # Stop the service
nastech gateway status                # Check status
journalctl --user -u nastech-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo nastech gateway install --system
sudo nastech gateway start --system
sudo nastech gateway status --system
journalctl -u nastech-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Nastech installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit nastech-gateway` (or `sudo systemctl edit nastech-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `nastech update`. When `nastech update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart nastech-gateway` command (it never blocks on an interactive password prompt).
...
```bash
nastech gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `nastech update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo nastech update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/nastech-gateway`:

```
nastech ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed nastech-gateway*, /usr/bin/systemctl --no-ask-password start nastech-gateway*, /usr/bin/systemctl --no-ask-password restart nastech-gateway*
```
:::
...
```bash
nastech gateway install               # Install as launchd agent
nastech gateway start                 # Start the service
nastech gateway stop                  # Stop the service
nastech gateway status                # Check status
tail -f ~/.nastech/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.nastech.gateway.plist`. It includes three environment variables:
...
- **NASTECH_HOME** — scopes the gateway to your Nastech installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `nastech gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `NASTECH_HOME` directory gets its own launchd label. The default `~/.nastech` uses `ai.nastech.gateway`; other installations use `ai.nastech.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `nastech-cli` | Full access |
| Telegram | `nastech-telegram` | Full tools including terminal |
| Discord | `nastech-discord` | Full tools including terminal |
| WhatsApp | `nastech-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `nastech-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `nastech-slack` | Full tools including terminal |
| Google Chat | `nastech-google_chat` | Full tools including terminal |
| Signal | `nastech-signal` | Full tools including terminal |
| SMS | `nastech-sms` | Full tools including terminal |
| Email | `nastech-email` | Full tools including terminal |
| Home Assistant | `nastech-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `nastech-mattermost` | Full tools including terminal |
| Matrix | `nastech-matrix` | Full tools including terminal |
| DingTalk | `nastech-dingtalk` | Full tools including terminal |
| Feishu/Lark | `nastech-feishu` | Full tools including terminal |
| WeCom | `nastech-wecom` | Full tools including terminal |
| WeCom Callback | `nastech-wecom-callback` | Full tools including terminal |
| Weixin | `nastech-weixin` | Full tools including terminal |
| BlueBubbles | `nastech-bluebubbles` | Full tools including terminal |
| QQBot | `nastech-qqbot` | Full tools including terminal |
| Yuanbao | `nastech-yuanbao` | Full tools including terminal |
| Microsoft Teams | `nastech-teams` | Full tools including terminal |
| API Server | `nastech-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `nastech-webhook` | Full tools including terminal |
| Raft | `nastech-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.nastech/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
