...
(device token, dashboard project id) lives in `~/.nastech/auth.json` under
...
**Pre-authorize specific numbers** (in `~/.nastech/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.nastech/.env`):
...
- **`sidecar deps : ✗ run nastech photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `nastech photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `nastech photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `nastech photon install-sidecar` completed without errors.
