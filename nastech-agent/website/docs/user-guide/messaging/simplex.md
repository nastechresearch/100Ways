...
> Run `nastech gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `NASTECH_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `nastech pairing approve simplex <CODE>`.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`nastech send` CLI](/guides/pipe-script-output):

```bash
nastech send simplex:<contact-id> "Done!"
```
