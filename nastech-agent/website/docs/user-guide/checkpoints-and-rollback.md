...
Or enable globally in `~/.nastech/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.nastech/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `nastech checkpoints` | Show total size, project count, per-project breakdown |
| `nastech checkpoints status` | Same as bare `checkpoints` |
| `nastech checkpoints list` | Alias for `status` |
| `nastech checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `nastech checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `nastech checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |
...
- Once per conversation turn (per directory), it:
  ...
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/nastech/<project-hash>`).
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `nastech checkpoints prune` manually.
...
```text
Checkpoint base: /home/you/.nastech/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
...
Clear with: nastech checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
nastech checkpoints prune --retention-days 3 --max-size-mb 200
```
...
~/.nastech/checkpoints/
  ...
  │   ├── refs/nastech/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `nastech checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.nastech/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.nastech/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
nastech checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `nastech chat --checkpoints` or per-profile `enabled: true`.
