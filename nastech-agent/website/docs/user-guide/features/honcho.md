...
**Multi-agent profiles**: When multiple Nastech instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.nastech/.env
```
...
- `per-session` — each `nastech` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Nastech syncs them back at session init.
...
The `nastech honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `nastech memory setup honcho` (or run `nastech memory setup` and pick it from the list); the `nastech honcho` subcommand then appears on the next invocation.

```bash
nastech memory setup honcho    # Configure Honcho directly (works before activation)
nastech honcho status          # Connection status, config, and key settings
nastech honcho setup           # Redirects to `nastech memory setup` (post-activation alias)
nastech honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
nastech honcho peer            # Show or update peer names + dialectic reasoning level
nastech honcho mode            # Show or set recall mode (hybrid/context/tools)
nastech honcho tokens          # Show or set token budget for context and dialectic
nastech honcho identity        # Seed or show the AI peer's Honcho identity
nastech honcho sync            # Sync Honcho config to all existing profiles
nastech honcho peers           # Show peer identities across all profiles
nastech honcho sessions        # List known Honcho session mappings
nastech honcho map             # Map current directory to a Honcho session name
nastech honcho enable          # Enable Honcho for the active profile
nastech honcho disable         # Disable Honcho for the active profile
nastech honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `nastech honcho`

If you previously used the standalone `nastech honcho setup`:
...
No re-login or re-setup needed. Run `nastech memory setup` and select "honcho" — the wizard detects your existing config.
