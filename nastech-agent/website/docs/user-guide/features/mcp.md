...
description: "Connect Nastech Agent to external tool servers via MCP — and control exactly which MCP tools Nastech loads"
---

# MCP (Model Context Protocol)

MCP lets Nastech Agent connect to external tool servers so the agent can use tools that live outside Nastech itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.
...
- Per-server filtering so you can expose only the MCP tools you actually want Nastech to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.nastech/config.yaml`:
...
4. Ask Nastech to use the MCP-backed capability.
...
## Catalog: one-click install for Nastech-approved MCPs

Nastech ships a curated catalog of MCP servers that Nastech staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
nastech mcp                # interactive picker (default)
nastech mcp catalog        # plain-text list, scriptable
nastech mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Nastech
...
Nastech approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Nastech prompts at install time and writes the value to
  `~/.nastech/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Nastech points you at
  `nastech auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Nastech probes the MCP server to list every
...
written (if not). Re-run `nastech mcp configure <name>` once the server is
reachable to refine.
...
the nastech-agent repo, so Nastech has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NastechResearch/nastech-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Nastech
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Nastech`) for that entry instead of silently hiding it. Run `nastech update`
to install the latest Nastech when you see that.
...
from environment variables (which include everything in `~/.nastech/.env`).
...
MCPs are never auto-updated. Re-run `nastech mcp install <name>` to refresh
after a Nastech update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/NastechResearch/nastech-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints Nastech connects to directly.
...
- you do not want Nastech spawning a local subprocess for that integration
...
On first connect, Nastech prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.nastech/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Nastech runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:
...
**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `nastech mcp login` can list the tools and look like it worked, but every real tool call later times out. `nastech mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `nastech mcp login googledrive` — with the pre-registered client, Nastech skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.nastech/config.yaml` from inside a running Nastech session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `nastech mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Nastech passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
For well-known MCP servers, `nastech mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
nastech mcp add codex --preset codex
```
...
You can pick any local name (`nastech mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Nastech registers MCP tools

Nastech prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Nastech sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Nastech also registers utility tools around MCP resources and prompts:
...
- Nastech only registers resource utilities if the MCP session actually supports resource operations
- Nastech only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Nastech, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Nastech skips the server completely and does not even attempt a connection.
...
You can also separately disable Nastech-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Nastech does not create an empty runtime MCP toolset for that server.
...
Nastech discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Nastech when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Nastech receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Nastech does not blindly pass your full shell environment.
...
Then verify your config and restart Nastech.
...
Because Nastech now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Nastech may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Nastech via the `sampling/createMessage` protocol. This allows an MCP server to ask Nastech to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Nastech
- You want a single MCP server that bridges to all of Nastech's connected messaging platforms at once
- You already have a running Nastech gateway with connected platforms
...
Add Nastech to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Nastech in a specific location:
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Nastech-specific channel browser:
...
The MCP server includes a live event bridge that polls Nastech's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
nastech mcp serve              # Normal mode
nastech mcp serve --verbose    # Debug logging on stderr
```
...
- The embedded `nastech mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Nastech, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
