...
The web dashboard is a browser-based UI for managing your Nastech Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nastech Portal OAuth; if you also want the dashboard to talk to a real backend, `nastech setup --portal` wires up the model and tool gateway too. See [Nastech Portal](/integrations/nastech-portal).
:::
...
```bash
# Custom port
nastech dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
nastech dashboard --host 0.0.0.0

# Start without opening browser
nastech dashboard --no-open
```
...
child with the selected profile's `NASTECH_HOME`, so the conversation runs
...
processes (manage them via `nastech -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `nastech-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.nastech/nastech-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.nastech/nastech-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `nastech dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `nastech dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `nastech --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.nastech/nastech-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.nastech/.env
ExecStart=/path/to/venv/bin/python -m nastech_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.nastech/.env` containing:

```bash
NASTECH_DASHBOARD_BASIC_AUTH_USERNAME=admin
NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
NASTECH_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `nastech config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Nastech instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `nastech profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.nastech/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `nastech skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `nastech mcp` reads from.
...
**Catalog:** browse the Nastech-approved MCP servers (the bundled `optional-mcps/`
...
`nastech mcp catalog` / `nastech mcp install` use.
...
Connect Nastech to any messaging platform from the browser — full parity with
`nastech setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.nastech/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Nastech versions, CPU core count + utilization, memory, disk usage of the Nastech home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Nastech version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `nastech update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nastech Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `nastech portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `nastech curator`.
...
![System admin page — host stats and Nastech Portal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.nastech/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`NASTECH_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
| `GET /api/mcp/catalog` | Browse the Nastech-approved MCP catalog |
...
| `GET /api/nastech/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Nastech Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Nastech Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (Nastech Portal)](#default-provider-nastech-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote Nastech Desktop connection](#connecting-nastech-desktop-to-a-remote-backend). Every login is verified against your Nastech account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No Nastech Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
|-------|-----------|----------|
| `nastech dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `nastech dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nastech plugin, no custom plugin), `nastech dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `nastech dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Nastech doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `nastech dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: Nastech Research

The bundled `plugins/dashboard_auth/nastech` plugin is **always installed** and auto-loaded. It auto-registers a `DashboardAuthProvider` named `nastech` when a client ID is configured.

Because every login is verified against Nastech Portal and protected by your Nastech account, **the Nastech provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Nastech provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `nastech dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nastech login (run `nastech setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `NASTECH_DASHBOARD_OAUTH_CLIENT_ID` into `~/.nastech/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
|---------|-----------|--------|----------------|
| `NASTECH_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `nastech dashboard register` |

Per the Nastech Agent convention (`~/.nastech/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nastech: NASTECH_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `nastech dashboard register` (Nastech Portal) or install a
...
#### Worked example: Nastech Research

From a logged-in Nastech install to a Nastech-gated dashboard in three steps.

**1. Log in and register the dashboard.** `nastech dashboard register` uses your existing Nastech login to provision an OAuth client and writes `NASTECH_DASHBOARD_OAUTH_CLIENT_ID` into `~/.nastech/.env` for you:

```bash
nastech setup            # if you're not already logged into Nastech Portal
nastech dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes NASTECH_DASHBOARD_OAUTH_CLIENT_ID to ~/.nastech/.env
```

**2. Run the dashboard on a reachable address.** A non-loopback bind engages the OAuth gate, and the `client_id` just written activates the `nastech` provider:

```bash
nastech dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Nastech Research** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: nastech`). For an internet-facing host, register with `--redirect-uri https://nastech.example.com/auth/callback` and set `NASTECH_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [Nastech Research provider](#default-provider-nastech-research) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like the Nastech provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `NASTECH_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `NASTECH_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `NASTECH_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.nastech/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.nastech/.env <<EOF
NASTECH_DASHBOARD_BASIC_AUTH_USERNAME=admin
NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
NASTECH_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Nastech Research](#default-provider-nastech-research) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Nastech Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the Nastech provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/nastech/   # required
      client_id: nastech-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `NASTECH_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `NASTECH_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `NASTECH_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-nastech.json` — it defines a `nastech` realm, a **public PKCE client** (`nastech-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "nastech-dashboard",
      "name": "Nastech Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-nastech.json:/opt/keycloak/data/import/realm-nastech.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/nastech/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/nastech`). The admin console is at
...
```bash
export NASTECH_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/nastech"
export NASTECH_DASHBOARD_OIDC_CLIENT_ID="nastech-dashboard"
export NASTECH_DASHBOARD_PUBLIC_URL="http://localhost:9119"
nastech dashboard --host 0.0.0.0 --port 9119 --no-open
```

`NASTECH_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → nastech-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `NASTECH_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/nastech"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `NASTECH_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `NASTECH_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Nastech Portal OAuth contract v1](https://github.com/NastechResearch/nastech-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Nastech Research" button → `/auth/login?provider=nastech`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.nastechresearch.com/oauth/authorize?…`.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `nastech_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `nastech_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `nastech_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
The sidebar widget shows `Logged in as <user_id…> via nastech` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.

### Audit log

Every login start, success, failure, and session-verify failure is written as a JSON line to `$NASTECH_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-Nastech OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.nastech/plugins/dashboard-auth-myidp/__init__.py
from nastech_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nastech-account-service` provisions a per-agent secret via `NASTECH_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
NASTECH_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  nastech dashboard --host 0.0.0.0
...
Nastech Desktop can drive a Nastech backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Nastech Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nastech Portal)** (register it with [`nastech dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nastech Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Nastech Research](#default-provider-nastech-research).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.nastech/.env (secrets file, 0600).
cat >> ~/.nastech/.env <<'EOF'
NASTECH_DASHBOARD_BASIC_AUTH_USERNAME=admin
NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
nastech dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.nastech/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.nastech/.env`, so the credentials are in the environment at boot.

:::warning
The dashboard reads and writes your `.env` (API keys, secrets) and can run agent commands. The **username/password** setup shown here is for a trusted network — never expose a password-protected dashboard directly to the open internet. Put it behind a VPN. [Tailscale](https://tailscale.com/) is the clean option: bind to the machine's tailscale IP (`--host <tailscale-ip>`) and use `http://<tailscale-ip>:9119` as the Remote URL. Only devices on your tailnet can reach it. To reach a backend over the public internet, use the **OAuth (Nastech Portal)** provider instead.
:::
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/nastech` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `NASTECH_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `NASTECH_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `NASTECH_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `NASTECH_DASHBOARD_BASIC_AUTH_USERNAME` / `NASTECH_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `NASTECH_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `nastech_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `nastech update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `nastech dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Nastech Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Nastech Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Nastech Blue** (`nastech-blue`) | Nastech-branded blue accents with airy spacing |
