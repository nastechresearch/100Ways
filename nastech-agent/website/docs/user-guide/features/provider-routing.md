...
description: Configure OpenRouter or Nastech Portal provider preferences to optimize for cost, speed, or quality.
...
When using [OpenRouter](https://openrouter.ai) or [Nastech Portal](/integrations/nastech-portal) as your LLM provider, Nastech Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through Nastech Portal respects the same provider preferences — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.nastech/config.yaml`:
...
:::info
Provider routing only applies when using OpenRouter or Nastech Portal. It has no effect with direct provider connections (e.g., connecting directly to the Anthropic API).
:::
...
Provider routing controls which **sub-providers behind OpenRouter or Nastech Portal** handle your requests. For automatic failover to an entirely different provider when your primary model fails, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
