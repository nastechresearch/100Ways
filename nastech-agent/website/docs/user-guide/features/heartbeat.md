...
Inspired by Prime-Agent's `/heartbeat`. The Nastech adaptation keeps the strict message-flow invariants: the heartbeat is injected only between turns (never mid-run), as a plain user-role message.

## Heartbeat vs cron: which one do I want?

They look similar but serve different jobs:

| | `/heartbeat` | [`nastech cron`](./cron) |
|---|---|---|
...
`/hb` is an alias. Works on the CLI and gateway platforms (on Slack, use `/nastech heartbeat …`).
...
Nastech: [Heartbeat — recurring instruction, fires every 15m]
