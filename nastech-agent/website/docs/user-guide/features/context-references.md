...
Type `@` followed by a reference to inject content directly into your message. Nastech expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Nastech env: `$NASTECH_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$NASTECH_HOME/skills/.hub/`
