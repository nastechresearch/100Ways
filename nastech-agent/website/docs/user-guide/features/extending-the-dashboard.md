...
description: "Build themes and plugins for the Nastech web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Nastech web dashboard (`nastech dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.nastech/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.nastech/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.nastech/dashboard-themes
```

```yaml
# ~/.nastech/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-nastech-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Nastech Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Nastech Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Nastech Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.nastech/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Nastech plugins in `~/.nastech/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__NASTECH_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
```bash
mkdir -p ~/.nastech/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.nastech/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.nastech/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
  ...
  window.__NASTECH_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Everything a plugin needs is on `window.__NASTECH_PLUGIN_SDK__`. Plugins should never import React directly.
...
// Nastech API client + raw fetcher
...
#### Calling built-in Nastech endpoints
...
```javascript
window.__NASTECH_PLUGINS__.registerSlot("my-plugin", "sidebar", MySidebar);
window.__NASTECH_PLUGINS__.registerSlot("my-plugin", "header-left", MyCrest);
```
...
| `header-left` | Before the Nastech brand in the top bar. |
...
window.__NASTECH_PLUGINS__.registerSlot("my-plugin", "sessions:top", PinnedSessionsBanner);
```
...
```json
// ~/.nastech/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.nastech/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__NASTECH_PLUGIN_SDK__;
  const { React } = SDK;
  ...
  window.__NASTECH_PLUGINS__.register("session-notes", function () { return null; });

  // The real work.
  window.__NASTECH_PLUGINS__.registerSlot("session-notes", "sessions:top", Banner);
})();
```
...
A reference plugin (`example-dashboard` in [`nastech-example-plugins`](https://github.com/NastechResearch/nastech-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.nastech/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing Nastech internals

Backend routes run inside the dashboard process, so they can import from the nastech-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.nastech/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.nastech/plugins/<name>/dashboard/` | `project` — only when `NASTECH_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `nastech dashboard`.

#### Plugin load lifecycle

1. Dashboard loads. `main.tsx` exposes the SDK on `window.__NASTECH_PLUGIN_SDK__` and the registry on `window.__NASTECH_PLUGINS__`.
...
4. The plugin's IIFE runs and calls `window.__NASTECH_PLUGINS__.register(name, Component)` — and optionally `.registerSlot(name, slot, Component)` for each slot.
...
The [`strike-freedom-cockpit`](https://github.com/NastechResearch/nastech-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `nastech-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NastechResearch/nastech-example-plugins.git

# Theme
cp nastech-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.nastech/dashboard-themes/

# Plugin
cp -r nastech-example-plugins/strike-freedom-cockpit ~/.nastech/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Nastech Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
|--------|------|----------|
| `window.__NASTECH_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__NASTECH_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__NASTECH_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
Check that the file is in `~/.nastech/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.nastech/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.nastech/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__NASTECH_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__NASTECH_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
2. Restart `nastech dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.nastech/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.nastech/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
