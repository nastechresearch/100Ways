...
persona in `~/.nastech/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.nastech/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.nastech/config.yaml` under the `mcp_servers`
