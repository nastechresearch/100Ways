...
| CLI | Saved to `~/.nastech/audio_cache/` | MP3 |
...
Each provider has a documented per-request input-character cap. Nastech splits longer replies into ordered, sentence-aware chunks before calling the provider, so the full normalized text is preserved instead of silently truncated:
...
| A voice-listing API used by `nastech setup` | **Plugin** (override `list_voices()`) |
...
Drop this in `~/.nastech/plugins/my-tts/`:
...
Enable it (`nastech plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `nastech tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `nastech tools` / `nastech setup`. Without this, the plugin still works but its row in the picker is minimal.
...
Drop this in `~/.nastech/plugins/my-stt/`:
...
Enable it (`nastech plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `nastech tools` / `nastech setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
