...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls. A [Nastech Portal](/user-guide/features/tool-gateway) subscription bundles model access plus web search, image gen, TTS, and cloud browsers under one bill — useful when you want stable cost-per-trajectory without juggling rate limits across five vendor accounts. Set up with `nastech setup --portal`, then point `--model` at a Nastech model.
:::
