...
Nastech can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.nastech/auth.json` and refreshed automatically on 401; you only log in once per machine (refresh tokens expire after ~6 months; re-run `nastech auth spotify` when they do).

Unlike Nastech' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `nastech auth spotify` walks you through it.
...
- Nastech Agent installed and running.
...
### One-shot: `nastech tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `nastech setup` / `nastech setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `nastech tools`.

Nastech drops you straight into the OAuth flow — if you don't have a Spotify app yet, it walks you through creating one inline. Once you finish, the toolset is enabled AND authenticated in one pass.
...
If no `NASTECH_SPOTIFY_CLIENT_ID` is set, Nastech walks you through the app registration inline:
...
4. Saves it to `~/.nastech/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.nastech/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
```bash
nastech auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Nastech restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `nastech auth logout spotify`.
...
If Home Assistant manages speakers that already support Spotify Connect (for example Sonos, Echo, Nest, or other Connect-capable speakers), they appear in `spotify_devices list` automatically whenever Spotify can see them. Nastech does not need a Home Assistant ↔ Spotify bridge for this path — Spotify handles the device routing natively.

Ask Nastech to transfer playback by the speaker's display name (for example, “transfer Spotify to the kitchen speaker”), or call `spotify_devices list` and pass the exact `device_id` to `spotify_devices transfer` when scripting. If the speaker is missing, open the Spotify app or the speaker's Spotify integration once so Spotify registers it as an active Connect target.
...
Because Spotify tools are regular Nastech tools, a cron job running in a Nastech session can trigger playback on any schedule. No new code needed.
...
1. Cron spins up a headless Nastech session.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `nastech tools` for the cron session to see the Spotify tools.
...
```bash
nastech auth logout spotify
```

Removes tokens from `~/.nastech/auth.json`. To also clear the app config, delete `NASTECH_SPOTIFY_CLIENT_ID` (and `NASTECH_SPOTIFY_REDIRECT_URI` if you set it) from `~/.nastech/.env`, or run the wizard again.
...
**`204 No Content` on `get_currently_playing`** — nothing is currently playing on any device. This is Spotify's normal response, not an error; Nastech surfaces it as an explanatory empty result (`is_playing: false`).

**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Nastech is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `NASTECH_SPOTIFY_REDIRECT_URI` in `~/.nastech/.env` to whatever you registered.

**`429 Too Many Requests`** — Spotify's rate limit. Nastech returns a friendly error; wait a minute and retry. If this persists, you're probably running a tight loop in a script — Spotify's quota resets roughly every 30 seconds.

**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `nastech auth spotify` again.

**Wizard doesn't open the browser** — If you're over SSH or in a container without a display, Nastech detects it and skips the auto-open. Copy the dashboard URL it prints and open it manually.

## Advanced: custom scopes

By default Nastech requests the scopes needed for every shipped tool. Override if you want to restrict access:

```bash
nastech auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
```bash
nastech auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.nastech/.env`:

```
NASTECH_SPOTIFY_CLIENT_ID=<your_id>
NASTECH_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| File | Contents |
|------|----------|
| `~/.nastech/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.nastech/.env` | `NASTECH_SPOTIFY_CLIENT_ID`, optional `NASTECH_SPOTIFY_REDIRECT_URI` |
