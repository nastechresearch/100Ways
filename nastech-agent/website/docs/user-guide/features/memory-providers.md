...
```bash
nastech memory setup      # interactive picker + configuration
nastech memory status     # check what's active
nastech memory off        # disable external provider
```

You can also select the active memory provider via `nastech plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.nastech/config.yaml`:
...
:::tip Migrating from `nastech honcho`
If you previously used `nastech honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::
...
`--clone` creates a `nastech.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Off-gateway these keys do nothing. `nastech memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
**Setup (Platform):**
```bash
nastech memory setup    # select "mem0" → "Platform"
# Or manually:
nastech config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.nastech/.env
```

**Setup (OSS):**
```bash
nastech memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
nastech memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```
...
**Config:** `$NASTECH_HOME/hindsight/config.json`
...
**Setup:**
```bash
nastech memory setup    # select "holographic"
# Or manually:
nastech config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.nastech-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$NASTECH_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
nastech memory setup    # select "retaindb"
# Or manually:
nastech config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.nastech/.env
```
...
- Knowledge tree stored at `$NASTECH_HOME/byterover/` (profile-scoped)
...
- **Local storage providers** (Holographic, ByteRover) use `$NASTECH_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$NASTECH_HOME/` so each profile has its own credentials
