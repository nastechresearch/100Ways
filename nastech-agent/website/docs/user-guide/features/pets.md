...
- Pets are installed into your profile's `pets/` directory
  (`<NASTECH_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
nastech pets list
nastech pets list cat

# Install a pet and make it active in one step
nastech pets install boba --select
...
## `nastech pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `nastech pets list [query] [--limit N]` |
| List installed pets | `nastech pets list --installed` |
| Install a pet | `nastech pets install <slug> [--select] [--force]` |
| Set the active pet | `nastech pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `nastech pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `nastech pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `nastech pets off` |
| Remove an installed pet | `nastech pets remove <slug>` |
| Diagnose setup | `nastech pets doctor` |

`nastech pets show` flags:
...
Run `nastech pets doctor` — it reports:
