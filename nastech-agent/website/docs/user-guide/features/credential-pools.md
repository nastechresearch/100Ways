...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Nastech Portal](/integrations/nastech-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
Run `nastech auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Nastech, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `nastech auth` | Interactive pool management wizard |
| `nastech auth list` | Show all pools and credentials |
| `nastech auth list <provider>` | Show a specific provider's pool |
...
Pool state is stored in `~/.nastech/auth.json` under the `credential_pool` key:
