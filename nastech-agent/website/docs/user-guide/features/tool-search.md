...
**Tool Search** is Nastech' opt-in progressive-disclosure layer for that
...
:::info Built-in Nastech tools never defer
The tools that make up Nastech' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_NASTECH_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Nastech **unwraps the bridge** and
...
- **No JS sandbox.** Nastech uses the simpler "structured tools" mode
