...
4. **Putting the state dir where the agent's sandbox can't write.** `$NASTECH_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
