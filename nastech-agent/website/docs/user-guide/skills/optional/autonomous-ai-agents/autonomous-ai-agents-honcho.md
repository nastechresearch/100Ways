...
See: https://docs.honcho.dev/v3/guides/integrations/nastech#running-honcho-locally-with-nastech
...
1. Creates a `nastech.coder` host block in `honcho.json`
...
Config file: `$NASTECH_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nastech honcho setup`. Ensure `memory.provider: honcho` is in `~/.nastech/config.yaml`.

### Memory not persisting across sessions
Check `nastech honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nastech profile create <name> --clone`. For existing profiles: `nastech honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nastech honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nastech honcho status` | Show resolved config, connection test, peer info for active profile |
| `nastech honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nastech honcho disable` | Disable Honcho for the active profile |
| `nastech honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
...
| `nastech honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
