...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/email/agentmail` |
| Path | `optional-skills/email/agentmail` |
| Version | `1.0.0` |
| Author | teyrebaz33, Nastech Agent |
| License | MIT |
...
Add to `~/.nastech/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `nastech-agent`)
   - The agent gets address: `nastech-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "nastech-outreach")
...
```
nastech --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
