...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NASTECH_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NASTECH_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NASTECH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nastech
```
...
```bash
PUBLISH="${NASTECH_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nastech
```
...
```bash
DRIVE="${NASTECH_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nastech`)    |
