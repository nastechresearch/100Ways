...
This optional skill gives Nastech practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${NASTECH_HOME:-~/.nastech}/.env`
...
It does **not** turn Nastech into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Nastech memory
...
### 1) "I want Nastech to own a real phone number"
Use **Twilio**.
...
- pairs well with Nastech `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${NASTECH_HOME:-~/.nastech}/.env`
...
```bash
SCRIPT="$(find ~/.nastech/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
nastech skills search telephony
nastech skills install official/productivity/telephony
```
...
Then save credentials into Nastech:
...
3. Buy it and save it into `${NASTECH_HOME:-~/.nastech}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Nastech calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Nastech's existing `text_to_speech` support.

Use this when:
- you want the call to use Nastech's configured TTS voice rather than Twilio `<Say>`
...
Recommended Nastech TTS -> Twilio Play workflow:

1. Generate the audio with Nastech `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Nastech to create the message audio with `text_to_speech`
...
Important note:
- Nastech TTS is great for prerecorded outbound messages
...
- Nastech STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Nastech memory.
...
- Do not store arbitrary third-party phone numbers in Nastech memory.
...
3. persist that number to `${NASTECH_HOME:-~/.nastech}/.env`
