...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/productivity/siyuan` |
...
3. Store it in `${NASTECH_HOME:-~/.nastech}/.env`:
   ```
...
```yaml
# In ~/.nastech/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
