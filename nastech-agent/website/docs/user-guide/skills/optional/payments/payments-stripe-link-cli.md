...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/payments/stripe-link-cli` |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Nastech can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Nastech cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Nastech attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Nastech" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Nastech' native MCP:

```
nastech mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `nastech mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
