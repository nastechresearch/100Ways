...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp/mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Nastech Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`nastech-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nastech-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Nastech Gateway

## Overview

Nastech' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Nastech process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Nastech runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Nastech.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Nastech' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Nastech is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Nastech** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Nastech process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the nastech user, an
empty or missing tokens directory (`$NASTECH_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Nastech detects a remote session it prints two
...
Both require an interactive terminal to the Nastech host. The rest of this skill
is for when there is NO interactive TTY — Nastech running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Nastech Dashboard (try this BEFORE manual token surgery)

A remote Nastech gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `nastech dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'nastech dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "NASTECH_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`NASTECH_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$NASTECH_HOME/.env`. Those
...
Nastech' `NastechTokenStorage` would have written, so on `/reload-mcp` Nastech finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "NASTECH|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Nastech' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find NASTECH_HOME and the config path

```bash
NASTECH_HOME=$(python3 -c 'from nastech_constants import get_nastech_home; print(get_nastech_home())')
echo "config: $NASTECH_HOME/config.yaml"
echo "tokens: $NASTECH_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Nastech Agent (manual OAuth)",
...
### 8. Write tokens in Nastech' exact schema

`tools/mcp_oauth.py::NastechTokenStorage` expects two files under
`$NASTECH_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "nastech-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Nastech (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Nastech sees `auth: oauth`, calls `NastechTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Nastech process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Nastech needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Nastech does natively).
...
- **Don't claim Nastech doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Nastech. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Nastech' config.
