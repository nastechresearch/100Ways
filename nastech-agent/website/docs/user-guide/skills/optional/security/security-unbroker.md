...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/security/unbroker` |
...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$NASTECH_HOME/.env`** so keys
  Nastech already loads for its own tools are picked up without re-exporting - each one converts a
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `browser_*` tools. Nothing is stored. This requires Nastech to be pointed at the operator's own
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.nastech/chrome-debug"` (a dedicated
...
The engine stores data under `$PDD_DATA_DIR` (default `$NASTECH_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `NASTECH_HOME`). For at-rest encryption run
  ...
  a full-`NASTECH_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
