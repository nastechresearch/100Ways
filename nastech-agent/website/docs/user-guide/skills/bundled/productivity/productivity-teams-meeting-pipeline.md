...
Everything operator-facing is a `nastech teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
```bash
nastech teams-pipeline validate              # config snapshot — run first after any change
nastech teams-pipeline token-health          # Graph token status
nastech teams-pipeline token-health --force-refresh   # force a fresh token acquisition
nastech teams-pipeline list                  # recent meeting jobs
nastech teams-pipeline list --status failed  # only failed jobs
nastech teams-pipeline show <job-id>         # full detail of one job
nastech teams-pipeline subscriptions         # current Graph webhook subscriptions
```
...
```bash
nastech teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

nastech teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
nastech teams-pipeline delete-subscription <sub-id>
nastech teams-pipeline maintain-subscriptions            # renew near-expiry ones
nastech teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `nastech teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `nastech cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
