...
- A long-running process (nastech gateway, tui_gateway) misbehaves and you can't restart it
...
The nastech test runner and pytest both support this:
...
```bash
source <nastech-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<nastech-agent-repo>" }
  ]
}
```
...
### `tui_gateway` subprocess (spawned by `nastech --tui`)
...
debugpy.wait_for_client()
```
Start `nastech --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
