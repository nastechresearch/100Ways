---
title: "Nastech Agent Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Nastech Agent Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Nastech Agent Skill Authoring
...
| Path | `skills/software-development/nastech-agent-skill-authoring` |
| Version | `2.0.0` |
...
1. **User-local:** `~/.nastech/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the nastech-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with nastech-agent
...
- Don't use for: personal skills in `~/.nastech/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `nastech skills install official/<category>/<skill>`.
...
- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.nastech/skills/`.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.nastech.{tags, related_skills}` all present
