...
Nastech can pull API keys from external secret managers at process startup instead of storing them in `~/.nastech/.env`. The bootstrap token for the secret manager lives in `.env`; every other provider key (OpenAI, Anthropic, OpenRouter, etc.) can stay in the manager and rotate centrally.
...
Every credential injected by a source is labelled with its origin — setup flows and `nastech model` show `(from Bitwarden)` next to detected keys so you always know where a value came from.
...
- **Profile aliasing** (on by default, `secrets.profile_alias: false` to disable) — when Nastech runs under a named profile, a vault secret named `FOO_<PROFILE>` (credential-shaped suffixes only: `*_API_KEY`, `*_TOKEN`, `*_SECRET`, `*_KEY`, `*_PASSWORD`) also hydrates the canonical `FOO`. Store `TELEGRAM_BOT_TOKEN_MILLA` in the shared project and the `milla` profile's adapters — which read the fixed name `TELEGRAM_BOT_TOKEN` — get the right value automatically. A var the vault supplies directly under its canonical name always beats an alias.
...
The bundled set is deliberately closed (same policy as memory providers): Bitwarden and 1Password ship in-tree. Everything else — Infisical, Proton Pass, HashiCorp Vault, AWS Secrets Manager, OS keystores — belongs in plugin repos; share them in the Nastech Research Discord (`#plugins-skills-and-skins`).
