...
secrets:
  command:
    enabled: true
    command: "cat /run/user/1000/nastech-secrets.env"
    # or any vault CLI that dumps KEY=VALUE lines:
    # command: "pass show nastech/env"
    # command: "secret-tool lookup service nastech-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Nastech' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Nastech discards its stderr on purpose) |
