...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.nastech/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
1. Download and verify `bws v2.0.0` into `~/.nastech/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.nastech/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
nastech secrets bitwarden status
```

From now on, every `nastech` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.
...
- **Single-machine personal setups** where `~/.nastech/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
