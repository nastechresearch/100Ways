...
:::tip Fastest path: Nastech Portal
[Nastech Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `nastech setup --portal` to log in and set Nastech as your provider in one command. Inspect what's wired up with `nastech portal info`.
...
|---|---|
| **Title Gen** | When title latency or cost matters more than matching the main model. Pin a known-good flash model, or set `auxiliary.title_generation.prefer_fast_model: true` to let Nastech choose the provider's fast tier. |
...
| **Skills Hub** | `nastech skills search` uses this. Usually fine at `auto`. |
| **MCP** | MCP tool routing. Usually fine at `auto`. |
| **Triage Specifier** | Routes the Kanban triage specifier (`nastech kanban specify`) that expands a rough one-liner into a concrete spec. A cheap, capable model works well. |
...
| **Profile Describer** | Routes profile-description generation (`nastech profile describe --auto` / the dashboard auto-generate button). Short, cheap call. |
...
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`nastech gateway restart`) if you want to force all sessions to pick up the change.
...
Inside any `nastech chat` session:
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `nastech config set` only writes scalar values, but it can't carry a custom `base_url`:
...
Both paths feed the same loader (`nastech_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
Edit `~/.nastech/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__NASTECH_SESSION_TOKEN__`) if you're scripting against a running dashboard.
