...
Nastech Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.nastech/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Nastech stores session history so it can resume conversations, but it does not
...
Nastech explicitly injects for that turn.
...
For example, if a user sends an image and asks Nastech to make a meme from it,
Nastech may inspect that image once with vision and run an image-processing
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`nastech` or `nastech chat`) |
...
nastech -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
nastech -c "my project"   # → resumes "my project #3"
```
...
nastech --resume 20250305_091523_a1b2c3d4
nastech -r 20250305_091523_a1b2c3d4

# Resume by title
nastech --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
nastech --resume latest

# Or with the chat subcommand
nastech chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `nastech sessions list`.
...
nastech --resume latest --in ./my-project

# Works with the TUI too
nastech --tui --resume latest --in ./my-project
```
...
```bash
nastech --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`nastech sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
nastech sessions list --workspace my-project
nastech sessions list --workspace ~/code/nastech-agent
```

### Conversation Recap on Resume

When you resume a session, Nastech displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Nastech session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.nastech/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `nastech -r "<title>"` from the shell) and pick up where the platform left off.
...
Nastech automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `nastech sessions list` or `nastech sessions browse`.
...
```bash
nastech sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Nastech creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`nastech -c "my project"`), it automatically picks the most recent session in the lineage.
...
Nastech provides a full set of session management commands via `nastech sessions`:
...
# Filter by platform
nastech sessions list --source telegram

# Show more sessions
nastech sessions list --limit 50
```
...
`nastech sessions export` is one surface for every export format, selected with `--format`:
...
nastech sessions export backup.jsonl

# Export sessions from a specific platform
nastech sessions export telegram-history.jsonl --source telegram

# Export a single session
nastech sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
...
nastech sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
nastech sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
nastech sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
nastech sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `nastech-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
nastech sessions export --format trace

# One session to a local trace file
nastech sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
nastech sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.nastech/session-exports`).

```bash
# Export one session to Markdown
nastech sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
nastech sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
nastech sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
nastech sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
nastech sessions export --format md --model sonnet --min-messages 50 --redact
...
nastech sessions delete 20250305_091523_a1b2c3d4
...
nastech sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
nastech sessions prune

# Custom age threshold — bare numbers are days
nastech sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
nastech sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
nastech sessions prune --newer-than 5h

# Explicit window with absolute timestamps
nastech sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
nastech sessions prune --source telegram
nastech sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
nastech sessions prune --newer-than 5h --title "smoke test"   # title substring
nastech sessions prune --older-than 30 --max-messages 3        # tiny sessions
nastech sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
nastech sessions prune --model gpt-5 --older-than 1w           # by model (substring)
nastech sessions prune --provider openrouter --older-than 60   # by billing provider
nastech sessions prune --branch feature/old-experiment         # by git branch
nastech sessions prune --user 12345678 --chat-type group       # by messaging origin
nastech sessions prune --max-tokens 500 --older-than 7         # by token usage
nastech sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
nastech sessions prune --newer-than 5h --dry-run

# Skip confirmation
nastech sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `nastech sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `nastech sessions prune` keeps the 90-day cutoff. Every
...
`nastech sessions archive` takes the same filters as `prune` but soft-hides
...
nastech sessions archive --newer-than 5h

# Archive by title substring, preview first
nastech sessions archive --title "dry run" --dry-run
nastech sessions archive --title "dry run" --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`nastech insights`](/reference/cli-commands#nastech-insights).
...
`nastech sessions repair-routing` finds message-bearing session rows with no
...
nastech sessions repair-routing
...
nastech sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
nastech sessions repair-routing --max-gap-seconds 300
```
...
When Nastech cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Nastech uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Nastech in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.nastech/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.nastech/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.nastech/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.nastech/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.nastech/sessions/sessions.json` is a **legacy mirror** of it,
...
`nastech sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.nastech/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `nastech sessions list`, the cause is
`state.db` not receiving them — run `nastech sessions repair` and watch for a
...
`*.jsonl` files in `~/.nastech/sessions/`. They are no longer written or
read by Nastech. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.nastech/config.yaml`:
...
nastech sessions delete <session_id>
