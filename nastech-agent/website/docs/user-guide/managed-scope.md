...
`~/.nastech/config.yaml`, `~/.nastech/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/nastech`:

```text
/etc/nastech/
├── config.yaml     # managed config layer (wins over ~/.nastech/config.yaml)
└── .env            # managed env layer (wins over ~/.nastech/.env + shell)
```
...
export NASTECH_MANAGED_DIR=/opt/org/nastech-policy
```

:::warning
A user who can set `NASTECH_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `nastech doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/nastech/config.yaml` (managed) | `/etc/nastech/.env` (managed) |
| 2 | `~/.nastech/config.yaml` (user) | `~/.nastech/.env` (user) |
...
```bash
nastech config        # shows a header naming the managed source + the pinned keys
...
(/etc/nastech/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `nastech config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/nastech
...
sudo tee /etc/nastech/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/nastech
