...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`nastech setup`、`nastech gateway install`、编辑配置）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期有所不同：配置存放在 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，CLI 配置命令被屏蔽。管理 nastech 的方式与管理其他 NixOS 服务相同。
:::
...
nix run github:NastechResearch/nastech-agent -- chat
...
就这些。`nixos-rebuild switch` 会创建 `nastech` 用户、生成 `config.yaml`、连接密钥并启动 gateway——这是一个长期运行的服务，将 Agent 连接到消息平台（Telegram、Discord 等）并监听传入消息。
...
设置 `addToSystemPackages = true` 有两个作用：将 `nastech` CLI 添加到系统 PATH，**并**在系统范围内设置 `NASTECH_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `nastech` 会创建独立的 `~/.nastech/` 目录。
:::
...
`hostUsers` 中列出的用户会自动加入 `nastech` 组以获得文件权限访问。
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo nastech chat`。
:::
...
```yaml
# secrets/nastech.yaml（使用 sops 加密）
nastech-env: |
...
:::tip
`env` 值中的环境变量在运行时从 `$NASTECH_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
Token 存储在 `$NASTECH_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。
...
# 确保：chown nastech:nastech，chmod 0600
```
...
当 nastech 通过 NixOS 模块运行时，以下 CLI 命令会被**屏蔽**，并显示指向 `configuration.nix` 的描述性错误：

| 被屏蔽的命令 | 原因 |
|---|---|
| `nastech setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `nastech config edit` | 配置由 `settings` 生成 |
| `nastech config set <key> <value>` | 配置由 `settings` 生成 |
| `nastech gateway install` | systemd 服务由 NixOS 管理 |
| `nastech gateway uninstall` | systemd 服务由 NixOS 管理 |
...
启用容器模式后，nastech 在持久化 Ubuntu 容器内运行，Nix 构建的二进制文件以只读方式从主机绑定挂载：
...
Nix 构建的二进制文件能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载——它携带自己的解释器和所有依赖，不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/nastech gateway run --replace`。执行 `nixos-rebuild switch` 时，只更新符号链接——容器继续运行。
...
仅当容器的**身份哈希**发生变化时才会重建容器。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 和入口点脚本。环境变量、settings、文档或 nastech 包本身的变更**不会**触发重建。

:::warning 可写层丢失
当身份哈希发生变化（镜像升级、新卷、新容器选项）时，容器会被销毁并从 `container.image` 的全新拉取重建。可写层中通过 `apt install`、`pip install` 或 `npm install` 安装的包将丢失。`/data` 和 `/home/nastech` 中的状态会保留（这些是绑定挂载）。

如果 Agent 依赖特定包，考虑将其烘焙到自定义镜像中（`container.image = "my-registry/nastech-base:latest"`），或在 Agent 的 SOUL.md 中编写安装脚本。
:::

### GC Root 保护

`preStart` 脚本在 `${stateDir}/.gc-root` 创建一个指向当前 nastech 包的 GC root。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC root 损坏，重启服务会重新创建它。

---

## 插件

NixOS 模块支持声明式插件安装——无需命令式的 `nastech plugins install`。
...
该包的 `site-packages` 会添加到 nastech wrapper 的 PYTHONPATH 中。`importlib.metadata` 在会话启动时发现入口点。
...
:::note
构建时冲突检查可防止插件包覆盖核心 nastech 依赖。如果插件提供了封闭 venv 中已有的包，`nixos-rebuild` 会以明确的错误失败。
:::
...
nix build .#checks.x86_64-linux.managed-guard       # NASTECH_MANAGED 屏蔽变更操作
...
| `bundled-skills` | skills 目录存在，包含 SKILL.md 文件，wrapper 中设置了 `NASTECH_BUNDLED_SKILLS` |
...
| `stateDir` | `str` | `"/var/lib/nastech"` | 状态目录（`NASTECH_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `nastech` CLI 添加到系统 PATH 并在系统范围内设置 `NASTECH_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$NASTECH_HOME/.env` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `nastech gateway` 的额外参数 |
| `extraPackages` | `listOf package` | `[]` | Agent 可用的额外包。添加到 nastech 用户的每用户 profile，终端命令、skills 和 cron 任务均可见 |
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$NASTECH_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.nastech` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `nastech` 组 |
...
```
/var/lib/nastech/                     # stateDir（归 nastech:nastech 所有，权限 0750）
├── .nastech/                         # NASTECH_HOME
