...
Nastech Agent 常用于大型、长期维护的仓库。当你需要：
...
本页介绍如何将 worktrees 与 Nastech 结合使用，使每个会话拥有干净、隔离的工作目录。

## 为什么在 Nastech 中使用 Worktrees？

Nastech 将**当前工作目录**视为项目根目录：

- CLI：运行 `nastech` 或 `nastech chat` 时所在的目录
...
- 新目录：`../repo-feature`
- 新分支：`feature/nastech-experiment`，已在该目录中检出

现在可以 `cd` 进入新 worktree 并在其中运行 Nastech：

```bash
cd ../repo-feature

# 在 worktree 中启动 Nastech
nastech
```
...
- 在各自的分支上工作（`feature/nastech-a` 与 `feature/nastech-b`）。
...
- `~/.nastech/checkpoints/` 下的 Nastech checkpoint 数据在移除 worktree 时不会自动清理，但通常体积很小。

## 最佳实践

- **每个 Nastech 实验对应一个 worktree**
  - 为每项重要变更创建专用的分支/worktree。
  - 这样可保持 diff 聚焦，PR 小而易于审查。
- **以实验内容命名分支**
  - 例如：`feature/nastech-checkpoints-docs`、`feature/nastech-refactor-tests`。
- **频繁提交**
  - 使用 git commit 记录高层级里程碑。
  - 使用 [checkpoints 与 /rollback](./checkpoints-and-rollback.md) 作为工具驱动编辑之间的安全网。
- **使用 worktrees 时避免从裸仓库根目录运行 Nastech**
  - 优先使用 worktree 目录，使每个 agent 拥有明确的作用范围。

## 使用 `nastech -w`（自动 Worktree 模式）

Nastech 内置 `-w` 标志，可**自动创建一个一次性 git worktree** 及其独立分支。无需手动配置 worktree——只需 `cd` 进入仓库并运行：
...
- 检出一个隔离分支（例如 `nastech/nastech-<hash>`）。
...
如需并行运行多个 agent，在多个终端中分别运行 `nastech -w`——每次调用都会自动获得独立的 worktree 和分支。

## 综合运用

- 使用 **git worktrees** 为每个 Nastech 会话提供独立的干净检出。
- 使用**分支**记录实验的高层级历史。
