...
**Profile 分发**将一个完整的 Nastech agent——个性、技能、cron 任务、MCP 连接、配置——打包为一个 git 仓库。任何有权访问该仓库的人都可以用一条命令安装整个 agent，就地更新，并保持自己的记忆、会话和 API 密钥不受影响。

如果说 [profile](./profiles.md) 是本地 agent，那么分发就是让该 agent 可共享的形式。

## 这意味着什么

在分发功能出现之前，共享一个 Nastech agent 意味着要发送：
...
```bash
nastech profile install github.com/you/my-research-agent --alias
```

……他们就拥有了完整的 agent。填入自己的 API 密钥（`.env.EXAMPLE` → `.env`），即可运行 `my-research-agent chat`，或通过 Telegram / Discord / Slack / 任何 gateway 平台与其交互。当你推送新版本时，他们运行 `nastech profile update my-research-agent` 即可拉取你的更改——他们的记忆和会话保持不变。
...
权衡之处：接收方需要安装 git。在 2026 年运行 Nastech 的任何机器上，这已是既成事实。
...
- **你只想在自己的机器上备份一个 profile。** 使用 [`nastech profile export` / `import`](../reference/profile-commands.md#nastech-profile-export)——那正是这两个命令的用途。
...
```bash
nastech profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.nastech/profiles/research-bot/SOUL.md
...
创建 `~/.nastech/profiles/research-bot/distribution.yaml`：
...
```bash
cd ~/.nastech/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
运行 `nastech profile update research-bot` 的接收方将拉取最新版本。
...
```bash
nastech profile install github.com/you/research-bot --alias
```
...
4. 请求确认。传入 `-y` / `--yes` 可跳过。
5. 将分发所有的文件复制到 `~/.nastech/profiles/research-bot/`（或 manifest 中 `name` 解析到的位置）。
...
```bash
# GitHub shorthand
nastech profile install github.com/you/research-bot

# Full HTTPS
nastech profile install https://github.com/you/research-bot.git

# SSH
nastech profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
nastech profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
nastech profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
nastech profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
nastech profile install github.com/acme/support-bot --name support-us --alias
# Bob（同一分发，不同本地名称）
nastech profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.nastech/profiles/research-bot/.env.EXAMPLE ~/.nastech/profiles/research-bot/.env
...
```bash
nastech profile info research-bot
```
...
`nastech profile list` 还会显示 `Distribution` 列，让你一眼看出哪些 profile 来自仓库，哪些是手动构建的：
...
```bash
nastech profile update research-bot
```
...
```bash
nastech profile delete research-bot
```
...
Path:    ~/.nastech/profiles/research-bot
...
在笔记本上的任何迭代（`git commit && push`）都可以通过 `nastech profile update research-bot` 同步到工作站。记忆按机器独立保存——笔记本记住自己的对话，工作站记住自己的，互不干扰。
...
```bash
# 工程负责人
cd ~/.nastech/profiles/pr-reviewer
# ... build and tune ...
...
# 每位工程师
nastech profile install git@github.com:your-org/pr-reviewer.git --alias
# 填写 .env，使用自己的 API 密钥（费用由自己承担），.env.EXAMPLE 指明了所需内容
pr-reviewer chat
```

当负责人发布 v1.1（更好的 SOUL、新技能）时，工程师运行 `nastech profile update pr-reviewer`，所有人在几分钟内就能用上新版本。
...
```bash
# 你
cd ~/.nastech/profiles/polymarket-trader
# 在仓库根目录写一个完整的 README.md——GitHub 会在仓库页面展示它
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# 发布到公开 GitHub 仓库
git remote add origin https://github.com/you/nastech-polymarket-trader.git
git push -u origin main --tags

# 任何人
nastech profile install github.com/you/nastech-polymarket-trader --alias
```
...
你在 Nastech 之上构建了产品——也许是合规监控框架、客服技术栈、特定领域的研究平台。你想以产品形式分发它。
...
# 每位值班人员
nastech profile install git@github.com:your-org/incident-2026-q2.git --alias

# 事故解决——清理
nastech profile delete incident-2026-q2
```
...
:::note
Git ref 固定（`#v1.2.0`）已在规划中，但不在初始版本中——目前安装时跟踪默认分支。通过 `nastech profile info <name>` 查看已安装版本，在准备好之前暂缓更新。
:::
...
```bash
# 你已安装的版本
nastech profile info research-bot | grep Version
...
```yaml
# ~/.nastech/profiles/research-bot/local/my-overrides.yaml
...
```bash
# 彻底删除并重新安装（记忆/会话也会丢失）
nastech profile delete research-bot --yes
nastech profile install github.com/you/research-bot --alias

# 更新到当前 main，但将 config.yaml 重置为分发默认值
nastech profile update research-bot --force-config --yes
```
...
nastech profile install github.com/yourname/forked-research-bot --alias

# 在 ~/.nastech/profiles/forked-research-bot/ 中本地迭代
...
```bash
# 从本地目录安装（无需 git push）
nastech profile install ~/.nastech/profiles/research-bot --name research-bot-test --alias

# 调整、删除、重新安装，直到满意
nastech profile delete research-bot-test --yes
nastech profile install ~/.nastech/profiles/research-bot --name research-bot-test
```
...
来自分发的 cron 任务**不会自动调度**——安装器会打印 `nastech -p <name> cron list`，你需要显式启用它们。SOUL.md 和技能在你开始与 profile 对话后立即生效，因此如果你从不熟悉的来源安装，请在第一次运行前阅读它们。
...
- `install`、`update`、`info` 位于 `nastech profile` 下——不是独立的命令树。
...
- 保留的 profile 名称（`nastech`、`test`、`tmp`、`root`、`sudo`）在安装时会被拒绝，以避免与常见二进制文件冲突。

## 另请参阅

- [Profiles：运行多个 Agent](./profiles.md) — 基础概念
- [Profile 命令参考](../reference/profile-commands.md) — 每个标志、每个选项
- [`nastech profile export` / `import`](../reference/profile-commands.md#nastech-profile-export) — 本地备份 / 恢复（非分发）
- [在 Nastech 中使用 SOUL](../guides/use-soul-with-nastech.md) — 编写个性
