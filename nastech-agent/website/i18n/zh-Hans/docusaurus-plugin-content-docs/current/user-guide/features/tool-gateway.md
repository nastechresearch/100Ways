---
title: "Nastech Tool Gateway（工具网关）"
description: "通过 Nastech 订阅统一使用网页搜索、文生图、语音合成与浏览器自动化，无需单独申请 Firecrawl、FAL、OpenAI、Browser Use 等 API Key"
...
上述四类能力均计入 Nastech 订阅计费。你可以按需组合——例如网页与文生图走网关，TTS 仍使用自己的 ElevenLabs Key。
...
在输出中找到 **Nastech Tool Gateway** 小节：会标明哪些工具经订阅网关启用、哪些使用直连 Key、哪些尚未配置。

## 如何启用 Tool Gateway

### 在模型配置流程中

运行 `nastech model` 并选择 Nastech Portal 作为提供商时，Nastech 会主动询问是否启用 Tool Gateway：

```
Your Nastech subscription includes the Tool Gateway.

  The Tool Gateway gives you access to web search, image generation,
  text-to-speech, and browser automation through your Nastech subscription.
...
选择工具类别（Web、Browser、Image Generation、TTS），再将提供商选为 **Nastech Subscription**。这会在配置里把对应工具的 `use_gateway` 设为 `true`。

### 手动编辑配置

在 `~/.nastech/config.yaml` 中直接设置 `use_gateway`：
...
当某工具的 `use_gateway: true` 时，运行时会把 API 调用路由到 Nastech Tool Gateway，而不是使用直连 Key：
...
网关使用 Nastech Portal 凭据认证（在 `nastech model` 完成后写入 `~/.nastech/auth.json`）。
...
在 `nastech tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
标记为 “active via Nastech subscription” 的即经网关路由；带自有 Key 的会显示当前激活的提供商。
...
Modal（无服务器终端后端）可作为 Nastech 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `nastech setup terminal` 或在 `config.yaml` 中配置。
