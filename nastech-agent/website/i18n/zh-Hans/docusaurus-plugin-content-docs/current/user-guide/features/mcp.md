...
2. 在 `~/.nastech/config.yaml` 中添加一个 MCP 服务器：
...
对于知名 MCP 服务器，`nastech mcp add` 接受 `--preset` 标志，自动填写传输层细节，无需手动查找命令和参数。预设只提供默认值——你在同一命令行传入的其他内容（环境变量、头信息、过滤规则）仍然优先生效。
...
```bash
nastech mcp serve              # 普通模式
nastech mcp serve --verbose    # 在 stderr 输出调试日志
```
