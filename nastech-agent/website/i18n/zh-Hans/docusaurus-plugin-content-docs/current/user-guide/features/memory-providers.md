...
```bash
nastech memory setup      # 交互式选择器 + 配置
nastech memory status     # 查看当前激活状态
nastech memory off        # 禁用外部提供者
```

也可以通过 `nastech plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。

或在 `~/.nastech/config.yaml` 中手动设置：
...
**安装向导：**
```bash
nastech memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```
...
:::tip 从 `nastech honcho` 迁移
如果你之前使用过 `nastech honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::
...
`--clone` 在 `honcho.json` 中创建一个 `nastech.coder` host 块，包含 `aiPeer: "coder"`、共享的 `workspace`、继承的 `peerName`、`recallMode`、`writeFrequency`、`observation` 等。AI peer 会在 Honcho 中提前创建，确保在第一条消息之前就已存在。
...
**安装：**
```bash
nastech memory setup    # 选择 "mem0"
# 或手动配置：
nastech config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.nastech/.env
```

**配置：** `$NASTECH_HOME/mem0.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `user_id` | `nastech-user` | 用户标识符 |
| `agent_id` | `nastech` | Agent 标识符 |
...
**安装：**
```bash
nastech memory setup    # 选择 "hindsight"
# 或手动配置：
...
**本地模式 UI：** `hindsight-embed -p nastech ui start`

**配置：** `$NASTECH_HOME/hindsight/config.json`
...
完整配置参考参见[插件 README](https://github.com/NastechResearch/nastech-agent/blob/main/plugins/memory/hindsight/README.md)。
...
**安装：**
```bash
nastech memory setup    # 选择 "holographic"
# 或手动配置：
nastech config set memory.provider holographic
```

**配置：** `plugins.nastech-memory-store` 下的 `config.yaml`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$NASTECH_HOME/memory_store.db` | SQLite 数据库路径 |
...
**安装：**
```bash
nastech memory setup    # 选择 "retaindb"
# 或手动配置：
...
- 知识树存储于 `$NASTECH_HOME/byterover/`（profile 范围隔离）
...
**安装：**
```bash
nastech memory setup    # 选择 "supermemory"
# 或手动配置：
...
- **本地存储提供者**（Holographic、ByteRover）使用 `$NASTECH_HOME/` 路径，各 profile 路径不同
- **配置文件提供者**（Honcho、Mem0、Hindsight、Supermemory）将配置存储在 `$NASTECH_HOME/` 中，每个 profile 拥有独立凭证
