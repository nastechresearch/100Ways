...
Nastech 有四套 hook 系统，可在关键生命周期节点运行自定义代码：

| 系统 | 注册方式 | 运行环境 | 使用场景 |
|------|---------|---------|---------|
| **[Gateway hooks](#gateway-event-hooks)** | `~/.nastech/hooks/` 下的 `HOOK.yaml` + `handler.py` | 仅 Gateway | 日志、告警、webhook |
| **[Plugin hooks](#plugin-hooks)** | [插件](/user-guide/features/plugins)中的 `ctx.register_hook()` | CLI + Gateway | 工具拦截、指标采集、护栏 |
| **[Shell hooks](#shell-hooks)** | `~/.nastech/config.yaml` 中 `hooks:` 块指向的 shell 脚本 | CLI + Gateway | 用于阻断、自动格式化、上下文注入的即插即用脚本 |
| **[Outbound webhooks](#outbound-webhooks)** | `~/.nastech/config.yaml` 中的 `hooks.outbound:` 列表 | CLI + Gateway | 将签名后的生命周期事件推送到外部 HTTP endpoint |
...
每个 hook 是 `~/.nastech/hooks/` 下的一个目录，包含两个文件：
...
LOG_FILE = Path.home() / ".nastech" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.nastech/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.nastech/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.nastech/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.nastech/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".nastech" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.nastech/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.nastech/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/nastech-events"
...
这是社区中流行的一种模式：在 `~/.nastech/BOOT.md` 放置一个 Markdown 检查清单，让 agent 在每次 gateway 启动时执行一次。适用于"每次启动时检查隔夜 cron 失败情况，若有失败则在 Discord 上通知我"，或"汇总过去 24 小时的 deploy.log 并发布到 Slack #ops"等场景。

本教程展示如何以用户自定义 hook 的方式自行构建。Nastech 不内置 BOOT.md hook——你可以精确配置自己想要的行为。

#### 我们要构建什么

1. 在 `~/.nastech/BOOT.md` 放置一个包含自然语言启动指令的文件。
...
创建 `~/.nastech/BOOT.md`。像给人类助手下达指令一样编写：

```markdown
# Startup Checklist

1. Run `nastech cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.nastech/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.nastech/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.nastech/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.nastech/hooks/boot-md/handler.py`**

```python
"""Run ~/.nastech/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".nastech" / "BOOT.md"
...
Nastech 早期版本将此作为内置 hook 发布，每次 gateway 启动时都会静默生成一个使用裸默认值的 agent。这让使用自定义端点的用户感到意外，也让不知道它在运行的用户无从察觉。将其作为文档化模式保留——由你在 hooks 目录中构建——意味着你能清楚地看到它的行为，并通过编写文件来选择启用。

### 工作原理

1. Gateway 启动时，`HookRegistry.discover_and_load()` 扫描 `~/.nastech/hooks/`
...
- 运行时事件名以 `nastech_cli.plugins.VALID_HOOKS` 为准。`nastech hooks list` 只列出已配置的 shell/outbound hook，并非可用事件目录；只有 `nastech hooks test <event>` 收到无效事件时才会打印有效集合。

### 已发布的 plugin-hook 目录

下表列出每个 call site 实际传入的事件专属字段。为保持向后兼容，`PluginManager` 还会向每个 plugin-hook 回调加入 `telemetry_schema_version="nastech.observer.v1"`。这个旧版 envelope 标记并不表示所有 hook payload 共用同一套语义 schema；新的版本化 contract 应归属于具体事件或 capability family。
...
**上下文注入位置：** 始终注入到**用户消息**，而非系统 prompt。这保留了 prompt 缓存——系统 prompt 在各轮次间保持不变，已缓存的 token 得以复用。系统 prompt 是 Nastech 的领域（模型指导、工具执行、个性、技能）。插件在用户输入旁边贡献上下文。

干净的用户消息 `content` 保持不变。为保证 replay 和 prompt cache 稳定，Nastech 可能把实际发送给 API 的消息（包括插件注入上下文）持久化到该行的 `api_content` sidecar。
...
在 `cli-config.yaml` 中声明 shell 脚本 hook，Nastech 会在对应的插件 hook 事件触发时将其作为子进程运行——在 CLI 和 gateway 会话中均适用。无需编写 Python 插件。
...
Shell hooks 通过在 CLI 启动（`nastech_cli/main.py`）和 gateway 启动（`gateway/run.py`）时调用 `agent.shell_hooks.register_from_config(cfg)` 来注册。它们与 Python 插件 hook 自然组合——两者都流经同一个分发器。

### 对比一览

| 维度 | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|------|-------------|-------------------------------|---------------------------------------|
| 声明位置 | `~/.nastech/config.yaml` 中的 `hooks:` 块 | 插件 `plugin.yaml` 中的 `register()` | `HOOK.yaml` + `handler.py` 目录 |
| 存放位置 | `~/.nastech/agent-hooks/`（约定） | `~/.nastech/plugins/<name>/` | `~/.nastech/hooks/<name>/` |
...
每次事件触发时，Nastech 为每个匹配的 hook（在 matcher 允许的情况下）生成一个子进程，将 JSON 载荷通过 **stdin** 传入，并从 **stdout** 读取 JSON 响应。
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Nastech-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.nastech/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.nastech/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.nastech/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.nastech/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.nastech/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.nastech/agent-hooks/inject-cwd-context.sh
...
Claude Code 的 `UserPromptSubmit` 事件在 Nastech 中没有对应的独立事件——`pre_llm_call` 在相同位置触发，且已支持上下文注入。在此使用即可。
...
hooks:
  subagent_stop:
    - command: "~/.nastech/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.nastech/agent-hooks/log-orchestration.sh
log=~/.nastech/logs/orchestration.log
...
每个唯一的 `(event, command)` 对在 Nastech 首次遇到时会提示用户审批，然后将决定持久化到 `~/.nastech/shell-hooks-allowlist.json`。后续运行（CLI 或 gateway）跳过提示。

三种方式可绕过交互式提示——满足其一即可：

1. CLI 上的 `--accept-hooks` 标志（如 `nastech --accept-hooks chat`）
2. `NASTECH_ACCEPT_HOOKS=1` 环境变量
3. `cli-config.yaml` 中的 `hooks_auto_accept: true`

非 TTY 运行（gateway、cron、CI）需要这三种方式之一——否则任何新添加的 hook 会静默保持未注册状态并记录警告。

**脚本编辑被静默信任。** 允许列表以精确的命令字符串为键，而非脚本的哈希值，因此编辑磁盘上的脚本不会使授权失效。`nastech hooks doctor` 会标记 mtime 漂移，以便你发现编辑并决定是否重新审批。

### `nastech hooks` CLI

| 命令 | 功能 |
|------|------|
| `nastech hooks list` | 列出已配置的 hook，包含 matcher、超时和授权状态 |
| `nastech hooks test <event> [--for-tool X] [--payload-file F]` | 对合成载荷触发所有匹配的 hook 并打印解析后的响应 |
| `nastech hooks revoke <command>` | 删除所有匹配 `<command>` 的允许列表条目（下次重启后生效） |
| `nastech hooks doctor` | 对每个已配置的 hook 检查：执行位、允许列表状态、mtime 漂移、JSON 输出有效性和大致执行时间 |
...
- 只引用你自己编写或完整审查过的脚本。
- 将脚本保存在 `~/.nastech/agent-hooks/` 内，便于审计路径。
- 拉取共享配置后重新运行 `nastech hooks doctor`，在新添加的 hook 注册前发现它们。
...
hooks:
  outbound:
    - url: "https://example.com/nastech-events"
      events: [post_tool_call, on_session_end]
      secret_env: NASTECH_WEBHOOK_SECRET
```

配置中的事件名同样以 `nastech_cli.plugins.VALID_HOOKS` 为准。`nastech hooks list` 会列出这些已配置 target，但不会输出完整的可用事件目录。