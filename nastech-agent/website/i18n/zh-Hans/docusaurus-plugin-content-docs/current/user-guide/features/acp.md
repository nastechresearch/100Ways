...
sidebar_position: 11
title: "ACP 宿主集成"
description: "在兼容 ACP 的编辑器和协作平台中使用 Nastech Agent"
---

# ACP 宿主集成

Nastech Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Nastech 通信并渲染：
...
当你希望 Nastech 表现得像编辑器原生的编码 agent，而非独立 CLI 或消息机器人时，ACP 是合适的选择。

## Nastech 在 ACP 模式下暴露的内容

Nastech 使用专为编辑器工作流设计的精选 `nastech-acp` 工具集运行，包括：
...
正常安装 Nastech 后，从安装检出目录添加 ACP 扩展：
...
Nastech 将日志输出到 stderr，以保留 stdout 用于 ACP JSON-RPC 流量。
...
```bash
nastech acp --setup-browser           # 交互式（下载约 400 MB 前会提示确认）
nastech acp --setup-browser --yes     # 非交互式接受下载
```

这是独立命令。终端认证流程（`nastech acp --setup`）在模型选择后也会将浏览器引导作为后续问题提供，因此大多数用户无需直接运行 `--setup-browser`。

具体操作：

- 若缺少 Node.js 22 LTS，将其安装到 `~/.nastech/node/`
- 将 `npm install -g agent-browser @askjo/camofox-browser` 安装到该前缀（无需 sudo — `npm` 的 `--prefix` 指向用户可写的 Nastech 管理 Node）
...
这是一种传输层集成，不是第二个 Nastech 安装。由 `buzz-acp` 启动的子进程使用该主机上
与 `nastech` 相同的配置、凭据、记忆、技能和状态。

（这与 [Buzz Desktop 的托管运行时](#buzz-desktop)不同——后者在本地将 Nastech 作为
预设 harness 启动。中继桥接用于以 agent 身份加入 Buzz *频道*，通常部署在服务器上。）

前置条件：

- 完成上文的 ACP 安装并通过 `nastech acp --check`。
- 从 [Buzz 仓库](https://github.com/block/buzz)构建 `buzz-acp` 和 `buzz` CLI
  （`cargo build --release -p buzz-acp`）。
- 为 Nastech 铸造专用的 Nostr 密钥对（`buzz-admin generate-key`）并将其注册为
...
若要持久化部署到服务器，请以拥有目标 Nastech home 的同一操作系统用户身份，
在服务管理器下运行 `buzz-acp`。安装、密钥生成、频道发现和各项 agent 选项见
[buzz-acp README](https://github.com/block/buzz/tree/main/crates/buzz-acp)。

桥接会发现 Nastech 身份所属的每个 Buzz 频道，并在其被加入新频道时自动订阅。
因此 Buzz 频道成员资格就是访问边界；Nastech 自身配置中无需单独的频道列表。

若要在所有者的 Buzz Desktop 中展示 Nastech 的 ACP 活动，添加：
...
2. 从内置 agent 列表中选择 **Nastech Agent**。
3. 连接并开始聊天。

如需手动定义 Nastech，通过 VS Code 设置在 `acp.agents` 下添加：
...
在 Zed 设置中将 Nastech 配置为自定义 agent 服务器：
...
3. 启动新的 Nastech 外部 agent 线程。

前提条件：

- 先通过 `nastech model` 配置 Nastech provider 凭据，或在 `~/.nastech/.env` / `~/.nastech/config.yaml` 中设置。

### JetBrains

使用兼容 ACP 的插件并将其指向 `nastech acp` 或 `nastech-acp`。

### Buzz Desktop

[Buzz](https://github.com/block/buzz) 将 Nastech Agent 作为预设运行时提供。
按常规方式安装 Nastech 后，Buzz 会自动发现它 —— 打开 **Settings → Runtimes**，
Nastech 就会出现在你的运行时列表中。
...
较新的安装会将 `nastech` 和 `nastech-acp` 两个启动器写入 `~/.local/bin`；
运行 `nastech update` 会为较旧的安装补上 `nastech-acp` 启动器。作为手动兜底方案，
可以将 Buzz 的 agent 命令配置为 `nastech`，参数为 `["acp"]`。

#### 将 Buzz agent 保持为 owner-only

Buzz 创建的每个 agent 默认都将 **Who can talk to this agent** 设为 `Owner only`。
当运行时为 Nastech 时，请保持该设置。
...
- `approvals.mode: manual` 确实会让 Nastech 发出权限请求，但 Buzz 仍会自动批准，
  命令照样执行。
...
Provider 解析使用 Nastech 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。Nastech 还为首次运行的 ACP 客户端提供终端认证方法（`--setup`）；这将打开 Nastech 的交互式模型/provider 设置。
...
底层 `AIAgent` 仍使用 Nastech 的正常持久化/日志路径，但 ACP 的 `list/load/resume/fork` 仅限于当前运行的 ACP 服务器进程。

## 工作目录行为

ACP 会话将编辑器的 cwd 绑定到 Nastech 任务 ID，使文件和终端工具相对于编辑器工作区运行，而非服务器进程的 cwd。
...
| `allow_always` | 始终允许 | 所有未来会话 | 是（写入 Nastech 永久允许列表） |
| `deny` | 拒绝 | 本次工具调用 | 否 |
...
ACP 桥接将这些选项映射到 Nastech 的内部审批语义——`allow_always` 与 CLI 相同地写入永久允许列表条目，而 `allow_session` 仅影响当前 ACP 会话的进程内审批缓存。
...
- 对于手动/本地开发，验证自定义 `agent_servers` 命令是否指向 `nastech acp`。
- Nastech 已安装且在 PATH 中。
- ACP 扩展已安装（`cd ~/.nastech/nastech-agent && uv pip install -e '.[acp]'`）。
...
ACP 模式使用 Nastech 现有的 provider 设置。通过以下方式配置凭据：

```bash
nastech model
```

或编辑 `~/.nastech/.env`。终端认证流程（`nastech acp --setup`）也可以触发交互式 provider/模型设置。
