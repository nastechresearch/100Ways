...
```bash
nastech chat --toolsets skills -q "What skills do you have?"
nastech chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.nastech/cache/chart-q4-2025.png
...
设置存储在 config.yaml 的 `skills.config` 下。`nastech config migrate` 会提示配置未设置的项，`nastech config show` 会显示它们。当 skill 加载时，其解析后的配置值会注入到上下文中，agent 会自动知晓已配置的值。
...
```text
~/.nastech/skills/                  # Single source of truth
...
```text
~/.nastech/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
捆绑包存放在 **`~/.nastech/skill-bundles/<slug>.yaml`** 中，格式如下：
...
- `description`（可选）——在 `/bundles` 和 `nastech bundles list` 中显示的简短文本。
...
nastech bundles create research

# 覆盖现有捆绑包
nastech bundles create backend-dev --skill ... --force

# 删除捆绑包
nastech bundles delete backend-dev

# 重新扫描 ~/.nastech/skill-bundles/ 并报告变更
nastech bundles reload
```
...
- 你想通过将捆绑包 YAML 提交到共享 dotfiles 仓库并符号链接到 `~/.nastech/skill-bundles/` 来发布团队范围的"任务配置文件"。

捆绑包只是一个 YAML 别名——它不会为你安装 skills。Skills 本身必须已经存在（在 `~/.nastech/skills/` 或外部 skill 目录中）。否则捆绑包调用只会跳过缺失的 skills。
...
```bash
nastech skills browse                              # Browse all hub skills (official first)
nastech skills browse --source official            # Browse only official optional skills
nastech skills search kubernetes                   # Search all sources
nastech skills search react --source skills-sh     # Search the skills.sh directory
nastech skills search https://mintlify.com/docs --source well-known
nastech skills inspect openai/skills/k8s           # Preview before installing
nastech skills install openai/skills/k8s           # Install with security scan
...
nastech skills update                              # Reinstall hub skills with upstream changes when needed
nastech skills audit                               # Re-scan all hub skills for security
nastech skills uninstall k8s                       # Remove a hub skill
nastech skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
nastech skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
nastech skills publish skills/my-skill --to github --repo owner/repo
nastech skills snapshot export setup.json          # Export skill config
nastech skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
```bash
nastech skills search https://mintlify.com/docs --source well-known
nastech skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
nastech skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```
...
```bash
nastech skills install openai/skills/k8s
...
```bash
nastech skills search airbnb --source browse-sh
nastech skills inspect browse-sh/airbnb.com/search-listings-ddgioa
nastech skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
nastech skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
nastech skills install https://example.com/SKILL.md --name sharethis-chat

# 或在聊天会话中：
/skills install https://example.com/SKILL.md --name sharethis-chat
```

信任级别始终为 `community`——与所有其他来源一样运行相同的安全扫描。URL 作为安装标识符存储，因此当你想刷新时，`nastech skills update` 会自动从同一 URL 重新获取。

### 安全扫描与 `--force`

所有通过 hub 安装的 skills 都经过**安全扫描器**检查，检测数据泄露、prompt 注入、破坏性命令、供应链信号及其他威胁。

`nastech skills inspect ...` 现在还会在可用时显示上游元数据：
- 仓库 URL
- skills.sh 详情页 URL
- 安装命令
- 每周安装量
...
```bash
nastech skills check          # Report which installed hub skills changed upstream
nastech skills update         # Reinstall only the skills with updates available
nastech skills update react   # Update one specific installed hub skill
```
...
- 每个 skill 目录必须包含一个带有标准 [SKILL.md frontmatter](#skillmd-format) 的 `SKILL.md`（`name`、`description`，以及可选的 `metadata.nastech.tags`、`version`、`author`、`platforms`、`metadata.nastech.config`）。
...
nastech skills search deploy
nastech skills install my-org/nastech-skills/deploy-runbook
```

#### 非默认路径

如果你的 skills 不在 `skills/` 下（当你向现有项目添加 `skills/` 子树时很常见），请编辑 `~/.nastech/.hub/taps.json` 中的 tap 条目：
...
`nastech skills tap add` CLI 默认将新 tap 的 `path` 设为 `"skills/"`；如果需要不同路径，请直接编辑该文件。`nastech skills tap list` 显示每个 tap 的有效路径。
...
```bash
nastech skills install owner/repo/skills/my-workflow
```
...
```bash
nastech skills tap list                                # show all configured taps
nastech skills tap add myorg/skills-repo               # add (default path: skills/)
nastech skills tap remove myorg/skills-repo            # remove
```
...
Tap 存储在 `~/.nastech/.hub/taps.json` 中（按需创建）。
...
nastech skills reset google-workspace --restore
...
:::note Profiles
每个 profile 在其自己的 `NASTECH_HOME` 下有自己的 `.bundled_manifest`，因此 `nastech -p coder skills reset <name>` 只影响该 profile。
:::
