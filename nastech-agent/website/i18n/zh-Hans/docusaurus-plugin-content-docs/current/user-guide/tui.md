...
| `/reload` | 将 `~/.nastech/.env` 重新读入运行中的 TUI 进程，使新添加的 API 密钥无需重启即可生效 |
...
1. `NASTECH_TUI_THEME` 环境变量——最高优先级。可选值：`light`、`dark`，或原始 6 位背景十六进制色值（如 `ffffff`、`1a1a2e`）。
...
默认情况下，`nastech --tui` 每次启动都会开启新会话。若要自动重新连接到最近的 TUI 会话（在终端或 SSH 连接意外断开时很有用），可选择启用：

```bash
export NASTECH_TUI_RESUME=1          # 最近的 TUI 会话
# 或：
export NASTECH_TUI_RESUME=<session-id>   # 指定会话
```
...
- **`⚠ YOLO`** — 每当 YOLO 模式开启时（`nastech --yolo`、`/yolo` 或 `NASTECH_YOLO_MODE=1`）显示的可见警告。同一徽章也出现在启动 banner 中，确保你不会在未注意到的情况下启动自动审批会话。
...
会话在 TUI 和 classic CLI 之间共享——两者均写入同一个 `~/.nastech/state.db`。可以在一个界面开始会话，在另一个界面恢复。会话选择器显示来自两个来源的会话，并带有来源标签。
...
你可能会在代码或日志中看到 `NASTECH_TUI_GATEWAY_URL` 环境变量。它是 **Web 仪表板的内部接线细节**，并非面向用户的远程连接开关。当你打开仪表板的 "Chat" 标签页（`nastech dashboard` → `/chat`）时，仪表板的 Web 服务器会派生一个内嵌的 TUI 子进程，并注入 `NASTECH_TUI_GATEWAY_URL`，让该子进程通过本地回环 WebSocket（`/api/ws`）连接到仪表板自己的进程内 `tui_gateway`。`/api/ws` 端点仅存在于仪表板服务器内部（`nastech_cli/web_server.py`），并绑定到该进程的生命周期和认证。

不存在通用的"将任意 TUI 指向任意独立 gateway 端口"的模式。特别是，OpenAI 兼容 API 服务器（`nastech gateway` / `api_server` 平台）**不**提供 `/api/ws`——它是模型后端接口（`/v1/chat/completions`、`/v1/models` 等），并刻意不暴露 TUI 的 JSON-RPC 控制通道。将 `NASTECH_TUI_GATEWAY_URL` 设置为该端口将返回 404。

如果你希望多个界面共享同一组会话，请使用共享的 `~/.nastech/state.db`（参见[会话](sessions.md)）或 Web 仪表板的内嵌聊天（参见 [Web Dashboard](features/web-dashboard.md#chat)）——而不是手动设置 gateway URL。
