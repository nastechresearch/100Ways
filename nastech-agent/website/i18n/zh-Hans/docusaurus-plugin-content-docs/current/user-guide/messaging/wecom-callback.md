...
通过回调/webhook 模式，将 Nastech 作为企业自建应用接入企业微信（WeCom）。

:::info WeCom Bot 与 WeCom 回调
Nastech 支持两种企业微信集成模式：
...
3. Nastech 解密消息，将其加入 agent 处理队列
...
（仅在通过 `nastech gateway install` 注册 systemd/launchd 服务后，才使用 `nastech gateway start`。）
