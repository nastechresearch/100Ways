...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Nastech Agent — Webhooks Tutorial"
...
1. 通过 `nastech gateway setup` 或环境变量启用
2. 在 `config.yaml` 中定义路由，**或**使用 `nastech webhook subscribe` 动态创建
...
| `script` | 否 | 位于 `~/.nastech/scripts/` 下的过滤/转换脚本。webhook payload 以 JSON 形式通过 stdin 传入。stdout 为 JSON 对象时会在模板渲染前替换 payload；文本 stdout 以 `script_output` 形式暴露；空 stdout、`[SILENT]` 或非零退出码会忽略该 webhook。 |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.nastech/data/todoist/watchlist.json"
...
当声明式过滤器不够用时，使用 `script`。脚本必须位于当前 profile 的 `~/.nastech/scripts/` 目录下；相对路径在该目录内解析，且禁止路径穿越到目录之外。`.sh` 和 `.bash` 脚本用 bash 运行，其他扩展名用当前 Python 解释器运行。

路由 payload 以 JSON 形式发送到 stdin：

```python
# ~/.nastech/scripts/todoist-nastech-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "nastech" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
- 空 stdout、精确的 `[SILENT]`、`{"__nastech_ignore__": true}`、超时、脚本缺失或非零退出码，均返回 HTTP 200 及 `{"status":"ignored","reason":"script"}`。
...
按照上方示例，将 `github-pr` 路由添加到 `~/.nastech/config.yaml`。
...
在仓库中打开一个 pull request。webhook 触发后，Nastech 处理事件并在 PR 上发布审查评论。
...
除了 `config.yaml` 中的静态路由，还可以使用 `nastech webhook` CLI 命令动态创建 webhook 订阅。当 agent 本身需要设置事件驱动触发器时，这尤为有用。
...
```bash
nastech webhook remove github-issues
```

### 测试订阅

```bash
nastech webhook test github-issues
nastech webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### 动态订阅的工作原理

- 订阅存储在 `~/.nastech/webhook_subscriptions.json`
...
agent 可通过 terminal 工具在 `webhook-subscriptions` skill 的引导下创建订阅。向 agent 请求"为 GitHub issues 设置 webhook"，它将运行相应的 `nastech webhook subscribe` 命令。
...
- 在前台运行 gateway 以查看日志：`nastech gateway run`
- 检查 prompt 模板是否正确渲染
- 验证投递目标已配置并连接
