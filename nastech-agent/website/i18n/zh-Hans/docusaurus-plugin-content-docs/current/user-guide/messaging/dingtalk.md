...
- **二维码设备流（推荐）。** 用钉钉手机 App 扫描终端中打印的二维码——Client ID 和 Client Secret 将自动返回并写入 `~/.nastech/.env`，无需前往开发者控制台。
...
:::tip
你可以在后台运行 `nastech gateway`，或将其配置为 systemd 服务以持续运行。详见部署文档。
:::
...
**解决方法**：确认 `DINGTALK_CLIENT_ID` 和 `DINGTALK_CLIENT_SECRET` 已在 `~/.nastech/.env` 中正确设置。Client ID 是你的 AppKey，Client Secret 是钉钉开发者控制台中的 AppSecret。
