...
内置 Web dashboard 在同一容器内作为受 s6-rc 监管的服务与 gateway 并行运行。设置 `NASTECH_DASHBOARD=1` 即可拉起它：
...
| 环境变量 | 描述 | 默认值 |
|---------------------|-------------|---------|
| `NASTECH_DASHBOARD` | 设为 `1`（或 `true` / `yes`）以启用受监管的 dashboard 服务 | *（未设置——服务已注册但保持关闭）* |
| `NASTECH_DASHBOARD_HOST` | dashboard HTTP 服务器的绑定地址 | `0.0.0.0` |
| `NASTECH_DASHBOARD_PORT` | dashboard HTTP 服务器的端口 | `9119` |
| `NASTECH_DASHBOARD_INSECURE` | **已弃用 / 空操作。** 以前用于绕过鉴权门控；自 2026 年 6 月的安全加固起，它不再禁用鉴权。任何非回环绑定都必须配置鉴权提供方 | *（被忽略——请改为配置提供方）* |

容器内的 dashboard 默认绑定 `0.0.0.0`，否则发布的 `-p 9119:9119` 端口将无法从宿主机访问。若你要把它限制在容器回环地址（例如 sidecar / 反向代理拓扑），请显式设置 `NASTECH_DASHBOARD_HOST=127.0.0.1`。
...
- **OAuth（Nastech Portal）** —— 适合托管/公网部署：设置 `NASTECH_DASHBOARD_OAUTH_CLIENT_ID` 后，`dashboard_auth/nastech` 提供者会自动激活。
...
`NASTECH_DASHBOARD_INSECURE=1` 现在是一个已弃用的空操作（它会
打印告警并被忽略）。请改为配置鉴权提供方，或设置
`NASTECH_DASHBOARD_HOST=127.0.0.1` 并通过 SSH 隧道 / Tailscale 访问。

:::warning 为什么移除了 `--insecure`
无鉴权的公网 dashboard 是 2026 年 6 月 MCP 配置持久化攻击活动的入口：互联网扫描器访问到暴露的 dashboard（以及 OpenAI API 服务器），诱导 agent 植入 SSH 密钥后门。现在每个非回环绑定都强制启用鉴权门控。对于可信局域网 / homelab 主机，内置的用户名/密码提供方（`NASTECH_DASHBOARD_BASIC_AUTH_USERNAME` + `_PASSWORD`）是满足该要求的零基础设施方式。
:::
...
- 每个 profile 独立的轮转日志：`${NASTECH_HOME}/logs/gateways/<name>/current`。
...
services:
  nastech:
    ...
    ports:
      ...
      - "9119:9119"   # dashboard（仅在 NASTECH_DASHBOARD=1 时生效）
    volumes:
      - ~/.nastech:/opt/data
    environment:
      - NASTECH_DASHBOARD=1
...
1. 以 root 身份运行 `/etc/cont-init.d/01-nastech-setup`（即 `docker/stage2-hook.sh`）：可选的 UID/GID 重映射、修复卷所有权、首次启动时初始化 `.env` / `config.yaml` / `SOUL.md`、同步内置技能。
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `nastech_cli.container_boot`）：遍历 `$NASTECH_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
3. 启动静态的 `main-nastech` 和 `dashboard` s6-rc 服务。
4. 将容器的 CMD 作为主程序 exec（`/opt/nastech/docker/main-wrapper.sh`），根据用户传给 `docker run` 的参数进行路由：
   - 无参数 → `nastech`（默认）
   - 第一个参数是 PATH 上的可执行文件（如 `sleep`、`bash`）→ 直接 exec
   - 其他情况 → `nastech <args>`（子命令透传）
   主程序退出时容器退出，并使用其退出码。
...
:::warning 权限模型
除非你在命令链中保留 `/init`（或等效的旧版 `docker/entrypoint.sh` shim，它会转发到 stage2 hook），否则不要覆盖镜像入口点。s6-overlay 的 `/init` 以 root 运行，以便在首次启动时对卷执行 chown，然后通过 `s6-setuidgid` 为每个受监管的服务**以及**主程序降权至 `nastech` 用户。在官方镜像内以 root 启动 `nastech gateway run` 默认会被拒绝，因为这可能在 `/opt/data` 中留下 root 所有的文件，导致后续 dashboard 或 gateway 启动失败。仅在你有意接受该风险时才设置 `NASTECH_ALLOW_ROOT_GATEWAY=1`。
:::

### Per-profile gateway 监管

在容器内，每个通过 `nastech profile create <name>` 创建的 profile 都会自动在 `/run/service/gateway-<name>/` 注册一个受 s6 监管的 gateway 服务。你在宿主机上运行的生命周期命令在此同样适用：

```sh
nastech profile create coder            # 注册 gateway-coder s6 槽
nastech -p coder gateway start          # s6-svc -u  → 受监管的 gateway
nastech -p coder gateway stop           # s6-svc -d  → 服务停止
nastech -p coder gateway restart        # s6-svc -t  → 向 supervisor 发送 SIGTERM
nastech profile delete coder            # 拆除 s6 槽
```

**相比 pre-s6 镜像的监管优势：**

- Gateway 崩溃后由 `s6-supervise` 在约 1 秒退避后自动重启。
- Dashboard 崩溃后自动重启（设置 `NASTECH_DASHBOARD=1` 以启动）。
- `docker restart` 保留运行中的 gateway：cont-init 协调器读取 `$NASTECH_HOME/profiles/<name>/gateway_state.json`，若上次记录状态为 `running` 则恢复该槽。已停止的 gateway 保持停止状态。
- 各 profile 的 gateway 日志持久化于 `$NASTECH_HOME/logs/gateways/<profile>/current`（由 `s6-log` 轮转），协调器的操作记录在每次启动时追加到 `$NASTECH_HOME/logs/container-boot.log`。

在容器内执行 `nastech status` 会显示 `Manager: s6 (container supervisor)`。使用 `/command/s6-svstat /run/service/gateway-<name>` 查看原始 supervisor 状态（注意 `/command/` 仅在监管树进程的 PATH 中；从 `docker exec` 调用时请传入绝对路径）。
...
当工具必须在每次容器启动时立即可用且无需重新安装延迟时，构建一个继承自 `nastechresearch/nastech-agent` 并在层中安装该工具的新镜像：
...
入口点脚本和 `/opt/data` 语义原样继承，本页其余内容仍然适用。拉取更新的上游 `nastechresearch/nastech-agent` 时记得重新构建镜像。
...
如果某个工具可能对大多数 Nastech Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [nastech-agent 仓库](https://github.com/NastechResearch/nastech-agent)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
...
然后在 `~/.nastech/config.yaml` 中，使用**容器名称**作为主机名：
...
1. 两个容器是否在同一 Docker 网络上（`docker network inspect nastech-net`）
...
检查日志：`docker logs nastech`。常见原因：
- `.env` 文件缺失或无效——先以交互方式运行以完成设置
- 开放端口时存在端口冲突

### "Permission denied" 错误

容器的 stage2 hook 通过 `s6-setuidgid` 在每个受监管的服务内将权限降至非 root 用户 `nastech`（UID 10000）。如果宿主机的 `~/.nastech/` 由不同 UID 拥有，请设置 `NASTECH_UID`/`NASTECH_GID` 以匹配宿主机用户，或确保数据目录可写：
...
docker run -it --rm nastechresearch/nastech-agent:latest version     # 验证版本
