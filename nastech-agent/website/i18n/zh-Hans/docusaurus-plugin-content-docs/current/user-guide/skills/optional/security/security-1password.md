...
| | |
|---|---|
| 来源 | 可选 — 通过 `nastech skills install official/security/1password` 安装 |
| 路径 | `optional-skills/security/1password` |
| 版本 | `1.0.0` |
| 作者 | arceus77-7，由 Nastech Agent 增强 |
| 许可证 | MIT |
...
- `tmux` 可用，用于在 Nastech 终端调用期间保持稳定的已认证会话（仅限桌面应用流程）
...
在 `~/.nastech/.env` 中设置 `OP_SERVICE_ACCOUNT_TOKEN`（skill 首次加载时会提示输入）。
...
Nastech 终端命令默认为非交互式，且在多次调用之间可能丢失认证上下文。
若要在桌面应用集成下可靠使用 `op`，请在专用 tmux 会话中执行登录和密钥操作。

注意：使用 `OP_SERVICE_ACCOUNT_TOKEN` 时**无需**此操作 — 令牌会在终端调用之间自动持久化。

```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nastech-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nastech-op.sock"
