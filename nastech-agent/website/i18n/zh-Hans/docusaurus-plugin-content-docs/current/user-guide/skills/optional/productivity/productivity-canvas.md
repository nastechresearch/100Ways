...
| | |
|---|---|
| 来源 | 可选 — 通过 `nastech skills install official/productivity/canvas` 安装 |
...
4. 为 token 命名（例如 "Nastech Agent"），设置可选的过期时间，然后点击 **Generate Token**
5. 复制 token 并添加到 `~/.nastech/.env`：
...
```bash
CANVAS="python $NASTECH_HOME/skills/productivity/canvas/scripts/canvas_api.py"
