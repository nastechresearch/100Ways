...
- 需要逐步执行某个函数并观察集合的变化
- 长期运行的进程（nastech gateway、tui_gateway）出现异常且无法重启
...
### `tui_gateway` 子进程（由 `nastech --tui` 启动）
...
debugpy.wait_for_client()
```
启动 `nastech --tui`。TUI 将显示为冻结状态（其后端正在等待）。附加客户端后，执行在你 `continue` 时恢复。
