...
| | |
|---|---|
| 来源 | 可选 — 通过 `nastech skills install official/productivity/here-now` 安装 |
...
- Skill 辅助脚本路径：
  - `${NASTECH_SKILL_DIR}/scripts/publish.sh` 用于发布站点
  - `${NASTECH_SKILL_DIR}/scripts/drive.sh` 用于私有 Drive 存储
...
| `--client {name}` | 用于归因的 agent 名称（如 `nastech`） |
