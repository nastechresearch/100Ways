---
sidebar_position: 1
title: "Nastech Portal"
description: "一个订阅，300+ 前沿模型，以及 Tool Gateway —— 运行 Nastech Agent 的推荐方式"
---

# Nastech Portal

[Nastech Portal](https://portal.nastechresearch.com) 是 Nastech Research 的统一订阅网关，也是**运行 Nastech Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
这条命令会完成 Portal OAuth 认证，让你选择一个 Nastech 模型，在 `config.yaml` 中将 Nastech 设为推理提供商，并开启 Tool Gateway。完成后即可立即运行 `nastech chat`。

还没有订阅？前往 [portal.nastechresearch.com/manage-subscription](https://portal.nastechresearch.com/manage-subscription) 注册，然后回来运行上面的命令。

## 订阅包含的内容

### 300+ 前沿模型，统一账单

Portal 代理了来自整个生态系统的精选 agentic 模型目录——统一计入你的 Nastech 订阅，而非每个厂商单独充值。
...
底层上，Portal 会为每个模型选择最合适的后端——部分模型通过 OpenRouter 路由，其他模型则通过专有或备用提供商，且某个模型的路由方式可能随时间调整。所有用量都统一计入你的 Nastech 订阅。在会话中途用 `/model` 即可在 Claude Sonnet 4.6（适合代码）和 Gemini 2.5 Pro（适合长上下文）之间切换——无需新凭证，无需充值，不会遇到余额为零的意外报错。
...
同一订阅还解锁了 [Tool Gateway](/user-guide/features/tool-gateway)，将 Nastech Agent 的工具调用路由至 Nastech 托管的基础设施。五个后端，一次登录：
...
Nastech Research 自家的 **Nastech 4** 系列（Nastech-4-70B、Nastech-4-405B）通过 Portal 提供，享有大幅折扣。这些是**前沿混合推理对话模型**——在数学、科学、指令遵循、schema 遵从、角色扮演和长文写作方面表现出色。
...
Portal 自身的[模型信息页](https://portal.nastechresearch.com/info)也有相同警告，因此这不是 Nastech 侧的主观意见——这是 Nastech Research 的官方指导。
...
1. 打开浏览器跳转至 portal.nastechresearch.com 进行 OAuth 登录
2. 将 refresh token 存储至 `~/.nastech/auth.json`
3. 让你从精选列表中选择一个 Nastech 模型（也可跳过以保留当前模型）
4. 在 `~/.nastech/config.yaml` 中将 Nastech 设为推理提供商（当你选择模型时）
5. 开启 Tool Gateway（网页、图像、TTS、浏览器路由）
6. 返回终端，即可运行 `nastech chat`

如果还没有订阅，请先在 [portal.nastechresearch.com/manage-subscription](https://portal.nastechresearch.com/manage-subscription) 注册。
...
```bash
nastech model
# 从提供商列表中选择 "Nastech Portal"
# 浏览器打开，登录，完成
```

你现有的提供商配置保持不变。可以在会话中途用 `/model` 切换，或在会话间用 `nastech model` 切换——Portal 成为你的可用提供商之一，而非唯一选项。
...
如果你使用 [Nastech profiles（配置文件）](/user-guide/profiles)，Portal 的 refresh token 会通过共享令牌存储自动在所有 profile 间共享。在任意 profile 上登录一次，其余 profile 自动获取——无需为每个 profile 重复 OAuth 流程。
...
```bash
nastech portal            # 登录 Nastech Portal 并完成配置（一键引导）
...
`nastech portal`（不带子命令）是 `nastech auth add nastech --type oauth` 的易记别名——它会登录、让你选择 Nastech 模型、把 Nastech 设为推理服务商，并提供 Tool Gateway 启用选项（与 `nastech setup --portal` 等价，与首次快速设置走的是同一套 Nastech 流程）。
...
如果你已有 Browserbase 账号并希望继续使用，同时通过 Nastech 路由网页搜索和图像生成，这是支持的。使用 `nastech tools` 为每个工具单独选择后端：

```bash
nastech tools
# → 网页搜索       → "Nastech Subscription"
# → 图像生成       → "Nastech Subscription"
...
- **网页端：** [portal.nastechresearch.com/manage-subscription](https://portal.nastechresearch.com/manage-subscription)
- **CLI 快捷方式：** `nastech portal open`（在默认浏览器中打开同一页面）

## 配置参考

运行 `nastech setup --portal` 后，`~/.nastech/config.yaml` 将如下所示：

```yaml
model:
  provider: nastech
  default: anthropic/claude-sonnet-4.6     # 或你选择的其他模型
  base_url: https://inference.nastechresearch.com/v1
```
...
```yaml
web:
  backend: nastech       # 网页搜索/抓取通过 Tool Gateway 路由
...
OAuth refresh token 单独存储在 `~/.nastech/auth.json`（不在 `config.yaml` 中——凭证与配置有意分开存放）。

## 令牌处理

Nastech 在每次推理调用时从存储的 Portal refresh token 生成短期 JWT，而非复用长期 API 密钥。令牌生命周期完全自动管理——刷新、生成、在瞬时 401 时重试——你无需关心这些细节。

如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，Nastech 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `nastech auth add nastech` 重新登录；隔离状态在下次成功登录时自动清除。

## 故障排查

### `nastech portal info` 显示"not logged in"
...
或使用 `nastech model` 重新选择 Nastech Portal。

### 会话中途收到"需要重新认证"提示

你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `nastech auth add nastech`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/NastechResearch/nastech-agent/issues)——我们将 Portal 目录同步至 Nastech，缺口通常意味着可以更新的路由配置。

### 账单未出现在我的 Portal 账号中

先检查 `nastech portal info`——如果显示你正在使用其他提供商（`Model: currently openrouter` 而非 `using Nastech as inference provider`），说明本地配置已偏离。运行 `nastech model`，选择 Nastech Portal，下一次请求将通过你的订阅路由。

## 另请参阅

- **[Tool Gateway](/user-guide/features/tool-gateway)** —— 每个 gateway 工具的完整详情、每工具配置及定价
- **[订阅代理](/user-guide/features/subscription-proxy)** —— 在非 Nastech 工具（其他 agent、脚本、第三方客户端）中使用你的 Portal 订阅
...
- **[Profiles](/user-guide/profiles)** —— 多个 Nastech 配置共享一个 Portal 登录
