---
sidebar_position: 1
title: "通过 Nastech Portal 运行 Nastech Agent"
description: "完整操作指南：订阅、配置、切换模型、启用 gateway 工具并验证路由"
---

# 通过 Nastech Portal 运行 Nastech Agent

本指南带你从头到尾完成在 [Nastech Portal](https://portal.nastechresearch.com) 订阅下运行 Nastech Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [Nastech Portal 集成页面](/integrations/nastech-portal)。本页是操作步骤脚本。

## 前提条件

- 已安装 Nastech Agent（[快速入门](/getting-started/quickstart)）
...
打开 [portal.nastechresearch.com/manage-subscription](https://portal.nastechresearch.com/manage-subscription)，注册并选择一个套餐。
...
3. 在 `~/.nastech/config.yaml` 中设置 `model.provider: nastech`
...
OAuth 需要浏览器，但 loopback 回调运行在 Nastech 所在的机器上。有两种方案：
...
如果任何一行显示的不是"via Nastech Portal"，或者 auth 行显示"not logged in"，请跳至下方的[故障排查](#troubleshooting)。
...
你应该看到 Nastech 调用 `web_search`（通过 gateway 由 Firecrawl 提供支持）并返回摘要。如果搜索正常执行且响应内容合理，说明配置完成——Portal 已端到端连通。

## 5. 选择你实际需要的模型

`nastech setup --portal` 会在设置过程中让你选择模型，但订阅的意义在于可以访问完整的模型目录——随时可在会话中使用 `/model` 切换：
...
### 不要在 agent 任务中使用 Nastech-4

Nastech-4-70B 和 Nastech-4-405B 在 Portal 上以大幅折扣提供，但它们是**对话/推理模型**，并非针对工具调用优化的模型。它们在多步骤 agent 循环中表现不佳。请通过[订阅代理](/user-guide/features/subscription-proxy)从非 agent 工具中将它们用于对话或研究工作。对于 Nastech Agent 本身，请坚持使用上述前沿 agentic 模型。

Portal 的[信息页面](https://portal.nastechresearch.com/info)也有此说明——这是 Nastech 官方指导，并非仅代表 Nastech 一方的意见。

## 6. （可选）自定义 Tool Gateway 路由

gateway 是按工具选择启用的，而非全部开启或全部关闭。如果你已有 Browserbase 账号并希望继续使用，同时将网页搜索和图像生成路由至 Nastech，这是支持的：

```bash
nastech tools
# → Web search       → "Nastech Subscription"     （推荐）
# → Image generation → "Nastech Subscription"     （推荐）
# → Browser          → "Browserbase"           （你自己的密钥）
# → TTS              → "Nastech Subscription"     （推荐）
```
...
你将看到每个工具的路由情况——通过订阅路由的工具显示 `via Nastech Portal`，使用你自己密钥的工具显示合作方名称（`browserbase`、`firecrawl` 等）。
...
```bash
nastech setup voice
# → 为 TTS 选择 "Nastech Subscription"
# → 选择语音转文字后端（本地 faster-whisper 免费，无需配置）
```

之后在任何消息平台会话中（Telegram、Discord、Signal 等），发送语音消息，Nastech 将转录内容、生成回复并以合成语音回复——全部通过你的 Portal 订阅完成。
...
```bash
nastech cron add "Daily AI news summary" "every day at 9am" \
...
如果你使用 [Nastech profiles](/user-guide/profiles)（例如每个项目单独一套配置），Portal refresh token 会通过共享 token 存储自动在所有 profiles 之间共享。在任意 profile 上登录一次，其余 profiles 会自动获取。

对于多人共用一台机器的团队场景，每个人有自己的 Portal 账号 → 每个 home 目录保存各自的 `~/.nastech/auth.json` → 用户之间不共享 token。这是正确的边界划分。

## 故障排查

### 运行 `nastech setup --portal` 后，`nastech portal info` 显示"not logged in"
...
### "Model: currently openrouter"（或其他 provider）而非"using Nastech as inference provider"
...
### Tool Gateway 工具显示合作方名称而非"via Nastech Portal"

按工具的配置覆盖了 gateway 设置。运行：

```bash
nastech tools
# 对需要通过 gateway 路由的工具选择 "Nastech Subscription"
```

部分用户会有意混合使用——例如网页搜索通过 Nastech 路由，但浏览器使用自己的 Browserbase 密钥。如果这是有意为之，保持不变即可。如果不是，此命令可修复。

### 会话中途出现"Re-authentication required"

你的 Portal refresh token 已失效（密码更改、手动撤销、会话过期）。该 token 现已在本地被隔离，以防 Nastech 无限重试。重新登录即可：
...
如果某个模型确实不可用，请[提交 issue](https://github.com/NastechResearch/nastech-agent/issues)——大多数缺失是我们可以更新的路由配置问题。

### 账单未出现在我的 Portal 账号中

`nastech portal info` 会告诉你是否真的在通过 Portal 路由，还是使用了其他 provider。常见原因：

- `model.provider` 设置为 `openrouter`/`anthropic`/等，而非 `nastech`
- OAuth refresh 失败后回退到了其他已配置的 provider
- 存在多个 Nastech profiles，你使用的是错误的那个（检查 `nastech profile list`）

### 想要撤销并重新开始

```bash
nastech auth logout nastech       # 清除本地 refresh token
...
- **[Nastech Portal 集成页面](/integrations/nastech-portal)** — 订阅内容概述
...
- **[Profiles](/user-guide/profiles)** — 在多个 Nastech 配置之间共享一个 Portal 登录
