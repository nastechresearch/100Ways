...
sidebar_position: 12
title: "将脚本输出推送到消息平台"
description: "使用 `nastech send` 将任意 shell 脚本、cron 任务、CI hook 或监控守护进程的文本发送到 Telegram、Discord、Slack、Signal 等平台。"
---

# 将脚本输出推送到消息平台

`nastech send` 是一个轻量、可脚本化的 CLI，能将消息推送到 Nastech 已配置的任意消息平台。可以把它理解为跨平台的通知专用 `curl`——无需运行中的 gateway，无需 LLM，也无需在每个脚本里重复粘贴 bot token。
...
- 需要将结果推送给你的 cron 脚本
- 从终端发送一次性消息
- 将任意工具的输出管道到任意平台（`make | nastech send --to slack:#builds`）

该命令复用 `nastech gateway` 已有的凭据和平台适配器，无需维护第二套配置。
...
`nastech send` 按以下顺序解析消息体：

1. **位置参数** — `nastech send --to telegram "hi"`
2. **`--file PATH`** — `nastech send --to telegram --file msg.txt`
3. **管道 stdin** — `echo hi | nastech send --to telegram`

当 stdin 是 TTY（无管道）时，Nastech **不会**等待输入——你会收到明确的用法错误提示。这可以防止脚本在意外省略消息体时挂起。
...
由于 `nastech send` 复用你的 Nastech 配置，同一脚本可在任何安装了 Nastech 的主机上运行——无需手动将 bot token 导出到每台机器的环境变量中。

:::tip 不要用 gateway 监控自身
对于可能在 gateway 本身出现问题时触发的 watchdog（OOM 告警、磁盘满告警），请继续使用最简单的 `curl` 调用，而非 `nastech send`。如果 Python 解释器因机器抖动无法加载，你仍然希望告警能发出去。
:::
...
## `nastech send` 需要 gateway 运行吗？

**通常不需要。** 对于所有基于 bot token 的平台——Telegram、Discord、Slack、Signal、SMS、WhatsApp Cloud API 等——`nastech send` 直接使用 `~/.nastech/.env` 和 `~/.nastech/config.yaml` 中的凭据调用平台的 REST 接口。它是一个独立的子进程，消息投递完成后即退出。

只有依赖持久适配器连接的**插件平台**才需要运行中的 gateway（例如，某个保持长连接 WebSocket 的自定义插件）。此时你会收到明确的错误提示，指引你启动 gateway；执行 `nastech gateway start` 后重试即可。
...
列表数据来源于 `~/.nastech/channel_directory.json`，gateway 运行期间每隔几分钟刷新一次。如果看到"尚未发现频道"，请先启动一次 gateway（`nastech gateway start`）以填充缓存。
...
| 方案 | 多平台 | 复用 Nastech 凭据 | 需要 gateway | 最适合 |
|----------|----------------|---------------------|---------------|----------|
| `nastech send` | ✅ | ✅ | 否（bot token） | 以下所有场景 |
| 对各平台直接 `curl` | 各自单独编写 | 手动管理 | 否 | 关键 watchdog |
| 带 `--deliver` 的 `cron` 任务 | ✅ | ✅ | 否 | 定时 agent 任务 |
| `send_message` agent 工具 | ✅ | ✅ | 否 | agent 循环内部 |

`nastech send` 有意保持最简接口。如果需要 agent 决定说什么，请在对话或 cron 任务中使用 `send_message` 工具。如果需要定时运行并生成 LLM 内容，请使用带 `deliver='telegram:...'` 的 `cronjob(action='create', prompt=...)`。如果只需要管道传输原始字符串，直接用 `nastech send`。
...
- [Gateway 内部机制](/developer-guide/gateway-internals) —
  `nastech send` 与 cron 投递共享的投递路由器。
