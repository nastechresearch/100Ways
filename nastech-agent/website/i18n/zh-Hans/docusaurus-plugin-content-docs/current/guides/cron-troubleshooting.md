...
如果你期望任务自动触发，需要运行一个 gateway（前台运行用 `nastech gateway`，安装为服务用 `nastech gateway start`）。如需单次调试，可手动触发一次 tick：`nastech cron tick`。
...
```bash
date
nastech cron list   # 将 next_run 时间与本地时间对比
```
...
| 目标 | 所需配置 |
|--------|----------|
| `telegram` | `~/.nastech/.env` 中的 `TELEGRAM_BOT_TOKEN` |
| `discord` | `~/.nastech/.env` 中的 `DISCORD_BOT_TOKEN` |
| `slack` | `~/.nastech/.env` 中的 `SLACK_BOT_TOKEN` |
| `whatsapp` | 已配置 WhatsApp gateway |
| `signal` | 已配置 Signal gateway |
| `matrix` | 已配置 Matrix homeserver |
| `email` | `config.yaml` 中已配置 SMTP |
| `sms` | 已配置 SMS 提供商 |
| `local` | 对 `~/.nastech/cron/output/` 有写权限 |
...
若投递失败，任务仍会执行——只是不会发送到任何地方。检查 `nastech cron list` 中的 `last_error` 字段（如有）。
...
Skill 必须先安装才能附加到 cron 任务。若 skill 缺失，先用 `nastech skills install <skill-name>` 安装，或在 CLI 中通过 `/skills` 安装。

### 检查 2：检查 skill 名称与 skill 文件夹名称

Skill 名称区分大小写，必须与已安装 skill 的文件夹名称完全匹配。若任务指定的是 `ai-funding-daily-report`，但 skill 文件夹也是 `ai-funding-daily-report`，请从 `nastech skills list` 确认确切名称。
...
1. 任务投递的聊天会话（若投递成功）
2. `~/.nastech/logs/agent.log`（调度器消息）或 `errors.log`（警告信息）
3. 通过 `nastech cron list` 查看任务的 `last_run` 元数据
...
Skill 必须安装在运行调度器的机器上。若你在不同机器间切换，skill 不会自动同步——请用 `nastech skills install <skill-name>` 重新安装。

**任务运行但没有投递任何内容**
可能是投递目标问题（见上方"投递失败"部分）或响应被静默抑制（`[SILENT]`）。

**任务挂起或超时**
调度器使用基于不活跃时间的超时机制（默认 600 秒，可通过 `NASTECH_CRON_TIMEOUT` 环境变量配置，`0` 表示无限制）。只要 agent 持续调用工具，就可以一直运行——计时器仅在持续不活跃后触发。长时间运行的任务应使用脚本处理数据采集，仅将结果投递出去。
...
任务存储在 `~/.nastech/cron/jobs.json`。若该文件对当前用户不可读写，调度器将静默失败：

```bash
ls -la ~/.nastech/cron/jobs.json
chmod 600 ~/.nastech/cron/jobs.json   # 应由你的用户拥有
```
...
1. 使用 `nastech cron run <job_id>` 运行任务（在下次 gateway tick 时触发），观察聊天输出中的错误
2. 查看 `~/.nastech/logs/agent.log` 中的调度器消息和 `~/.nastech/logs/errors.log` 中的警告
3. 在 [github.com/NastechResearch/nastech-agent](https://github.com/NastechResearch/nastech-agent) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息
