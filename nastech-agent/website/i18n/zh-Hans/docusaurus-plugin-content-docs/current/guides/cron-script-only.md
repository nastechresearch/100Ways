...
- **Bash 或 Python。** `.sh` / `.bash` 文件在 `/bin/bash` 下运行；其他扩展名在当前 Python 解释器下运行。`~/.nastech/scripts/` 中的任何文件均可接受。
...
# 3. 验证
nastech cron list
nastech cron run <job_id>    # 触发一次以测试
```
...
脚本必须位于 `~/.nastech/scripts/`。这在任务创建时和运行时均会强制检查——绝对路径、`~/` 展开以及路径穿越模式（`../`）均会被拒绝。该目录与 LLM 任务使用的预检脚本门控共享。
...
```bash
nastech cron create "every 5m"        # 间隔
nastech cron create "every 2h"
nastech cron create "0 9 * * *"       # 标准 cron：每天上午 9 点
nastech cron create "30m"             # 单次：30 分钟后运行一次
```
...
--deliver local                          # 仅保存到 ~/.nastech/cron/output/
```

对于使用 bot token 的平台（Telegram、Discord、Slack、Signal、SMS、WhatsApp），脚本运行时无需运行中的 gateway——工具直接使用 `~/.nastech/.env` / `~/.nastech/config.yaml` 中已有的凭据调用各平台的 REST 端点。

## 编辑与生命周期

```bash
nastech cron list                                    # 查看所有任务
nastech cron pause <job_id>                          # 停止触发，保留定义
nastech cron resume <job_id>
nastech cron edit <job_id> --schedule "every 10m"    # 调整频率
nastech cron edit <job_id> --agent                   # 切换为 LLM 模式
nastech cron edit <job_id> --no-agent --script …     # 切换回无 agent 模式
nastech cron remove <job_id>                         # 删除任务
```
