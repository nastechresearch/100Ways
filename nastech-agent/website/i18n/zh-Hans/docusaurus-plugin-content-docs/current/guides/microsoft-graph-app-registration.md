...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.nastech/.env`。
...
3. **Description：**`nastech-graph-secret`。**Expires：**根据你的轮换策略选择合适的值（通常为 6-24 个月）。
4. 点击 **Add**。
...
2. 用新值更新 `~/.nastech/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
3. 重启 gateway 以使新密钥生效：`nastech gateway restart`。
4. 使用上述冒烟测试进行验证。
5. 在 Azure 门户中删除旧密钥。
