...
本页涵盖所有与 [Nastech profiles](../user-guide/profiles.md) 相关的命令。通用 CLI 命令请参阅 [CLI 命令参考](./cli-commands.md)。
...
管理 profile 的顶级命令。不带子命令运行 `nastech profile` 将显示帮助信息。
...
```bash
nastech profile use <name>
```

将 `<name>` 设为活跃 profile。此后所有 `nastech` 命令（不带 `-p`）都将使用该 profile。
...
nastech profile use default
```

## `nastech profile create`

```bash
nastech profile create <name> [options]
```
...
| `--description "<text>"` | 一到两句话描述该 profile 的用途。供 kanban 编排器根据角色而非仅凭 profile 名称来路由任务。可跳过，稍后通过 `nastech profile describe` 添加。持久化保存在 `<profile_dir>/profile.yaml` 中。 |
| `--no-skills` | 创建一个**空** profile，不启用任何内置 skill。会在 profile 目录中写入 `.no-bundled-skills` 标记，使后续 `nastech update` 不再重新植入内置 skill 集，且拒绝与 `--clone`、`--clone-from` 或 `--clone-all` 组合使用（因为这些选项会复制 skill）。适用于不应继承完整 skill 目录的窄化编排器 profile 或沙箱 profile。 |
...
```bash
# 空白 profile — 需要完整配置
nastech profile create mybot

# 仅从当前 profile 克隆 config
nastech profile create work --clone

# 从当前 profile 克隆所有内容
nastech profile create backup --clone-all

# 从指定 profile 克隆 config
nastech profile create work2 --clone-from work

# 从指定 profile 克隆所有内容
nastech profile create work2-backup --clone-from work --clone-all
```

## `nastech profile describe`

```bash
nastech profile describe [<name>] [options]
```
...
```bash
# 读取当前描述
nastech profile describe researcher

# 显式设置描述
nastech profile describe researcher --text "Reads source code and writes findings."

# 让 LLM 生成描述
nastech profile describe researcher --auto

# 为所有没有描述的 profile 填充描述
nastech profile describe --all --auto
```

## `nastech profile delete`

```bash
nastech profile delete <name> [options]
```
...
```bash
nastech profile delete mybot
nastech profile delete mybot --yes
```
...
此处显示的是 profile 的 Nastech 主目录，而非终端工作目录。终端命令从 `terminal.cwd` 启动（或在本地后端 `cwd: "."` 时从启动目录启动）。
...
```bash
$ nastech profile show work
Profile: work
Path:    ~/.nastech/profiles/work
...
## `nastech profile alias`

```bash
nastech profile alias <name> [options]
```

重新生成位于 `~/.local/bin/<name>` 的 shell alias 脚本。适用于 alias 被意外删除，或移动 Nastech 安装目录后需要更新的情况。
...
```bash
nastech profile alias work
# 创建/更新 ~/.local/bin/work

nastech profile alias work --name mywork
# 创建 ~/.local/bin/mywork

nastech profile alias work --remove
# 移除 wrapper 脚本
```

## `nastech profile rename`

```bash
nastech profile rename <old-name> <new-name>
```
...
```bash
nastech profile rename mybot assistant
# ~/.nastech/profiles/mybot → ~/.nastech/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `nastech profile export`

```bash
nastech profile export <name> [options]
```
...
```bash
nastech profile export work
# 在当前目录创建 work.tar.gz

nastech profile export work -o ./work-2026-03-29.tar.gz
```

## `nastech profile import`

```bash
nastech profile import <archive> [options]
```
...
```bash
nastech profile import ./work-2026-03-29.tar.gz
# 从归档文件推断 profile 名称

nastech profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`nastech profile export` / `import` 仍是在**本机进行 profile 本地备份和恢复**的正确命令。发行版（`install` / `update` / `info`）是独立概念：通过 git 分发 profile，供他人安装。
:::

### `nastech profile install`

```bash
nastech profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | 同时创建 shell wrapper（例如 `telemetry` → `nastech -p telemetry`）。 |
...
```bash
# 从 GitHub 仓库安装（简写）
nastech profile install github.com/kyle/telemetry-distribution --alias

# 从完整 HTTPS git URL 安装
nastech profile install https://github.com/kyle/telemetry-distribution.git

# 从 SSH 安装
nastech profile install git@github.com:kyle/telemetry-distribution.git

# 开发时从本地目录安装
nastech profile install ./telemetry/
```

### `nastech profile update`

```bash
nastech profile update <name> [--force-config] [--yes]
```
...
### `nastech profile info`

```bash
nastech profile info <name>
```

打印 profile 的发行版 manifest — 名称、版本、所需 Nastech 版本、作者、环境变量要求、来源 URL/路径，以及发行版最后一次 `install` 或 `update` 时记录的 `Installed:` 时间戳。适用于安装前检查共享 profile 的需求，以及发现"该 profile 已安装 6 个月未更新"等情况。

`nastech profile list` 也会在 `Distribution` 列中显示发行版名称和版本，`nastech profile show <name>` / `delete <name>` 会显示来源 URL，让你一眼看出哪些 profile 来自 git 仓库，哪些是本地创建的。
...
nastech profile install git@github.com:your-org/internal-assistant.git

# 使用 git credential helper
nastech profile install https://github.com/your-org/internal-assistant.git
```
...
`nastech_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<`，或裸版本号（视为 `>=`）。若当前 Nastech 版本不满足规格，安装将失败并给出明确错误。
...
2. 初始化 git 仓库（或使用已有仓库），推送到 GitHub / GitLab / 任何 Nastech 可克隆的托管平台。
3. 告知接收方运行 `nastech profile install <your-repo-url>`。

使用 git tag 进行版本化发布 — 克隆 `HEAD` 的接收方将获得最新状态，你也可以随时在 manifest 中更新 `version:`。

## `nastech -p` / `nastech --profile`

```bash
nastech -p <name> <command> [options]
nastech --profile <name> <command> [options]
```

全局标志，用于在不更改默认 profile 的情况下，在指定 profile 下运行任意 Nastech 命令。仅在该命令执行期间覆盖活跃 profile。
...
```bash
nastech -p work chat -q "Check the server status"
nastech --profile dev gateway start
nastech -p personal skills list
nastech -p work config edit
```

## `nastech completion`

```bash
nastech completion <shell>
```
...
安装后，Tab 补全适用于：
- `nastech profile <TAB>` — 子命令（list、use、create 等）
- `nastech profile use <TAB>` — profile 名称
- `nastech -p <TAB>` — profile 名称
