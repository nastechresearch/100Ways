...
| `--ignore-user-config` | 忽略 `~/.nastech/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
| `--ignore-rules` | 跳过 `AGENTS.md`、`SOUL.md`、`.cursorrules`、memory（记忆）和预加载 skill 的自动注入。 |
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `NASTECH_TUI=1`。 |
...
| 命令 | 用途 |
|---------|---------|
| `nastech chat` | 与 agent 进行交互式或单次聊天。 |
| `nastech model` | 交互式选择默认 provider 和模型。 |
| `nastech fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `nastech gateway` | 运行或管理消息 gateway 服务。 |
| `nastech proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `nastech lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
...
| `nastech auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nastech/Anthropic 的 OAuth 流程。 |
| `nastech login` / `logout` | **已弃用** — 请改用 `nastech auth`。 |
| `nastech status` | 显示 agent、auth 和平台状态。 |
| `nastech cron` | 检查并触发 cron 调度器。 |
| `nastech kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `nastech webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `nastech hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `nastech doctor` | 诊断配置和依赖问题。 |
...
| `nastech skills` | 浏览、安装、发布、审计和配置 skill。 |
...
| `nastech portal` | Nastech Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
...
```bash
nastech
nastech chat -q "Summarize the latest PRs"
...
### `nastech -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`nastech -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
单次运行覆盖（不修改 `~/.nastech/config.yaml`）：

| 标志 | 等效环境变量 | 用途 |
|---|---|---|
| `-m` / `--model <model>` | `NASTECH_INFERENCE_MODEL` | 覆盖本次运行的模型 |
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `nastech chat -q`；`-z` 专门用于"我只需要最终答案"的场景。
...
- 登录基于 OAuth 的 provider（Anthropic、Copilot、Codex、Nastech Portal）
- 输入或更新 API 密钥
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `nastech model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `NASTECH_HOME`。当你并行运行多个 profile 并希望在 `nastech update` 后全部重启时很有用。 |
| `--no-supervise` | 在 `run` 时：在 s6-overlay Docker 镜像内部，跳过 s6 自动监管，退回到 pre-s6 前台语义——gateway 作为容器主进程运行，无自动重启。在 s6 镜像之外为空操作。等同于设置 `NASTECH_GATEWAY_NO_SUPERVISE=1`。 |

:::tip WSL 用户
使用 `nastech gateway run` 而非 `nastech gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s nastech 'nastech gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-nastech-gateway-start-fails)。
:::
...
| `--portal` | 一键 Nastech Portal 设置：通过 OAuth 登录，将 Nastech 设为推理 provider，并选择加入 [Tool Gateway](../user-guide/features/tool-gateway.md)。跳过向导其余部分。 |
...
检查 Nastech Portal 认证、Tool Gateway 路由，并访问订阅页面。不带子命令时运行 `status`。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | Portal 认证状态 + 每个工具的 Tool Gateway 路由摘要。不带子命令时也会显示。 |
| `open` | 在默认浏览器中打开 `portal.nastechresearch.com/manage-subscription`。 |
| `tools` | 列出每个 Tool Gateway 合作伙伴（Firecrawl、FAL、OpenAI TTS、Browser Use、Modal）及哪些通过 Nastech 路由。 |
...
```bash
nastech slack manifest              # 将 manifest 打印到 stdout
nastech slack manifest --write      # 写入 ~/.nastech/slack-manifest.json
nastech slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
`nastech update` 后重新运行 `nastech slack manifest --write` 以获取新增命令。


## `nastech login` / `nastech logout` *（已弃用）*

:::caution
`nastech login` 已被移除。请使用 `nastech auth` 管理 OAuth 凭据，使用 `nastech model` 选择 provider，或使用 `nastech setup` 进行完整的交互式设置。
:::
...
```bash
nastech auth                                              # 交互式向导
nastech auth list                                         # 显示所有池
nastech auth list openrouter                              # 显示特定 provider
nastech auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
nastech auth add anthropic --type oauth                   # 添加 OAuth 凭据
nastech auth remove openrouter 2                          # 按索引删除
...
```bash
nastech cron <list|create|edit|pause|resume|run|remove|status|tick>
```
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.nastech/kanban.db`（向后兼容）；其他看板位于 `~/.nastech/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。

**全局标志（适用于以下所有操作）：**

| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `nastech kanban boards switch`、`NASTECH_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `nastech kanban`。Worker 的环境中固定了 `NASTECH_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.nastech/kanban/current`）。 |
...
nastech kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `NASTECH_KANBAN_BOARD` 环境变量 → `~/.nastech/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/nastech-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
:::tip
`nastech dump` 专为分享而设计。交互式诊断请使用 `nastech doctor`。可视化概览请使用 `nastech status`。
:::
...
```bash
nastech debug share              # 上传调试报告，打印 URL
nastech debug share --lines 500  # 包含更多日志行
nastech debug share --expire 30  # 粘贴保留 30 天
nastech debug share --local      # 在终端打印报告（不上传）
```
...
| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/nastech-backup-<timestamp>.zip`）。 |
...
```bash
nastech backup                           # 完整备份到 ~/nastech-backup-*.zip
nastech backup -o /tmp/nastech.zip        # 完整备份到指定路径
nastech backup --quick                   # 仅状态快速快照
nastech backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
检查和管理 `~/.nastech/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。

| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `nastech checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
nastech checkpoints                                  # 状态概览
nastech checkpoints prune --retention-days 3         # 激进清理
nastech checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
nastech checkpoints clear-legacy -f                  # 删除 v1 归档目录
nastech checkpoints clear -f                         # 清除所有内容
```
...
### 示例
```bash
nastech import ~/nastech-backup-20260423.zip           # 覆盖现有配置前提示确认
nastech import ~/nastech-backup-20260423.zip --force   # 不提示直接覆盖
```
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.nastech/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.nastech/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.nastech/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.nastech/skills/`（默认使用最新快照） |
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `nastech update` 后的第一次 tick 时立即执行 curator。使用 `nastech curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `nastech model` 相同的选择器）并追加到链末尾 |
...
检查 `~/.nastech/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.nastech/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `nastech <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `nastech honcho`）。未激活的 provider 不暴露其子命令。运行 `nastech --help` 查看当前已连接的命令。
:::
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `nastech plugins` 会打开包含两个部分的复合交互界面：
...
| `--stop` | — | 停止正在运行的 `nastech dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `nastech dashboard` 进程并退出。 |
...
| `--backup` | 在拉取前创建 `NASTECH_HOME` 的带标签预更新快照（config、auth、会话、skill、配对数据）。默认**关闭**——之前的始终备份行为在大型主目录上每次更新会增加数分钟。通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
