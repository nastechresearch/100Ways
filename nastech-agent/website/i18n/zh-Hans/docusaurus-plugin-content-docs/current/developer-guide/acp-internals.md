...
```text
nastech acp / nastech-acp / python -m acp_adapter
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["nastech-acp"])
...
- ACP 会话持久化至共享的 `~/.nastech/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `nastech-acp` toolset 定义
- `nastech_cli/main.py` — `nastech acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `nastech-acp` 脚本