...
多个 nastech 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.nastech/state.db
...
默认路径：`~/.nastech/state.db`

该路径由 `nastech_constants.get_nastech_home()` 推导，默认解析为 `~/.nastech/`，或 `NASTECH_HOME` 环境变量的值。
