...
sidebar_position: 5
title: "Prompt 组装"
description: "Nastech 如何构建系统 prompt、保持缓存稳定性并注入临时层"
---
...
1. agent 身份 — 优先使用 `NASTECH_HOME` 中的 `SOUL.md`，否则回退到 `prompt_builder.py` 中的 `DEFAULT_AGENT_IDENTITY`
2. 工具感知行为指导
3. Honcho 静态块（激活时）
4. 可选系统消息
5. 冻结的 MEMORY 快照
...
You are Nastech, an AI assistant created by Nastech Research.
...
`SOUL.md` 位于 `~/.nastech/SOUL.md`，作为 agent 的身份标识——系统 prompt 的第一个部分。`prompt_builder.py` 中的加载逻辑如下：
...
```
You are Nastech Agent, an intelligent AI assistant created by Nastech Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    # SOUL.md from NASTECH_HOME (independent of project context)
    if not skip_soul:
...
- **剥离 YAML frontmatter** — `.nastech.md` 的 frontmatter 会被移除（保留供未来配置覆盖使用）
...
大多数用户应将 `agent/prompt_builder.py` 视为实现代码，而非配置入口。推荐的自定义路径是修改 Nastech 已加载的 prompt 输入，而非直接编辑 Python 模板。
...
- 若想改变 Nastech 为所有人组装 prompt 的方式，修改 Python 代码并将其视为代码贡献
