#!/bin/sh
# shellcheck shell=sh
# /opt/nastech/bin/nastech — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# returns "Provider authentication failed: Nastech is not logged into Nastech
# Portal" on every incoming message — even though `docker exec <c> nastech
...
# Root, no opt-out. Drop to the nastech user.
#
...
# Reset HOME to the nastech user's home before dropping privileges. Without
