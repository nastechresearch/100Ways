...
if [ "$$" -eq 1 ]; then
    exec /init /opt/nastech/docker/main-wrapper.sh "$@"
fi

echo "[nastech] WARNING: container entrypoint is not PID 1; skipping s6-overlay /init and falling back to direct bootstrap. Supervised services are unavailable in this runtime, but the requested command will still run." >&2
...
/opt/nastech/docker/stage2-hook.sh
exec /opt/nastech/docker/main-wrapper.sh "$@"
