...
import { SelectionSwitcher } from "@nastech-research/ui/ui/components/selection-switcher";
import { Spinner } from "@nastech-research/ui/ui/components/spinner";
import { Typography } from "@nastech-research/ui/ui/components/typography/index";
import { ConfirmDialog } from "@nastech-research/ui/ui/components/confirm-dialog";
import { cn } from "@/lib/utils";
import { SidebarFooter } from "@/components/SidebarFooter";
import { SidebarStatusStrip, gatewayLine } from "@/components/SidebarStatusStrip";
import { useBelowBreakpoint } from "@nastech-research/ui/hooks/use-below-breakpoint";
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'nastech update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
