 ...
 *  - loopback / ``--insecure``: attach the ``X-Nastech-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``nastech_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__NASTECH_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  checkNastechUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/nastech/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** nastech-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``nastech update``. */
  can_update_nastech?: boolean;
  config_path: string;
