...
export function NastechConsoleModal({ open, onClose }: NastechConsoleModalProps) {
  ...
  const promptRef = useRef("nastech> ");
  const inputPromptRef = useRef("nastech> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "nastech> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Nastech Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="nastech-console-title"
    >
      <div
        className={cn(
          themedBody,
