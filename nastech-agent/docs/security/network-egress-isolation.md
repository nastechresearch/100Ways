...
When running Nastech inside Docker, the default `network_mode: host` gives the
...
The Nastech [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ nastech-agent │   │ nastech-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   NASTECH_UID=$(id -u) NASTECH_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=nastech,nastech-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://nastech-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Nastech trust model and vulnerability reporting
