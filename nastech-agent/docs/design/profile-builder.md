...
Supersedes: PR #31781 (prompt_toolkit `nastech profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `nastech profile wizard` in the terminal.
...
needs, and a profile is just a NASTECH_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.nastech/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — NASTECH_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with NASTECH_HOME override |
...
### Seam #1 — hub-skill install cannot use the NASTECH_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = NASTECH_HOME / "skills"` at **module
import time**. The context-local `set_nastech_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_nastech_action`
runs `python -m nastech_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_nastech_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's NASTECH_HOME bound
...
page afterward. Mirror `_write_profile_model`'s NASTECH_HOME-override helper for
...
- Backend E2E with isolated NASTECH_HOME: POST a full create body
