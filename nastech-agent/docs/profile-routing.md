...
`nastech config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.nastech/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `NASTECH_HOME`, secret scope, and
