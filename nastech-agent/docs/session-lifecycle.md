...
The canonical transcript store is SQLite via `SessionDB` (from `nastech_state`). The
...
Counts consecutive restarts via a JSON file (`{NASTECH_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `nastech update`, `nastech gateway restart`,
or `/restart`.
