...
Nastech includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Nastech and does not require `nastech plugins enable
observability/nemo_relay`. Nastech remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Nastech execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `nastech-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Nastech requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Nastech installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Nastech process so configured interceptors can
...
Collection remains off unless Nastech policy enables it:
...
Nastech core owns one Relay host and one isolated Relay session scope per Nastech
session. Core lifecycle producers use
`nastech_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Nastech plugin registration. Shared-metrics marks must
...
```text
Nastech turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Nastech shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Nastech sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Nastech used for the logical call, such as
...
New calls use `nastech.model_route.count`. The previous
`nastech.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `nastech.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Nastech
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Nastech processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `nastech.task_run`, parented to
the owning Nastech session. The start counter contains only bounded execution
...
provider attempts for the same Nastech API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Nastech tool-call
...
cancellations. Active task ownership follows the task ID if Nastech rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`nastech.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Nastech
...
tool or plugin names. Nastech does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `nastech.tool_approval` marks and
...
Successful skill mutations emit `nastech.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `nastech.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Nastech derives reuse and patch-generation
...
```text
$NASTECH_HOME/telemetry/shared_metrics/metrics.sqlite3
$NASTECH_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Nastech version,
...
schema field name, its current scope is one `NASTECH_HOME`, so it is more
...
Deleting `$NASTECH_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `NASTECH_HOME`. To reset it, stop Nastech
processes and remove `$NASTECH_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Nastech CLI turn against the deterministic local model server:
