...
Nastech supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.nastech/config.yaml`:
...
Or set the env var: `NASTECH_KANBAN_DISPATCH_IN_GATEWAY=false`
