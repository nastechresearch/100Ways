...
- `telemetry_schema_version`: currently `nastech.observer.v1`
- `middleware_schema_version`: currently `nastech.middleware.v1`
...
For isolated local testing, use one `NASTECH_HOME` for plugin enablement and the
agent run:
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["nastech_middleware_demo"] = True
