import type { InputEvent, Key } from '@nastech/ink'
import * as Ink from '@nastech/ink'
 ...
 * but that accepts `key.meta` on macOS — and nastech-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.NASTECH_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
