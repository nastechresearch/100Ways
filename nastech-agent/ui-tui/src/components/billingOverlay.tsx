...
function OverviewScreen({ ctx, onClose, onPatch, s, t }: ScreenProps) {
  ...
  const note = !s.is_admin
    ...
    : !s.cli_billing_enabled
      ? "Remote spending is off for this org — a billing admin can turn it on from the portal's Nastech Agent page."
      : null
...
function ConfirmScreen({
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color={t.color.muted}>By confirming, you allow Nastech Research to charge your card.</Text>
      <Text />
...
function AutoReloadScreen({ ctx, onClose, onPatch, s, t }: ScreenProps) {
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text />
      <Text color={t.color.muted}>
        By confirming, you authorize Nastech Research to charge {chargeCardName} whenever your balance falls below the
