...
describe('fromSkin', () => {
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ NASTECH_TUI_BACKGROUND: '#ffffff' })
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ NASTECH_TUI_BACKGROUND: '#ffffff' })
...
describe('derived tone ladder', () => {
  it('reproduces the original hand-tuned tones from seeds (reverse-engineered knobs)', async () => {
    ...
    const light = await importThemeWithEnv({ NASTECH_TUI_BACKGROUND: '#ffffff' })
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  it('renders a dark-authored skin on light like minimumContrastRatio hosts do (the standardized look)', async () => {
    const { contrastRatio, fromSkin } = await importThemeWithEnv({ NASTECH_TUI_BACKGROUND: '#ffffff' })
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ NASTECH_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ NASTECH_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late NASTECH_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ NASTECH_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
