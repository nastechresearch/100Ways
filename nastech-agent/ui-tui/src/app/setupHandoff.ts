import type { RunExternalProcess } from '@nastech/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nastech ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nastech: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nastech ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
