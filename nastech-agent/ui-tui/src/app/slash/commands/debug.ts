...
import { terminalBackgroundHex } from '@nastech/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $NASTECH_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see NASTECH_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['NASTECH_TUI_BACKGROUND', process.env.NASTECH_TUI_BACKGROUND ?? '(unset)'],
            ['NASTECH_TUI_THEME', process.env.NASTECH_TUI_THEME ?? '(unset)'],
