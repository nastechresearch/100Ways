...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    help: 'session usage + Nastech credits',
    name: 'usage',
    run: (_arg, ctx) => {
      ctx.gateway.rpc<SessionUsageResponse>('session.usage', { session_id: ctx.sid }).then(r => {
        ...
        // Nastech balance block is agent-independent (a portal fetch), so it shows
        ...
        if (usageModel?.available && (barLines.length || usageModel.status === 'free')) {
          ...
          if (creditsLines.length) {
            ctx.transcript.panel('Nastech balance', [{ text: creditsLines.join('\n') }])
            showedBalance = true
          }
        }
