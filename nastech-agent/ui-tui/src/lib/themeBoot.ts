 ...
 * `nastech-boot-background` / `nastech-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports NASTECH_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.nastech matches get_nastech_home()'s default.
const bootFilePath = () => join(process.env.NASTECH_HOME ?? join(homedir(), '.nastech'), 'tui-theme-boot.json')

// Never touch the user's real ~/.nastech from test runs (the TS suite has no
// NASTECH_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into NASTECH_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.NASTECH_TUI_THEME || env.NASTECH_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.NASTECH_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.NASTECH_TUI_BACKGROUND
  ) {
    env.NASTECH_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.NASTECH_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.NASTECH_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into NASTECH_TUI_THEME.
