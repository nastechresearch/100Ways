...
describe('heapdump retention guard (#21767)', () => {
  ...
  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.NASTECH_HEAPDUMP_DIR
    } else {
      process.env.NASTECH_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.NASTECH_HEAPDUMP_MAX_BYTES
    } else {
      process.env.NASTECH_HEAPDUMP_MAX_BYTES = savedMax
    }
