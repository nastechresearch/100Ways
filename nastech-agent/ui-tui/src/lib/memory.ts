...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because NASTECH_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.NASTECH_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.NASTECH_HEAPDUMP_MAX_BYTES?.trim()
