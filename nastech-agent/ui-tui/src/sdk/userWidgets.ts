...
/**
 * User widget apps — Nastech authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$NASTECH_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.nastech/plugins/`: files under NASTECH_HOME execute
...
const widgetsDir = () => join(process.env.NASTECH_HOME?.trim() || join(homedir(), '.nastech'), 'tui-widgets')
 ...
 *  every change, so a widget Nastech writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Nastech) ever writes must hot-load too — a 10s poll here read
