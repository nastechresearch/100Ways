...
import type { FrameEvent } from '@nastech/ink'
...
if (!process.stdin.isTTY) {
  console.log('nastech-tui: no TTY')
  process.exit(0)
}
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `nastech-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    try {
      process.stderr.write(`nastech-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
    } catch {
  ...
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`nastech-tui lifecycle: received ${signal}\n`)
  },
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write(
      `nastech-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`
    )
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('nastech-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `nastech-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.NASTECH_HEAPDUMP_ON_START === '1') {
