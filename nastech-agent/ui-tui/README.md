...
- `src/app/setupHandoff.ts` — launches external `nastech setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Nastech remote spending — buy credits, auto-reload, limits
...
### Billing (`billing.ts`)
`/billing` — manage Nastech remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Nastech credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `nastech setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external nastech setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Nastech remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.nastech/tui-parent.log
