"""Run the read-tool eval through the REAL Nastech AIAgent.

For each task: fresh temp NASTECH_HOME, fresh fixture workspace, real
...
Usage:
  python3 evals/readtool/runner.py --model anthropic/claude-opus-4.8 \\
      --provider nastech --reps 3 --label baseline
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    nastech_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".nastech"
    ...
    try:
        ...
        shutil.rmtree(nastech_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.nastech/.env; set +a  — then relaunch."
        )
