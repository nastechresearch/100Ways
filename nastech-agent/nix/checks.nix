...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nastech-agent = self'.packages.default;
      nastechVenv = nastech-agent.nastechVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nastech-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nastech-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "nastech-build-package" { } ''
          echo "PASS: package built at ${nastech-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "nastech-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "nastech-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nastech-agent}/bin/nastech || (echo "FAIL: nastech binary missing"; exit 1)
          test -x ${nastech-agent}/bin/nastech-agent || (echo "FAIL: nastech-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nastech-agent}/bin/nastech version 2>&1 | grep -qi "nastech" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nastech-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nastech nastech-agent nastech-acp; do
            test -x ${nastech-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nastech-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nastech --help ==="
          ${nastech-agent}/bin/nastech --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nastech-agent}/bin/nastech --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nastech-bundled-skills" { } ''
          set -e
        ...
        bundled-plugins = pkgs.runCommand "nastech-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nastech-agent}/share/nastech-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nastech-agent}/share/nastech-agent/plugins/platforms/irc/plugin.yaml || \
        ...
        bundled-locales = pkgs.runCommand "nastech-bundled-locales" { } ''
          set -e
        ...
        bundled-tui = pkgs.runCommand "nastech-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nastech-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nastech-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
        ...
        extra-python-packages = let
          ...
          nastechWithExtra = nastech-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nastech-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nastechWithExtra}/bin/nastech || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nastechWithExtra}/bin/nastech || \
          ...
          if grep -q "PYTHONPATH" ${nastech-agent}/bin/nastech; then
        ...
        extra-dependency-groups = let
          nastechWithGroups = nastech-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nastech-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nastechWithGroups}"
          echo "venv: ${nastechWithGroups.nastechVenv}"
        ...
        messaging-variant = pkgs.runCommand "nastech-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.nastechVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "nastech-config-roundtrip" {
