...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Nastech derivation. This is deliberately a derivation environment
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        nastech-agent = prev.nastech-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "nastech-agent-env" {
    nastech-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "nastech-agent-editable-env" {
    nastech-agent = dependency-groups;
  };
}
