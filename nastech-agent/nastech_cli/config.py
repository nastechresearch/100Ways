"""
Configuration management for Nastech Agent.

Config files are stored in ~/.nastech/ for easy access:
- ~/.nastech/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nastech/.env         - API keys and secrets

This module provides:
- nastech config          - Show current configuration
- nastech config edit     - Open config in editor
- nastech config get      - Print a resolved configuration value
- nastech config set      - Set a specific value
- nastech config unset    - Remove a user configuration value
- nastech config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``nastech config set`` (which rewrites
    ...
    ``config.yaml`` in place: nastech never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.nastech/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nastech logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nastech config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nastech makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nastech itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nastech update``, the TUI build.
...
#   on every plugin install / ``nastech update``.
...
# * ``NASTECH_HOME`` / ``NASTECH_PROFILE`` / ``NASTECH_CONFIG`` /
#   ``NASTECH_ENV`` — Nastech runtime location flags. Writing these into
...
#
# IMPORTANT: ``NASTECH_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NASTECH_LANGFUSE_PUBLIC_KEY,
# NASTECH_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Nastech runtime location — never via dashboard env writer.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # NASTECH_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # NASTECH_TOOL_PROGRESS variant is fully unsupported since the v12 config
    ...
    # Activation is via plugins.enabled (opt-in through `nastech plugins enable
    # observability/langfuse` or `nastech tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NASTECH_LANGFUSE_ENV",
    "NASTECH_LANGFUSE_RELEASE",
    "NASTECH_LANGFUSE_SAMPLE_RATE",
    "NASTECH_LANGFUSE_MAX_CHARS",
    "NASTECH_LANGFUSE_CAPTURE",
    "NASTECH_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "NASTECH_ACP_AUTH_METHOD",
    "NASTECH_ACP_AUTO_APPROVE",
    "NASTECH_COPILOT_ACP_COMMAND",
    "NASTECH_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from nastech_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set NASTECH_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NASTECH_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nastech_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Nastech is running in package-manager-managed mode.

    Two signals: the NASTECH_MANAGED env var (set by the systemd service),
    or a .managed marker file in NASTECH_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Nastech through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``nastech_cli/`` — i.e. the git checkout for source
    installs, ``/opt/nastech`` inside the published image. It is a property of
    the running interpreter, NOT of ``$NASTECH_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nastech was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$NASTECH_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. NASTECH_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.nastech``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$NASTECH_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.nastech:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$NASTECH_HOME``, a
    ...
    ``docker`` and ``nastech update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nastechresearch/nastech-agent`` image bakes a ``docker``
        stamp into ``/opt/nastech`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $NASTECH_HOME.
    try:
    ...
    #    host install whose shared $NASTECH_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``nastech update``.
    try:
        method = (
            (get_nastech_home() / ".install_method")
    ...
    # detect Nix installs that don't set NASTECH_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``nastech_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$NASTECH_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/nastech`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nastechresearch/nastech-agent:latest"
    return "nastech update"
...
# Long-form text for ``nastech update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# nastech_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Nastech, it doesn't update
...
✗ ``nastech update`` doesn't apply inside the Docker container.

Nastech Agent runs as a published image (nastechresearch/nastech-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nastechresearch/nastech-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nastech-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nastechresearch/nastech-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nastechresearch/nastech-agent/tags
  • Your config and session history live under ``$NASTECH_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nastech update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nastech installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Nastech installation is managed by NixOS "
            f"(NASTECH_MANAGED={env_hint}).\n"
            "Edit services.nastech-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NASTECH_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nastech_bin
    or None if container mode is not active, we're already inside the
    container, or NASTECH_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NASTECH_DEV") == "1":
        return None
    ...
    container_mode_file = get_nastech_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nastech-agent")
    exec_user = info.get("exec_user", "nastech")
    nastech_bin = info.get("nastech_bin", "/data/current-package/bin/nastech")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "nastech_bin": nastech_bin,
    }
...
from nastech_constants import get_nastech_home, get_process_nastech_home  # noqa: F811,E402
...
def _resolve_nastech_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Nastech commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NASTECH_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NASTECH_UID", "").strip()
    gid_str = os.environ.get("NASTECH_GID", "").strip()
    try:
...
def _chown_to_nastech_uid(path) -> None:
    """Chown ``path`` to ``NASTECH_UID:NASTECH_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_nastech_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nastech group can
    share state with the gateway service.

    The mode can be overridden via the NASTECH_HOME_MODE environment variable
    (e.g. NASTECH_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NASTECH_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NASTECH_UID``/``NASTECH_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NASTECH_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_nastech_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Nastech runs in a container with volume-mounted config files, forcing
...
def _secure_file(path):
    ...
    permissions.  Set NASTECH_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NASTECH_HOME, upgrading legacy empty templates.
...
# ensure_nastech_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_NASTECH_HOME_ENSURED: set = set()


def ensure_nastech_home():
    """Ensure ~/.nastech directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``NASTECH_HOME`` we only re-run
    ...
    ``get_nastech_home()`` and therefore re-run for the new path.
    """
    home = get_nastech_home()
    key = str(home)

    if key in _NASTECH_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``nastech profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old NASTECH_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_nastech_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _NASTECH_HOME_ENSURED.add(key)


def _ensure_nastech_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NASTECH_HOME {home} does not exist. "
...
from nastech_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Nastech Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `nastech config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nastech.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nastech update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Nastech' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nastech versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nastech_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (nastech_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (nastech_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (nastech_cli/setup.py)
    "known_plugin_toolsets", # written/read by nastech_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nastech won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, nastech send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nastech doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_nastech_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "nastech update / nastech -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ nastech config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in nastech_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from nastech_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    # `nastech update`, but only the version bump is persisted.
    ...
    # metadata.nastech.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.nastech/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_nastech_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``nastech config set``
    ...
    backends) — while display paths (``nastech status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nastech_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in nastech_cli.personality.
from nastech_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`nastech_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from nastech_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from nastech_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``NASTECH_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`nastech_cli.personality` (single owner).
    """
    from nastech_cli.personality import resolve_ephemeral_system_prompt
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_nastech_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nastech/config.yaml.
    ...
    (which change ``NASTECH_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Nastech host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/nastech/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_nastech_dotenv()
...
#   nastech         (OAuth — nastech auth) — Nastech Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.nastech/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Nastech writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nastech/.env.
    ...
    interactive menu render (`nastech tools`, `nastech setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nastech tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nastech/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nastech/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``NASTECH_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"NASTECH_CUSTOM_{slug}_API_KEY" if slug else "NASTECH_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nastech/.env and os.environ.
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from nastech_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Nastech — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nastech vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.nastech/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.nastech/.env`` over ``os.environ``.

    Used for Nastech-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    try:
        from nastech_cli.personality import active_personality_name
    ...
    print(color("  nastech config edit     # Edit config file", Colors.DIM))
    print(color("  nastech config set <key> <value>", Colors.DIM))
    print(color("  nastech setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``NASTECH_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("NASTECH_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `nastech cron list`, then pin the "
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (nastech_cli/plugins_cmd.py, nastech_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `nastech config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``nastech config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``nastech config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``nastech config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (nastech config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``nastech config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``nastech config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    nastech config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    nastech config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `nastech config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_nastech_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `nastech config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `nastech config unset <KEY>` fully removes
        # the provider instead of leaving it resurrectable (#51071 family).
        from nastech_cli.credential_lifecycle import remove_provider_env_credential
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: nastech config get <key> [--json]")
            print()
            print("Examples:")
            print("  nastech config get model")
            print("  nastech config get terminal.backend")
            print("  nastech config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: nastech config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  nastech config set model anthropic/claude-sonnet-4")
            print("  nastech config set terminal.backend docker")
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: nastech config unset <key>")
            print()
            print("Examples:")
            print("  nastech config unset model")
            print("  nastech config unset terminal.backend")
            print("  nastech config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        print("  nastech config           Show current configuration")
        print("  nastech config edit      Open config in editor")
        print("  nastech config get <key>          Print a resolved config value")
        print("  nastech config set <key> <value>   Set a config value")
        print("  nastech config unset <key>        Remove a config value")
        print("  nastech config check     Check for missing/outdated config")
        print("  nastech config migrate   Update config with new options")
        print("  nastech config path      Show config file path")
        print("  nastech config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nastech config`` UI so users
