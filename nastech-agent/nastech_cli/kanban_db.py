...
``<root>`` is the **shared Nastech root** (the parent of any active
...
``nastech -p <profile>`` joins the same board as the dispatcher that
...
the profile's Nastech home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NASTECH_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NASTECH_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``nastech kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.nastech``. In Docker / custom
deployments where ``NASTECH_HOME`` points outside ``~/.nastech`` (e.g.
``/opt/nastech``), ``<root>`` is ``NASTECH_HOME``. Legacy env-var
overrides still work:

* ``NASTECH_KANBAN_DB`` — pin the database file path directly.
* ``NASTECH_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NASTECH_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NASTECH_KANBAN_DB``,
``NASTECH_KANBAN_WORKSPACES_ROOT``, and ``NASTECH_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/nastech-kanban-v1-spec.pdf`` for the full
...
from nastech_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``nastech_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from nastech_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("NASTECH_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active NASTECH_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from nastech_cli.lifecycle import invoke_hook
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from nastech_cli.lifecycle import has_hook
...
# ``NASTECH_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``NASTECH_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("NASTECH_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``NASTECH_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("NASTECH_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "nastech_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``nastech-agent``
...
def kanban_home() -> Path:
    """Return the shared Nastech root that anchors the kanban board.

    Resolution order:

    1. ``NASTECH_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_nastech_root()``, which already returns ``<root>``
       when ``NASTECH_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NASTECH_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NASTECH_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NASTECH_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``nastech kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NASTECH_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``nastech kanban boards
    ...
    env = os.environ.get("NASTECH_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``nastech kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NASTECH_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NASTECH_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NASTECH_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NASTECH_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``NASTECH_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("NASTECH_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``nastech kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``nastech_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``NASTECH_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (nastech_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``NASTECH_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("NASTECH_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``nastech_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from nastech_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``nastech gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from nastech_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see nastech_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from nastech_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Nastech treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``nastech kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NASTECH_KANBAN_DB`` env → ``NASTECH_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `nastech kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        from nastech_state import preflight_db_writability
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``nastech kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``NASTECH_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from nastech_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"nastech_nested_{secrets.token_hex(8)}"
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from nastech_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``nastech --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from nastech_cli import projects_db as _pdb
    ...
    # `nastech --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``nastech kanban list --sort``.
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``nastech kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``nastech kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``nastech kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``nastech kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``nastech kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``NASTECH_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Nastech' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Nastech-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``nastech kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Nastech checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Nastech treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Nastech creates/reuses that linked worktree. With
      no ``workspace_path``, Nastech anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Nastech uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Nastech
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``nastech kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``nastech kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Nastech profile.
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Nastech ships the ``sdlc-review`` skill and the
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from nastech_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Nastech profile.
        # `_default_spawn` invokes ``nastech -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Nastech
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `nastech kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Nastech bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from nastech_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_nastech_argv() -> list[str]:
    """Return the interpreter-bound Nastech CLI invocation."""
    # ``nastech_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``nastech`` package — there is no
    # ``nastech`` package to import.
    return [sys.executable, "-m", "nastech_cli.main"]


def _absolute_nastech_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Nastech shim."""
...
def _nastech_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Nastech executable path.
    ...
    return [_absolute_nastech_path(path)]


def _resolve_nastech_argv() -> list[str]:
    """Resolve the ``nastech`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$NASTECH_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("nastech")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\nastech.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m nastech_cli.main`` — fallback for setups where
       Nastech is launched from a venv and the ``nastech`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_nastech_bin`` for the same reason. Kept
    local (not imported from gateway) because ``nastech_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("NASTECH_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _nastech_path_argv(env_bin)
...
def _resolve_worker_cli_toolsets(nastech_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``NASTECH_KANBAN_TASK`` is set.
    """
    if not nastech_home:
        return None
    try:
        ...
        token = set_nastech_home_override(nastech_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for NASTECH_HOME=%r (%s)",
            nastech_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``nastech -p <profile> chat -q ...`` subprocess.
    ...
    ``NASTECH_KANBAN_DB`` / ``NASTECH_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject NASTECH_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `nastech -p <name>` the
    # _apply_profile_override() runs *before* nastech_constants is imported.
    # If NASTECH_HOME is absent from the child's env, get_nastech_home() falls
    # back to Path.home() / ".nastech" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from nastech_cli.profiles import resolve_profile_env
    try:
        env["NASTECH_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via NASTECH_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NASTECH_HOME never had profiles created.
        pass
    if task.tenant:
        env["NASTECH_TENANT"] = task.tenant
    env["NASTECH_KANBAN_TASK"] = task.id
    env["NASTECH_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `nastech kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["NASTECH_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["NASTECH_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["NASTECH_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["NASTECH_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`nastech -p <name>`
    # rewrites NASTECH_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_nastech_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NASTECH_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NASTECH_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["NASTECH_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NASTECH_KANBAN_BOARD"] = resolved_board
    # NASTECH_PROFILE is the author the kanban_comment tool defaults to.
    # `nastech -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NASTECH_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited NASTECH_TUI=1
    ...
    # older nastech builds on PATH that predate the flag's precedence.
    env.pop("NASTECH_TUI", None)

    cmd = [
        *_resolve_nastech_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped NASTECH_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("NASTECH_HOME"))
    if worker_toolsets:
    ...
    # `nastech kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`nastech` executable not found on PATH. "
            "Install Nastech Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``nastech kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled NASTECH_PROFILE values like "nastech-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Nastech root exists

    Reads profile paths directly so this module has no import dependency on
    ``nastech_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from nastech_constants import get_default_nastech_root
        default_root = get_default_nastech_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``nastech kanban assignees`` for the terminal.
