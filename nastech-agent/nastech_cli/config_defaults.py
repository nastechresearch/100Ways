"""Default configuration data for Nastech Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from nastech_cli/config.py. Must not import from nastech_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    # SQLite journal mode used by every Nastech database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Nastech server processes.
    ...
    "agent": {
        ...
        # the Nastech-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nastech
        ...
        # identity slot (SOUL.md). Empty by default. The NASTECH_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Nastech adds a coding operating brief
        ...
        # `nastech status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``NASTECH_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             NASTECH_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NASTECH_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Nastech auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nastech sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nastech
        ...
        # bundled Nastech image, which drops to the `nastech` user via
    ...
    # Opt in via ``nastech chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nastech/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nastech sweeps the checkpoint base at startup
        ...
        # ``nastech checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .nastech.md, or .cursorrules before Nastech applies
    ...
    # Single-query (``nastech -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Nastech truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Nastech' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); nastech =
                                      # Nastech' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Nastech' local
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Nastech does.
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | nastech | codex | custom
        ...
        # Invoked by ``nastech kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nastech kanban decompose`` and the kanban
        ...
        # ``nastech profile describe <name> --auto`` and the dashboard's
    ...
    "display": {
        ...
        # Which interface bare `nastech` (and `nastech chat`) launches by default:
        ...
        # Mirrors `nastech -c` muscle memory.  Default off so existing
        # users aren't surprised.  NASTECH_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nastech --tui` drops a one-time hint
        ...
        # Nastech credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see nastech_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to NASTECH_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/nastech`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `nastech pets`. Disabled until
        ...
        "pet": {
            ...
            # get_nastech_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Nastech Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``NASTECH_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NASTECH_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.nastechresearch.com).
        "oauth": {
        ...
        # nastech-account-service at deploy time via the
        # ``NASTECH_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``NASTECH_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/nastech``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Nastech appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # "Hey Nastech" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey nastech",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_nastech" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nastech/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Nastech, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.nastech/cache/delegation/
    ...
    # response. If not, Nastech feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nastech auto-pauses the goal and
    ...
    "moa": {
        ...
        # <nastech_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.nastech/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NASTECH_SKILL_DIR} and ${NASTECH_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nastech curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `nastech curator run --consolidate` overrides
        ...
        # normally restored on every `nastech update`, so pruning them only
        ...
        # skipped), snapshot ~/.nastech/skills/ into
        # ~/.nastech/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nastech curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nastech-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nastech Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NASTECH_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Nastech' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.nastech/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NASTECH_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .nastech config) always require human
        ...
        # longer trigger the startup banner. Add via `nastech doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nastech_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nastech to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $NASTECH_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Nastech Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.nastechresearch.com",
        ...
        # Also overridable via NASTECH_CRON_MAX_PARALLEL env var.
        ...
        # NASTECH_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # NASTECH_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `nastech -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `nastech` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `nastech kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nastech-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nastech tools (terminal, read_file, write_file, patch,
    ...
    # curated model lists for OpenRouter and Nastech Portal from this URL,
    ...
    "gateway": {
        ...
        # internal NASTECH_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.nastech/sessions/sessions.json entirely.
        ...
        # the NASTECH_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.nastech.gateway` that defenses 1-2 don't
        ...
        # `nastech update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``NASTECH_GATEWAY_MAX_STARTS`` / ``NASTECH_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nastech/.env,
        ...
        # behavior: files must live under the Nastech cache, under
        ...
        # user. Bridged to NASTECH_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nastech
        # cache (~/.nastech/cache/{documents,images,audio,video,screenshots})
        ...
        # to NASTECH_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NASTECH_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NASTECH_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nastech/state.db.
    ...
    "sessions": {
        ...
        # the default of ``nastech sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nastech/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `nastech sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `nastech update` prints a one-line notice with
        ...
        # ~/.nastech/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to NASTECH_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # NASTECH_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `nastech sessions export`. Checked
    ...
    # ``nastech doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `nastech doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``nastech update`` behaviour.
    "updates": {
        ...
        #     DBs) into <NASTECH_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``nastech backup``-style zip
        #     of NASTECH_HOME into <NASTECH_HOME>/backups/, restorable with
        #     ``nastech import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `nastech update` does with uncommitted local changes to the
        ...
        # Refresh an already-installed cua-driver during `nastech update`.
    ...
    "lsp": {
        ...
        #              ``<NASTECH_HOME>/lsp/bin/`` on first use.
    ...
    # `nastech tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nastech/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.nastech/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.nastech/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Nastech may use this encrypted
            ...
            # ~/.nastech/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.nastech/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Nastech disables it for our users unless they
        # explicitly opt in here. When false (default), Nastech sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Nastech also calls
    ...
    #
    # Configure with `nastech egress setup`.  Disabled by default — the rest of
    # Nastech works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.nastech/bin/ on
        ...
        # Together, DeepSeek, Nastech).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Nastech Desktop (Electron app) launch options. These only affect
    # `nastech desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the NASTECH_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the NASTECH_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "NASTECH_BASE_URL": {
        "description": "Nastech Portal base URL override",
        "prompt": "Nastech Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Nastech mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nastech model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Nastech Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nastech Subscribers only, used to derive vendor hosts, e.g. nastechresearch.com -> firecrawl-gateway.nastechresearch.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Nastech Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nastech Subscriber access token for tool-gateway requests (optional; otherwise read from the Nastech auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Nastech' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Nastech, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nastech:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NASTECH_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nastech)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nastech-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nastech-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nastech API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nastech API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nastech API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NASTECH_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to NASTECH_TOOL_PROGRESS_MODE for backward
    ...
    # NASTECH_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "NASTECH_PREFILL_MESSAGES_FILE": {
