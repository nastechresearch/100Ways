...
storage discipline as the other indexes). On a populated database, run
    ...
    nastech sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.nastech/config.yaml` to disable. Override
the .so location with `NASTECH_FTS5_CJK_SO`.
