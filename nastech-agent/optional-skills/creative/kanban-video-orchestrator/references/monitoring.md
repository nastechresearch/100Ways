...
nastech kanban watch --tenant <project-slug>

# Snapshot of the board
nastech kanban list --tenant <project-slug>
nastech kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nastech kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nastech dashboard

# Inspect a specific task (includes comments + events)
nastech kanban show <task-id>

# Follow a single task's event stream
nastech kanban tail <task-id>
```

Verify available subcommands with `nastech kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nastech kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nastech kanban block <id>` then `nastech kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nastech kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nastech kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nastech profile list` (profile exists?), `nastech status` (gateway/dispatcher up?), and verify tenant. |
...
nastech kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
nastech kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(nastech kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nastech kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nastech kanban block <task-id>
nastech kanban archive <task-id>

# Diagnose what happened
nastech kanban show <task-id>      # task body, comments, recent events
nastech kanban tail <task-id>      # follow the live event stream
nastech kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nastech kanban create` for the director
...
while true; do
    clear
    nastech kanban list --tenant <slug>
    echo "---"
    nastech kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nastech kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nastech profile list` and start
