...
the actual `setup.sh` that creates the project workspace, configures Nastech
...
> [NastechResearch/kanban-video-pipeline](https://github.com/NastechResearch/kanban-video-pipeline).
...
2. **Create profiles** — `nastech profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.nastech/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `nastech kanban create` assigned to the director
...
```bash
nastech profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.nastech/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.nastech/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Nastech
...
Each profile gets a `SOUL.md` at `~/.nastech/profiles/<name>/SOUL.md` that
...
```bash
nastech kanban create "Direct production of <video title>" \
    --assignee director \
...
the Nastech `.env` (`${NASTECH_HOME:-$HOME/.nastech}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _nastech_env="${NASTECH_HOME:-$HOME/.nastech}/.env"
    if grep -q "^${var}=" "$_nastech_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_nastech_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_nastech_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY nastech ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY nastech OPENROUTER_API_KEY || exit 1
# ...
```
