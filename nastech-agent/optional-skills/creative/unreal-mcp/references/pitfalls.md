...
### 1. Start order: editor first, Nastech second

Nastech probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Nastech session.
...
restart the editor, restart the Nastech session.
...
server fails to bind (Output Log) or Nastech' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.nastech/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Nastech

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Nastech' connection
lives in `~/.nastech/config.yaml` via `nastech mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Nastech — just don't
mistake it for the Nastech setup step.
...
explicitly warns against overlapping calls. Nastech executes same-turn tool
...
### 9. Timeouts: Nastech gives up before Unreal does

Nastech' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Nastech runs elsewhere (SSH backend,
