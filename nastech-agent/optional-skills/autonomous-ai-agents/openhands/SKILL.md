...
metadata:
  nastech:
    ...
    related_skills: [claude-code, codex, opencode, nastech-agent]
---
...
- User wants a coding agent that can run on a non-Anthropic / non-OpenAI provider (DeepSeek, Qwen, Ollama, vLLM, Nastech, etc.) — sibling skills `claude-code` and `codex` are tied to one vendor.
