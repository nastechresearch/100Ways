...
$NASTECH_OSINT_CACHE/icij/ (default: ~/.cache/nastech-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("NASTECH_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "nastech-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "nastech-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $NASTECH_OSINT_CACHE/icij or ~/.cache/nastech-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
