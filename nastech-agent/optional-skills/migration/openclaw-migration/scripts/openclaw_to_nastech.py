...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.nastech/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.nastech/tts/.",
    },
    "discord-settings": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'nastech cron'.",
    },
    "hooks-config": {
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-nastech/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-nastech/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: nastech config set",
                        )
                    continue
        ...
        for oc_key, nastech_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and nastech_key not in secret_additions:
                secret_additions[nastech_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, nastech_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and nastech_key not in secret_additions:
                        secret_additions[nastech_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nastech_config = load_yaml_file(destination)
        current_model = nastech_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = nastech_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                nastech_config["model"] = {"default": model_str}
            dump_yaml_file(destination, nastech_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nastech_config = load_yaml_file(destination)
        existing_tts = nastech_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nastech_cfg = load_yaml_file(nastech_cfg_path)
        existing_mcp = nastech_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            # STDIO transport
            if srv.get("command"):
                nastech_srv["command"] = srv["command"]
                if srv.get("args"):
                    nastech_srv["args"] = srv["args"]
                if srv.get("env"):
                    nastech_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    nastech_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                nastech_srv["url"] = srv["url"]
                if srv.get("headers"):
                    nastech_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    nastech_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                nastech_srv["enabled"] = False
            if srv.get("timeout"):
                nastech_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                nastech_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                nastech_srv["tools"] = {}
                if tools_cfg.get("include"):
                    nastech_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    nastech_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                nastech_srv["sampling"] = {
                    k: v for k, v in {
        ...
        if added > 0 and self.execute:
            ...
            nastech_cfg["mcp_servers"] = existing_mcp
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'nastech cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = nastech_cfg.get("agent") or {}
        ...
        if compaction:
            compression = nastech_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = nastech_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            nastech_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = nastech_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'nastech gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("NASTECH_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nastech_cfg_path = self.target_root / "config.yaml"
        nastech_cfg = load_yaml_file(nastech_cfg_path)
        sr = nastech_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        nastech_cfg_path = self.target_root / "config.yaml"
        nastech_cfg = load_yaml_file(nastech_cfg_path)
        custom_providers = nastech_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(nastech_cfg_path)
            nastech_cfg["custom_providers"] = custom_providers
            dump_yaml_file(nastech_cfg_path, nastech_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            nastech_cfg_path = self.target_root / "config.yaml"
            nastech_cfg = load_yaml_file(nastech_cfg_path)
            discord_nastech = nastech_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_nastech["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_nastech["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                nastech_cfg["discord"] = discord_nastech
                dump_yaml_file(nastech_cfg_path, nastech_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if browser.get("headless") is not None:
            browser_nastech["headless"] = browser["headless"]
            changed = True

        if changed:
            nastech_cfg["browser"] = browser_nastech
            if self.execute:
                self.maybe_backup(nastech_cfg_path)
                dump_yaml_file(nastech_cfg_path, nastech_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            nastech_mode = mode_map.get(mode, "manual")
            nastech_cfg.setdefault("approvals", {})["mode"] = nastech_mode
            if self.execute:
                self.maybe_backup(nastech_cfg_path)
                dump_yaml_file(nastech_cfg_path, nastech_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{nastech_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "- Run `nastech setup` to configure any remaining settings",
            "- Run `nastech mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `nastech cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `nastech cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `nastech gateway install` if you need the gateway service",
            "- Review `~/.nastech/config.yaml` for any adjustments",
            "",
        ])
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.nastech/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.nastech/config.yaml")
        print("    2. Run: nastech mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: nastech cron")
