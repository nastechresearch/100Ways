#!/usr/bin/env python3
"""Telephony helper for the Nastech optional telephony skill.

Capabilities:
- Persist telephony provider credentials to the Nastech .env file ($NASTECH_HOME/.env)
...
def _nastech_home() -> Path:
    return Path(os.environ.get("NASTECH_HOME", "~/.nastech")).expanduser()


def _env_path() -> Path:
    return _nastech_home() / ".env"


def _config_path() -> Path:
    return _nastech_home() / "config.yaml"


def _state_path() -> Path:
    return _nastech_home() / "telephony_state.json"
...
def _load_root_config() -> dict[str, Any]:
    ...
    if not path.exists():
        return {}
    try:
        import yaml  # optional dependency; Nastech already ships PyYAML
    except Exception:
        return {}
    try:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Nastech telephony helper")
    sub = parser.add_subparsers(dest="command", required=True)

    sub.add_parser("diagnose", help="Show saved telephony state and provider readiness")

    p = sub.add_parser("save-twilio", help="Save Twilio credentials to the Nastech .env file")
    p.add_argument("account_sid")
    p.add_argument("auth_token")
    p.add_argument("--phone-number", default="")
    p.add_argument("--phone-sid", default="")

    p = sub.add_parser("save-bland", help="Save Bland.ai settings to the Nastech .env file")
    p.add_argument("api_key")
    p.add_argument("--voice", default=BLAND_DEFAULT_VOICE)

    p = sub.add_parser("save-vapi", help="Save Vapi settings to the Nastech .env file")
    p.add_argument("api_key")
