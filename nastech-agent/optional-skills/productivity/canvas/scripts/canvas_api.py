#!/usr/bin/env python3
"""Canvas LMS API CLI for Nastech Agent.
...
def _check_config():
    ...
    if missing:
        nastech_env = os.path.join(
            os.environ.get("NASTECH_HOME", os.path.expanduser("~/.nastech")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {nastech_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Nastech Agent"
    )
