...
is typically only in `~/.nastech/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Nastech,
...
`nastech-custom-providers` skill, pitfall 2). Nastech hits this because it forwards
