...
Usage:
  ...
  <server_name> matches the files in $NASTECH_HOME/mcp-tokens/<server>.json etc.
...
def _nastech_home():
    # Prefer Nastech' own resolver (profile-safe); fall back to env then ~/.nastech.
    try:
        from nastech_constants import get_nastech_home
        return str(get_nastech_home())
    except Exception:
        return os.environ.get("NASTECH_HOME") or os.path.expanduser("~/.nastech")


def _tokens_dir():
    return os.path.join(_nastech_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "nastech-diag", "version": "1.0"}},
        },
        headers={
