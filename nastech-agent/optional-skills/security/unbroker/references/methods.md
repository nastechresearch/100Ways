...
How the agent executes each broker `optout.method` using native Nastech tools. Obey **least-disclosure**:
...
1. **browser mode (no password, autonomous):** the command returns a recipient-locked `compose`
   ...
   stored. Requires the inbox signed in in the browser Nastech uses.
...
- **Phase 1 (scan, read-only):** a cloud stealth browser (Browserbase) or the `scrapling` skill is
  ...
  `$NASTECH_HOME/.env`, not just the shell env, so `doctor`/`setup --auto` detect the key Nastech
...
- **Phase 2 (execute: opt-out forms, webmail sends, session-bound multi-step gates):** the work must
  ...
  binding).
- **How to drive the operator's browser (CDP).** Point Nastech's browser tools at the operator's real
  Chrome over the DevTools protocol: launch
  `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.nastech/chrome-debug"` and connect the
  ...
  fingerprint + logged-in sessions, which is precisely what Phase-2 flows need. (This is a Nastech-side
