...
The design is **Nastech-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
nastech skills install official/security/unbroker
```

Then start a new Nastech session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Nastech session:
...
- **PII handling.** Dossiers live under the Nastech home directory (`0600`, optionally
