...
Run it through the `terminal` tool (it can read PII files under NASTECH_HOME);
...
def cmd_setup(args) -> None:
    if getattr(args, "auto", False):
        ...
        # asking anyone. Read creds from $NASTECH_HOME/.env too (the terminal shell doesn't export
...
def cmd_doctor(args) -> None:
    ...
    cfg = config_mod.load_config()
    caps = config_mod.detect_capabilities(config_mod.dotenv_env())  # see creds in $NASTECH_HOME/.env too
    ...
    if engaged:
        L.append(f"        key: {crypto.identity_path()} (0600) - guards casual/backup/commit "
                 "exposure, NOT a full-NASTECH_HOME read")
    ...
    if cfg["email_mode"] == "draft_only":
        ...
        L.append("  Email mode: browser (no password) - the agent sends opt-outs and opens verify "
                 "links via the operator's logged-in webmail. This needs Nastech pointed at the "
                 "operator's OWN Chrome over CDP (launch with --remote-debugging-port=9222 "
                 "--user-data-dir=~/.nastech/chrome-debug, signed into the webmail once); else it falls "
    ...
    if not crypto.is_engaged():
        L.append("  Storage: dossiers are PLAINTEXT JSON (0600 under NASTECH_HOME). "
...
def cmd_cdp(args) -> None:
    ...
    Nastech at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({"running": True, "endpoint": f"127.0.0.1:{port}",
              ...
              "note": "a debuggable browser is already listening; point Nastech's browser tools at "
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Nastech's browser tools at 127.0.0.1:{port} (CDP)",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--profile",
                   help="user-data-dir (default: $NASTECH_HOME/chrome-debug, a dedicated debug profile)")
