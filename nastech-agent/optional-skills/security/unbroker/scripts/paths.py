...
All per-subject data lives under PDD_DATA_DIR (default: $NASTECH_HOME/unbroker),
which is the same trust boundary Nastech uses for .env and OAuth tokens.
"""
...
def nastech_home() -> Path:
    return Path(os.environ.get("NASTECH_HOME") or (Path.home() / ".nastech"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else nastech_home() / "unbroker"
