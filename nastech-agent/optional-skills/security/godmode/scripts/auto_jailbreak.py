...
PREFILL_PATH = NASTECH_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nastech/Nastech models — already uncensored, just needs clean prompt
    "nastech": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nastech" in model_lower or "nastech" in model_lower:
        return "nastech"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nastech/prefill.json."""
