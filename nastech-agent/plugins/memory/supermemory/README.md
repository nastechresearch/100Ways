...
- Hosted: API key from [app.supermemory.ai/integrations?connect=nastech](http://app.supermemory.ai/integrations?connect=nastech)
...
Then run `nastech memory setup` and enter the local server's API key. Configuring
...
| `container_tag` | `nastech` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nastech-{identity}` → `nastech-coder`). |
...
`https://api.supermemory.ai`. Nastech resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: nastech`, and document writes stamp
`metadata.sm_source: nastech`. This is a **functional routing key, not telemetry**:
it groups Nastech-written memories into a dedicated "Nastech" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Nastech profile:

```json
{
  "container_tag": "nastech-{identity}"
}
```

For a profile named `coder`, this resolves to `nastech-coder`. The default profile resolves to `nastech-default`. Without `{identity}`, all profiles share the same container.
