...
```bash
nastech memory setup    # select "mem0"
```
...
Behavioral settings live in `$NASTECH_HOME/mem0.json` (set them via `nastech memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.nastech/.env`.
...
| `user_id` | `nastech-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Nastech session and call `mem0_search` — it connects to your server.
...
```bash
nastech memory setup mem0 --mode oss \
...
```bash
nastech memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$NASTECH_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.nastech/mem0_qdrant"}}
  }
}
```
...
```bash
nastech memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$NASTECH_HOME/mem0.json`).
