...
>
> **Share cards (2026-05-04, vendored in nastech-agent v0.4.0):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.
...
```bash
git clone https://github.com/PCinkusz/nastech-achievements ~/nastech-achievements
ln -s ~/nastech-achievements ~/.nastech/plugins/nastech-achievements
```
...
If backend API routes 404, restart `nastech dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.nastech/plugins/nastech-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `nastech dashboard` after pulling.
...
```text
/api/plugins/nastech-achievements/
```
