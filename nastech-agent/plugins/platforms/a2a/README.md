# A2A — Agent-to-Agent protocol for Nastech
...
peer (another Nastech, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
nastech gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Nastech serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.nastech/a2a_audit.jsonl`.
- Conversations persist to `~/.nastech/a2a_conversations/` — they survive context
