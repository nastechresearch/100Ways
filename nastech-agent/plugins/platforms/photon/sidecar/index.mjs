...
//
// Inbound  (gRPC -> Nastech): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Nastech -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Nastech-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `nastech photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Nastech can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "nastech-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Nastech'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Nastech allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Nastech, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-nastech-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Nastech supplied them so a known-good
