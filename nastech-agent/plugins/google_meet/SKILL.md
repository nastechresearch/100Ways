...
nastech meet install                 # pip deps + Chromium (transcribe only)
nastech meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
nastech meet auth                    # optional; skips guest-lobby wait
nastech meet setup                   # preflight checks
```

`nastech meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or NASTECH_MEET_REALTIME_KEY in ~/.nastech/.env
```
...
pip install playwright websockets && python -m playwright install chromium
nastech plugins enable google_meet
nastech meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
nastech meet node approve my-mac ws://<mac-ip>:18789 <token>
nastech meet node ping my-mac                   # confirm reachable
```

Run `nastech meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Nastech agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `nastech meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `NASTECH_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `nastech_meet_src`). |
...
Local:
```
$NASTECH_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$NASTECH_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `nastech meet node approve`.
